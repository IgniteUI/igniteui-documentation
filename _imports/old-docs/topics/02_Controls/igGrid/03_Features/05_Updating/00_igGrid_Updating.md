...
-   [**Adding igGrid with Updating feature enabled using the %%ProductFamilyName%% CLI**](#adding-using-CLI)
...
## <a id="adding-using-CLI"></a> Adding igGrid with Updating feature enabled using the %%ProductFamilyName%% CLI
To install the %%ProductFamilyName%% CLI:
```
npm install -g igniteui-cli
```
Once the %%ProductFamilyName%% CLI is installed the commands for generating an %%ProductName%% project, adding a new igGrid component with Updating feature configured, building and serving the project are as following:
```
