...
この詳細なガイドのコードによって、このトピックの上部にあるスクリーンショットと非常に似た PDF レポートを作成することができます。後はレポートをパブリッシュするだけです。レポートのパブリッシュについての詳細は、[「レポートをパブリッシュ」](DocumentEngine-Publish-a-Report.html)を参照してください。
