...
%%ProductName%% コントロール ファミリの主な配布方法は、NPM、JSPM、NuGet などのパッケージ マネージャーを使用することです。
...
%%ProductName%% が jQuery および jQuery UI ライブラリに依存するため、%%ProductName%% スクリプトの前にそれへの参照を追加する必要があります。また、%%ProductName%% コントロールをページに追加するために複数のオプションがあります。
