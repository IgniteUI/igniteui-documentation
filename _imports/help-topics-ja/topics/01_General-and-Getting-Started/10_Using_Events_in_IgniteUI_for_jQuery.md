...
-   Infragistics.Web.Mvc.dll (%%ProductNameMVC%% を含む) への参照
...
- ***バインド:***
 ...
 このサンプルは、`igTextEditor` のインスタンスが MVC コンテキストで作成され、`valueChanged` イベントへバインドされています。%%ProductNameMVC%% を使用して `igEditor` コントロールから継承したコントロールのインスタンスを作成する場合、レンダリング メソッドは適切な型の値で `igEditor` コントロールを生成し、エディターを構成します。そのため、イベントをバインドまたはバインド解除する場合、「igeditor」プレフィックスが必要です。
