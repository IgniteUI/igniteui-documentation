...
以下の表は、`igSplitter` コントロールの API 参照ドキュメントのリンクの一覧です。(この参照コンテンツは、コントロールの開発者ガイドの他、[こちら](igSplitter.html)で使用できます。)

API マニュアル|説明
---|---
[igSpitter jQuery API](%%jQueryApiUrl%%/ui.igsplitter)|これは、コントロールの概要と、そのオプション、イベントおよびメソッドの完全なリスト、コード例を含む一連のドキュメントです。
