...
6.  %%ProductNameMVC%% コントロールなど jQuery Upload コントロールにはサーバー イベントはありません。ポイントできるのは、ハンドラーとすべてのファイルがアップロードされるフォルダーですが、アップロードが完了したらファイルを処理して削除したり移動することはできません。しかし、ASP.NET MVC のコンテキストを使えば回避できます。Global.asax ファイルで、手動でサーバー側イベントにアタッチして必要なロジックを実行し、アップロードを行うことができます。
