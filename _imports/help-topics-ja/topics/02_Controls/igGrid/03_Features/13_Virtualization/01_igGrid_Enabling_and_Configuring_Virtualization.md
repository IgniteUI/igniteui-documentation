...
仮想化モード|[virtualizationMode](%%jQueryApiUrl%%/ui.iggrid#options:virtualizationMode)|行仮想化モードを決定します。
平均の行の高さ|[avgRowHeight](%%jQueryApiUrl%%/ui.iggrid#options:avgRowHeight)|固定行仮想化で使用されます。描画する行の数を計算するために使用されるピクセル単位の平均値を決定します。すべての行の高さは自動的にこの値に設定されます。
列の平均幅|[avgColumnWidth](%%jQueryApiUrl%%/ui.iggrid#options:avgColumnWidth)|列仮想化で使用されます。これは列幅の平均値 (ピクセル単位) です。

## <a id="fixed-row"></a> 固定行の仮想化の有効化と構成

固定行仮想化は、`igGrid` コントロールの [`rowVirtualization`](%%jQueryApiUrl%%/ui.iggrid#options:rowVirtualization) オプションを `true` に設定することで有効化します。
...
[virtualizationMode](%%jQueryApiUrl%%/ui.iggrid#options:virtualizationMode) (オプション)|"fixed" (デフォルト値)
[height](%%jQueryApiUrl%%/ui.iggrid#options:height)|"600px"
[avgRowHeight](%%jQueryApiUrl%%/ui.iggrid#options:avgRowHeight) (オプション)|"30px" (デフォルト値)
...
列仮想化は、`igGrid` コントロールの [`columnVirtualization`](%%jQueryApiUrl%%/ui.iggrid#options:columnVirtualization) オプションを `true` に設定することで有効化します。有効な場合、**固定行仮想化も有効にします**。

以下のオプションを設定する必要があります:

- [width](%%jQueryApiUrl%%/ui.iggrid#options:width)
- [defaultColumnWidth](%%jQueryApiUrl%%/ui.iggrid#options:defaultColumnWidth) または各列の列 [width](%%jQueryApiUrl%%/ui.iggrid#options:columns.width)。
- [avgColumnWidth](%%jQueryApiUrl%%/ui.iggrid#options:avgColumnWidth)
- [height](%%jQueryApiUrl%%/ui.iggrid#options:height) (行仮想化の自動有効化のため)
- [avgRowHeight](%%jQueryApiUrl%%/ui.iggrid#options:avgRowHeight) (行仮想化の自動有効化のため)
...
igGrid コントロールの [rowVirtualization](%%jQueryApiUrl%%/ui.iggrid#options:rowVirtualization) オプションを `true` に設定し、[virtualizationMode](%%jQueryApiUrl%%/ui.iggrid#options:virtualizationMode) を "continuous" にすることで、仮想化が継続的になります。
