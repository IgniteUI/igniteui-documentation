...
ただし、%%ProductNameMVC%% Grid は、Infragistics.Web.Mvc.dll が提供する組み込みリモート機能サポートにより使用される異なる要求パラメーターを使用します。そのため、oData サービスへのバインドにそれを使用する場合は、対応する機能 URL パラメーターを null に設定する必要があります。
