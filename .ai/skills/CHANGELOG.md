...
| The two populations of the xplat content tree written down, because both rules above are scoped to one of them: an **xplat doc** publishes to WinUI and Uno and owes `apiTerms: full` plus a `json-snippet`-first habit; a **web-only** topic owes neither and is not to be reworked to match. The test for telling them apart lives in the `xplat-docs-json-snippets` skill. | igniteui-doc-topics/references/house-style.md |
