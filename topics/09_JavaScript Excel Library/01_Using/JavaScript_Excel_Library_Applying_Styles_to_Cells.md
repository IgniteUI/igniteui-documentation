...
ほとんどのスタイルは、[`WorksheetCell`](%%jQueryApiUrl%%/ig.excel.WorksheetCell)、[`WorksheetRow`](%%jQueryApiUrl%%/ig.excel.WorksheetRow)、[`WorksheetColumn`](%%jQueryApiUrl%%/ig.excel.WorksheetColumn)、および [`WorksheetMergedCellsRegion`](%%jQueryApiUrl%%/ig.excel.WorksheetMergedCellsRegion) の `cellFormat` プロパティでプロパティを設定し、適用できます。
...
![](images/ExcelEngine_Applying_Styles_to_Cells_01.png)
