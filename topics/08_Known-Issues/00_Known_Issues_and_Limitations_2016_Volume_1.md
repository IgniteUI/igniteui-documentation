...
![](images/negative.png)
        ...
        <tr>
            <td>
Infragistics テーマは、デフォルトの [jQuery UI アイコン](https://api.jqueryui.com/theming/icons/) を置き換えるフォント アイコン セットを含みます。
			</td>
            <td>
...
背景画像の使用を続行するには、`!important` のルール オーバーライドが必要で、`::before` 擬似要素の非表示も必要がある可能です。

`igRating` および `igHtmlEditor` コントロールのカスタム アイコンは変更されないことに注意してください。また、その他の利用可能なテーマが変更されていません。
            </td>
            <td>
![](images/positive.png)
			</td>
...
埋め込み iframe (`igHtmlEditor` などのコントロールを含めて) の再読み込みにより、コンテンツを失う場合があります。|`igDialog` の最大化、最小化がコンテンツのフレーム要素 (`igHtmlEditor` などのコントロールも) を再読み込むことを実行できます。これは、ドキュメント本文の下に配置するためにダイアログが DOM で移動されるためです。構成によっては、ピン固定で同様の動作が可能です。 | ![](images/positive.png)
...
問題|説明|状態
---|---|---
`igOlapXmlaDataSource` は、Android オペレーティング システム用ブラウザーでサポートされていません。 | `igOlapXmlaDataSource`™ コンポーネントは、Android オペレーティング システム用ブラウザーでサポートされていません。 | ![](images/negative.png) ![](images/plannedFix.png)
...
`igZoombar` は igDataChart と正しく動作しません。| `igZoombar` はターゲットとする元の `igDataChart` のクローンを作成します。デフォルトで `igZoombar` は元のチャート オブジェクトからクローンのオプションを暗示します (`igZoombar` の [`clone`](%%jQueryApiUrl%%/ui.igzoombar#options:clone) オプション参照)。より複雑な構造を持つ積み上げシリーズの場合、元のチャートはユーザーが内部的に提供するシリーズを変更します。 これにより、`igZoombar` は元のシリーズの取得およびクローンを自動で再作成することはできません。| ![](images/positive.png)
