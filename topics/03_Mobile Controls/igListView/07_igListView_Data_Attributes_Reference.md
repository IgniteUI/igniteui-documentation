...
プロパティ|タイプ|説明|デフォルト値
---------|------|-------------|--------------
data-role |文字列|`igListView` の識別子|iglistview
...
data-data-source-url |文字列|AJAX コール ($.ajax) を使用してデータが取得されるデータ ソースとしてリモート URL を指定します。|null
data-data-source-type |文字列|データ ソースの種類 (**json** など) を明示的に設定します。|null
data-response-data-key |文字列|これは、応答がラップした場合にデータ レコードが保持される応答内のプロパティです。|null
data-response-total-rec-count-key |文字列|これは、サーバー上のレコード総数を指定する応答内のプロパティです。|null
data-local-schema-transform |bool|このオプションが false と設定されている場合、データ スキーマへの変換は適用されません。|true
data-item-template |文字列|リスト項目を描画するために使用される、IG templating スタイル テンプレート。|null
data-item-details-template |文字列|サブ ページ内のリスト項目の詳細を描画するために使用される、IG templating スタイル テンプレート。|null
data-image-mode |文字列|画像、アイコン、またはiサムネイルを表示するかどうかを決定します。|サムネイル
data-show-count |bool|カウント バブルを表示するかどうかを判断します。|true
data-item-header-size |文字列|LI に見出しタグを表示するかどうかを決定します。値によって、H1 から H6 になります。|h2
data-bindings-text-key |文字列|その値がノード テキストになる、data source プロパティの名前を指定します。|null
data-bindings-text-x-path |文字列|テキスト属性／ノードへの XPath を指定します。クライアントのみのXML への直接バインディングで使用されます。|null
data-bindings-image-url-key |文字列|値がノード イメージに対する URL として使用されるデータ ソース プロパティの名前を指定します。|null
data-bindings-image-url-x-path |文字列|イメージ URL 属性／ノードへの XPath を指定します。クライアントのみのXML への直接バインディングで使用されます。|null
data-bindings-navigate-url-key |文字列|その値がノード アンカーの **href** 属性として使用される、data source プロパティの名前を指定します。|null
data-bindings-navigate-url-x-path |文字列|ナビゲート URL 属性／ノードへの XPath を指定します。クライアントのみのXML への直接バインディングで使用されます。|null
data-bindings-count-key |文字列|その値がノードのカウントとして使用される、data source プロパティの名前を指定します。|null
data-bindings-count-x-path |文字列|ノード カウントへの XPath を指定します。クライアントのみのXML への直接バインディングで使用されます。|null
data-bindings-header-key |文字列|その値がノードのカウントとして使用される、data source プロパティの名前を指定します。|null
data-bindings-header-x-path |文字列|リスト項目ヘッダーへの XPath を指定します。クライアントのみのXML への直接バインディングで使用されます。|null
data-bindings-description-key |文字列|その値がノードのカウントとして使用される、data source プロパティの名前を指定します。|null
data-bindings-description-x-path |文字列|クライアントのみの XML への直接バインディングで使用されるリスト項目記述への XPath を指定します。|null
data-bindings-primary-key |文字列|その値がデータのプライマリ キー属性として使用される、data source プロパティの名前を指定します。ロード オン デマンドが有効であり、指定されたノードのパスがインデックスの代わりにプライマリ キーを使用して生成される場合に、このプロパティが使用されます。|null
data-bindings-is-divider-key |文字列|項目が読み取り専用のリスト ディバイダーを表すかどうかを決定するために使用される data source プロパティの名前を指定します。|null
data-bindings-details-title-key |文字列|その値がノードのサブ ページのタイトルになる、data source プロパティの名前を指定します。|null
data-bindings-details-title-x-path |文字列|タイトル属性／ノードへの XPath を指定します。クライアントのみのXML への直接バインディングで使用されます。|null
...
data-default-children-data-property |文字列|子供が位置する応答内にデフォルトのプロパティを指定します。|Children
data-path-separator |文字列|データの階層的ルックアップのためのパスを作るために区切り文字を指定します。|/
data-auto-generate-layouts |bool|true の場合、キーのデフォルト値を想定してすべてのレイアウトを自動生成します。|false
data-child-layout-key |文字列|レイアウト キーを指定します。これは、子レイアウトのデータ レコードを保持するプロパティになります。|null
data-init-selector |文字列|このウィジェットに変換される html 要素を見つけるためにデフォルトで使用されるセレクターです。|:jqmData(role='iglistview')
...
data-sorting-type |文字列|ローカルまたはリモート並べ替えを定義。|null
data-sorting-case-sensitive |bool|並べ替えの大文字と小文字の区別です。|false
data-sorting-sort-presets-label |文字列|並べ替えプリセットの上に表示されるテキスト。|null
data-sorting-sort-state |文字列|開始時に選択されるプリセットの数です。|default
data-sorting-sort-url-key |文字列|並び替え式が URL 中でどのようにエンコードされるかを指定する URL パラメータ名です。OData 規則を使用します。`例: ?sort(col1)=asc`|null
data-sorting-sort-url-key-asc-value |文字列|昇順の並び替え用の URL パラメータ値です。OData 規則を使用します。|null
data-sorting-sort-url-key-desc-value |文字列|降順の並び替え用の URL パラメータ値です。OData 規則を使用します。|null
...
data-sorting-divider-template |文字列|jQuery テンプレート スタイルはリスト区分線目を描画するために使われるテンプレートです。カウントと名前のキー／値ペアを含むデータオブジェクトで描画します。|null
...
data-filtering-type |文字列|フィルタリングのローカルとリモートのいずれかを決定します。|null
data-filtering-case-sensitive |bool|フィルタリングの大文字小文字の区別を有効化／無効化します。|false
data-filtering-filter-expr-url-key |文字列|フィルタリング式がリモート要求用にどのようにエンコードされるかを指定する URL キー名です。たとえば、`&filter('col') = startsWith.`。デフォルト値は OData です。|null
...
data-filtering-search-bar-field-name |文字列|これはキーワード検索のフィールドを決めます。これが空の場合、すべてのフィールドを検索します。|””
data-filtering-search-bar-placeholder |文字列|これが空の時に検索バーにプレースホルダを表示するかどうかを決めます。|null
...
data-filtering-filter-presets-label |文字列|フィルター プリセットの上に表示されるテキストです。|null
data-filtering-filter-state |文字列|使用されるプリセットのインデックス。*デフォルト* はなし。|default
...
data-load-on-demand-type |文字列|ローディングのローカルとリモートのいずれかを決定します。|null
data-load-on-demand-page-size |number|*0* よりも大きい場合、最初にフェッチされてロードされる項目をいくつにするかをコントロールします。|10
data-load-on-demand-record-count-key |文字列|データ ソース中のレコード総数を保持する応答内のプロパティです。|null
data-load-on-demand-page-size-url-key |文字列|現在要求されているページ サイズが何であるかを示すエンコードされた URL パラメータの名前を示します。|null
data-load-on-demand-page-index-url-key |文字列|現在要求されているページ インデックスが何であるかを示すエンコードされた URL パラメータの名前を示します。|null
data-load-on-demand-mode |文字列|追加小目をロードするためのボタンを表示するか、自動的にフェッチするかを定めます。|ボタン
