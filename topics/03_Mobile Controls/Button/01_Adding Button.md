...
1. ビュー ページにリソースを追加します。
	...
	> **注:** `Infragistics.Web.Mvc.Mobile.dll` への参照と、Infragistics Mobile Loader への参照が必要です。次の例では、`js` および `css` ファイルのすべてが `ig_mobileui` という仮想ディレクトリーに置かれています。この手順を完了するためには、このフォルダーは、アプリケーションの正しい `js` と `css` の場所に従って、名前を変更する必要があります。
