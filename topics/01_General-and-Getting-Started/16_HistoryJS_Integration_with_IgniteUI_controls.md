...
# History.js と %%ProductName%% コントロールの統合

## トピックの概要

### 目的

%%ProductName%% コントロールはブラウザーの履歴機能を提供する history.js をサポートします。このトピックは、統合に必要な要件の説明および gGrid コントロールを history.js に統合する方法について示します。
...
%%ProductName%% コントロールは 完全に history.js と統合できます。%%ProductName%% コントロールのステートを保存する場合、後者のクライアント側イベント API を使用します。これらのイベントは %%ProductName%% コントロールの現在のステートを持ちます。使用できるステートはブラウザー履歴スタックにプッシュします。以下に igGrid を HistotyJS フレームワークと統合する方法の詳細を示します。
