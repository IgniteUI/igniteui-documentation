...
-   [%%ProductName%% NuGet パッケージのインストール ファイル](#whatIsInstalled)
-   [%%ProductName%% NuGet パッケージのアンインストール](#uninstalling)
-   [ASP.NET Core MVC の NuGet パッケージの使用](#aspnetcore)
...
## <a id="whatIsInstalled"></a> %%ProductName%% NuGet パッケージのインストール ファイル
...
## <a id="aspnetcore"></a> ASP.NET Core MVC の NuGet パッケージの使用

ASP.NET Core MVC の %%ProductName%% NuGet パッケージの 2017.2 以後のバージョンを使用する場合、ASP.NET Core アプリケーションの作成で次の情報に注意してください。%%ProductName%% 2017.2 バージョン以前、ASP.NET Core MVC の NuGet パッケージ名は Infragistics.Web.Mvc として配布されていました。2017.2 バージョンで、同じく Infragistics.Web.Mvc と呼ばれる MVC4 および MVC5 パッケージと区別するために、パッケージは Infragistics.Web.AspNetCore に名前変更されました。
...
スクリプト ファイルの追加後、%%ProductName%% コントロールを追加する .cshtml ページで使用できます。名前空間を以下のようにインポートする必要があります。

```js
@using Infragistics.Web.Mvc 
```

以下、Ignite UI スクリプトを参照します。たとえば、以下のようです:
...
Ignite UI スクリプトを使用する前に jQuery および jQueryUI を参照する必要があります。完了後、シナリオに応じて %%ProductName%% コントロールを作成できます。この例では、以下のコードを使用して数値エディターを作成します。
