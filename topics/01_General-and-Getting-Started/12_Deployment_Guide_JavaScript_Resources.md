...
# %%ProductName%% で JavaScript リソースを使用
...
このトピックでは、Web アプリケーションで %%ProductName%% を操作して、必要なリソースを管理する方法について説明します。
...
%%ProductName%% を使用して作業する場合、Web アプリケーションで Infragistics リソースを参照してください。これらのリソースは、3 種類の方法で参照できます。

-	**カスタム JavaScript ファイルを含む**: これは %%ProductName%% JavaScript ファイルを参照する方法です。%%ProductName%% コントロールの[カスタム ダウンロード](%%SamplesUrl%%/download)を作成できます。
...
%%ProductName%% カスタム ビルドを作成するには、[カスタム ダウンロード ページ](%%SamplesUrl%%/download)を参照してください。カスタム ビルドには 2 つの利点があります。1. アプリケーションで使用されるコントロールおよび機能のみをダウンロードすることにより、最小限の JavaScript で実行することができます。2. JavaScript を 1 つのファイルに結合し、ブラウザーがサーバーへの要求の数を減らします。この利点により、アプリケーションのパフォーマンスを向上します。
...
%%ProductName%% が機能するには、ベース JavaScript ファイルを含めていくつかのリソースを手動で参照する必要があります。
...
**Modernizr**、**JQuery**、および **JQuery UI** JavaScript ライブラリは、%%ProductName%% を含むプロジェクトに必ず必要なライブラリです。Modernizr ライブラリがブラウザの機能を検出し、これによってコントロールはタッチまたは非タッチ環境であるかを識別することができます。
...
%%ProductName%% は、英語と日本語のリソースで出荷されます。

Volume 2012.2 より製品の一部としてロシア語 ([ru])、ブルガリア語 ([bg])、ドイツ語 ([de])、スペイン語 ([es])、およびフランス語 ([fr]) のローカライズが含まれます。ローカライズ ファイルは、ローカライズ フォルダー `../js/i18n` に入れる必要があります (*js* は %%ProductName%% プログラムの JavaScript ファイルのルート フォルダーです)。
...
JavaScript ファイルは、jQuery エディターに日付、数字、通貨などのローカライズ フォーマットを提供します。これらは、*`../js/modules/i18n/regional`  にあります* (js は %%ProductName%% プログラムの JavaScript ファイルのルート フォルダーです)。
...
- [%%ProductName%% 対応 Infragistics コンテンツ配信ネットワーク (CDN)](Deployment-Guide-Infragistics-Content-Delivery-Network%28CDN%29.html): %%ProductName%% 対応 Infragistics Content Delivery Network (CDN) の使用方法。
