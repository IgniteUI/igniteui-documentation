...
デバッグ バージョンの例外を除き、すべての JavaScript ファイルは縮小されています。デバッグには、縮小されていないファイルを使用します。これらのファイルは縮小されたファイルと同じファイル構造を持ち、ファイル名も同じです。縮小されていないファイルは、.zip アーカイブで提供されます (`infragistics.jquery.debug.zip`)。
...
### 順 2: その他の必要なリソースをダウンロードおよび追加します。
