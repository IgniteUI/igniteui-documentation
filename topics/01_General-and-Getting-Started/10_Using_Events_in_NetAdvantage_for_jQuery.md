...
このトピックは、%%ProductName%%™ コントロールが発生させるイベントの処理方法について説明します。また、初期化と初期化後のイベントのバインドの違いについても説明します。
...
-   Infragistics.Web.Mvc.dll (ASP.NET MVC ラッパーを含む) への参照
...
-   jQuery および MVC に必要なスクリプトは同じですが、これは MVC ラッパーが jQuery ウィジットに似た JavaScript を描画するためです。次が必要になります。
	...
	2.  jQuery UI ライブラリ
	3.  ページで使用するウィジェットに必要な %%ProductName%% スクリプト ファイル
...
>**注:** : As of jQuery 1.7, the `live()` method is deprecated and as of jQuery 3.0 the `delegate()` and `bind()` methods are also deprecated. These methods were superseded by the `on()` and `one()` functions as preferred replacements.
...
>**注:** %%ProductName%% API ガイドでは、各コントロールのオプション、メソッド、イベントすべての一覧を参照できます。
...
- ***バインド:***
 ...
 このサンプルは、`igTextEditor` のインスタンスが MVC コンテキストで作成され、`valueChanged` イベントへバインドされています。ASP.NET MVC ヘルパーを使用して `igEditor` コントロールから継承したコントロールのインスタンスを作成する場合、レンダリング メソッドは適切な型の値で `igEditor` コントロールを生成し、エディターを構成します。そのため、イベントをバインドまたはバインド解除する場合、「igeditor」プレフィックスが必要です。
...
-   [ランタイム時の igEditors の構成](Configuring-igEditors-at-Runtime.html)
-   [igGrid イベント](%%jQueryApiUrl%%/ui.iggrid#events)
-   [igCurrencyEditor イベント](%%jQueryApiUrl%%/ui.igcurrencyeditor#events)
-   [igDateEditor イベント](%%jQueryApiUrl%%/ui.igdateeditor#events)
-   [igEditor イベント](%%jQueryApiUrl%%/ui.igeditor#events)
-   [igMaskEditor イベント](%%jQueryApiUrl%%/ui.igmaskeditor#events)
-   [igNumericEditor イベント](%%jQueryApiUrl%%/ui.ignumericeditor#events)
-   [igPercentEditor イベント](%%jQueryApiUrl%%/ui.igpercenteditor#events)
-   [igTextEditor イベント](%%jQueryApiUrl%%/ui.igtexteditor#events)
-   [igCombo イベント](%%jQueryApiUrl%%/ui.igcombo#events)
-   [igDatePicker イベント](%%jQueryApiUrl%%/ui.igdatepicker#events)
-   [igRating イベント](%%jQueryApiUrl%%/ui.igrating#events)
-   [igTree イベント](igtree-event-reference.html)
-   [igUpload イベント](%%jQueryApiUrl%%/ui.igupload#events)
-   [igVideoPlayer イベント](%%jQueryApiUrl%%/ui.igvideoplayer#events)

##<a id="_Related_Samples"></a>関連サンプル


-   [エディター API およびイベント](%%SamplesUrl%%/grid/editing-api-events)
-   [グリッド API およびイベント](%%SamplesUrl%%/grid/grid-api-events)
-   [ツリー API およびイベント](igtree-event-reference.html#attaching-handlers-jquery)
-   [ファイル アップロード API およびイベント](%%SamplesUrl%%/file-upload/api-events)
