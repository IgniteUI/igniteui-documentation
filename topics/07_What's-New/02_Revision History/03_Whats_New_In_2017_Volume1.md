...
このトピックでは、%%ProductFamilyName%%™ 2017 Volume 1 リリースのコントロールと新機能および拡張機能を紹介します。
...
igDataSource コンポーネントは、[`filterByText`](%%jQueryApiUrl%%/ig.datasource#methods:filterByText) メソッドによってすべてのフィールドで特定の単語または句を検索する方法を提供します。
...
[`enableUTCDates`](%%jQueryApiUrl%%/ui.iggrid#options:enableUTCDates) オプションが変更しました。日付のシリアル化のみに影響します。 日付はローカル時間およびゾーン値の代わりにクライアント側の日付を [UTC ISO 8061](https://en.wikipedia.org/wiki/ISO_8601#UTC) 文字列としてのシリアル化されます。

日付の表示を処理するには、新しい [`dateDisplayType`](%%jQueryApiUrl%%/ui.iggrid#options:columns.dateDisplayType) オプションはグリッドの列定義に設定できます。`date` 型の列のみに影響します。
