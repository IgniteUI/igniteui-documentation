...
[AddStartingUploadEventHandler](Infragistics.Web.Mvc.UploadProgressManager~AddStartingUploadEventHandler.html) メソッドを各のコントロールのために呼び出します。最初のパラメーターが関連する `igUpload` コントロールで定義される `ControlId` オプションと一致する必要があります。 
