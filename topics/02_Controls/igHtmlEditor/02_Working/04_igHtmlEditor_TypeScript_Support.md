...
# TypeScript サポート (igHtmlEditor)

## トピックの概要
本トピックは、TypeScript で `igHtmlEditor` を構成する方法について説明します。
...
-   [手順](#Steps)
    ...
    -   [TypeScript コードの追加](#typescript_code)
...
<a id="create_html_markup"></a>HTML の作成 - `igHtmlEditor` を 1 つ使用します。

**HTML の場合:**
```html
<div id="htmlEditor">
    高いパフォーマンスを提供する jQuery データ グリッドは、インタラクティブな操作が可能な HTML テーブルやデータを表示します。本リリースでは業務向け機能を多数追加しており、高いクオリティとパフォーマンスが提供されます。
</div>
...
<a id="typescript_code"></a>TypeScript コードの作成 - ツールバーを追加します。太字、イタリック、下線操作をサポートします。
