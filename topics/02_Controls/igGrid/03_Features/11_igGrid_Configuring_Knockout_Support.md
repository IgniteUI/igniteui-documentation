...
1.  バージョン 13.1 以降に必要な Ignite UI の JavaScript ファイルと CSS ファイル
...
- [Knockout サポート (エディター)](Configuring-Knockout-Support-%28Editors%29.html): このトピックは、Knockout ライブラリにより管理されるビューモデル オブジェクトをバインドするために Ignite UI エディター コントロールを構成する方法について説明します。
...
- [グリッド Knockout の結合](%%SamplesUrl%%/grid/bind-grid-with-ko): このサンプルでは、`igGrid` ウィジェットを使用する knockout.js バインディングのサポートを紹介します。この実装では、グリッドの選択された行は標準の TwoWay バインディングによってバインドされます。
