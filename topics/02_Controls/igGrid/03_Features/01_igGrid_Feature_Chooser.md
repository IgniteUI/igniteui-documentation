...
以下の画像は、グリッドで複数の機能が有効な場合、デスクトップで機能セレクターの外観を示します。

![デスクトップ環境の機能セレクター](images/feature-chooser-desktop.png)
...
![タッチ対応環境の機能セレクター](images/feature-chooser-touch.png)
...
ギア アイコンの表示動作はグリッドの [`featureChooserIconDisplay`](http://help.infragistics.com/jQuery/2014.2/ui.iggrid#options:featureChooserIconDisplay) オプションにより制御されます。このオプションのデフォルト値は `desktopOnly` です。操作は上記に説明があります。ギア アイコンをすべての環境で表示するには、`featureChooserIconDisplay` を always に設定します。
...
- [列チューザーの構成 (igGrid)](igGrid_Hiding_Column_Chooser.html)
- [igGrid 複数並べ替えモーダル](igGrid_Multiple_Sorting_Dialog.html)
- [igGrid フィルタリング](igGrid_Filtering.html)
- [Ignite UI コントロールのタッチ サポート](Touch_Support_for_NetAdvantage_for_jQuery_Controls.html)
