...
Ignite UI™ グリッド、つまり `igGrid` のページング機能では、オリジナル データ ソースから事前に定義された数のレコードだけフェッチすることで、パフォーマンスが向上できるよう、ページ中のデータを分割できます。
...
- **リスト 1: グリッド ページングに必要なスクリプトと CSS ファイル**
	...
	**HTML の場合:**
	...
	ページングに最小限必要な Ignite UI™ スクリプトのみを組み込む場合は、リスト 2 に示すようにスクリプトを参照するだけで組み込むことができます。
...
pageSizeDropDownLabel (“Show “)|ページ サイズ ドロップダウン ラベルの前に描画されるラベル テキスト。
pageSizeDropDownTrailingLabel (“records”)|ページ サイズ ドロップダウンの描画後に描画される後続ラベル テキスト。
...
pagerRecordsLabelTemplate ("$startRecord$ - $endRecord$ of $recordCount$ records")|カスタマイズに使用できるページャー レコード ラベルのテンプレート。
nextPageLabelText (“next”)|次ページの画像またはアイコン前に描画されるラベル テキスト。
prevPageLabelText (“prev”)|前ページの画像またはアイコン後に描画されるラベル テキスト。
...
currentPageDropDownLeadingLabel (“Pg”)|特定のページ インデックスの選択に使用するドロップダウンの前にあるスパン要素の先頭ラベル テキスト。
currentPageDropDownTrailingLabel ("of $count$")|上記と同じですが、後続ラベル スパンの場合に指定されます。
...
page|ページ番号を保持する `<UL>` のすべての `<li>` に適用されたクラス。|ui-iggrid-page ui-state-default ui-corner-all
...
pageCurrent |現在のページ インデックスに対応する `<li>` タグに適用されたクラス。|ui-iggrid-pagecurrent ui-state-active ui-corner-all
...
pageDropDownLabels|「Pg &lt;index&gt; Out of XXX」など、ページ インデックスを選択するドロップダウンの前後のラベルを保持する先頭および後続の SPAN に適用されます。|ui-iggrid-pagedropdownlabels
