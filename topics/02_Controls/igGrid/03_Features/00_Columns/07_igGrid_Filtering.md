...
`filterDialogFilterConditionTemplate` オプションのデフォルト値が変更されました。
...
|以前の動作|新しい動作|
|---|---|
|日付列にフィルターする場合、エディターのテキストが変更された際にフィルターは適用されます。| 日付列をフィルターする場合、エディター以外にフォーカスを移動するか、Enter キーを押すときのみにフィルターが適用されます。|
