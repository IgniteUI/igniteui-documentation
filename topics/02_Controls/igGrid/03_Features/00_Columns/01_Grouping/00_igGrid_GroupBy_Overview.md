...
グループ化データのグループ行は、グループ行の生成された ID を含むオブジェクトです。コードで行を [`collapse`](%%jQueryApiUrl%%/ui.iggridgroupby#methods:collapse) および [`expand`](%%jQueryApiUrl%%/ui.iggridgroupby#methods:expand) するために ID を使用します。
