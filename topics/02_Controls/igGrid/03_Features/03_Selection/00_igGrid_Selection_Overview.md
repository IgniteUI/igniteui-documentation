...
左矢印、右矢印、下矢印、上矢印の任意の矢印キーを使用して、キーボード ナビゲーションを利用できます。その他に次のようなキーボード ナビゲーションがあります。

-   セルまたは行が選択された状態で CTRL キーと SHIFT キーを押すと、それらは選択解除されます。
-   CTRL キーを押して、矢印キーを使用すると、セルと行のアクティブ化が切り替わります。SPACE キーを押すと、選択のみ変更されます。
-   したがって CTRL キーを押したまま SPACE キーを押すと、行またはセルの選択が切り替わります。つまり、それらが選択されている場合は選択解除され、選択解除されている場合は選択されます。CTRL キーを離し、いずれかの矢印キーを押すと、現在の既存の連続選択がクリアされ、アクティブになるため、選択された行またはセルも同じくアクティブになります (つまり、それにしたがって移動します)。これは基本的に Windows Explorer と同じ動作です。
...
`igGridSelection` を有効にする場合、[`persist`](%%jQueryApiUrl%%/ui.iggridselection#options:persist) モードにあります。つまり、選択した行およびセルは、グリッドの並べ替え、列フィルタリングなどのその他の機能と操作したか、データ バインドした後に選択は残ります。
...
ユーザーがグリッドと操作した後に選択をクリアする現在の動作に戻るには、[`persist`](%%jQueryApiUrl%%/ui.iggridselection#options:persist) オプションで機能を無効にできます。以下はコード スニペットです。
...
行選択を有効にすると、ユーザーはいずれかの行のセルをクリックして、行を選択できます。また、このドキュメントで以前にリストした Cell Selection の例に示すように、公開されたメソッドを使用してコードの行を選択することもできます。

### <a id="cell-selection"></a> セル選択 

セル選択を有効にするには、選択動作を初期化する際に選択モードを「cell」に設定する必要があります。

セル選択を有効にした後でセルを選択する方法はいくつかあります。まず、セルをクリックするか、キーボードでセルまでナビゲートすることでセルを選択できます。また、このドキュメントで以前にリストした Cell Selection の例に示すように、メソッドを使用してコードの行を選択および選択解除することもできます。
...
rowSelectionChanging
            ...
            <td>
**row:** 現在選択されているグリッド行。 <br />

**selectedRows:** 現在選択されている行の配列。 <br />

**selectedFixedRows:** ある場合、固定行の配列 <br />

**owner:** 選択ウィジェット オブジェクトへの参照。 <br />

**startIndex:** 開始行のインデックス (複数選択) – オプション <br />

**endIndex:** 終了行のインデックス (複数選択) – オプション
            </td>
        </tr>
...
`rowSelectionChanging` と同じですが、行「selectedRows」には選択に追加された新しい行が入っています。
				<blockquote>
**注:** このイベントには `startIndex` プロパティと `endIndex` プロパティはありません。
                </blockquote>
			</td>
        </tr>
...
**firstRowIndex:** 選択された最初の行のインデックス。 <br />

**lastRowIndex:** 選択された最後の行のインデックス。 <br />

**firstColumnIndex:** 選択された最初の列のインデックス。 <br />

**lastColumnIndex:** 選択された最後の列のインデックス。 <br />

				<blockquote>
**注:** 最後の 4 つのプロパティは、連続複数選択の場合にのみ使用されます。
                </blockquote>

**selectedCells:** 現在選択されているセルの配列。 <br />

**owner:** 選択ウィジェット オブジェクトへの参照。
			</td>
        </tr>
...
-   [ページング (igGrid)](igGrid-Paging.html)
-   [フィルタリング (igGrid)](igGrid-Filtering.html)
-   [並べ替えの概要 (igGrid)](igGrid-Sorting-Overview.html)

### <a id="samples"></a> サンプル

-   [選択](%%SamplesUrl%%/grid/selection)
