...
-   [**更新を有効にする**](#enable)
    ...
    -   [CSS および JavaScript参照を静的に読み込む – 更新のみに必要](#minimal-required)
...
### <a id="user-interactions"></a> ユーザー相互作用チャート 
...
更新をコミットして次のセルを編集|TAB キーを押下|現在のセルに対する更新をコミットし、次の編集可能なセルを編集モードにします。次のセルは、同じ行のすぐ右側のセルになり、最後の列に値をコミットした場合は、次の行の最初の編集可能なセルになります。この操作は、`editMode` が セルに設定されている時のみサポートされます。
...
## <a id="column-settings-editors"></a> 列の設定およびエディター 
...
[readOnly](%%jQueryApiUrl%%/ui.igGridUpdating#options:columnSettings.readOnly) (false)|列のセルを編集不可にします。
[validation](%%jQueryApiUrl%%/ui.igGridUpdating#options:columnSettings.validation) (false)|igEditor の値の検証を有効にします。
[defaultValue](%%jQueryApiUrl%%/ui.igGridUpdating#options:columnSettings.defaultValue) (null)|Add-New-Row の列のセルのデフォルト値。
...
### <a id="datadirty-event"></a> dataDirty イベントの処理
...
### <a id="example-committing-data"></a> 例: dataDirty イベントでのデータのコミット 
...
### <a id="programmatically-start-edit"></a> セルをプログラムで開始および編集 (UI の更新を使用) 
...
## <a id="localization"></a> ローカライズのサポート 
...
### <a id="api-delete-row"></a> 行をプログラムで削除 
...
### <a id="api-update-row"></a> 行をプログラムで更新 
**JavaScript の場合:**
...
## <a id="customization"></a> カスタマイズの更新 
...
### <a id="setting-option"></a> 初期化後の更新機能オプションの設定 
**JavaScript の場合:**
