...
機能は [$.ig.DataSource](%%jQueryApiUrl%%/ig.datasource) のページング機能の一番上に構築されるため、`$.ig.DataSource` でサポートされるすべての範囲のデータ ソースをサポートします。しかし、この機能はページング機能自体と互換性がありません。統合シナリオのサポートについては、｢[機能マトリックス (igGrid)](Feature-Compatibility-Matrix%28igGrid%29.html%29｣のトピックを参照してください。

![]%28images/igGrid_Append_Rows_on_Demand_Overview_1.png)
...
前述したように、この機能の名称は、他の機能との混同を避けるために「ロード オン デマンド｣から｢オンデマンドによる行追加｣に変更されました。Ignite UI の以前のバージョンからアップグレードする場合、以下の情報を知っておく必要があります。
...
-   機能の名称は「igGridLoadOnDemand」から [igGridAppendRowsOnDemand](%%jQueryApiUrl%%/ui.igGridAppendRowsOnDemand) に変更されています。ユーザーのすべての API 呼出しを、新しい機能名に置き換える必要があります。
-   igGrid [features](%%jQueryApiUrl%%/ui.iggrid_tg#options:features) オプションの配列では、機能の名称が「LoadOnDemand」から「AppendRowsOnDemand」に変更されています。
...
- [機能マトリックス (igGrid)](Feature-Compatibility-Matrix%28igGrid%29.html)

### <a id="samples"></a>サンプル

このトピックについては、以下のサンプルも参照してください。

- [オン デマンドによる行追加](%%NewSamplesUrl%%/grid/append-rows-on-demand)
