...
物理セル結合を *"duplicate"* 形式で初期化するためにデフォルトを使用してオプションを設定する必要はありません。
...
この場合、セル結合機能はデータおよび表示方法を最初から正しく予測できません。ここで、カスタム `mergeStrategy` を使用します。結合される列の以前のレコード、現在のレコード、および列のキーを受け取る関数を割り当てることが可能です。この関数を使用すると、上記のカスタム要件は以下のように実装できます。
