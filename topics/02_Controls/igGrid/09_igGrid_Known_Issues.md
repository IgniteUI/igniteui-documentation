...
[サイズ変更が固定仮想化を使用しても機能しない](#resizing-virtualization)|固定仮想化が有効になっていると、列のサイズ変更機能は動作しません。| ![](images/positive.png)
...
グリッドが右側にスクロールされた場合、列のサイズ変更はより速いスピードで行います。|グリッドが右側にスクロールされ、列が右端から左へドラッグしてサイズ変更される場合、サイズ変更スピードはより早くなります。この動作はブラウザーのレイアウト エンジンによるものです。|![](images/negative.png)
...
>**回避策 1**: 連続仮想化を使用します。行の高さが動的にサイズ変更されるシナリオで操作します。

>**回避策 2**: 動的にサイズ変更すると、行の高さが変更されないことを確認するセルのスタイル設定を適用します。たとえば、テキストの長さが行の高さに影響しないために、セルに “white-space:nowrap” を適用します。
...
## igGridFiltering
### <a="simple-filtering"></a> 簡易フィルタリングが列の仮想化で機能しない
