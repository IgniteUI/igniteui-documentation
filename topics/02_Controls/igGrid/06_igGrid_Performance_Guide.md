...
仮想化がサポートされる機能に制限を追加するので、グリッド構成のデザインに注意してください。詳細については、[機能互換性マトリックス (igGrid)](Feature-Compatibility-Matrix(igGrid).html) および[既知の問題と制限 (igGrid)](igGrid-Known-Issues.html) トピックを参照してください。 ページングは仮想化よりも他の igGrid 機能とより良く統合できるため、ローカルまたはリモート ページングの使用を推薦します。
