...
`igDialog` はモーダルに設定できます。その場合、背後のコンテンツはすべて無効で、非表示になりまる。モーダル ダイアログは複数表示できます。複数のモーダル ダイアログ インスタンスがあるとき、最後に開かれたダイアログがページの先頭になります。1 ページで複数のモーダル ダイアログを処理するときは、[複数のダイアログ](igDialog-Multiple-Dialogs.html) トピックを参照してください。以下の例で示したのは、1 つのモーダル ダイアログの構成方法だけです。
