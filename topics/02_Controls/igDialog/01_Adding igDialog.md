...
コントロールのインスタンスを作成する方法はいくつかかあります。このトピックでは、一般的な jQuery UI メソッドの作成方法、(属性を使用することによる) jQuery モバイル メソッドの作成方法、および MVC ダイアログ ラッパーを使用したコントロール インスタンスの作成方法を示します。
...
> **注:** MVC ダイアログ ラッパーを使用する際には、***Infragistics.Web.Mvc*** dll への参照をプロジェクトに追加することを忘れないでください。
...
> `Dialog.ContentID(“igDialog1”)` - # を付けずにセレクターを定義します。この場合、セレクターは、MVC ラッパーによって自動的に表示されます。
