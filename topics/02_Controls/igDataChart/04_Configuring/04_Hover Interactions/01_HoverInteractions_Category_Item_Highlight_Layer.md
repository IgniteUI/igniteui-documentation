...
以下のスクリーンショットは、以下の設定の結果、`categoryItemHighlightLayer` オブジェクトの `bandHighlightType` および `brush` プロパティを持つ `igDataChart` コントロールの外観がどのようになるか示します。
...
![](images/jQuery_Item_Highlight_Layer_02.png)
