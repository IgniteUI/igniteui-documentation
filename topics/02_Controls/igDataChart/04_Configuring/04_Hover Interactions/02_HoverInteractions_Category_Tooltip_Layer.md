...
toolTipStyle|style|このプロパティは、表示されるヒントに適用するスタイルを指定します。デフォルトではヒントはチャートからスタイルを継承しますが、このプロパティを設定するとこのデフォルト設定を上書きできます。
...
以下のスクリーンショットは、以下の設定で構成された `categoryTooltipLayer` オブジェクトの `toolTipPosition` プロパティで `igDataChart` コントロールを描画する方法を示します。
...
![](images/jQuery_Category_Tooltip_Layer_02.png)
