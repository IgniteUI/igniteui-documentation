...
3. MVC ビューを追加します。
 ...
 新しい空の MVC ビューを作成するには、コントローラーの `Index()` メソッドを右クリックします。コンテキスト メニューから、[ビューを追加] を選択します。[igDataChart の追加](igDataChart-Adding.html)のトピックのステップに従って、チャートを組み込むスケルトン MVC ビューを作成します。この例では、最初のステップ[必要なリソースへの参照の追加](igDataChart-Adding.html#_AddReferencesToRequiredResources)が必要です。
