...
新しいコンボおよびその Knockout 拡張子の更新され、アーキテクチャの変更があります。コンボの選択コレクションがドロップダウンの項目に利用可能な値のみを許可するためにデザインされました。Knockout 拡張子はそのパターンを使用し、コンボの変更をモニターする監視可能な項目は選択された項目の配列です。 
以前のバージョンで拡張子がコンボ フィールドをモニターしました。新しい実装では、単一の選択だけでなく、複数の選択された項目をモニターできます。 

この方法で、入力監視可能要素が監視可能配列として定義できないため、igCombo を HTML 入力要素にバインドできません。igEditors およびその拡張子は、HTML 入力要素と操作するためにデザインされました。igCombo を HTML 入力要素と結合するには、以前のセクション (Knockout 結合の変更) に説明した実装を使用できます。 
