...
autoComplete|入力フィールドのテキストをドロップダウン リストの最初に一致する項目でオートコンプリートするために使用されていました。|このオプションは現在サポートされていません*。
headerTemplate footerTemplate|ヘッダー / フッター テンプレートを指定するために使用されていました。|このオプションは現在サポートされていません*。
...
*古いコンボと新しいコンボの動作の等価性において、15.1 の一部で確認作業が完了しておりません。これらの動作がお客様のソリューションにおいて重大で、また 15.1 へのアップグレードをご希望の場合は、弊社までお問い合わせください。
...
noMatchFound|`autoComplete` 機能が現在サポートされていないため、このイベントは削除されました。
