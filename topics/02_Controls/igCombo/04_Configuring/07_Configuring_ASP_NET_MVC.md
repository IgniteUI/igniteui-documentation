...
ビューでコンボをインスタンス化するために igCombo ASP.NET MVC ヘルパーが使用されます。また、従業員のコレクションのリモート要求を処理し、リモート フィルタリング パラメーターを処理するために `ComboDataSourceAction` 属性が使用されます。最後に、フォームで モデルのフィールドを更新するために使用されるコンボを確認できます。

#### 概念

-   ASP.NET MVC Helper として igCombo を使用します。
...
-	[igCombo データ バインドについての概要](igCombo-Data-Binding.html): このトピックでは、`igCombo` コントロールでの各種データ バインド方式について説明し、データ バインディングに関するその他の詳細情報を示します。
