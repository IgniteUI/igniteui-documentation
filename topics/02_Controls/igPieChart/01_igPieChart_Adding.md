...
1. Add references to the required resources.
	...
	**Referencing resources in MVC using the MVC Loader**
	...
	For MVC views %%ProductNameMVC%% Loader must be used:
	...
	**In ASPX:**
	...
	The %%ProductNameMVC%% Loader automatically detects required resources and specifying resources is not necessary.
...
2. Add HTML markup required by the `igPieChart`.
	...
	**ASP.NET Example**
	...
	For ASP.NET MVC, no container elements are needed because the %%ProductNameMVC%% adds the required markup automatically.
...
4. Instantiate the pie chart.
	...
	**ASP.NET example**
	...
	The code below instantiates and sets the main features of the `igPieChart` using the %%ProductNameMVC%% PieChart provided in the `Infragistics.Web.Mvc` assembly. The data model is associated with the control with the PieChart(Model) call and the rest of the calls act similarly to the HTML example.
...
- [jQuery and %%ProductNameMVC%% API Reference Links (igPieChart)](igPieChart-API-Links.html): This topic provides links to the API documentation for jQuery and %%ProductNameMVC%% class for `igDataChart`™ control.
