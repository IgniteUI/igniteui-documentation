...
この設定は、コントロールの [`orientation`](%%jQueryApiUrl%%/ui.igBulletGraph#options:orientation) プロパティで定義されます。
...
スケールの方向は、コントロールの [`isScaleInverted`](%%jQueryApiUrl%%/ui.igBulletGraph#options:isScaleInverted) プロパティで定義されます。デフォルトの方向は、標準です。
...
構成可能な項目|詳細|プロパティ
---|---|---
スケールの向き|コントロール内のブレット グラフのスケールの向き (水平または垂直)。|[orientation](%%jQueryApiUrl%%/ui.igBulletGraph#options:orientation)
スケールの方向|ブレット グラフの方向 (標準または反転)。|[isScaleInverted](%%jQueryApiUrl%%/ui.igBulletGraph#options:isScaleInverted)


## <a id="scale-orientation"></a> スケールの向きの構成

ブレット グラフの向き (水平または垂直) は、コントロールの [`orientation`](%%jQueryApiUrl%%/ui.igBulletGraph#options:orientation) プロパティで指定します。
...
目的:|使用するプロパティ:|設定の選択肢:
--------------|--------------------|--------------
水平方向を指定する|[`orientation`](%%jQueryApiUrl%%/ui.igBulletGraph#options:orientation)|*horizontal*
...
水平方向では、スケールの標準 (デフォルト) 方向は「左から右」で、これはスケールが [**グラフ領域**](igBulletGraph-Overview.html#logical-areas) の左端から開始され、右端で終了することを意味します ([`scaleStartExtent`](%%jQueryApiUrl%%/ui.igBulletGraph#options:scaleStartExtent) はグラフ領域の左端の始まりを示し、[`scaleEndExtent`](%%jQueryApiUrl%%/ui.igBulletGraph#options:scaleEndExtent) は、グラフ領域の左端からスケールの終りまでの距離を示します)。
...
目的:|使用するプロパティ:|設定の選択肢:
---|---|---
標準方向の構成|[`isScaleInverted`](%%jQueryApiUrl%%/ui.igBulletGraph#options:isScaleInverted)|“false”
...
### <a id="topics"></a>トピック
- [視覚要素の構成 (*igBulletGraph*)](igBulletGraph-Configuring-the-Visual-Elements.html): このトピック グループは、igBulletGraph コントロールの視覚要素 (スケール要素、パフォーマンス バー、比較マーカーおよび範囲など) を詳細に説明し、コード例を使用してコントロールの視覚要素を構成する方法を示します。
...
- [垂直方向](%%SamplesUrl%%/bullet-graph/vertical-orientation): このサンプルでは、igBulletGraph コントロールの方向を変更し、スケールを反転する方法を紹介します。
