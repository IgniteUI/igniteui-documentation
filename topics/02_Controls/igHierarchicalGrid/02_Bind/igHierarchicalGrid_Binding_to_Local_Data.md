...
階層グリッドを正しく構成するためには、以下のオプションこのサンプル JSON データを使用する
...
[columnLayouts](%%jQueryApiUrl%%/ui.igHierarchicalGrid#options:dataSource) | 子グリッドの構造を指定する columnLayout オブジェクトのリスト。 | 列レイアウトのリスト。列レイアウト オブジェクトのオプションは igGrid の| 
[columnLayouts.key](%%jQueryApiUrl%%/ui.igHierarchicalGrid:columnLayouts.key) | これは、現在の列レイアウトのデータ レコードを保持するプロパティです。|columnLayouts: [{  key: "Orders", ...}]
...
|オプション | 説明 | 設定値|
--------|-------------|--------
[searchField](%%jQueryApiUrl%%/ig.dataschema#options:schema.searchField)|フィールドを定義するために Xpaths ベース ノードとして使用されます。|"OrgChart"
