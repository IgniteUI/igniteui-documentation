...
Ignite UI™ 日付エディター、つまり `igDateEditor` は日付に書式設定されたデータを編集できる入力フィールドを描画するコントロールです。`igDateEditor` コントロールはローカライズおよび様々な地域オプションをサポートします。
...
>**注:** [`dateInputFormat`](%%jQueryApiUrl%%/ui.igdateeditor#options:dateInputFormat) オプションはほとんどの短い数値指定子をサポートしません (たとえば行間 0 なしの月数の `"M"`)。値が編集中または入力しなかった場合は適用できません。この場合、エディターは月のために 2 つのプロンプトを提供し、編集モードで `"MM"` を結果的に使用しています。
