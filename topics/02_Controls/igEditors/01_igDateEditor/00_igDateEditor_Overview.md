...
日付の単一フィールドを変更すると、エディターは新しく作成された日付が正しいかどうかを検証します。正しくない場合は再計算されます。Fたとえば、最初に入力された日付値が 2016/01/31 で、月を 2 月に変更します。2 月は 29 日迄のため、エディターは日フィールドを再計算します。日付は 2016/02/29 になります。もう 1 つの例について説明します。日フィールドのみを変更した結果はどうなるでしょう。たとえば、2016/02/29 の日付を手動で 2016/02/30 に変更すると、エディターは日フィールドを 29 に戻します。存在しない日付の場合、存在する日付に変更されます。

特定の月の最後の日に設定されるその他のケースもあります。その場合、編集モードで月のみを入力した場合、結果はどうなるでしょう。エディターは JavaScript Date オブジェクトを使用して日付を作成しようとします。次の例で詳しく見てみましょう。最初に設定された値は 2016/01/31 で、エディターが編集モードに入り、月フィールドに 2 を入力します。その場合、編集モードの日付は `__/_2/__` になります。エディターは以前使用された年および日を使用しますが、2 月は 29 日迄です。この日付 (2016/02/31) は 2 月最終日の 2 日後ですので、JavaScript オブジェクトは 2016/02/29 に 2 日を追加し、日付は 2016/03/02 になります。
