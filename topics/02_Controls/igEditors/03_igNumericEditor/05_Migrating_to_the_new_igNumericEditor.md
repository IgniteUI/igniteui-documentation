...
isLimitedToListValues|<a name='isLimitedToListValues'></a>リスト項目のみに値の設定を許可する機能を設定します。この検証は、エディターをぼかす場合、または Enter キーが押された場合のみに実行されます。`$('#numericEditor').igNumericEditor({ listItems: [10, 25, 50, 100], isLimitedToListValues: true })`。
...
spin|パラメーターに従って、エディター内の値を増分 (正の差分) または減分 (負の差分) します。|このメソッドは、`spinUp` と `spinDown` という新しい 2 つのメソッドに置き換えられました。このメソッドは値のみに動作し、ドロップダウンのナビゲーションには影響を与えません。ドロップダウンのナビゲーションには、[selectListIndexUp](#selectListIndexUp) または [selectListIndexDown](#selectListIndexDown) メソッドを使用します。
...
selectListIndexUp|<a name='selectListIndexUp'></a>リスト内の現在の項目より上に表示されている項目に選択インデックスを移動します。
selectListIndexDown|<a name='selectListIndexDown'></a>リスト内の現在の項目より上に表示されている項目に選択インデックスを移動します。
clearButton|<a name='clearButton'></a>エディターのクリア ボタンへの参照を返します。
insert|<a name='insert'></a>カレットの位置にテキストを貼り付けます。
spinUp|<a name='spinUp'></a>パラメーターに従って、エディター内の値を増分します。
spinDown|<a name='spinDown'></a>パラメーターに従って、エディター内の値を減分します。
