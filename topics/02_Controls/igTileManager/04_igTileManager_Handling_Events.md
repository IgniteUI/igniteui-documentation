...
- [igTileManager の追加](igTileManager-Adding.html): このトピックでは、主要機能、最小要件およびユーザー機能性など、`igTileManager` コントロールについて概念的な情報を提供します。
...
イベント|説明
---|---
[dataBinding](%%jQueryApiUrl%%/ui.igtilemanager#options:dataBinding)|データのバインド前に発生します。
[dataBound](%%jQueryApiUrl%%/ui.igtilemanager#options:dataBound)|データのバインドが完了した後、描画がまだ開始されていないときに発生します。
[rendered](%%jQueryApiUrl%%/ui.igtilemanager#options:rendered)|ウィジェットが作成され、それが描画する DOM をすべてアタッチした後に発生します。
[rendering](%%jQueryApiUrl%%/ui.igtilemanager#options:rendering)|ウィジェットの描画を開始する前に発生します。項目説明が明示的に提供されず内部的に生成される場合、この時点で項目説明が存在していることが必要です。
[tileMaximized](%%jQueryApiUrl%%/ui.igtilemanager#options:tileMaximized)|アニメーションの最大化が完了した後に発生します。
[tileMaximizing](%%jQueryApiUrl%%/ui.igtilemanager#options:tileMaximizing)|最小化された状態から最大化された状態にタイルを移動する前に発生します。
[tileMinimized](%%jQueryApiUrl%%/ui.igtilemanager#options:tileMinimized)|アニメーションの最小化が完了した後に発生します。
[tileMinimizing](%%jQueryApiUrl%%/ui.igtilemanager#options:tileMinimizing)|最大化された状態から最小化された状態にタイルを移動する前に発生します。
[tileRendered](%%jQueryApiUrl%%/ui.igtilemanager#options:tileRendered)|タイルが描画された後に発生します。
[tileRendering](%%jQueryApiUrl%%/ui.igtilemanager#options:tileRendering)|タイルの描画を開始する前に発生します。
...
この例は、初期化時に [rendered](%%jQueryApiUrl%%/ui.igtilemanager#options:rendered) イベントにイベント処理関数を割り当てます。
...
この例は、実行時に [rendered](%%jQueryApiUrl%%/ui.igtilemanager#options:rendered) イベントにイベント ハンドラーを割り当てます。
...
- [タイル マネージャーの JSON バインド](%%SamplesUrl%%/tile-manager/bind-json): このサンプルでは、タイル マネージャー コントロールを JSON データ ソースにバインドする方法を紹介します。

- [タイル マネージャーの項目構成](%%SamplesUrl%%/tile-manager/item-configurations): このサンプルは、`igTileManager` 内部で各項目の位置およびサイズを構成する方法を紹介します。
