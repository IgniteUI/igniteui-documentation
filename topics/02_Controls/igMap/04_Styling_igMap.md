...
>**注:** Ignite UI のベース テーマはマップには不要で、マップのみ表示されたページでは問題なく省略できます。
...
Ignite UI は、`igMap` コントロールで使用できる以下のテーマを提供しています。
...
プロパティ|説明|関連先
---|---|---
[backgroundContent](%%jQueryApiUrl%%/ui.igMap#options:backgroundContent)|マップをプロットする場合に使用する、背景コンテンツ プロバイダーを構成します。現在サポートされているプロバイダーは Bing® Maps、CloudMade Maps、OpenStreetMap です。詳細は、[マップ プロバイダーの構成 (igMap)](igMap-Configuring-Map-Provider.html) のトピックを参照してください。|すべてのシリーズ
