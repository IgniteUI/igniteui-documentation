...
画面上にコントロールをレンダリングするには、データ配列で定義されるデータ ポイントを三角形に変換する必要があります。それによって、色分けされたすべての領域の正しい位置とサイズを特定できるようになります。三角測量のプロセスの詳細は、[関連内容: 外部リソース](igMap-Configuring-Geographic-Scatter-Area-Series.html#external-resources)セクションのリンク先を参照してください。
