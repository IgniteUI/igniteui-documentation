...
以下の表は、`igMap` コントロールがサポートする **Bing Maps** の画像セットをまとめたものです。スタイル コードを [backgroundContent.imagerySet](%%jQueryApiUrl%%/ui.igMap#options:backgroundContent.imagerySet) オプションに割り当てることによって、選択したスタイルを構成します。以下の表カタログは、**Bing Maps** スタイルから利用できます。[imagerySet](%%jQueryApiUrl%%/ui.igMap#options:imagerySet)
