...
<Sample src="/lists/tree-basic-sample" height={400} alt="Angular Tree Example" />
...
<ApiLink type="TreeNode" /> is the representation of every node that belongs to the <ApiLink type="Tree" />.  
Nodes provide <ApiLink type="TreeNode" member="disabled" label="disabled" />, <ApiLink type="TreeNode" member="active" label="active" />, <ApiLink type="TreeNode" member="selected" label="selected" /> and <ApiLink type="TreeNode" member="expanded" label="expanded" /> properties, which give you opportunity to configure the states of the node as per your requirement.
<ApiLink type="TreeNode" member="data" label="data" /> property can be used to add a reference to the data entry the node represents. Binding `[data]` is required for searching through nodes using <ApiLink type="Tree" member="findNodes" label="IgxTreeComponent.findNodes()" />.
...
<ApiLink type="TreeNode" /> could be expanded or collapsed:

- by clicking on the node expand indicator _(default behavior)_.
- by clicking on the node if the `igx-tree` <ApiLink type="Tree" member="toggleNodeOnClick" label="toggleNodeOnClick" /> property is set to `true`.
...
By default, multiple nodes could be expanded at the same time. In order to change this behavior and allow expanding only single branch at a time, the <ApiLink type="Tree" member="singleBranchExpand" label="singleBranchExpand" /> property could be enabled. This way when a node is expanded, all of the others already expanded branches in the same level will be collapsed.
...
You can find a specific node within an IgxTree by using the <ApiLink type="Tree" member="findNodes" label="findNodes" /> method. It returns an array of nodes, which match the specified data.
...
Additionally, by using the <ApiLink type="Tree" member="expandIndicator" label="expandIndicator" /> input you have the ability to set a custom template to be used for rendering the expand/collapse indicators of nodes.
...
In order to setup node selection in the `igx-tree`, you just need to set its <ApiLink type="Tree" member="selection" label="selection" /> property. This property accepts the following three modes: **None**, **BiState** and **Cascading**. Below we will take a look at each of them in more detail.
...
To enable bi-state node selection in the `igx-tree` just set the <ApiLink type="Tree" member="selection" label="selection" /> property to **BiState**. This will render a checkbox for every node. Each node has two states - selected or not. This mode supports multiple selection.
...
<Sample src="/lists/tree-advanced-sample" height={400} alt="Tree Load On Demand Example" />

After the user clicks the expand icon, it is replaced by a loading indicator. When the <ApiLink type="TreeNode" member="loading" label="loading" /> property resolves to `false`, the loading indicator disappears and the children are loaded.
...
Following the simplest approach, we create a new theme that extends the <ApiLink kind="sass" module="themes" type="function-tree-theme" label="tree-theme" /> and provide just the `$background` parameter, the theme will automatically calculate all the other necessary colors, of course you can override any of the other properties:
...
<Sample src="/lists/tree-styling" height={400} alt="Tree Styling" />
...
You can find the full list of properties in the <ApiLink kind="sass" module="themes" type="function-tree-theme" label="IgxTree Theme" />. The syntax is as follows:
