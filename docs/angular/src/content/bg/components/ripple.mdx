...
The <ApiLink type="RippleDirective" label="igxRipple" /> uses the Web Animation API and runs natively on [browsers that support it.](http://caniuse.com/#feat=web-animation)
...
By default, the ripple effect starts from the position of the click event. You can change this behavior using the <ApiLink type="RippleDirective" member="centered" label="igxRippleCentered" /> property and setting the center of the element as origin.
...
We can use the <ApiLink type="RippleDirective" member="rippleDuration" label="igxRippleDuration" /> property to change the duration of the ripple animation, which, by default, is set to 600 milliseconds.
...
Use the <ApiLink type="RippleDirective" member="igxRippleTarget" label="igxRippleTarget" /> property to attach a ripple effect to a specific element inside a parent element.
