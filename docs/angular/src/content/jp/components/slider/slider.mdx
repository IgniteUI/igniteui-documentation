...
上記では、<ApiLink suffix={false} type="ThumbFromTemplateDirective" /> ディレクティブと <ApiLink suffix={false} type="ThumbToTemplateDirective" /> ディレクティブの両方を使用して、カスタム `label` テンプレートを提供する方法を意図的に示しました。直感的に <ApiLink suffix={false} type="ThumbFromTemplateDirective" /> は <ApiLink type="Slider" member="lowerLabel" label="lowerLabel" />に対応し、<ApiLink suffix={false} type="ThumbToTemplateDirective" /> は <ApiLink suffix={false} type="ThumbToTemplateDirective" /> に対応すると想定できます。 <br />
...
<ApiLink type="TickLabelsOrientation" kind="variable" prefixed={false} label="TickLabelsOrientation" /> を <ApiLink kind="variable" type="TickLabelsOrientation" prefixed={false} /> に設定してすべての表示ラベルを回転します。
