...
`16.0.0` 以降、`IgxSliderComponent` はスタンドアロン コンポーネントです。直接インポートするか、コンポーネントとそのすべてのサポート コンポーネントおよびディレクティブを含む `IGX_SLIDER_DIRECTIVES` トークンを使用してください。
...
NgModule を使用するアプリケーションの場合は、**app.module.ts** で `IgxSliderModule` をインポートします:
...
<Aside type="caution">
`IgxSliderModule` は後方互換性のためにのみ保持されています。新しいプロジェクトでは、スタンドアロンの `IGX_SLIDER_DIRECTIVES` または `IgxSliderComponent` のインポートを使用してください。
</Aside>
...
最初に、スライダーの <ApiLink type="Slider" member="type" label="type" /> を <ApiLink kind="enum" type="SliderType" prefixed={false} member="Range" /> に設定します。次に、スライダー値を `lower` と `upper` 値のプロパティを持つオブジェクトにバインドします。
...
上記では、<ApiLink kind="interface" prefixed={false} suffix={false} type="SliderThumbFrom" /> ディレクティブと <ApiLink kind="interface" prefixed={false} suffix={false} type="SliderThumbTo" /> ディレクティブの両方を使用して、カスタム `label` テンプレートを提供する方法を意図的に示しました。直感的に <ApiLink kind="interface" prefixed={false} suffix={false} type="SliderThumbFrom" /> は <ApiLink type="Slider" member="lowerLabel" label="lowerLabel" />に対応し、<ApiLink kind="interface" prefixed={false} suffix={false} type="SliderThumbTo" /> は <ApiLink kind="interface" prefixed={false} suffix={false} type="SliderThumbTo" /> に対応すると想定できます。 <br />
...
<ApiLink type="TickLabelsOrientation" kind="enum" prefixed={false} member="range" label="TickLabelsOrientation" /> を <ApiLink kind="enum" type="TickLabelsOrientation" prefixed={false} /> に設定してすべての表示ラベルを回転します。
...
| イベント | 説明 | キャンセル可能 |
|--------|------|:----------:|
| `valueChange` | スライダーの値が変更されたときに発生します。`oldValue` と `value` を含む `ISliderValueChangeEventArgs` を提供します。 | いいえ |
| `lowerValueChange` | 範囲スライダーで下側のつまみの値が変更されたときに発生します。新しい `number` 値を提供します。 | いいえ |
| `upperValueChange` | 範囲スライダーで上側のつまみの値が変更されたときに発生します。新しい `number` 値を提供します。 | いいえ |
| `dragFinished` | ユーザーがつまみのドラッグを終了したときに発生します。最終値 (`number` または `IRangeSliderValue`) を提供します。 | いいえ |

`valueChange` を使用してスライダーの移動に応答します。
...
public onValueChange(args: ISliderValueChangeEventArgs) {
    console.log('以前の値:', args.oldValue, '新しい値:', args.value);
}
```

```html
<igx-slider [minValue]="0" [maxValue]="100" (valueChange)="onValueChange($event)">
</igx-slider>
```

範囲スライダーでは、個々のつまみの動きを追跡するために `lowerValueChange` と `upperValueChange` を使用するか、ユーザーがリリースした時のみ処理するために `dragFinished` を使用します。

```html
<igx-slider type="range"
    (lowerValueChange)="onLower($event)"
    (upperValueChange)="onUpper($event)"
    (dragFinished)="onDragFinished($event)">
</igx-slider>
```
