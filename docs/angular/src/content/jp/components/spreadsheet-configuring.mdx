...
Angular `Spreadsheet` は、コントロールの `IsFormulaBarVisible` プロパティを設定して数式バーの表示/非表示を設定できます。
...
<ApiLink type="SpreadsheetCellRange" pkg="spreadsheet" suffix={false} prefixed={false} /><br />
<ApiLink type="SpreadsheetSelection" pkg="spreadsheet" suffix={false} prefixed={false} /><br />
