...
<DocsAside type="info" title="情報">
サンプルでは、[Bootstrap Light](themes/sass/schemas.mdx#predefined-schemas) スキーマを使用します。
</DocsAside>

### Tailwind によるスタイル設定

カスタム Tailwind ユーティリティ クラスを使用して `button` をスタイル設定できます。まず [Tailwind を設定して](themes/misc/tailwind-classes.mdx)ください。
