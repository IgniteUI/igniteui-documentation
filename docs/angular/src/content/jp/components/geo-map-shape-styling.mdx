...
このトピックでは、Angular `GeographicMap` で <ApiLink pkg="maps" type="GeographicShapeSeries" /> にカスタム スタイリングを適用する方法を説明します。
...
次のコード例は、シェイプ スタイリングを設定する 4 つの異なる方法を提供する[シェイプ スタイリング ユーティリティ](geo-map-resources-shape-styling-utility) ファイルを使用していることに注意してください。
