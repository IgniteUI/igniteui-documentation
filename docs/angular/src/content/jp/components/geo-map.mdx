...
以下のサンプルは、Bubble Series (バブル シリーズ) とも呼ばれる <ApiLink pkg="maps" type="GeographicProportionalSymbolSeries" /> を使用して `GeographicMap` にデータを表示する方法を示しています。
...
`GeographicMap` には以下のモジュールが必要ですが、DataChartInteractivityModule は、マップ コンテンツのパンやズームなどのマウス操作にのみ必要です。
