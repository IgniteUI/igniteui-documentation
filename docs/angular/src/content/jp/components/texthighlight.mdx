...
`16.0.0` 以降、`IgxTextHighlightDirective` はスタンドアロン ディレクティブです。直接インポートしてください。
...
NgModule を使用するアプリケーションの場合は、**app.module.ts** で `IgxTextHighlightModule` をインポートします:
...
<Aside type="caution">
`IgxTextHighlightModule` は後方互換性のためにのみ保持されています。新しいプロジェクトでは、スタンドアロンの `IgxTextHighlightDirective` のインポートを使用してください。
</Aside>
...
<ApiLink type="TextHighlight" suffix={false} /> ディレクティブは、指定された文字列のすべての発生の色と背景を変更してスタイル設定できます。まず、すべてのテーマ関数とコンポーネント ミックスインが存在する `index` ファイルをインポートする必要があります。
...
<ApiLink type="TextHighlight" suffix={false} /> ディレクティブの <ApiLink type="TextHighlightDirective" suffix={false} member="cssclass" label="cssClass" /> と <ApiLink type="TextHighlightDirective" suffix={false} member="activecssclass" label="activeCssClass" /> 入力を利用できます。これらのクラスを [`highlight-theme`]({environment:sassApiUrl}/themes#function-highlight-theme) のスタイルと組み合わせて、優れたユーザー エクスペリエンスを提供できます。
...
- <ApiLink type="TextHighlight" suffix={false} />
