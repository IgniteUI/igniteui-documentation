...
Angular Single Select ComboBox コンポーネントは、[ComboBox コンポーネント](/combo)を単一選択用に変更したものです。このコンポーネントを Simple Combo と呼びます。元の ComboBox コンポーネントの単一選択モードに対する需要が高かったため、ユーザーが事前定義された項目リストからオプションを選択し、カスタム値を入力できるようにする、テキストを入力して検索できる入力フィールドを備えた拡張コンポーネントを作成しました。
