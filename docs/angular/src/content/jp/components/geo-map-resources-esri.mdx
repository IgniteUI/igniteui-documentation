...
_language: ja
llms:
  description: "リソース トピックは、Esri Maps が GeographicMap で提供する ArcGISOnlineMapImagery の使用に役立つユーティリティの実装を提供します。"
---
import ApiLink from 'igniteui-astro-components/components/mdx/ApiLink.astro';

# Angular Esri ユーティリティ

リソース トピックは、Esri Maps が <ApiLink type="ArcGISOnlineMapImagery" member="geographicMap" label="GeographicMap" /> で提供する <ApiLink type="ArcGISOnlineMapImagery" /> の使用に役立つユーティリティの実装を提供します。
