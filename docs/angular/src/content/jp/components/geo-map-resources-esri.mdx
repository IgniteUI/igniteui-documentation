...
リソース トピックは、Esri Maps が <ApiLink pkg="maps" type="ArcGISOnlineMapImagery" member="geographicMap" label="GeographicMap" excludeSuffixFor="WebComponents" /> で提供する <ApiLink pkg="maps" type="ArcGISOnlineMapImagery" suffix={false} /> の使用に役立つユーティリティの実装を提供します。
