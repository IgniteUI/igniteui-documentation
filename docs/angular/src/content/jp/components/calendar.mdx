...
`igxCalendar` に `specialDates` を追加します。これを行うには、[`DateRangeType.Specific`]({environment:angularApiUrl}/enums/daterangetype.html#specific) タイプの <ApiLink type="DateRangeDescriptor" kind="interface" prefixed={false} /> 項目を作成し、日付の配列を [`dateRange`]({environment:angularApiUrl}/interfaces/daterangedescriptor.html#dateRange) として渡します。
...
- <ApiLink type="DateRangeDescriptor" kind="interface" prefixed={false} />
