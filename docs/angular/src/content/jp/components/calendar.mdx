...
<ApiLink type="DateRangeType" kind="enum" prefixed={false} /> は無効にする範囲を指定するために使用します。
...
`igxCalendar` に `specialDates` を追加します。これを行うには、<ApiLink kind="enum" type="DateRangeType" prefixed={false} member="Specific" /> タイプの <ApiLink type="DateRangeDescriptor" kind="interface" suffix={false} prefixed={false} /> 項目を作成し、日付の配列を <ApiLink kind="interface" prefixed={false} suffix={false} type="DateRangeDescriptor" member="dateRange" /> として渡します。
