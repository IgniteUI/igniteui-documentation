...
Ignite UI PDF Exporter の使用を開始するには、まず <ApiLink type="PdfExporterService" suffix={false} /> をコンポーネントにインポートします。
...
`inject` 関数を使用して <ApiLink type="PdfExporterService" suffix={false} /> にアクセスできます。データを PDF 形式でエクスポートするには、エクスポーター サービスの <ApiLink type="PdfExporterService" suffix={false} member="exportdata" label="exportData" /> メソッドを起動します。このメソッドは、エクスポートするデータを最初の引数として受け取り、エクスポート プロセスを構成できる <ApiLink type="PdfExporterOptions" suffix={false} /> インスタンスを 2 番目の引数として受け取ります。
...
上記の例では、PDF Exporter サービスはすべての使用可能なデータをエクスポートします。ただし、行または列全体のエクスポートをスキップしたい場合があります。これを実現するには、各列および/または各行に対してそれぞれ発生する <ApiLink type="PdfExporterService" suffix={false} member="columnexporting" label="columnExporting" /> イベントおよび/または <ApiLink type="PdfExporterService" suffix={false} member="rowexporting" label="rowExporting" /> イベントをサブスクライブできます。その後、イベント引数オブジェクトの <ApiLink kind="interface" prefixed={false} suffix={false} type="IRowExportingEventArgs" member="cancel" /> プロパティを `true` に設定することで、エクスポートをキャンセルできます。
...
<ApiLink type="PdfExporterOptions" suffix={false} /> クラスは、PDF エクスポートをカスタマイズするためのいくつかのプロパティを提供します。
