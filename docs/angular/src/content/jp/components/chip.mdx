...
_language: ja
llms:
  description: "Angular Chip コンポーネントは、楕円形のコンテナーに情報を表示する視覚的要素です。"
---
...
Angular チップは、<ApiLink type="Chip" member="outlined" label="outlined" /> プロパティを設定することで、アウトライン スタイルでレンダリングし、周囲にボーダーを表示することもできます。
...
選択は、<ApiLink type="Chip" member="selectable" label="selectable" /> 入力を `true` に設定して有効にできます。チップを選択すると、<ApiLink type="Chip" member="selectedChanging" label="selectedChanging" /> イベントが発生します。新しい <ApiLink kind="interface" type="IChipSelectEventArgs" member="selected" /> 値を提供することにより、新しいステートとこの選択の変更をトリガーした <ApiLink kind="interface" type="IChipSelectEventArgs" member="originalEvent" /> の元のイベントを取得できます。<ApiLink kind="interface" type="IChipSelectEventArgs" member="selected" /> プロパティをプログラムで設定して行う場合、<ApiLink kind="interface" type="IChipSelectEventArgs" member="originalEvent" /> 引数に値 `null` になります。
...
カスタム Tailwind ユーティリティ クラスを使用して chip  をスタイル設定できます。まず [Tailwind を設定して](/themes/misc/tailwind-classes)ください。
...
詳細については、[サイズ](/display-density)の記事をご覧ください。
