...
Angular チップは、いくつかの定義済みスタイル バリアントをサポートしています。<ApiLink type="Chip" member="variant" label="variant" /> 入力プロパティにサポートされている値 (`primary`、`info`、`success`、`warning`、または `danger`) のいずれかを割り当てることで、バリアントを変更できます。
