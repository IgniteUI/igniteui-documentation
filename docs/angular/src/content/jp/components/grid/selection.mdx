...
以下のサンプルは、Grid の**セル選択**動作の 3 つのタイプを示します。以下のボタンを使用して、使用可能な各選択モードを有効にします。Snackbar メッセージ ボックスで各ボタンの操作について簡単に説明します。
...
Ignite UI for Angular Grid コンポーネントは、[行選択](/grid/row-selection)、[セル選択](/grid/cell-selection)、[列選択](/grid/column-selection)の 3 つの選択モードを提供します。デフォルトでは、Grid で**複数セル選択**モードのみが有効になっています。選択モードを変更/有効化するには、<ApiLink type="Grid" member="rowSelection" label="rowSelection" />、<ApiLink type="Grid" member="cellSelection" label="cellSelection" /> 、または <ApiLink type="Column" member="selectable" label="selectable" /> プロパティを使用できます。
...
> 詳細については、[行選択トピック](/grid/row-selection)を参照してください。
...
> 詳細については、[セル選択トピック](/grid/cell-selection)を参照してください。
...
> 詳細については、[列選択トピック](/grid/column-selection)を参照してください。
