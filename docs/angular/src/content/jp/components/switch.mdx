...
`16.0.0` 以降、`IgxSwitchComponent` はスタンドアロン コンポーネントです。直接インポートしてください。
...
NgModule を使用するアプリケーションの場合は、**app.module.ts** で `IgxSwitchModule` をインポートします:
...
<Aside type="caution">
`IgxSwitchModule` は後方互換性のためにのみ保持されています。新しいプロジェクトでは、スタンドアロンの `IgxSwitchComponent` のインポートを使用してください。
</Aside>
