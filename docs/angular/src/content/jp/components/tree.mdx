...
`16.0.0` 以降、`IgxTreeComponent` はスタンドアロン コンポーネントです。直接インポートするか、コンポーネントとそのすべてのサポート コンポーネントおよびディレクティブを含む `IGX_TREE_DIRECTIVES` トークンを使用してください。
...
NgModule を使用するアプリケーションの場合は、**app.module.ts** で `IgxTreeModule` をインポートします:
...
<Aside type="caution">
`IgxTreeModule` は後方互換性のためにのみ保持されています。新しいプロジェクトでは、スタンドアロンの `IGX_TREE_DIRECTIVES` または `IgxTreeComponent` のインポートを使用してください。
</Aside>
...
| イベント | 説明 | キャンセル可能 |
|--------|------|:----------:|
| `nodeSelection` | ノードの選択が変更されたときに発生します。`newSelection`、`oldSelection`、`added`、および `removed` を含む `ITreeNodeSelectionEvent` を提供します。 | はい |
| `nodeExpanding` | ノードが展開される前に発生します。`node` を含む `ITreeNodeTogglingEventArgs` を提供します。 | はい |
| `nodeExpanded` | ノードが展開し終わった後に発生します。 | いいえ |
| `nodeCollapsing` | ノードが折りたたまれる前に発生します。`node` を含む `ITreeNodeTogglingEventArgs` を提供します。 | はい |
| `nodeCollapsed` | ノードが折りたたまれ終わった後に発生します。 | いいえ |
| `activeNodeChanged` | アクティブ (フォーカスされた) ノードが変更されたときに発生します。新しいアクティブな `IgxTreeNode` を提供します。 | いいえ |

`nodeExpanding` を使用して特定のノードの展開を防ぐことができます。
...
```html
<igx-tree (nodeExpanding)="onNodeExpanding($event)" (nodeExpanded)="onNodeExpanded($event)">
    <igx-tree-node *ngFor="let node of data" [data]="node">
        {{ node.label }}
    </igx-tree-node>
</igx-tree>
