...
<DocsAside type="info" title="情報">
<ApiLink type="Combo" /> は、[`IgxOverlay`](overlay.md) サービスを使用して、コンボボックス項目のリスト コンテナーを保持および表示します。スタイルを適切にスコープするには、<ApiLink kind="interface" type="OverlaySettings" member="outlet" /> を使用してください。詳細については、[`IgxOverlay スタイル ガイド`](overlay-styling.md)を確認してください。また、コンポーネントのスタイルを設定するときに `::ng-deep` を使用する必要があります。
</DocsAside>
