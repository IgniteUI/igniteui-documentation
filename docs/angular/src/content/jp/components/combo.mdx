...
また、コンボボックスは、選択が変更されるたびにイベントを発生させます - <ApiLink type="Combo" member="selectionChanging" label="selectionChanging()" />。発行されたイベント引数 <ApiLink type="IComboSelectionChangingEventArgs" kind="interface" prefixed={false} /> には、変更前の選択、現在の選択、追加または削除された項目に関する情報が含まれています。また、イベントをキャンセルして、新しいアイテムの配列で選択を更新できないようにすることもできます。
