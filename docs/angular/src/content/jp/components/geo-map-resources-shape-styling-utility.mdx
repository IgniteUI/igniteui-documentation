...
_language: ja
llms:
  description: "リソース トピックは、Angular GeographicMap コンポーネントで GeographicShapeSeries の UI 要素のスタイリングに役立つユーティリティの実装を提供します。"
---
import ApiLink from 'igniteui-astro-components/components/mdx/ApiLink.astro';

# Angular シェイプ スタイリング ユーティリティ

リソース トピックは、Angular <ApiLink type="GeographicMap" /> コンポーネントで <ApiLink type="GeographicShapeSeries" /> の UI 要素のスタイリングに役立つユーティリティの実装を提供します。
