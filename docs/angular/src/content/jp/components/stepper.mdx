...
`16.0.0` 以降、`IgxStepperComponent` はスタンドアロン コンポーネントです。直接インポートするか、コンポーネントとそのすべてのサポート コンポーネントおよびディレクティブを含む `IGX_STEPPER_DIRECTIVES` トークンを使用してください。
...
NgModule を使用するアプリケーションの場合は、**app.module.ts** で `IgxStepperModule` をインポートします:
...
<Aside type="caution">
`IgxStepperModule` は後方互換性のためにのみ保持されています。新しいプロジェクトでは、スタンドアロンの `IGX_STEPPER_DIRECTIVES` または `IgxStepperComponent` のインポートを使用してください。
</Aside>
...
<ApiLink type="Stepper" /> はステップ間のナビゲーション時に以下のイベントを発生します:

| イベント | 説明 | キャンセル可能 |
|--------|------|:----------:|
| `activeStepChanging` | アクティブなステップが変更される前に発生します。`newIndex`、`oldIndex`、および `owner` を提供します。 | はい |
| `activeStepChanged` | アクティブなステップが変更された後 (トランジション アニメーションの完了後) に発生します。`index` と `owner` を提供します。 | いいえ |

キャンセル可能な `activeStepChanging` イベントを使用して後方ナビゲーションを防止したり、カスタム バリデーションを適用したりします。
...
`activeStepChanged` を使用してナビゲーション アニメーションの完了後に反応します (例: 進捗状況の保存)。
...
public onActiveStepChanged(event: IStepChangedEventArgs) {
    console.log(`ステップ ${event.index} に移動しました`);
