...
<Sample
  src="/scheduling/datepicker-sample-1"
  height={550}
  alt="Angular Datepicker Example"
/>
...
<DocsAside type="info" title="情報">
  ピッカーが <ApiLink type="Calendar" label="**IgxCalendarComponent**" />{" "}
  を使用し、**BrowserAnimationsModule**
  に依存関係があり、モジュールに追加する必要があります。
</DocsAside>
...
<DocsAside type="info" title="情報">
  ピッカーは常に `Date`
  値を返します。これは、モデルにバインドされている場合、または文字列変数への双方向バインドの場合、新しい日付が選択された後、タイプが
  `Date` になることを意味します。
</DocsAside>

### コンポーネントの投影

<ApiLink type="DatePicker" /> は、
<ApiLink type="InputDirective" /> を除く <ApiLink type="InputGroup" />
がサポートする子コンポーネントの投影を許可します。それは、[`igxLabel`](label-input.md)、[`igx-hint
/ IgxHint`](input-group.md#hint)、[`igx-prefix /
igxPrefix`](input-group.md#prefix-および-suffix)、[`igx-suffix /
igxSuffix`](input-group.md#prefix-および-suffix) です。詳細については、[Label
および Input](label-input.md) トピックを参照してください。
...
<ApiLink type="DatePicker" /> は <ApiLink type="PickerToggle" /> と
...
<ApiLink type="DatePicker" /> は
[`IgxDateTimeEditorDirective`](date-time-editor.md) を使用するため、キーボード
ナビゲーションを継承します。
...
<ApiLink type="DatePicker" member="inputFormat" label="inputFormat" /> および
<ApiLink type="DatePicker" member="displayFormat" label="displayFormat" />
は、ピッカーのエディターが指定された形式に従うように設定できるプロパティです。
<ApiLink type="DatePicker" member="inputFormat" label="inputFormat" />
プロパティは、ピッカーが `dropdown`
モードのときに使用され、入力の編集可能なマスクとそのプレースホルダー
(何も設定されていない場合) を管理します。さらに、
<ApiLink type="DatePicker" member="inputFormat" label="inputFormat" />
はロケール
ベースであるため、何も指定されていない場合、ピッカーはデフォルトでブラウザーで使用されるものになります。
...
<ApiLink type="DatePicker" /> は、
<ApiLink type="DatePicker" member="increment" label="increment" /> メソッドと
<ApiLink type="DatePicker" member="decrement" label="decrement" />
メソッドを公開します。どちらも
[`IgxDateTimeEditorDirective`](date-time-editor.md#increment-decrement)
から取得され、現在設定されている日付の特定の
<ApiLink type="DatePart" kind="enum" /> を増加および減少するために使用できます。

```html
<igx-date-picker #datePicker>
  <igx-icon igxPrefix (click)="datePicker.increment(DatePart.Month, 3)"
    >keyboard_arrow_up</igx-icon
  >
  <igx-icon igxPrefix (click)="datePicker.decrement(DatePart.Year. 4)"
...
<ApiLink type="DatePicker" /> は、コア
[FormsModule](https://angular.io/api/forms/FormsModule)、[NgModel](https://angular.io/api/forms/NgModel)、および
[ReactiveFormsModule](https://angular.io/api/forms/ReactiveFormsModule)
([`FormControl`](https://angular.io/api/forms/FormControl)、[`FormGroup`](https://angular.io/api/forms/FormGroup)
など) からのすべてのディレクティブをサポートします。これには、[フォーム
バリデータ](https://angular.io/api/forms/Validators)機能も含まれます。さらに、コンポーネントの
<ApiLink type="DatePicker" member="minValue" label="minValue" /> プロパティと
<ApiLink type="DatePicker" member="maxValue" label="maxValue" />
...
<ApiLink type="DatePicker" /> は [`IgxCalendarComponent`](calendar.md)
を使用し、日付ピッカーが公開するプロパティを介してその設定の一部を変更できます。これらの一部には、ピッカーが展開されたときに複数のカレンダーを表示できる
<ApiLink
  type="DatePicker"
  member="displayMonthsCount"
  label="displayMonthsCount"
/>
、週の開始日を決定する
<ApiLink type="DatePicker" member="weekStart" label="weekStart" />
、年の各週の番号を表示する
<ApiLink type="DatePicker" member="showWeekNumbers" label="showWeekNumbers" />
などが含まれます。

## インターナショナリゼーション

<ApiLink type="DatePicker" /> のローカライズは、
<ApiLink type="DatePicker" member="locale" label="locale" />
入力で制御できます。さらに、
<ApiLink type="Calendar" /> によって提供される `igxCalendarHeader` と
`igxCalendarSubheader`
テンプレートを使用して、ヘッダーとサブヘッダーの外観を指定できます。このテンプレートを使用する方法の詳細については、[**IgxCalendarComponent**](calendar.md)
トピックを参照してください。
...
<igx-date-picker locale="ja-JP" [value]="date">
  <ng-template igxCalendarHeader let-format>
    {{ format.month.combined | titlecase }}{{format.day.combined }}{{
    format.weekday.combined }}
  </ng-template>
  <ng-template igxCalendarSubheader let-format>
    <span (click)="format.yearView()">{{ format.year.combined }}</span>
    <span (click)="format.monthView()"
      >{{ format.month.combined | titlecase }}</span
    >
  </ng-template>
</igx-date-picker>
```
...
- <ApiLink
    kind="sass"
    module="themes"
    type="function-calendar-theme"
    label="IgxCalendarComponent スタイル"
  />
- <ApiLink
    kind="sass"
    module="themes"
...
- <ApiLink
    kind="sass"
    module="themes"
    type="function-drop-down-theme"
    label="IgxDropDown テーマ"
  />
