...
各グループの上にある (<ApiLink kind="enum" type="FilteringLogic" prefixed={false} member="And" /> or <ApiLink kind="enum" type="FilteringLogic" prefixed={false} member="Or" />) ボタンをクリックすると、グループ タイプを変更したり、内部の条件をグループ化解除したりするためのオプションを含むメニューが開きます。
...
条件の検索値は、<ApiLink type="QueryBuilderSearchValueTemplateDirective" suffix={false} label="igxQueryBuilderSearchValue" /> ディレクティブを使用してテンプレート化でき、`igx-query-builder` 本体内の `<ng-template>` に適用されます。
