...
デフォルトの Time Picker モードは、編集可能なドロップダウン モードです。Time Picker モードを読み取り専用ダイアログ モードに変更するには、<ApiLink type="TimePicker component" suffix={false} member="mode" label="mode" /> 入力を [`dialog`]({environment:angularApiUrl}/index.html#pickerinteractionmode) に設定します。
