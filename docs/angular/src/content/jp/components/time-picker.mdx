...
デフォルトの Time Picker モードは、編集可能なドロップダウン モードです。Time Picker モードを読み取り専用ダイアログ モードに変更するには、<ApiLink type="TimePicker" member="mode" label="mode" /> 入力を <ApiLink kind="enum" type="PickerInteractionMode" prefixed={false} /> に設定します。
