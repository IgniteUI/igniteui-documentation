...
<ApiLink type="TimePicker" /> は、アクション ボタンのカスタマイズをサポートしています。これを実現するには、<ApiLink type="PickerActionsDirective" suffix={false} /> ディレクティブ セレクターでマークされた `ng-template` でボタンをラップします。
...
デフォルトの Time Picker モードは、編集可能なドロップダウン モードです。Time Picker モードを読み取り専用ダイアログ モードに変更するには、<ApiLink type="TimePicker" member="mode" label="mode" /> 入力を <ApiLink kind="variable" type="PickerInteractionMode" prefixed={false} /> に設定します。
