...
新しいローカライズでは、ローカライズ文字列およびすべての利用可能なロケールに対する書式設定の両方について、より少ない要件でより多くの機能を提供します。書式設定は現在、`Intl` API によって導入された標準に基づいています。
...
`igniteui-angular` パッケージから提供される `setCurrentI18n` メソッドを使用して、グローバルに使用されるロケールを設定できます。これは、すべてのコンポーネントで使用される書式設定および登録済みのリソース文字列の両方に影響します。リソース文字列の詳細については、[ローカライズされたリソース文字列](ローカライズされたリソース文字列)を参照してください。
