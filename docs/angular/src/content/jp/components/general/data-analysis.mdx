...
| `formatCells` | 選択したセルに条件付き書式を適用します。使用方法: <br/> **this.conditionalFormatting.formatCells(ConditionalFormattingType.dataBars)** | `formatterName`: **string**, `formatRange`?: <ApiLink type="GridSelectionRange" kind="interface" suffix={false} prefixed={false} /> [ ], <br /> `reset`: boolean (**true** by default) |
