...
Angular はタッチ ジェスチャー用の `HammerModule` の提供を終了しました。Ignite UI for Angular コンポーネントでパン、スワイプ、タップなどのタッチ操作を引き続き使用するには、`angular.json` の `scripts` 配列でグローバル スクリプトとして HammerJS を読み込みます。
...
Angular 9 の重大な変更により、Hammer プロバイダーは暗黙的に追加されなくなりました。このため、以下のコンポーネントの**タッチ**操作が正しく動作するには、アプリケーションのルート モジュールに `HammerModule` をインポートする必要があります。
