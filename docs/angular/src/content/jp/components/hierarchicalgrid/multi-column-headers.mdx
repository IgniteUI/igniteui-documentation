...
各 [`igx-column-group`]({environment:angularApiUrl}/classes/igxcolumngroupcomponent.html) は、[`移動`](/hierarchicalgrid/column-moving)、[`ピン固定`](/hierarchicalgrid/column-pinning)と[`非表示`](/hierarchicalgrid/column-hiding)をサポートします。
