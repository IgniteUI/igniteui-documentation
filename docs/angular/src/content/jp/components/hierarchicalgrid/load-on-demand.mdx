...
ルート グリッドが最初に読み込まれたときにフィルター処理されるように、グリッドに初期フィルタリング ルールを追加しましょう。<ApiLink type="FilteringExpressionsTree" suffix={false} prefixed={false} /> を作成し、`ngOnInit` ライフサイクル フックを使用して `IgxHierarchicalGrid` の <ApiLink type="HierarchicalGrid" member="advancedFilteringExpressionsTree" label="advancedFilteringExpressionsTree" /> プロパティに設定します。
