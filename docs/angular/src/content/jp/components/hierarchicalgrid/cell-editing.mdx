...
<Aside type="note" title="注">
編集モードでセルの [`editValue`]({environment:angularApiUrl}/classes/igxgridcell.html#editValue) に加えられた変更は、終了時に適切な[編集イベント](/hierarchicalgrid/editing#イベントの引数とシーケンス)をトリガーし、[トランザクション状態](/hierarchicalgrid/batch-editing)に適用されます (トランザクションが有効な場合)。
</Aside>
...
この例では、[`cellEdit`]({environment:angularApiUrl}/classes/igxhierarchicalgridcomponent.html#cellEdit) イベントにバインドすることにより、入力されたデータに基づいてセルを検証します。セルの新しい値が事前定義された基準を満たしていない場合、イベントをキャンセルすることでデータソースに到達しないようにします (`event.cancel = true`)。また、[`IgxToast`](/toast) を使用してカスタム エラーメッセージを表示します。
