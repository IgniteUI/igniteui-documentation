---
title: Angular Hierarchical Grid の縮小可能な列グループ - インフラジスティックス
description: Angular Hierarchical Grid の縮小可能な列グループを使用して、より小さく簡潔なデータのセットを表示/非表示にする機能を利用します。今すぐお試しください。
...
Ignite UI for Angular については、「[はじめに](/hierarchicalgrid/general/getting-started)」トピックをご覧ください。

次に app.module.ts ファイルに `IgxHierarchicalGridModule` をインポートします。そのため、[複数列グループ](/hierarchicalgrid/multi-column-headers)のトピックを簡単に確認することを強くお勧めします。グリッドで列グループを設定する方法の詳細情報を参照してください。
