...
ただし、[高度なフィルタリング](/hierarchicalgrid/advanced-filtering)を有効にするには、[`allowAdvancedFiltering`]({environment:angularApiUrl}/classes/igxhierarchicalgridcomponent.html#allowAdvancedFiltering) 入力プロパティを `true` に設定します。
