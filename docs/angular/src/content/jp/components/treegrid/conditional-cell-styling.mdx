...
Ignite UI for Angular の IgxTreeGrid コンポーネントは、カスタム ルールに基づいて**行の条件付きスタイル設定**を作成する次の 2 つの方法を提供します:

- IgxTreeGrid コンポーネントで <ApiLink type="TreeGrid" member="rowClasses" label="rowClasses" /> 入力を設定する方法。
- IgxTreeGrid コンポーネントで <ApiLink type="TreeGrid" member="rowStyles" label="rowStyles" /> 入力を設定する方法。

さらにこのトピックでは、両方について詳しく説明します。

### rowClasses の使用

<ApiLink type="TreeGrid" member="rowClasses" label="rowClasses" /> 入力を設定し、カスタム ルールを定義することで、IgxTreeGrid 行のスタイルを条件付きで設定できます。
...
<ApiLink type="Column" /> <ApiLink type="Column" member="cellClasses" label="cellClasses" /> 入力を設定してカスタム条件を定義することにより、IgxTreeGrid の条件付きセルのスタイルを設定できます。
...
- [IgxTreeGridComponent スタイル]({environment:sassApiUrl}/themes#mixin-grid)
...
- [列の移動](/treegrid/column-moving)
- [列のピン固定](/treegrid/column-pinning)
- [列のサイズ変更](/treegrid/column-resizing)
- [列の非表示](/treegrid/column-hiding)
- [選択](/treegrid/selection)
- [検索](/treegrid/search)
- [ツールバー](/treegrid/toolbar)
- [複数行ヘッダー](/treegrid/multi-column-headers)
- [サイズ](/treegrid/display-density)
