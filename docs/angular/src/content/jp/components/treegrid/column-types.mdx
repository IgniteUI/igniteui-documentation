...
# Angular Tree Grid 列タイプ

Ignite UI for Angular Tree Grid は、デフォルトおよび編集テンプレートの外観に基づいて、_number_ (数値)、_string_ (文字列)、_date_ (日付)、_boolean_ (ブール値)、_currency_ (通貨) および _percent_ (パーセント) 列のデータ型のデフォルト処理を提供します。
...
Tree Grid は、**Date オブジェクト**、**数値 (ミリ秒)** または **ISO 日付/時刻文字列**の日付値を受け取ります。このセクションは、[カスタム表示書式を構成する方法](/treegrid/grid#カスタム表示形式)を示します。
...
デフォルトのテンプレートには、プレフィックスまたはサフィックスが付いた通貨記号を含む数値を表示します。通貨記号の位置と数値の書式設定は、提供された Application [`LOCALE_ID`](https://angular.io/api/core/LOCALE_ID) または Tree Grid の <ApiLink type="Grid" member="locale" label="locale" /> に基づいています。
...
セルの値を編集すると、**通貨記号**がサフィックスまたはプレフィックスとして表示されます。詳細については、公式の[セル編集トピック](/treegrid/cell-editing#セル編集テンプレート)を参照してください。
...
[Tree Grid 編集トピック](/treegrid/editing#テンプレートの編集)の編集テンプレート部分を参照してください。
...
- Tree Grid <ApiLink type="Grid" member="locale" label="locale" />
...
- [Tree Grid 概要トピック](/treegrid/tree-grid)
- [編集トピック](/treegrid/editing)
- [集計トピック](/treegrid/summaries)
