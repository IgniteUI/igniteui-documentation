...
<Sample lob src="/tree-grid/treegrid-childdatakey" height={850} alt="Angular ツリー グリッドの例" />
...
<ApiLink type="TreeGrid" /> は <ApiLink type="Grid" /> と多数の機能を共有しますが、データを階層で表示する機能も追加されています。
<ApiLink type="TreeGrid" /> は、各データオブジェクトの[子コレクション](#子コレクション)または各データオブジェクトに[プライマリキーまたは外部キー](#プライマリと外部キー)を使用してデータ オブジェクト内の関係を定義できます。

### ツリー セル

ツリー グリッド階層の構築に使用するオプション (子コレクションまたは主キーまたは外部キー) にかかわらず、ツリー グリッドの行は 2 タイプのセルで構成されます。

- <ApiLink type="GridCell" /> - 値を含む標準セル。
- <ApiLink type="GridCell" /> - セル行のレベルに基づいた値、インジケーターの展開/縮小、インデント div 要素を含むツリー セル。インナー <ApiLink type="TreeGridRow" member="treeRow" /> の <ApiLink kind="interface" type="ITreeGridRecord" member="level" /> プロパティでアクセスできる行コンポーネント レベル。
...
初期時のツリーグリッドは、すべてのノード レベルを展開して表示します。この動作は、<ApiLink type="TreeGrid" member="expansionDepth" /> プロパティを使用して設定できます。デフォルトの値は **Infinity** ですべてのノードが展開されます。初期時の展開の深さは、このプロパティを数値に設定して制御できます。たとえば、**0** はルート レベルのノードのみを表示し、**1** はルートレベルと子ノードを表示します。

### 子コレクション

**child collection** オプションを使用して各データ オブジェクトは子コレクションを含み、親データ オブジェクトとして同じタイプの項目で生成します。これによりツリー グリッドの各レコードがその子への直接の参照を持つことができます。元のデータソースを含むツリー グリッドの <ApiLink type="TreeGrid" member="data" /> プロパティが階層が定義されたコレクションになります。
...
データ コレクションをインポートしてツリーグリッドの <ApiLink type="TreeGrid" member="data" /> 入力にバインドします。
...
IgxTreeGridComponent を階層にバインドするには、<ApiLink type="TreeGrid" member="childDataKey" /> プロパティを各データ オブジェクトで使用される子コレクションの名前に設定します。このサンプルでは **Employees** コレクションです。
...
<ApiLink type="Grid" member="rowselection" /> と <ApiLink type="TreeGrid" member="paging" /> プロパティを使用してツリー グリッドの行選択とページング機能を有効にします。
...
最後に <ApiLink type="GridToolbar" />、<ApiLink type="GridToolbarHiding" />、<ApiLink type="GridToolbarPinning" /> および <ApiLink type="GridToolbarExporter" /> を個別に使用して、列非表示、列ピン固定、エクスポートの機能を含むツリー グリッドのツールバーを有効にします。
...
**Primary and foreign keys** オプションを使用した際に各データオブジェクトはプライマリキーと外部キーを含みます。プライマリキーは現在のデータ オブジェクトの一意識別子、外部キーは親の一意識別子です。元のデータソースを含むツリー グリッドの <ApiLink type="TreeGrid" member="data" /> プロパティがフラットコレクションになります。
...
親子関係は、ツリーグリッドの  <ApiLink type="TreeGrid" member="primaryKey" /> と <ApiLink type="TreeGrid" member="foreignKey" /> プロパティを使用して設定されます。
...
更に <ApiLink type="Grid" member="rowSelection" /> プロパティを使用してツリーグリッドの行選択機能、および各列でフィルタリング、ソート、編集、移動、サイズ変更機能を有効にします。
...
<Sample lob src="/tree-grid/treegrid-primaryforeignkey" height={450} alt="Angular ツリー グリッドの主キー / 外部キーの例" />
...
- 最初の列 (<ApiLink type="Column" member="visibleIndex" /> 0 を持つ列) は、常にツリー列です。
- 列ピン固定、列の非表示、列移動などの処理後に 0 の <ApiLink type="Column" member="visibleIndex" /> で終わる列はツリー列になります。
...
[Grid サイズ変更](/treegrid/sizing) トピックを確認してください。

## スタイル設定

Tree Grid は、[`Ignite UI for Angular テーマ ライブラリ`](/themes/sass/component-themes)でスタイルを設定できます。ツリー グリッドの <ApiLink kind="sass" module="themes" type="function-grid-theme" label="grid-theme" code={true} />  は、さまざまなプロパティを公開します。これにより、ツリーグリッドのすべての機能をカスタマイズできます。
...
次に、カスタム テーマを作成する必要があります。`igx-tree-grid` をスタイル設定するための最も簡単で推奨される方法は、<ApiLink kind="sass" module="themes" type="function-grid-theme" label="grid-theme" code={true} /> を使用して、`background`、`foreground`、`accent-color` の 3 つのメイン カラーを指定することです。
...
<Sample src="/tree-grid/treegrid-style" height={600} alt="Angular ツリー グリッド スタイル設定のデモ" />
