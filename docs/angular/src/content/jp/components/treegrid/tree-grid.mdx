...
Tree Grid は、[`Ignite UI for Angular テーマ ライブラリ`](/themes/sass/component-themes)でスタイルを設定できます。ツリー グリッドの [`grid-theme`]({environment:sassApiUrl}/themes#function-grid-theme)  は、さまざまなプロパティを公開します。これにより、ツリーグリッドのすべての機能をカスタマイズできます。
