...
Tree Grid コンポーネントはインライン行追加や Angular CRUD 操作のための強力な API を通して便利なデータ操作方法を提供します。グリッドのテンプレートで編集アクションが有効になっている[アクションストリップ](/action-strip)コンポーネントを追加し、ホバーして提供されたボタンを使用するか、<kbd>ALT</kbd> + <kbd>+</kbd> を押して、行追加 UI を生成するか、<kbd>ALT</kbd> + <kbd>SHIFT</kbd> + <kbd>+</kbd> を押して、選択した行に子を追加するための UI を生成します。
...
<Sample src="/tree-grid/treegrid-add-row" height={520} alt="Angular Tree Grid 行追加の例" />
...
次に、バインドしたデータソースに Tree Grid を定義をして <ApiLink type="TreeGrid" member="rowEditable" label="rowEditable" /> を true に設定し、編集アクションを有効にした[アクション ストリップ](/action-strip) コンポーネントを定義します。<ApiLink type="GridEditingActions" member="addRow" label="addRow" /> 入力は、行追加 UI を生成するボタンの表示状態を制御します。
...
内部の <ApiLink type="BaseTransactionService" suffix={false} /> は Tree Grid に自動的に提供されます。行の状態が送信またはキャンセルされるまで、保留中のセルの変更を保持します。
...
行追加 UI は、エンド ユーザーに整合性のある編集エクスペリエンスを提供するように設計されているため、行編集 UI と同じ動作をします。詳細については、[Tree Grid 行編集](/treegrid/row-editing)トピックを参照してください。
...
- [Tree Grid 行追加](/treegrid/row-editing#スタイル設定)
...
- [Tree Grid の概要](/treegrid/tree-grid)
