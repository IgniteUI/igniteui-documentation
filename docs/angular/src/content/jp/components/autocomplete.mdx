...
_language: ja
llms:
  description: "Angular Autocomplete は、入力中に候補のリストから項目を簡単に検索、フィルタリング、および選択できるようにする検索ボックス ディレクティブです。"
---
...
<ApiLink type="AutocompleteDirective" label="igxAutocomplete" /> ディレクティブは <ApiLink type="DropDown" label="igxDropDown" /> と候補オプションを表示してテキスト入力を向上する方法を提供します。候補は、テキスト入力の開始時、または `上矢印`/`下矢印` キーを使用して表示します。
</div>
...
はじめに、**app.module** で **IgxAutocompleteModule** と **IgxDropDownModule** をインポートします。<ApiLink type="AutocompleteDirective" label="igxAutocomplete" /> が <ApiLink type="InputDirective" label="igxInput" /> に適用される場合、**igxInputGroupModule** も必要です。
...
<ApiLink type="AutocompleteDirective" label="igxAutocomplete" /> は、利用可能なオプションのプロバイダーとして <ApiLink type="DropDown" label="igxDropDown" /> を使用します。これはドロップダウン コンポーネントのすべての機能をオートコンプリートで使用できることを意味します。
</DocsAside>

### Angular Autocomplete の無効化

Angular Autocomplete を無効にするには、<ApiLink type="AutocompleteDirective" member="disabled" label="IgxAutocompleteDisabled" /> 入力を使用します。
...
`igx-autocomplete` ドロップダウンの配置、スクロール ストラテジとアウトレットは、<ApiLink type="AutocompleteDirective" member="autocompletesettings" label="IgxAutocompleteSettings" /> を使用して設定できます。
...
`igxAutocomplete` のスタイルを設定するには、それに含まれるコンポーネントのスタイルを設定します。この場合、<ApiLink kind="sass" module="themes" type="function-input-group-theme" label="input-group-theme" /> と <ApiLink kind="sass" module="themes" type="function-drop-down-theme" label="drop-down-theme" /> を使用します。

これら 2 つのコンポーネントのスタイル設定については、[`igxInputGroup`](/input-group#スタイル設定) および [`igxDropdown`](/drop-down#スタイル設定) のスタイル設定セクションを参照してください。
...
- [IgxDropDown](/drop-down#styling)
- [IgxInputGroup](/input-group#styling)
- [テンプレート駆動フォームの統合](/input-group#styling)
