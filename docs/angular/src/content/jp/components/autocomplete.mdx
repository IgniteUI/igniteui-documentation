...
<ApiLink type="AutocompleteDirective" suffix={false} label="igxAutocomplete" /> ディレクティブは <ApiLink type="DropDown" label="igxDropDown" /> と候補オプションを表示してテキスト入力を向上する方法を提供します。候補は、テキスト入力の開始時、または `上矢印`/`下矢印` キーを使用して表示します。
</div>
...
はじめに、**app.module** で **IgxAutocompleteModule** と **IgxDropDownModule** をインポートします。<ApiLink type="AutocompleteDirective" suffix={false} label="igxAutocomplete" /> が <ApiLink type="InputDirective" suffix={false} label="igxInput" /> に適用される場合、**igxInputGroupModule** も必要です。
...
<ApiLink type="AutocompleteDirective" suffix={false} label="igxAutocomplete" /> は、利用可能なオプションのプロバイダーとして <ApiLink type="DropDown" label="igxDropDown" /> を使用します。これはドロップダウン コンポーネントのすべての機能をオートコンプリートで使用できることを意味します。
</DocsAside>

### Angular Autocomplete の無効化

Angular Autocomplete を無効にするには、<ApiLink type="AutocompleteDirective" suffix={false} member="disabled" label="IgxAutocompleteDisabled" /> 入力を使用します。
...
`igx-autocomplete` ドロップダウンの配置、スクロール ストラテジとアウトレットは、<ApiLink type="AutocompleteDirective" suffix={false} member="autocompletesettings" label="IgxAutocompleteSettings" /> を使用して設定できます。
