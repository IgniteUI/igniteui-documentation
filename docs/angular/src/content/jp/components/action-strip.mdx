...
3 つ以上のアクション項目が表示される場合に、<ApiLink type="ActionStripMenuItemDirective" suffix={false} /> ディレクティブの使用をお勧めします。`*igxActionStripMenuItem` 構造ディレクティブでマークされたアクション ストリップ内の項目はドロップダウンで表示され、[その他] ボタン (最後のアクションを表す 3 つのドット) を切り替えると表示されます。
