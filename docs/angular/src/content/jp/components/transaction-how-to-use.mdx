...
すべての変更が完了したら、<ApiLink type="TransactionService" suffix={false} /> の <ApiLink type="TransactionService" suffix={false} member="commit" label="commit" /> メソッドを使用して、一度にすべてをコミットできます。指定されたデータにすべてのトランザクションを適用します。
...
リストとのやり取りのどの時点でも、<ApiLink type="TransactionService" suffix={false} member="clear" label="clear" /> メソッドを使用して、トランザクション ログをクリアできます。
