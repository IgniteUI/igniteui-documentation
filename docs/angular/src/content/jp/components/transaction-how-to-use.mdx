...
`ts` ファイルは、アプリケーションで必要となる <ApiLink type="TransactionService" /> を `igniteui-angular` ライブラリからインポートし、<ApiLink type="State" kind="interface" /> および <ApiLink type="Transaction" kind="interface" /> インターフェイスと <ApiLink type="TransactionType" kind="enum" /> enum をインポートする必要があります。
