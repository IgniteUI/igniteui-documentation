...
`ts` ファイルは、アプリケーションで必要となる <ApiLink type="igxTransactionService" suffix={false} /> を `igniteui-angular` ライブラリからインポートし、<ApiLink type="State" kind="interface" prefixed={false} /> および <ApiLink type="Transaction" kind="interface" prefixed={false} /> インターフェイスと <ApiLink type="TransactionType" kind="enum" prefixed={false} /> enum をインポートする必要があります。
