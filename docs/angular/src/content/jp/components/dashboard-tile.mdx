...
- [IgxCategoryChart](charts/chart-overview)
- [IgxDataChart](charts/chart-overview)
- [IgxDataPieChart](charts/types/data-pie-chart)
- [IgxGeographicMap](geo-map)
- [IgxLinearGauge](linear-gauge)
- [IgxRadialGauge](radial-gauge)

デフォルトで選択されるデータ視覚化は、主にスキーマとバインドした `DataSource` の数によって決まります。たとえば、単一の数値をバインドすると `RadialGauge` が取得されますが、互いに区別しやすい値とラベルのペアのコレクションをバインドすると `DataPieChart` が取得されます。より多くの値パスを持つ `DataSource` をバインドすると、バインドされたコレクションの数に応じて、複数の列シリーズまたは線シリーズを持つ <ApiLink pkg="charts" type="DataChart" /> を受け取ります。また、`GeographicMap` を取得するために、`ShapeDataSource` または地理的ポイントを含むデータにバインドすることもできます。
