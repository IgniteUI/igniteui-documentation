...
_language: ja
llms:
  description: "Grid Lite は、データ ソースでのフィルター操作をサポートします。"
---
...
<Sample src="/grid-lite/filtering-simple" height={600} alt="Angular Grid Lite フィルタリング構成" />

## フィルター モデル

グリッド内のフィルター操作の構成要素は、次の構造を持つ <ApiLink type="GridLiteFilteringExpression" /> です。
...
Grid Lite は、API からフィルター操作を適用する 2 つの方法を提供します。<ApiLink type="GridLite" member="filter" label="filter()" />/<ApiLink type="GridLite" member="clearFilter" label="clearFilter()" /> メソッドまたは <ApiLink type="GridLite" member="filterExpressions" /> プロパティのいずれかを使用します。
...
<ApiLink type="GridLite" member="filterExpressions" label="filterExpressions" /> プロパティの動作は、**`filter()`** メソッド呼び出しと非常に似ています。これはグリッド内のフィルター状態を制御する宣言的な方法を公開していますが、最も便利なプロパティは、Grid Lite コンポーネントが最初にレンダリングされるときに初期フィルター状態を設定できることです。
...
<Sample src="/grid-lite/filtering-events" height={510} alt="Angular Grid Lite フィルタリング構成イベント" />

## リモート フィルター操作

フィルタリングをリモートで実行する必要がある場合、または現在の状態/データをどこかのサーバーに保存する必要がある場合、Grid Lite は、この動作を実装およびカスタマイズできるフックを公開します。

<ApiLink type="GridLite" member="dataPipelineConfiguration" label="dataPipelineConfiguration" /> プロパティを使用すると、フィルター操作が実行されるたびに呼び出されるカスタム フックを提供できます。コールバックには **`DataPipelineParams`** オブジェクトが渡されます。
...
<Sample src="/grid-lite/filtering-pipeline" height={510} alt="Angular Grid Lite フィルタリング リモート構成" />
