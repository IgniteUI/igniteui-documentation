...
_language: ja
llms:
  description: "Grid Lite はデータ ソースに対してソート操作をサポートします。"
---
...
<Sample src="/grid-lite/sorting-simple" height={510} alt="Angular Grid Lite ソート構成" />
...
<Sample src="/grid-lite/sorting-grid-config" height={600} alt="Angular Grid Lite Sorting Config Grid" />
...
<Sample src="/grid-lite/sorting-events" height={510} alt="Angular Grid Lite ソート構成イベント" />

## リモート ソート操作

ソートをリモートで実行する必要がある場合、または現在の状態/データをどこかのサーバーに保存する必要がある場合、Grid Lite は、この動作を実装およびカスタマイズできるフックを公開します。

<ApiLink type="GridLite" member="dataPipelineConfiguration" label="dataPipelineConfiguration" /> プロパティを使用すると、ソート操作が実行されるたびに呼び出されるカスタム フックを提供できます。コールバックには **`DataPipelineParams`** オブジェクトが渡されます。
...
<Sample src="/grid-lite/sorting-pipeline" height={510} alt="Angular Grid Lite ソート構成パイプライン" />
