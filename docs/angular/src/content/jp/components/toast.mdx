...
`16.0.0` 以降、`IgxToastComponent` はスタンドアロン コンポーネントです。直接インポートしてください。
...
NgModule を使用するアプリケーションの場合は、**app.module.ts** で `IgxToastModule` をインポートします:
...
<Aside type="caution">
`IgxToastModule` は後方互換性のためにのみ保持されています。新しいプロジェクトでは、スタンドアロンの `IgxToastComponent` のインポートを使用してください。
</Aside>
