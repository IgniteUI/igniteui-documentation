...
_language: ja
llms:
  description: "Ignite UI for Angular Pivot Grid は、当社の最高の Angular コンポーネントの 1 つであり、グループ化された値と集計のテーブルを表す UI コンポーネントであり、データを表形式で整理して要約することができます。"
---
...
<Sample src="/pivot-grid/pivot-grid-datasource" height={870} alt="Angular ピボット セレクターを備えたピボット グリッドの例" />
...
Angular ピボット グリッド コンポーネントは、<ApiLink type="PivotGrid" member="pivotConfiguration" /> プロパティを介して構成できます。
...
<Sample src="/pivot-grid/pivot-grid-basic" height={530} alt="Angular ピボット グリッドの基本例" />
...
ピボット グリッドは、[`Ignite UI for Angular テーマ ライブラリ`](/themes/sass/component-themes)を使用してスタイルを設定できます。グリッドの <ApiLink kind="sass" module="themes" type="function-grid-theme" label="grid-theme" code={true} /> は、ピボット グリッドのすべての機能をカスタマイズできるさまざまなプロパティを公開します。

### グローバル テーマのインポート

ピボット グリッドのスタイル設定を開始するには、まずすべてのテーマ関数とコンポーネント ミックスインが含まれている `index` ファイルをインポートする必要があります。
...
次に、カスタム テーマを作成する必要があります。`igx-pivot-grid` のスタイルを設定する最も簡単で推奨される方法は、<ApiLink kind="sass" module="themes" type="function-grid-theme" label="grid-theme" code={true} /> を使用して、3 つの主要な色 (`background`、`foreground`、`accent-color`) のみを提供することです。

<DocsAside type="info">
ピボット グリッド専用の `sass` 関数はありません。
</DocsAside>

これらはコアのテーマ プロパティです。これらを設定すると、グリッドのすべての部分と内部コンポーネントがそれらの値から色を導出し、グリッド全体で一貫した外観が実現されます。ボタン、アイコン、入力、ドロップダウン、チェックボックス、スクロールバー、チップ、その他のヘルパー コンポーネントなどのネストされたコンポーネントも、統一された外観のためにメインの <ApiLink kind="sass" module="themes" type="function-grid-theme" label="grid-theme" code={true} /> からスタイリング トークンを導出します。
...
<DocsAside type="info">
先ほどのようにカラー値をハードコーディングする代わりに、[`palette`]({environment:sassApiUrl}/palettes#function-palette) および [`color`]({environment:sassApiUrl}/palettes#function-color) 関数を使用することで、色に関してより高い柔軟性を実現できます。これらの使用方法の詳細については、[`パレット`](/themes/sass/palettes) トピックを参照してください。
</DocsAside>

### カスタム テーマの適用

最後の手順は、アプリケーションにコンポーネント テーマを**含める**ことです。
...
<Sample src="/pivot-grid/pivot-grid-styling" height={530} alt="Angular ピボット グリッド スタイル設定のデモ" />
...
- <ApiLink kind="sass" module="themes" type="function-grid-theme" label="grid-theme" />


## その他のリソース


- [Angular ピボット グリッド機能](pivot-grid-features.mdx)
- [Angular ピボット グリッド カスタム集計](pivot-grid-custom.mdx)
