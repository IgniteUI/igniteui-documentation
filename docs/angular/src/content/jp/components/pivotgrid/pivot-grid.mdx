...
ピボット グリッドは、[`Ignite UI for Angular テーマ ライブラリ`](/themes/sass/component-themes)を使用してスタイルを設定できます。グリッドの <ApiLink kind="sass" module="themes" type="function-grid-theme" label="grid-theme" code={true} /> は、ピボット グリッドのすべての機能のカスタマイズを可能にする多くのプロパティを公開しています。
...
// 重要: Ignite UI for Angular バージョン 13 より前の場合は以下を使用します:
// @import '~igniteui-angular/lib/core/styles/themes/index';
```

### カスタム テーマの定義

次に、カスタム テーマを作成する必要があります。`igx-pivot-grid` をスタイルする最も簡単で推奨される方法は、<ApiLink kind="sass" module="themes" type="function-grid-theme" label="grid-theme" code={true} /> を使用して、3 つの主要な色 (`background`、`foreground`、`accent-color`) のみを提供することです。

<DocsAside type="info">
特定の `sass` ピボット グリッド関数はありません。
</DocsAside>
...
最後のステップは、アプリケーションにコンポーネント テーマを **include** することです。
...
<Sample src="/pivot-grid/pivot-grid-styling" height={530} alt="Angular ピボット グリッド スタイリング デモ" />

<ApiLink kind="sass" module="themes" type="function-grid-theme" label="grid-theme" code={true} /> には、より具体的なカスタマイズを行う場合に使用できる追加のパラメーターもあります。
