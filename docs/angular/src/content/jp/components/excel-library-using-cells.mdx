...
Infragistics Angular Excel Library は、<ApiLink pkg="excel" prefixed={false} type="Worksheet" /> オブジェクトの `GetCell` と `GetRegion` メソッドによって、名前によるセルおよび領域の参照をサポートします。そのセルまたは領域を参照する `NamedReference` インスタンスを使用してセルまたは領域を参照します。
