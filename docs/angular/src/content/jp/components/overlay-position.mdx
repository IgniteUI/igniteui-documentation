...
[`overlaySettings`]({environment:angularApiUrl}/interfaces/overlaysettings.html) からの開始点と [`positionSettings`]({environment:angularApiUrl}/interfaces/positionsettings.html) から渡された方向に基づいて要素を配置します。開始点 (<ApiLink type="Point" suffix={false} prefixed={false} />) または `HTMLElement` を配置決めのベースとして渡すことができます。デフォルトは:
...
次に、オーバーレイで使用される配置ストラテジを指定します。<ApiLink type="OverlayService" suffix={false} member="attach" label="overlay.attach()" /> メソッドが呼び出されると、配置ストラテジは [`overlaySettings`]({environment:angularApiUrl}/interfaces/overlaysettings.html) パラメーターのプロパティとして渡されます。以下の例では、デフォルトの `GlobalPositionStrategy` を `ConnectedPositionStrategy` で変更しています。
...
- <ApiLink type="IPositionStrategy" kind="interface" prefixed={false} />
