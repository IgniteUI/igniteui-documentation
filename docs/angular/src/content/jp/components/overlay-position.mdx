...
<ApiLink kind="interface" type="OverlaySettings" /> からの開始点と <ApiLink kind="interface" type="PositionSettings" /> から渡された方向に基づいて要素を配置します。開始点 (<ApiLink type="Point" />) または `HTMLElement` を配置決めのベースとして渡すことができます。デフォルトは:
