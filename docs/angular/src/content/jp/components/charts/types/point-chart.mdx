...
<ApiLink type="CategoryChart" /> コントロールで Angular  ポイント チャートを作成するには、以下の例のように、データを <ApiLink type="CategoryChart" member="itemsSource" label="ItemsSource" pkg="charts" /> プロパティにバインドし、<ApiLink type="CategoryChart" member="chartType" label="CategoryChart.ChartType" /> プロパティを **Point** 列挙型に設定します。
