...
データ項目を文字列と数値データでバインドすることで、`PieChart` の円チャートが作成できます。これらのデータ値を足すと可視化率 100% になります。この例では部門ごとの予算支出の全体的な内訳を示しています。
...
また、凡例項目の外観をカスタマイズするために <ApiLink pkg="charts" type="PieChart" member="legendItemTemplate" label="LegendItemTemplate" /> および <ApiLink pkg="charts" type="PieChart" member="legendItemBadgeTemplate" label="LegendItemBadgeTemplate" />プロパティ、`ItemLegend` の複数のフォント プロパティも使用できます。
