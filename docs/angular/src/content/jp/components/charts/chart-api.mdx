...
Ignite UI for Angular チャートは、`CategoryChart`、`FinancialChart`、`DataChart`、`DataPieChart`、`DoughnutChart`、`PieChart`、および `Sparkline` UI 要素でデータをプロットするためのシンプルで使いやすい API を提供します。

## Angular カテゴリ チャート API

Angular `CategoryChart` には次の API メンバーがあります。
...
Angular `FinancialChart` には次の API メンバーがあります。
...
Angular `DataChart` には次の API メンバーがあります。
...
Angular `DataChart` は、`Series` から継承する次のタイプのシリーズを使用できます。
...
Angular `DataLegend` には次の API メンバーがあります:
...
Angular `DoughnutChart` には、次の API メンバーがあります。
...
Angular `DataPieChart` には、次の API メンバーがあります。
...
Angular `PieChart` には、次の API メンバーがあります。
...
Angular `Sparkline` には、次の API メンバーがあります。
