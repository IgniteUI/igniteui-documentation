...
description: Ignite UI CLI MCP を AI クライアントに接続して、プロジェクトのスキャフォールディング、既存アプリの変更、コンポーネントの作成・更新、Ignite UI for Angular のドキュメントへの質問を行います。VS Code、GitHub、Cursor、Claude Desktop、Claude Code、JetBrains、その他の MCP クライアントの設定方法について説明します。
