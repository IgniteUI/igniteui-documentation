...
Carousel [アニメーション](/carousel#angular-carousel-のアニメーション)はタッチ デバイスで完全にサポートされているため、プラットホームに合わせてプログレッシブ Web アプリケーション ([PWA](https://developer.mozilla.org/ja/docs/Web/Progressive_web_apps)) を構築するための完璧なツールです。
...
カルーセル構成の準備ができました。次に、[リスト](/list) コンポーネントを追加して、両方のコンポーネントを同期します。
...
カスタム Tailwind ユーティリティ クラスを使用して `carousel` をスタイル設定できます。最初に必ず [Tailwind](/themes/misc/tailwind-classes) を設定してください。
