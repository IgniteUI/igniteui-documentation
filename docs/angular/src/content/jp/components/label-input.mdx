...
<ApiLink type="InputDirective" suffix={false} label="igxInput" /> および <ApiLink type="LabelDirective" suffix={false} label="igxLabel" /> を使用するには、`<igx-input-group>` コンテナーにラップします。
...
<ApiLink type="InputDirective" suffix={false} member="required" label="required" /> 属性を使用して `input` を検証できます。ラベルの隣にアスタリスクが追加され、このフィールドは必須入力であることを示します。入力は検証が成功/失敗したかどうかによって緑/赤に変わります。
...
<ApiLink type="igxFocus" suffix={false} /> ディレクティブを使用して、input 要素に `focus` を強制するロジックを追加できます。
...
<ApiLink type="igxFocus" suffix={false} /> ディレクティブを使用するには、<ApiLink type="FocusModule" suffix={false} /> をインポートする必要があります。
</DocsAside>

`igxInput` でマークされた input 要素内のテキストをフォーカス際に選択したい場合、<ApiLink type="igxTextSelection" suffix={false} /> ディレクティブを有効にする必要があります。
...
<DocsAside type="note" title="注">
<ApiLink type="igxTextSelection" suffix={false} /> ディレクティブを使用するには、<ApiLink type="TextSelectionModule" suffix={false} /> をインポートする必要があります。
</DocsAside>
...
- <ApiLink type="LabelDirective" suffix={false} />
