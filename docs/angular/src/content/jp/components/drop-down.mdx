...
選択可能な複数のオプション項目を提供するシンプルなドロップダウンを作成します。これを実現するには、<ApiLink type="DropDown" /> と <ApiLink type="ToggleActionDirective" suffix={false} /> を使用してドロップダウンを開きます / 閉じます。
...
より有益な視覚情報を提供するには、<ApiLink type="DropDownItemComponent" suffix={false} member="isheader" label="isHeader" /> プロパティを使用して項目をセマンティックにグループ化するか、<ApiLink type="DropDownItemComponent" suffix={false} member="disabled" label="disabled" /> プロパティを使用して項目を非インタラクティブとして表示します。<ApiLink type="DropDownItemComponent" suffix={false} member="selected" label="selected" /> プロパティを特定の項目に設定して選択済の項目にできます。`igx-drop-down` 項目は、HTML 要素/他の Web コンポーネントに含める、または設定できる `igxPrefix`、`igxSuffix` および `igx-divider` ディレクティブをサポートしています。
...
追加のドロップダウンを開くように <ApiLink type="DropDownItemComponent" suffix={false} /> を構成するには、ネストされたドロップダウンの <ApiLink kind="interface" prefixed={false} suffix={false} type="OverlaySettings" label="overlay settings" /> を処理し、その `innerDropdown` プロパティを通じてその開閉状態を管理する `multiLevel` ディレクティブを追加します。
...
以下のサンプルでは、クリックで `igxDropDown` インスタンスを開閉できます。入力自体に <ApiLink type="DropDownItemNavigationDirective" suffix={false} /> ディレクティブを適用すると、上下矢印キーを使用するキーボード ナビゲーションが有効になります。これは、<ApiLink type="DropDown" member="allowItemsFocus" label="allowItemsFocus" /> プロパティを `false` に設定し、入力でフォーカスを維持できるデフォルトのドロップダウン動作に依存します。
