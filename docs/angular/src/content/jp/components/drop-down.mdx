...
ドロップダウンをメニューとして動作するように構成できます。<ApiLink type="DropDown" member="selectionChanging" label="selectionChanging" /> イベント ハンドラーで <ApiLink type="ISelectionEventArgs" kind="interface" prefixed={false} /> インターフェイスの [cancel]({environment:angularApiUrl}/interfaces/iselectioneventargs.html#cancel) メンバーを true に設定します。この方法では、メニューを開いた際に選択した項目が保持されず、前の選択が無効になります。クリックされた項目は、イベントの [newSelection]({environment:angularApiUrl}/interfaces/iselectioneventargs.html#newSelection) メンバー値で取得できます。
...
追加のドロップダウンを開くように <ApiLink type="DropDownItem" suffix={false} /> を構成するには、ネストされたドロップダウンの [`overlay settings`]({environment:angularApiUrl}/interfaces/overlaysettings.html) を処理し、その `innerDropdown` プロパティを通じてその開閉状態を管理する `multiLevel` ディレクティブを追加します。
