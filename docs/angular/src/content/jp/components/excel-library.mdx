...
Infragistics Angular Excel ライブラリは、<ApiLink pkg="excel" prefixed={false} type="Workbook" />、<ApiLink pkg="excel" prefixed={false} type="Worksheet" />、<ApiLink pkg="excel" prefixed={false} type="WorksheetCell" label="Cell" />、<ApiLink pkg="excel" prefixed={false} type="Formula" /> などの人気の Microsoft® Excel® スプレッドシート オブジェクトを使用してスプレッドシート データで作業をすることができます。Infragistics Angular Excel ライブラリによって Excel スプレッドシートでアプリケーションのデータを表示するだけでなく、Excel からアプリケーションへのデータの転送も簡単になります。
