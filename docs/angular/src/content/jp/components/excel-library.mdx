...
次のコード スニペットでは、外部の [ExcelUtility](excel-utility.md) クラスを使用して <ApiLink pkg="excel" type="Sheet" member="workbook" /> を保存およびロードしています。



<ApiLink pkg="excel" type="Sheet" member="workbook" /> オブジェクトを読み込んで保存するために、実際の <ApiLink pkg="excel" type="Sheet" member="workbook" /> の保存メソッドや static な `Load` メソッドを使用できます。
