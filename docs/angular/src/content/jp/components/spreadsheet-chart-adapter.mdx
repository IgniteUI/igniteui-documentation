...
Angular Spreadsheet コンポーネントを使用して <ApiLink pkg="spreadsheet" type="Spreadsheet" exclude="Blazor" /> にチャートを表示できます。
...
以下のコード スニペットは、<ApiLink pkg="spreadsheet" type="Spreadsheet" exclude="Blazor" /> コントロールで現在表示されているワークシートにハイパーリンクを追加する方法を示しています。
...
<ApiLink pkg="spreadsheet" type="SpreadsheetChartAdapterBase" suffix={false} exclude="Blazor" excludePrefixFor="Angular,React,WebComponents" /><br />
