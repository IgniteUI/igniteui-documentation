...
アイコン ボタンを無効にしたい場合は、<ApiLink type="IconButtonDirective" suffix={false} member="disabled" label="disabled" /> プロパティを使用できます。このサンプルでは、`igxIconButton` ディレクティブを使用してさまざまなファミリのアイコンを使用する方法も示します。
