...
`16.0.0` 以降、`IgxSplitterComponent` はスタンドアロン コンポーネントです。直接インポートするか、コンポーネントとそのすべてのサポート コンポーネントおよびディレクティブを含む `IGX_SPLITTER_DIRECTIVES` トークンを使用してください。
...
NgModule を使用するアプリケーションの場合は、**app.module.ts** で `IgxSplitterModule` をインポートします:
...
<Aside type="caution">
`IgxSplitterModule` は後方互換性のためにのみ保持されています。新しいプロジェクトでは、スタンドアロンの `IGX_SPLITTER_DIRECTIVES` または `IgxSplitterComponent` のインポートを使用してください。
</Aside>
...
<ApiLink type="Splitter" /> はペインのサイズ変更中に以下のイベントを発生します:

| イベント | 説明 |
|--------|------|
| `resizeStart` | ユーザーがスプリッター バーのドラッグを開始したときに発生します。`pane` と `sibling` の参照を提供します。 |
| `resizing` | ユーザーがスプリッター バーをドラッグしている間、継続的に発生します。`pane` と `sibling` の参照を提供します。 |
| `resizeEnd` | ユーザーがスプリッター バーを離したときに発生します。`pane` と `sibling` の参照を提供します。 |

これらのイベントを使用してペインのサイズ変更を追跡または応答します。
...
public onResizeStart(event: ISplitterBarResizeEventArgs) {
    console.log('サイズ変更が開始されました。ペイン:', event.pane.id);
}

public onResizeEnd(event: ISplitterBarResizeEventArgs) {
    console.log('ペインの新しいサイズ:', event.pane.size);
    this.savePaneLayout();
}
```
...
<igx-splitter
    ...
    <igx-splitter-pane>左</igx-splitter-pane>
    <igx-splitter-pane>右</igx-splitter-pane>
</igx-splitter>
```
...
- <ApiLink type="SplitterType" kind="enum" prefixed={false} />
