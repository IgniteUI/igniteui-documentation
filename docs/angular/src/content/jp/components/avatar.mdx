...
最もシンプルな方法として、<ApiLink kind="sass" module="themes" type="function-avatar-theme" label="avatar-theme" code={true} /> を拡張し、`$background` および `$border-radius` パラメーターを指定して新しいテーマを作成します。`$color` (または `$icon-color`) は、指定された背景に対してよりコントラストの高い色 (黒または白) が自動的に設定されます。なお、`$border-radius` プロパティはアバターの `shape` が `rounded` に設定されている場合のみ適用されます。
