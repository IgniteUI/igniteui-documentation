...
Select コンポーネントは、[入力グループ](/input-group)に適用可能な次のディレクティブをサポートしています。
...
[`Input Group`](/input-group#スタイル設定) と [`Drop Down`](/drop-down#スタイル設定) のスタイル設定セクションを参照して、これら 2 つのコンポーネントのスタイル設定方法をより深く理解してください。
...
カスタム Tailwind ユーティリティ クラスを使用して select をスタイル設定できます。まず [Tailwind を設定して](/themes/misc/tailwind-classes)ください。
