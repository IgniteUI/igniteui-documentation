...
スクロールバーをさらにカスタマイズするには、<ApiLink kind="sass" module="themes" type="function-scrollbar-theme" label="scrollbar-theme" code={true} /> 関数を使用して新しいスクロールバー スタイルを作成します。この関数は、スクロールバーのスタイル変更に関する引数を受け入れます。スクロールバーのサイズ、カラー、境界線、最小高さなどをカスタマイズできます。
