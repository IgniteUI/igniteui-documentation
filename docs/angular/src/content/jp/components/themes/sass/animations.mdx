...
Ignite UI for Angular には、アニメーションをイーズ イン/アウトするために使用できるタイミング関数のセットが含まれています。<ApiLink type="EaseIn" kind="enum" prefixed={false} />、<ApiLink type="EaseOut" kind="enum" prefixed={false} />、<ApiLink type="EaseInOut" kind="enum" prefixed={false} /> の 3 つの主要なタイミング関数グループがあり、それぞれ以下のタイミングを含みます。
- quad
- cubic
- quart
- quint
...
- <ApiLink type="AnimationSettings" kind="interface" suffix={false} prefixed={false} />
- <ApiLink type="IAnimationParams" kind="interface" suffix={false} prefixed={false} />
