...
境界半径は、コンポーネントの[テーマ スキーマ](https://github.com/IgniteUI/igniteui-theming/blob/18f878033898e1b6a3bb0ed28993e9a4037d1a80/sass/themes/schemas/components/light/_toast.scss#L44)で定義されます (以下の例を参照)。この方法で定義されたコンポーネントの境界半径は、<ApiLink kind="sass" module="themes" type="mixin-theme" label="theme" /> ミックスインの `$roundness` パラメーター、または `--ig-radius-factor` と呼ばれる単一の CSS 変数を介して制御できます。
...
各テーマのデフォルトおよび最小/最大半径値を確認するには、各コンポーネントの <ApiLink kind="sass" module="schemas" label="スキーマ" /> ドキュメントを参照してください。
