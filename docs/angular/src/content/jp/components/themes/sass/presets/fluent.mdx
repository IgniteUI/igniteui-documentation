...
llms:
  description: "バージョン 8.2 以降、インフラジスティックスのコンポーネントでは、Microsoft Fluent デザイン システム に基づいて構築された新しいテーマが含まれます。"
---
...
`Material` から `Fluent` に切り替えるには、<ApiLink kind="sass" module="themes" type="mixin-theme" label="theme" /> 組み込みのミキシンを使用します。
