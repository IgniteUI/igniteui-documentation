...
コンボボックス コンポーネントでは、ユーザーが項目リストの表示方法を変更できます。これを行うには、カスタム <ApiLink type="OverlaySettings" kind="interface" prefixed={false} /> を定義し、それらを ComboBox の <ApiLink type="Combo" member="overlaySettings" label="overlaySettings" /> 入力に渡します。
