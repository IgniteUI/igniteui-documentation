...
コンボボックス コンポーネントでは、ユーザーが項目リストの表示方法を変更できます。これを行うには、カスタム <ApiLink type="OverlaySettings" kind="interface" suffix={false} prefixed={false} /> を定義し、それらを ComboBox の <ApiLink type="Combo" member="overlaySettings" label="overlaySettings" /> 入力に渡します。
...
<DocsAside type="info" title="情報">
コンボボックス コンポーネントは、デフォルトの配置ストラテジとして <ApiLink type="AutoPositionStrategy" suffix={false} prefixed={false} /> を使用します。
</DocsAside>
