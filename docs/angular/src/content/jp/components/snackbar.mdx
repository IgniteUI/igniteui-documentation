...
`16.0.0` 以降、`IgxSnackbarComponent` はスタンドアロン コンポーネントです。直接インポートしてください。
...
NgModule を使用するアプリケーションの場合は、**app.module.ts** で `IgxSnackbarModule` をインポートします:
...
<Aside type="caution">
`IgxSnackbarModule` は後方互換性のためにのみ保持されています。新しいプロジェクトでは、スタンドアロンの `IgxSnackbarComponent` のインポートを使用してください。
</Aside>
...
| イベント | 説明 | キャンセル可能 |
|--------|------|:----------:|
| `clicked` | スナックバーがクリックされたときに発生します。`IgxSnackbarComponent` インスタンスを提供します。 | いいえ |
| `animationStarted` | 表示/非表示アニメーションが開始されたときに発生します。`ToggleViewEventArgs` を提供します。 | いいえ |
| `animationDone` | 表示/非表示アニメーションが完了したときに発生します。`ToggleViewEventArgs` を提供します。 | いいえ |

`clicked` を使用してユーザーがスナックバーを操作したときにアクションを追加します。

```typescript
public onSnackbarClicked(snackbar: IgxSnackbarComponent) {
    console.log('スナックバーがクリックされました:', snackbar);
    snackbar.close();
}
```

```html
<igx-snackbar #snackbar message="アイテムが削除されました" (clicked)="onSnackbarClicked(snackbar)">
    元に戻す
</igx-snackbar>
```
