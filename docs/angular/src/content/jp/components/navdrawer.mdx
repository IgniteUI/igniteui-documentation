...
<ApiLink type="NavigationDrawer" suffix={false} /> はタッチ操作の [`BrowserAnimationsModule`](https://angular.io/api/platform-browser/animations/BrowserAnimationsModule) と [`HammerModule`](https://angular.io/api/platform-browser/HammerModule) (**オプション**) に依存関係があり、これらも AppModule に追加する必要があります。
</DocsAside>
...
<DocsAside type="note" title="注">
Navigation Drawer はデフォルトでレスポンシブです。画面サイズに基づいて固定解除および固定モード間で切り替わります。この動作は <ApiLink type="NavigationDrawer" member="pinThreshold" label="pinThreshold" suffix={false} /> プロパティによって制御され、falsy 値 (0 など) を設定すると無効になります。
</DocsAside>
...
- <ApiLink type="NavigationDrawer" suffix={false} />
