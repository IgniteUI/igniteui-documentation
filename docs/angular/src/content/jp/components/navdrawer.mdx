...
任意のコンテンツをテンプレートに設定できますが、<ApiLink type="igxDrawerItem" suffix={false} /> ディレクティブ ([項目のスタイル](#スタイル設定)を参照) が定義済みのスタイル設定を項目に適用します。
...
Navigation Drawer は <ApiLink type="igxNavigationService" suffix={false} /> とも統合し、[`igxToggleAction`](toggle.md#トグル自動操作) ディレクティブで id によって対象にされます。
...
<Aside type="note" title="注">
Navigation Drawer はデフォルトでレスポンシブです。画面サイズに基づいて固定解除および固定モード間で切り替わります。この動作は <ApiLink type="NavigationDrawer" member="pinThreshold" label="pinThreshold" /> プロパティによって制御され、falsy 値 (0 など) を設定すると無効になります。
</Aside>
...
```html
<igx-nav-drawer
    (opening)="onDrawerOpened()"
    (closing)="onDrawerClosed()">
</igx-nav-drawer>
```
