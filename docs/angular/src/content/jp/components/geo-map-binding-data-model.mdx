...
## コード スニペット
以下のコードは、<ApiLink pkg="maps" type="GeographicSymbolSeries" /> を、経度と緯度の座標を使用して格納された世界の一部の都市の地理的位置を含むカスタム データ モデルにバインドする方法を示します。また、[WorldUtility](geo-map-resources-world-util) を使用してこれらの場所間の最短の地理的経路をプロットするために <ApiLink pkg="maps" type="GeographicPolylineSeries" /> を使用します。
