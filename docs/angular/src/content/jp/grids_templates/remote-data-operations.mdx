...
データを要求時、[`startIndex`]({environment:angularApiUrl}/interfaces/iforofstate.html#startIndex) と [`chunkSize`]({environment:angularApiUrl}/interfaces/iforofstate.html#chunkSize) プロパティを提供する <ApiLink type="IForOfState" kind="interface" prefixed={false} /> インターフェイスを使用する必要があります。
