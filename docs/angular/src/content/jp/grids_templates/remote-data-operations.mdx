...
<ApiLink type="{ComponentApiType}" /> は、データ チャンクがリモート サービスから要求されるシナリオをサポートし、内部で使用される <ApiLink type="ForOfDirective" label="igxForOf" /> ディレクティブで実装された動作を公開します。
...
データを要求時、<ApiLink kind="interface" type="IForOfState" member="startIndex" /> と <ApiLink kind="interface" type="IForOfState" member="chunkSize" /> プロパティを提供する <ApiLink type="IForOfState" kind="interface" /> インターフェイスを使用する必要があります。

<DocsAside type="info" title="情報">
最初の <ApiLink kind="interface" type="IForOfState" member="chunkSize" /> は常に 0 で、特定のアプリケーション シナリオに基づいて設定する必要があります。
</DocsAside>
...
これまでの例で、リモート データで {ComponentName} を設定する方法を説明しました。次に、[一括編集のトピック](./batch-editing.mdx)に従ってグリッドの一括編集を有効にします。
