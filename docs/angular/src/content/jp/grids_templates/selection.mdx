...
Ignite UI for Angular {ComponentTitle} コンポーネントは、[行選択](./row-selection.mdx)、[セル選択](./cell-selection.mdx)、[列選択](./column-selection.mdx)の 3 つの選択モードを提供します。デフォルトでは、{ComponentTitle} で**複数セル選択**モードのみが有効になっています。選択モードを変更/有効化するには、<ApiLink type="Grid" member="rowSelection" label="rowSelection" />、<ApiLink type="Grid" member="cellSelection" label="cellSelection" /> 、または <ApiLink type="Column" member="selectable" label="selectable" /> プロパティを使用できます。
...
> 詳細については、[行選択トピック](./row-selection.mdx)を参照してください。
...
> 詳細については、[セル選択トピック](./cell-selection.mdx)を参照してください。
...
> 詳細については、[列選択トピック](./column-selection.mdx)を参照してください。
