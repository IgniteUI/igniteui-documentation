...
列をピン固定すると、一番右に配置されたピン固定列の右にピン固定されます。ピン固定列の順序を変更するには、<ApiLink type="{ComponentApiType}" member="columnPin" label="columnPin" /> イベントでイベント引数の <ApiLink kind="interface" prefixed={false} suffix={false} type="IPinColumnEventArgs" member="insertAtIndex" /> プロパティを適切な位置インデックスに変更します。
