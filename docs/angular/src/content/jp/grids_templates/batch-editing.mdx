...
<ApiLink kind="interface" prefixed={false} type="HierarchicalTransactionService" /> を <ApiLink type="HierarchicalGrid" label="{ComponentName}" /> と一緒に使用し、アイランドごとに個別のトランザクション ログを蓄積するには、代わりにサービス ファクトリを提供する必要があります。<ApiLink kind="interface" prefixed={false} type="HierarchicalTransactionService" /> として使用できるようになります。
