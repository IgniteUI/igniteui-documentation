...
次に、現在のセルのエディターと背景をより明確にするためにスタイルを設定します。セル スタイリングの詳細については、[こちら](./cell-editing.mdx#スタイル設定)をご覧ください。
...
バナーとボタンのスタイルを設定後、[編集モードのセル](./cell-editing.mdx#スタイル設定)のカスタム スタイルも定義します。以下は、すべてのスタイルを組み合わせた結果です。
