...
{ComponentTitle} は、**Date オブジェクト**、**数値 (ミリ秒)** または **ISO 日付/時刻文字列**の日付値を受け取ります。このセクションは、[カスタム表示書式を構成する方法](./grid.mdx#カスタム表示形式)を示します。
...
セルの値を編集すると、**通貨記号**がサフィックスまたはプレフィックスとして表示されます。詳細については、公式の[セル編集トピック](./cell-editing.mdx#セル編集テンプレート)を参照してください。
...
[{ComponentTitle} 編集トピック](./editing.mdx#テンプレートの編集)の編集テンプレート部分を参照してください。
...
- [{ComponentTitle} 概要トピック](./{ComponentMainTopic}.mdx)
- [編集トピック](./editing.mdx)
- [集計トピック](./summaries.mdx)
