...
{ComponentName} は、[`Ignite UI for Angular テーマ ライブラリ`](/themes/sass/component-themes.md)でスタイルを設定できます。{ComponentTitle} の [`grid-theme`]({environment:sassApiUrl}/themes#function-grid-theme) は、{ComponentTitle} のすべての機能をカスタマイズできるさまざまなプロパティを公開します。
