...
_language: ja
llms:
  description: "{ComponentApiType} は、共通の列ヘッダー下で行のグループ化が可能な複数列ヘッダーをサポートします。"
---
...
各 <ApiLink type="ColumnGroup" label="igx-column-group" /> は、[`移動`](/{igPath}/column-moving)、[`ピン固定`](/{igPath}/column-pinning)と[`非表示`](/{igPath}/column-hiding)をサポートします。
