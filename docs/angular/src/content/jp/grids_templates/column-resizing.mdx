...
**列サイズ変更**も列レベルで有効化にできます。つまり、[**{ComponentSelector}**]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html) にサイズ変更可能な列とサイズ変更不可の列の両方を含むことが可能です。[`igx-column`]({environment:angularApiUrl}/classes/igxcolumncomponent.html) の [`resizable`]({environment:angularApiUrl}/classes/igxcolumncomponent.html#resizable) 入力で行うことができます。
...
[`{ComponentSelector}`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html) の [`columnResized`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#columnResized) イベントにサブスクライブして列がサイズ変更されたにカスタム ロジックを実装します。以前の列幅、新しい列幅、および [`IgxColumnComponent`]({environment:angularApiUrl}/classes/igxcolumncomponent.html) オブジェクトがイベント引数で公開されます。
