...
<DocsAside type="note" title="注">
[行の追加] のボタンの表示状態を制御する IgxGridEditingActions 入力は、アクション ストリップ コンテキスト (タイプ <ApiLink type="RowType" kind="interface" suffix={false} prefixed={false} />) を使用して、ボタンが表示するレコードを調整できます。
</DocsAside>
</ComponentBlock>
<ComponentBlock for="TreeGrid">
