...
To use the <ApiLink type="InputDirective" label="igxInput" /> and <ApiLink type="LabelDirective" label="igxLabel" />, you have to wrap them in an `<igx-input-group>` container:
...
We can validate an `input` using the <ApiLink type="InputDirective" member="required" label="required" /> attribute. This will add an asterisk next to the label, indicating that this field must be completed. The input will turn green/red depending on whether the validation passes/fails.
...
You can add logic to force `focus` on input elements using the <ApiLink type="FocusDirective" /> directive.
...
<DocsAside type="info">
To use the <ApiLink type="FocusDirective" /> directive, you have to import the <ApiLink type="FocusDirective" />.
</DocsAside>

If you want the text in an input element, marked with `igxInput`, to be selected on focus, you have to enable the <ApiLink type="TextSelectionDirective" /> directive.
...
<DocsAside type="info">
To use the <ApiLink type="TextSelectionDirective" /> directive, you have to import the <ApiLink type="TextSelectionDirective" />.
</DocsAside>
