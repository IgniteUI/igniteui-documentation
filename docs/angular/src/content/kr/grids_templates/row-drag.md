...
---
title: 트리 그리드 행 드래그
description: The Ignite UI for Angular Tree Grid control features the fastest, touch-responsive data-rich tree grid with popular features, including hierarchical and list views.
...
---
title: 계층 그리드 행 드래그
description: The Ignite UI for Angular Hierarchical Grid control features the fastest, touch-responsive data-rich tree grid with popular features.
...
버튼을 누른 상태에서 드래그 핸들을 클릭하고 _커서를 움직이면_ 그리드의 [`rowDragStart`]({environment:angularApiUrl}/classes/@@igTypeDoc.html#rowDragStart) 이벤트가 실행됩니다. 언제든지 클릭을 해제하면 [`rowDragEnd`]({environment:angularApiUrl}/classes/@@igTypeDoc.html#rowDragEnd) 이벤트가 실행됩니다.
...
행 드래그를 활성화하는 것은 매우 간단한데 다음은 행 _드롭_을 처리하는 방법을 설정해야 합니다.
...
먼저, `onEnter` 및 `onLeave` 핸들러를 살펴보겠습니다. 이 방법은 드래그 _고스트_ 아이콘을 변경하여 사용자가 행을 드롭할 수 있는 영역 위에 있음을 표시할 수 있습니다:
...
다음으로, 사용자가 실제로 드롭 영역 내에 행을 _드롭_할 때 실행되는 것을 정의해야 합니다.
@@if (igxName === 'IgxTreeGrid' || igxName === 'IgxHierarchicalGrid') {
