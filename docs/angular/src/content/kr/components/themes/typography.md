...
Ignite UI for Angular defines a `$default-type-scale` (illustrated by the table above), which is in turn used by the `typography` mixin to set the initial typography styles. The user can, however, pass a different type scale to be used by `typography` mixin.
...
To overwrite the default typography, include the `typography` mixin anywhere after the `core` mixin. Let's take advantage of the type scale `$my-type-scale` we defined above and make it the default type scale.
