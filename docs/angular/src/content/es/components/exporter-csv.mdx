...
The exporting functionality is encapsulated in the <ApiLink type="CsvExporterService" /> class.
</div>
...
To start using the IgniteUI CSV Exporter first import the <ApiLink type="CsvExporterService" /> in the app.module.ts file and add the service to the `providers` array:
...
You may access the exporter service by defining an argument of type <ApiLink type="CsvExporterService" /> in the component's constructor and the Angular framework will provide an instance of the service. To export some data in CSV format you need to invoke the exporter service's <ApiLink type="CsvExporterService" member="exportdata" label="exportData" /> method. This method accepts as a first argument the data you want to export and the second argument is of type <ApiLink type="CsvExporterOptions" /> and allows you to configure the export process.
...
<ApiLink type="CsvExporterService" />'s <ApiLink type="CsvExporterService" member="export" label="export" /> method and pass the [**IgxGrid**](/grid/grid) as first argument.
...
- as a second argument of the <ApiLink type="CsvExporterOptions" /> objects's constructor
- using the <ApiLink type="CsvExporterOptions" /> object's <ApiLink type="CsvExporterOptions" member="filetype" label="fileType" /> property
...
In the above examples the CSV Exporter service was exporting all available data. There are situations in which you may want to skip exporting a row or even an entire column. To achieve this you may hook to the <ApiLink type="CsvExporterService" member="columnexporting" label="columnExporting" /> and/or <ApiLink type="CsvExporterService" member="rowexporting" label="rowExporting" /> events which are fired respectively for each column and/or each row and cancel the respective event by setting the event argument object's <ApiLink kind="interface" type="IRowExportingEventArgs" member="cancel" /> property to `true`.
