...
By applying the <ApiLink type="MaskDirective" label="igxMask" /> directive on a **text input field**, the developer can control user input and format the visible value, based on configurable mask rules. It provides different input options and ease in use and configuration.
...
<DocsAside type="info">
<ApiLink type="MaskDirective" label="igxMask" /> directive is used on an input of type **text**.
</DocsAside>
...
Use the <ApiLink type="MaskDirective" member="includeLiterals" label="includeLiterals" /> input to configure which input value (formatted or raw) to bind in your form when a specific mask is applied. By default, <ApiLink type="MaskDirective" member="includeLiterals" label="includeLiterals" /> is set to _false_ and the raw value is used.
...
You can force the component to select all of the input text on focus using <ApiLink type="TextSelectionDirective" />. Find more info on `igxTextSelection` at [Label & Input](/label-input#focus--text-selection).
...
Pass an instance of each pipe to the <ApiLink type="MaskDirective" member="focusedValuePipe" label="focusedValuePipe" /> and <ApiLink type="MaskDirective" member="displayValuePipe" label="displayValuePipe" /> input properties as follows:
...
The user can also take advantage of the <ApiLink type="MaskDirective" member="placeholder" label="placeholder" /> input property, which serves the purpose of the native input placeholder attribute. If no value is provided for the <ApiLink type="MaskDirective" member="placeholder" label="placeholder" />, the value set for the mask is used.
