...
By applying the <ApiLink type="MaskDirective" suffix={false} label="igxMask" /> directive on a **text input field**, the developer can control user input and format the visible value, based on configurable mask rules. It provides different input options and ease in use and configuration.

## Angular Mask Example

<Sample src="/data-display/mask-sample-2" height={100} alt="Angular Mask Example" />
...
<DocsAside type="note">
<ApiLink type="MaskDirective" suffix={false} label="igxMask" /> directive is used on an input of type **text**.
</DocsAside>
...
Use the <ApiLink type="MaskDirective" suffix={false} member="includeLiterals" label="includeLiterals" /> input to configure which input value (formatted or raw) to bind in your form when a specific mask is applied. By default, <ApiLink type="MaskDirective" suffix={false} member="includeLiterals" label="includeLiterals" /> is set to _false_ and the raw value is used.
...
<p *ngIf="socialSecurityNumber.length > 0">Value: {{ socialSecurityNumber }}
</div>
```
...
<Sample src="/data-display/mask-sample-3" height={160} alt="" />
...
<Sample src="/data-display/mask-sample-1" height={120} alt="" />

<hr/>

### Text Selection

You can force the component to select all of the input text on focus using <ApiLink type="igxTextSelection" suffix={false} />. Find more info on `igxTextSelection` at [Label & Input](/label-input#focus--text-selection).
...
In addition to the default mask behavior, the user can implement his own custom pipes and take advantage of the <ApiLink type="MaskDirective" suffix={false} member="focusedValuePipe" label="focusedValuePipe" /> and <ApiLink type="MaskDirective" suffix={false} member="displayValuePipe" label="displayValuePipe" /> input properties, to transform the value to a desired output when the input gets or loses focus. This will not affect the underlying model value. Let's demonstrate how this can be achieved!
...
Pass an instance of each pipe to the <ApiLink type="MaskDirective" suffix={false} member="focusedValuePipe" label="focusedValuePipe" /> and <ApiLink type="MaskDirective" suffix={false} member="displayValuePipe" label="displayValuePipe" /> input properties as follows:
...
<Sample src="/data-display/mask-sample-4" height={100} alt="" />

### Adding a placeholder

The user can also take advantage of the <ApiLink type="MaskDirective" suffix={false} member="placeholder" label="placeholder" /> input property, which serves the purpose of the native input placeholder attribute. If no value is provided for the <ApiLink type="MaskDirective" suffix={false} member="placeholder" label="placeholder" />, the value set for the mask is used.
...
<Sample src="/data-display/mask-sample-5" height={100} alt="" />
...
- <ApiLink type="MaskDirective" suffix={false} />
