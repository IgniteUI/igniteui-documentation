...
You can configure the drop-down to behave as a menu. To do this, set the <ApiLink type="ISelectionEventArgs" kind="interface" prefixed={false} /> interface [cancel]({environment:angularApiUrl}/interfaces/iselectioneventargs.html#cancel) member to _true_ in the <ApiLink type="DropDown" member="selectionChanging" label="selectionChanging" /> event handler. In this way, the selected item is not preserved when opening the menu and previous selections get invalidated. Still, you can get the clicked item through the [newSelection]({environment:angularApiUrl}/interfaces/iselectioneventargs.html#newSelection) member value in the event.
...
In order to configure the <ApiLink type="DropDownItem" suffix={false} /> to open an additional drop down, add the `multiLevel` directive that would handle the [`overlay settings`]({environment:angularApiUrl}/interfaces/overlaysettings.html) of the nested drop down and manages its opened/closed state through its `innerDropdown` property.
