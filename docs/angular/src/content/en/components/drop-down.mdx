...
Let's create a simple drop-down that provides several option items to choose from. To achieve this, we will use the <ApiLink type="DropDown" /> as well as the <ApiLink type="ToggleActionDirective" suffix={false} /> to open/close the drop-down.
...
To provide a more useful visual information, use the <ApiLink type="DropDownItemComponent" suffix={false} member="isheader" label="isHeader" /> property to group items semantically or the <ApiLink type="DropDownItemComponent" suffix={false} member="disabled" label="disabled" /> property to display an item as a non-interactive. You can also set the <ApiLink type="DropDownItemComponent" suffix={false} member="selected" label="selected" /> property on a particular item to make it the selected item. The `igx-drop-down` items have out-of-the-box support for `igxPrefix`, `igxSuffix`, and `igx-divider` directives that can contain or be set on HTML elements or other web components.
...
You can configure the drop-down to behave as a menu. To do this, set the <ApiLink type="ISelectionEventArgs" kind="interface" suffix={false} prefixed={false} /> interface <ApiLink kind="interface" prefixed={false} suffix={false} type="ISelectionEventArgs" member="cancel" /> member to _true_ in the <ApiLink type="DropDown" member="selectionChanging" label="selectionChanging" /> event handler. In this way, the selected item is not preserved when opening the menu and previous selections get invalidated. Still, you can get the clicked item through the <ApiLink kind="interface" prefixed={false} suffix={false} type="ISelectionEventArgs" member="newSelection" /> member value in the event.
...
In order to configure the <ApiLink type="DropDownItemComponent" suffix={false} /> to open an additional drop down, add the `multiLevel` directive that would handle the <ApiLink kind="interface" prefixed={false} suffix={false} type="OverlaySettings" label="overlay settings" /> of the nested drop down and manages its opened/closed state through its `innerDropdown` property.
