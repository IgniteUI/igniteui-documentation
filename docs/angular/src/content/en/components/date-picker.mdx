...
- the whole buttons can be templated using the <ApiLink type="igxPickerActions" suffix={false} /> directive:
...
Use the <ApiLink type="DatePicker" member="disabledDates" label="disabledDates" /> and <ApiLink type="DatePicker" member="specialDates" label="specialDates" /> inputs to mark specific days in the calendar. Both accept an array of `DateRangeDescriptor` objects, each with a `type` (`DateRangeType`) and an optional `dateRange` array.
...
```html
<igx-date-picker [disabledDates]="disabledDates">
    <label igxLabel>Select a date</label>
</igx-date-picker>
```
...
```html
<igx-date-picker [specialDates]="specialDates">
    <label igxLabel>Select a date</label>
</igx-date-picker>
```
