...
You can read about the <ApiLink type="LabelDirective" label="igxLabel" /> and <ApiLink type="InputDirective" label="igxInput" /> directives as well as their validation, data binding and API in the [Label & Input documentation](/label-input).
...
The <ApiLink type="HintDirective" label="igx-hint" /> directive provides a helper text placed below the input. It can be at the beginning or at the end of the input depending on the value of the <ApiLink type="HintDirective" member="position" label="position" /> property. Let's add a hint to our phone input:
...
Using the <ApiLink kind="variable" type="IGX_INPUT_GROUP_TYPE" /> injection token allows to specify a type on an application level for all input-group instances. It provides an easy way to style all related components at once.
To set the type, use the <ApiLink kind="variable" type="IGX_INPUT_GROUP_TYPE" /> injection token to create a DI provider.
...
<DocsAside type="info">
The <ApiLink type="InputGroup" member="type" label="type" /> property has precedence over a <ApiLink kind="variable" type="IGX_INPUT_GROUP_TYPE" />, thus a token value can be overridden on a component level if the type property is set explicitly.
...
Similar to the template-driven form sample, when having additional validation like `email` and `minlength`, an <ApiLink type="HintDirective" label="igx-hint" /> directive could be used to notify the end user if the validation has failed.
