...
<Sample src="/data-entries/input-group-sample-1" height={100} alt="Angular Input Group Example" />
...
You can read about the <ApiLink type="LabelDirective" suffix={false} label="igxLabel" /> and <ApiLink type="InputDirective" suffix={false} label="igxInput" /> directives as well as their validation, data binding and API in the [Label & Input documentation](/label-input).
...
<Sample src="/data-entries/input-group-sample-3" height={110} alt="Angular Input Group Example" />

<hr/>

### Hints

The <ApiLink type="HintDirective" suffix={false} label="igx-hint" /> directive provides a helper text placed below the input. It can be at the beginning or at the end of the input depending on the value of the <ApiLink type="HintDirective" suffix={false} member="position" label="position" /> property. Let's add a hint to our phone input:
...
Using the <ApiLink kind="variable" type="IGX_INPUT_GROUP_TYPE" prefixed={false} /> injection token allows to specify a type on an application level for all input-group instances. It provides an easy way to style all related components at once.
To set the type, use the <ApiLink kind="variable" type="IGX_INPUT_GROUP_TYPE" prefixed={false} /> injection token to create a DI provider.
...
<DocsAside type="info">
The <ApiLink type="InputGroup" member="type" label="type" /> property has precedence over a <ApiLink kind="variable" type="IGX_INPUT_GROUP_TYPE" prefixed={false} />, thus a token value can be overridden on a component level if the type property is set explicitly.
...
<Sample src="/data-entries/input-group-sample-5" height={695} alt="" />

### Input Group Theme

The input group component supports several themes - `material`, `fluent`, `bootstrap`, and `indigo-design`; The <ApiLink type="InputGroup" member="theme" label="theme" /> is automatically set during component initialization and is inferred from the currently used stylesheet. If you plan to support several themes in your application with runtime switching, you can explicitly set the theme using the `theme` Input property.
...
<Sample src="/data-entries/typed-form" height={770} alt="" />
...
The result from the above configurations could be seen in the below sample. Start typing into the Email and Password fields and you will notice that the <ApiLink type="HintDirective" suffix={false} label="igx-hint" /> is shown if the entered values are invalid. The sample also demonstrates how to toggle the password's visibility by using the <ApiLink type="Icon" label="igx-icon" /> and the [`igx-suffix`](#prefix--suffix) directive.

<Sample src="/data-entries/template-driven-form-validation" height={480} alt="" />
...
Similar to the template-driven form sample, when having additional validation like `email` and `minlength`, an <ApiLink type="HintDirective" suffix={false} label="igx-hint" /> directive could be used to notify the end user if the validation has failed.
...
The result from the above configurations could be seen in the below sample. Similar to the template-driven form sample, it also demonstrates how to toggle the password's visibility by using the <ApiLink type="Icon" label="igx-icon" /> and the [`igx-suffix`](#prefix--suffix) directive.

<Sample src="/data-entries/reactive-form-validation" height={480} alt="" />
...
<Sample src="/data-entries/reactive-form-custom-validation" height={480} alt="" />
...
To customize the appearance of input groups, you can create a new theme by extending the <ApiLink kind="sass" module="themes" type="function-input-group-theme" label="input-group-theme" code={true} />. This approach allows you to override only the parameters you want to change, while the rest are automatically handled by the base theme.
...
<Sample src="/data-entries/input-group-style" height={230} alt="" />
...
You can find the full list of properties in the <ApiLink kind="sass" module="themes" type="function-input-group-theme" label="input-group-theme" />. The syntax is as follows:
...
<Sample src="/data-entries/input-group-tailwind-style" height={350} iframeOnly />
