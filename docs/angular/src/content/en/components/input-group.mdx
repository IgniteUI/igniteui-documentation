...
>[!NOTE]
>The <ApiLink type="InputGroup" suffix={false} member="type" label="type" /> property has precedence over a [IGX_INPUT_GROUP_TYPE]({environment:angularApiUrl}/index.html#igx_input_group_type), thus a token value can be overridden on a component level if the type property is set explicitly.
