...
<Sample src="/layouts/stepper-overview-sample" height={725} alt="Angular Stepper Example" />

<hr/>

Here is the a sample demonstrating how to achieve the above functionality using Angular Reactive Forms.

<Sample src="/layouts/stepper-sample-reactive-forms" height={725} alt="Angular Stepper Example with Reactive Forms" />
...
@Component({
    ...
    template: `<igx-stepper>
        <igx-step> 
            <p igxStepTitle>Step 1
</div>
        </igx-step>
        <igx-step> 
            <p igxStepTitle>Step 2
</div>
        </igx-step>
    </igx-stepper>`,
...
<ApiLink type="Step" /> is the representation of every step that belongs to the <ApiLink type="Stepper" />. Steps provide <ApiLink type="Step" member="isValid" label="isValid" />, <ApiLink type="Step" member="active" label="active" />, <ApiLink type="Step" member="optional" label="optional" />, <ApiLink type="Step" member="disabled" label="disabled" /> and <ApiLink type="Step" member="completed" label="completed" /> properties, which give you the ability to configure the step states according to your business requirement.
...
<igx-stepper>
    <igx-step>
       ...
       <p igxStepSubtitle>Home Sub Title
</div>
       <div igxStepContent>
          ...
...
You can customize the stepper orientation through the exposed <ApiLink type="Stepper" member="orientation" label="orientation" /> property. It takes a member of the `IgxStepperOrientation` enum - `Horizontal` _(default value)_ or `Vertical`.

**Horizontal Stepper Orientation**

`horizontal` is the default value for the `igx-stepper` <ApiLink type="Stepper" member="orientation" label="orientation" /> property.
When the stepper is horizontally orientated you have the opportunity to determine whether the steps’ content would be displayed above or below the steps’ headers. This could be achieved by setting the <ApiLink type="Stepper" /> <ApiLink type="Stepper" member="contentTop" label="contentTop" /> boolean property, which default value is `false`. In case it is enabled the steps’ content would be displayed above the steps’ headers.
...
The sample below demonstrates how stepper <ApiLink type="Stepper" member="orientation" label="orientation" /> and [titles position](/stepper#customizing-the-steps) could be changed runtime.

<Sample src="/layouts/stepper-label-position-and-orientation-sample" height={528} alt="Angular Stepper Orientation Example" />

<hr/>

### Step States

<ApiLink type="Stepper" /> supports four steps states and each of them apply different styles by default:

- <ApiLink type="Step" member="active" label="**active**" /> - Determines whether the step is the currently displayed. By design, if the user does not explicitly set some step’s active attribute to `true`, the initial active step would be the first non-disabled step.
- <ApiLink type="Step" member="disabled" label="**disabled**" /> - Determines whether the step is interactable. By default, the disabled attribute of a step is set to `false`.
- <ApiLink type="Step" member="optional" label="**optional**" /> - By default, the optional attribute of a step is set to `false`. If validity of a step in linear stepper is not required, then the optional attribute can be enabled in order to be able to move forward independently from the step validity.
- <ApiLink type="Step" member="completed" label="**completed**" /> - By default, the completed attribute of a step returns `false`. The user, however, can override this default completed behavior by setting the completed attribute as needed. When step is marked as completed not only that the style of the step header is changed by default, but also the style of the progress line between the completed step and the next one. Both styles could be modified using the exposed <ApiLink kind="sass" module="themes" type="function-stepper-theme" label="CSS variables" />.
...
The `igx-stepper` gives you the opportunity to set its steps flow using the <ApiLink type="Stepper" member="linear" label="linear" /> property. By default, linear is set to `false` and the user is enabled to select any non-disabled step in the <ApiLink type="Stepper" />.

When the <ApiLink type="Stepper" member="linear" label="linear" /> property is set to `true`, the stepper will require the current non-optional step to be valid before proceeding to the next one.
...
<Sample src="/layouts/stepper-linear-sample" height={430} alt="Angular Linear Stepper Example" />
...
- <ApiLink type="Stepper" member="navigateTo" label="**navigateTo**" /> – activates the step by given index.
- <ApiLink type="Stepper" member="next" label="**next**" /> - activates the next non-disabled step.
- <ApiLink type="Stepper" member="prev" label="**prev**" /> – activates the previous non-disabled step.
- <ApiLink type="Stepper" member="reset" label="**reset**" /> – resets the stepper to its initial state.
...
This could be achieved through the <ApiLink type="Stepper" member="stepType" label="stepType" /> property of the <ApiLink type="Stepper" />. It takes a member of the `IgxStepType` enum:
...
The user can configure the title position using the <ApiLink type="Stepper" member="titlePosition" label="titlePosition" /> property. Both properties take member of `IgxStepperTitlePosition` enum:
...
<DocsAside type="info">
<ApiLink type="Stepper" member="titlePosition" label="titlePosition" /> property is applicable **only** when the stepper <ApiLink type="Stepper" member="stepType" label="stepType" /> property is set to `full`.
</DocsAside>

**Indicator**

If you want to display only indicators for the steps, set the <ApiLink type="Stepper" member="stepType" label="stepType" /> option to `indicator`.

The step indicator supports any content, however with the restriction that its size would be always **24 pixels**. Having this in mind, we recommend using [IgxIconComponent](/icon) or [IgxAvatarComponent](/avatar) as step indicators.

**Title**

If you want to display only titles for the steps, set the <ApiLink type="Stepper" member="stepType" label="stepType" /> option to `title`.
...
<Sample src="/layouts/stepper-steptypes-sample" height={300} alt="Angular Step Types Example" />
...
When the stepper is **horizontally orientated**, it is configured to use the `slide` animation by default. It also supports `fade` as an alternative. The animations are configured through the <ApiLink type="Stepper" member="horizontalAnimationType" label="horizontalAnimationType" /> input.

In **vertically orientated** layout, the animation type could be defined using the <ApiLink type="Stepper" member="verticalAnimationType" label="verticalAnimationType" /> property. By default, its value is `grow` and the user has the ability to set it to `fade` as well.

Setting `none` to the both animation type inputs disables stepper animations.

<ApiLink type="Stepper" /> gives you the ability to configure the duration of the transition between the steps. This could be achieved through the <ApiLink type="Stepper" member="animationDuration" label="animationDuration" /> property, which takes a number as an argument and it is common to the both orientations.
...
The <ApiLink type="Stepper" /> navigation is compliant with [W3 accessibility standards](https://www.w3.org/WAI/ARIA/apg/example-index/tabs/tabs-manual.html#accessibilityfeatures) and convenient to use.
...
<Sample src="/layouts/stepper-styling-sample" height={264} alt="Angular Stepper Styling Example" />
...
You can find the full list of properties in the <ApiLink kind="sass" module="themes" type="function-stepper-theme" label="stepper-theme" />. The syntax is as follows:
