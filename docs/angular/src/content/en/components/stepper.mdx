...
The <ApiLink type="Stepper" /> emits the following events when navigating between steps:
...
```html
<igx-stepper (activeStepChanging)="onActiveStepChanging($event)">
    ...
</igx-stepper>
```
...
```html
<igx-stepper (activeStepChanged)="onActiveStepChanged($event)">
    ...
</igx-stepper>
```
