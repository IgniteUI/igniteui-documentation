...
In our `ts` file, we should import <ApiLink type="TransactionService" /> from the `igniteui-angular` library, as well as the <ApiLink type="State" kind="interface" /> and <ApiLink type="Transaction" kind="interface" /> interfaces and the <ApiLink type="TransactionType" kind="enum" /> enum, which will be needed by our application:
