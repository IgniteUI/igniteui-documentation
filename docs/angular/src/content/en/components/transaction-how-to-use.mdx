...
Once we are done with all our changes, we may commit them all at once using the <ApiLink type="TransactionService" suffix={false} member="commit" label="commit" /> method of the <ApiLink type="TransactionService" suffix={false} />. It applies all transactions over the provided data:
...
If we are using the <ApiLink type="HierarchicalTransactionService" suffix={false} /> we can also use an overload of the <ApiLink type="TransactionService" suffix={false} member="commit" label="commit" /> method which expects primaryKey and childDataKey as arguments.
...
At any point of our interaction with the list, we may clear the Transaction log, using the <ApiLink type="TransactionService" suffix={false} member="clear" label="clear" /> method.
