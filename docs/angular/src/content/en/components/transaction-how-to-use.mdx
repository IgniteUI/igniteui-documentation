...
In our `ts` file, we should import <ApiLink type="igxTransactionService" suffix={false} /> from the `igniteui-angular` library, as well as the <ApiLink type="State" kind="interface" prefixed={false} /> and <ApiLink type="Transaction" kind="interface" prefixed={false} /> interfaces and the <ApiLink type="TransactionType" kind="enum" prefixed={false} /> enum, which will be needed by our application:
