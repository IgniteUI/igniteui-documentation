...
The month picker uses the calendar's theme, so we have to create a new theme that extends the <ApiLink kind="sass" module="themes" type="function-calendar-theme" label="calendar-theme" code={true} />. To style the month picker's items, you can set the `$content-background` parameter. Optionally, you can also set `$header-background` if you want to override the rest of the properties.
