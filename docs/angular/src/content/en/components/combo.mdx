...
Since `16.0.0`, `IgxComboComponent` is a standalone component. Import it directly or use the `IGX_COMBO_DIRECTIVES` token, which includes the component and all of its supporting components and directives.
...
For applications using NgModules (Angular versions prior to standalone support), import `IgxComboModule` in your **app.module.ts**:
