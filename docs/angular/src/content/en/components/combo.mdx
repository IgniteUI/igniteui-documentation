...
The <ApiLink type="Combo" /> uses the <ApiLink type="DropDown" /> internally as an item container. It also includes the <ApiLink type="InputGroup" /> and the <ApiLink type="Checkbox" /> components. Creating new themes, that extend these components' themes, and scoping them under the respective classes will let you change the combobox styles:
