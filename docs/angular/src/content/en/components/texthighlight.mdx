...
Let's say we want to provide an even richer styling to our highlighted text parts. In order to do this, we can take advantage of the <ApiLink type="TextHighlightDirective" suffix={false} member="cssclass" label="cssClass" /> and the <ApiLink type="TextHighlightDirective" suffix={false} member="activecssclass" label="activeCssClass" /> inputs of the <ApiLink type="TextHighlightService" suffix={false} /> directive. We can combine these classes with the styles from the [`highlight-theme`]({environment:sassApiUrl}/themes#function-highlight-theme) and provide an awesome experience to our users!
