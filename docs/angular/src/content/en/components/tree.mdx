...
<ApiLink type="TreeNodes" /> is the representation of every node that belongs to the <ApiLink type="Tree" />.  
...
<ApiLink type="TreeNode" member="data" label="data" /> property can be used to add a reference to the data entry the node represents. Binding `[data]` is required for searching through nodes using <ApiLink type="TreeComponent.findNodes()" suffix={false} member="findNodes" label="IgxTreeComponent.findNodes()" />.
