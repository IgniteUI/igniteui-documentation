...
The Ignite UI for Angular Excel Exporter service can export data in Microsoft® Excel® format from raw data (array) or from the [**IgxGrid**](/grid/grid), [**IgxTreeGrid**](/treegrid/tree-grid) and [**IgxHierarchicalGrid**](/hierarchicalgrid/hierarchical-grid) components. The exporting functionality is encapsulated in the <ApiLink type="ExcelExporterService" suffix={false} /> class and the data is exported in MS Excel table format. This format allows features like filtering, sorting, etc.
</div>
<igc-divider></igc-divider>

## Angular Excel Exporter Example

<Sample src="/services/export-excel" height={100} alt="Angular Excel Exporter Example" />

<hr/>

## Usage

To start using the IgniteUI Excel Exporter first import the <ApiLink type="ExcelExporterService" suffix={false} /> in the app.module.ts file and add the service to the `providers` array:
...
You may access the exporter service by defining an argument of type <ApiLink type="ExcelExporterService" suffix={false} /> in the component's constructor and the Angular framework will provide an instance of the service. To export some data in MS Excel format you need to invoke the exporter service's <ApiLink type="ExcelExporterService" suffix={false} member="exportdata" label="exportData" /> method. This method accepts as a first argument the data you want to export and the second argument is of type <ApiLink type="ExcelExporterOptions" suffix={false} /> and allows you to configure the export process.
