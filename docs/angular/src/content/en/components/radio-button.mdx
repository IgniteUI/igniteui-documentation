...
To get started, create an <ApiLink type="RadioGroupDirective" label="igxRadioGroup" /> and add several <ApiLink type="Radio" label="igxRadio" /> components.

Note that, setting a <ApiLink type="RadioGroupDirective" member="name" label="name" /> property for the radio group is **mandatory**.
...
Use the <ApiLink type="RadioGroupDirective" member="alignment" label="alignment" /> input property to change the orientation of the `igxRadio` components in the radio group. Users can choose between `horizontal` and `vertical`. By default the radio group alignment is horizontal.
