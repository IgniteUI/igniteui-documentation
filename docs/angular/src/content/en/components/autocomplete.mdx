...
The next step is to import the **IgxAutocompleteModule** and **IgxDropDownModule** in our **app.module**. If <ApiLink type="AutocompleteDirective" suffix={false} label="igxAutocomplete" /> is applied on an <ApiLink type="InputDirective" suffix={false} label="igxInput" />, the **igxInputGroupModule** is also required:
...
You can disable the Angular autocomplete by using the <ApiLink type="AutocompleteDirective" suffix={false} member="disabled" label="IgxAutocompleteDisabled" /> input:
...
The `igx-autocomplete` dropdown positioning, scrolling strategy, and outlet can be configured using the <ApiLink type="AutocompleteDirective" suffix={false} member="autocompleteSettings" label="IgxAutocompleteSettings" />.
