...
<DocsAside type="note">
The <ApiLink type="AutocompleteDisabled" suffix={false} label="igxAutocomplete" /> uses the <ApiLink type="DropDown" label="igxDropDown" /> as a provider for the available options, which means that all capabilities of the dropdown component can be used in the autocomplete.
</DocsAside>

### Disable Angular Autocomplete

You can disable the Angular autocomplete by using the <ApiLink type="AutocompleteDisabled" suffix={false} member="disabled" label="IgxAutocompleteDisabled" /> input:
...
The `igx-autocomplete` dropdown positioning, scrolling strategy, and outlet can be configured using the <ApiLink type="AutocompleteSettings" suffix={false} member="autocompleteSettings" label="IgxAutocompleteSettings" />.
