...
The AI will call `create_custom_palette` with `mode: "explicit"` for the primary color and auto-generate the rest. See [Palettes](/themes/palettes) for more detail on shade generation.
...
Also confirm that `core()` is called before any other theming mixin in your `styles.scss`. See [Theming with Sass](/themes/sass) for the correct file setup.

## Additional Resources

Related topics:

- [Theming Overview](/themes)
