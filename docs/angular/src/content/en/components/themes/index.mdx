...
llms:
  description: "Ignite UI for Angular allows you to modify the styles of all component themes using CSS variables."
---
...
To find out which palette colors are used by what component, take a look at the <ApiLink kind="sass" module="themes" label="Components Documentation" />.
</DocsAside>
...
These are essentially stacked CSS [`box-shadow`](https://developer.mozilla.org/en-US/docs/Web/CSS/box-shadow) declarations. You can replace them with any other valid `box-shadow` value. The higher the elevation level number is, the bigger the shadow is. Again, different components use different elevation levels, to find out which elevation levels are used by which component, take look at the <ApiLink kind="sass" module="themes" label="Components Documentation" />. We will take a deeper look at elevations in the [Elevations](/themes/elevations) of the documentation.
