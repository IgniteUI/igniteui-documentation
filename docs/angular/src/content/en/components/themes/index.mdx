...
Some components do not use colors from the palettes. In those instances, target the component's CSS variables directly to modify their colors. To find out which palette colors a component uses, see the <ApiLink kind="sass" module="themes" label="Sass Theming API Reference" />.
</DocsAside>
...
Each variable is a stacked CSS [`box-shadow`](https://developer.mozilla.org/en-US/docs/Web/CSS/box-shadow) declaration; you can replace any of them with another valid `box-shadow` value. The higher the elevation level, the larger the shadow. Different components use different elevation levels by default — see [Elevations](/themes/elevations) for the full breakdown, and the <ApiLink kind="sass" module="themes" label="Sass Theming API Reference" /> for which level each component uses.
