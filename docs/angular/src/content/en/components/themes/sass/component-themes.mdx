...
There are even more ways you can organize and scope your component themes. The most straightforward way to do that is in the same file you defined your [**global theme**](/themes/sass/global-themes).
...
Any CSS you author outside of an `@layer` block is treated by the browser as unlayered, and unlayered styles always take precedence over layered styles, no matter their specificity. This means plain, unlayered overrides in your application stylesheet (including ones added via `::ng-deep`) will continue to beat Ignite UI's own layered styles, so you generally don't need extra specificity tricks to override component styles with your own CSS rules.
</DocsAside>
