...
Ignite UI for Angular includes a set of timing functions that can be used to ease in or out an animation. There are three main timing function groups - <ApiLink type="EaseIn" kind="enum" prefixed={false} />, <ApiLink type="EaseOut" kind="enum" prefixed={false} />, and <ApiLink type="EaseInOut" kind="enum" prefixed={false} />, each containing the following timings:
