...
This method returns a <ApiLink pkg="excel" prefixed={false} type="CellFormat" /> instance which refers back to the associated <ApiLink pkg="excel" prefixed={false} type="CellFormat" /> on which it is based. So subsequent changes to the <ApiLink pkg="excel" prefixed={false} type="WorksheetCell" member="cellFormat" label="CellFormat" /> property will be reflected in the instance returned from a `GetResolvedCellFormat` call.
