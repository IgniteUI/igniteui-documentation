...
Every [`igx-column-group`]({environment:angularApiUrl}/classes/igxcolumngroupcomponent.html) supports [`moving`](/treegrid/column-moving), [`pinning`](/treegrid/column-pinning) and [`hiding`](/treegrid/column-hiding).
> [!NOTE]
