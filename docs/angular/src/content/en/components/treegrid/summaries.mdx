...
> [!NOTE]
> In order to calculate the summary row height properly, the Tree Grid needs the <ApiLink type="SummaryOperand" suffix={false} member="operate" label="operate" /> method to always return an array of <ApiLink type="IgxSummaryResult" kind="interface" prefixed={false} /> with the proper length even when the data is empty.
