...
In addition to changing the exported filename, the user can further configure the exporter options by waiting for the [toolbarExporting]({environment:angularApiUrl}/classes/igxtreegridcomponent.html#toolbarExporting) event and customizing the options entry in the event properties.
