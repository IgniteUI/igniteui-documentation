...
<Sample src="/tree-grid/treegrid-remote-filtering" height={550} alt="Angular Tree Grid Remote Data Operations Overview Example" />
...
To provide remote filtering, you need to subscribe to the <ApiLink type="TreeGrid" member="filteringExpressionsTreeChange" label="filteringExpressionsTreeChange" /> output so that you make the appropriate request based on the arguments received. Let's use a flat collection as a data source for our Tree Grid by providing a <ApiLink type="TreeGrid" member="primaryKey" label="primaryKey" /> and a <ApiLink type="TreeGrid" member="foreignKey" label="foreignKey" />.
...
When remote filtering is provided, usually we do not need the built-in filtering of the Tree Grid. We can disable it by setting the <ApiLink type="TreeGrid" member="filterStrategy" label="filterStrategy" /> input of the Tree Grid to the `NoopFilteringStrategy` instance.
...
<Aside type="note">
When the <ApiLink type="TreeGrid" member="uniqueColumnValuesStrategy" label="uniqueColumnValuesStrategy" /> input is provided, the default unique values generating process in the excel style filtering will not be used.
</Aside>
