...
- <ApiLink type="GridCell" /> - Tree cell that contains a value, an expand/collapse indicator and an indentation div element, which is based on the level of the cell's row. The level of a row component can be accessed through the <ApiLink kind="interface" type="ITreeGridRecord" member="level" /> property of its inner <ApiLink type="TreeGridRow" member="treeRow" />.
...
The Tree Grid allows styling through the [`Ignite UI for Angular Theme Library`](/themes/sass/component-themes). The tree grid's <ApiLink kind="sass" module="themes" type="function-grid-theme" label="grid-theme" code={true} /> exposes a wide variety of properties, which allows the customization of all the tree grid's features.
...
Next we need to create a custom theme, the easiest and recommended way to style the `igx-tree-grid` is to use the <ApiLink kind="sass" module="themes" type="function-grid-theme" label="grid-theme" code={true} /> and provide just the three main colors: `background`, `foreground`, and `accent-color`.
