...
The column selection feature can be enabled through the [`columnSelection`]({environment:angularApiUrl}/classes/igxtreegridcomponent.html#columnSelection) input, which takes [GridSelectionMode]({environment:angularApiUrl}/index.html#gridselectionmode) values.
...
<Aside type="note">
The keyboard combinations are available only when the grid [`columnSelection`]({environment:angularApiUrl}/classes/igxtreegridcomponent.html#columnselection) input is set to `multiple`.
</Aside>
