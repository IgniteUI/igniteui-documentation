...
In it, you can see how to define an `igx-accordion` and its <ApiLink type="ExpansionPanel" label="expansion panels" />. The sample also demonstrates the two types of expansion behavior. The switch button sets the <ApiLink type="Accordion" member="singleBranchExpand" label="singleBranchExpand" /> property to toggle between single and multiple branches to be expanded at a time.
