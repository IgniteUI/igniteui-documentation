...
If you still need the detailed setup reference for each client, see [Angular Schematics & Ignite UI CLI](~/components/general/cli-overview.mdx) and [Ignite UI Theming MCP](~/components/ai/theming-mcp.mdx).
...
- [Ignite UI for Angular Skills](~/components/ai/skills.mdx)
