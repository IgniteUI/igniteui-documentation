...
- Public APIs, which used to return an instance of one of the above, now return objects implementing the public <ApiLink type="RowType" kind="interface" prefixed={false} /> interface:
...
While the public API of <ApiLink type="RowType" kind="interface" prefixed={false} /> is the same as what _IgxRowComponent_ and others used to expose, please note:
