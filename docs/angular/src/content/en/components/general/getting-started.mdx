...
SCSS is recommended because the [Ignite UI for Angular Theming Library](../themes/sass/index.mdx) is built on it and `ng add` configures the default theme automatically. Then add Ignite UI for Angular:
...
<Image src={igniteuiProject} alt="Ignite UI for Angular application with IgxGridComponent rendered" class="responsive-img" />
