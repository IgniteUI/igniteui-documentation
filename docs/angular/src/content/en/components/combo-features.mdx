...
To get started with the combobox component, import `IGX_COMBO_DIRECTIVES`. Our sample also uses the <ApiLink type="Switch" label="igx-switch" /> component to toggle combobox properties' values, so we will need `IgxSwitchComponent` as well:
