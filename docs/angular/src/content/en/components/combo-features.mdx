...
The combobox component allows users to change the way a list of items is shown. This can be done by defining <ApiLink kind="interface" prefixed={false} suffix={false} type="OverlaySettings" label="Custom OverlaySettings" /> and passing them to the <ApiLink type="Combo" member="overlaySettings" label="ComboBox's OverlaySettings" /> input:
...
<DocsAside type="info">
The combobox component uses the <ApiLink type="AutoPositionStrategy" suffix={false} prefixed={false} /> as a default position strategy.
</DocsAside>
