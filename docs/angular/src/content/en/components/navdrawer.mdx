...
While any content can be provided in the template, the <ApiLink type="igxDrawerItem" suffix={false} /> directive (see [Item styling](#styling)) is available to apply out-of-the-box styling to items.
...
The Navigation Drawer also integrates with <ApiLink type="igxNavigationService" suffix={false} /> and can be targeted by id with an [`igxToggleAction`](/toggle#automatic-toggle-actions) directive.
...
<DocsAside type="note">
By default the Navigation Drawer is responsive, actively changing between unpinned and pinned mode based on screen size. This behavior is controlled by the <ApiLink type="NavigationDrawer" member="pinThreshold" label="pinThreshold" /> property and can be disabled by setting a falsy value (e.g. 0).
</DocsAside>
