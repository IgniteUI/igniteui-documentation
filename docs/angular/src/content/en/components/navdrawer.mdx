...
While any content can be provided in the template, the <ApiLink type="NavDrawerItemDirective" suffix={false} /> directive (see [Item styling](#styling)) is available to apply out-of-the-box styling to items.
...
The Navigation Drawer also integrates with <ApiLink type="NavigationService" suffix={false} /> and can be targeted by id with an [`igxToggleAction`](/toggle#automatic-toggle-actions) directive.
