...
<DocsAside type="note">
By default the Navigation Drawer is responsive, actively changing between unpinned and pinned mode based on screen size. This behavior is controlled by the <ApiLink type="NavigationDrawer" member="pinThreshold" label="pinThreshold" suffix={false} /> property and can be disabled by setting a falsy value (e.g. 0).
</DocsAside>
