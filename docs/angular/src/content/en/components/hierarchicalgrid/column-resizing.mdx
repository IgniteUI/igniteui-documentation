...
<Sample src="/hierarchical-grid/hierarchical-grid-resizing" height={510} alt="Angular Hierarchical Grid Column Resizing Example" />

<div class="divider--half"></div>

**Column resizing** is also enabled per-column level, meaning that the <ApiLink type="HierarchicalGrid" /> can have a mix of resizable and non-resizable columns. This is done via the <ApiLink type="Column" member="resizable" label="resizable" /> input of the <ApiLink type="Column" label="igx-column" />.

```html
<igx-column [field]="'Artist'" [resizable]="true"></igx-column>
```

You can subscribe to the <ApiLink type="HierarchicalGrid" member="columnResized" label="columnResized" /> event of the <ApiLink type="HierarchicalGrid" /> to implement some custom logic when a column is resized. Both, previous and new column widths, as well as the <ApiLink type="Column" /> object, are exposed through the event arguments.
...
<Sample src="/hierarchical-grid/hGrid-columnAutosizing-sample" height={510} alt="Angular Hierarchical Grid Column Resizing Example" />
