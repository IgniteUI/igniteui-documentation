...
<Sample lob src="/hierarchical-grid/hierarchical-grid-lod" height={620} alt="Angular Hierarchical Grid Load On Demand Example" />
...
Now let's add initial filtering rules to our grid so that the root grid is filtered when first loaded. We will create a <ApiLink type="FilteringExpressionsTree" suffix={false} prefixed={false} /> and set it to the <ApiLink type="HierarchicalGrid" member="advancedFilteringExpressionsTree" label="advancedFilteringExpressionsTree" /> property of the `IgxHierarchicalGrid` using the `ngOnInit` lifecycle hook.
