...
The Hierarchical Grid in Ignite UI for Angular provides an <ApiLink type="GridToolbar" /> which is essentially a container for **UI** operations. The Angular toolbar is located at the top of the Angular component, i.e the Hierarchical Grid and it matches its horizontal size. The toolbar container can host predefined UI controls for the following Hierarchical Grid's features:
...
- <ApiLink type="HierarchicalGrid" member="toolbarExporting" label="toolbarExporting" />
