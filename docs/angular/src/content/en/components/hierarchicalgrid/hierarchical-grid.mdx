...
<Sample lob src="/hierarchical-grid/hierarchical-grid-resizing" height={520} alt="Angular Hierarchical Data Grid Example" />
...
Note that setting the <ApiLink type="HierarchicalGrid" member="haschildrenkey" /> property is not required. In case you don't provide it, expansion indicators will be displayed for each row.

Additionally if you wish to show/hide the header expand/collapse all indicator you can use the <ApiLink type="HierarchicalGrid" member="showExpandAll" /> property.
...
Next, you need to create a custom theme, the easiest and recommended way to style the `igx-hierarchical-grid` is to use the <ApiLink kind="sass" module="themes" type="function-grid-theme" label="grid-theme" code={true} /> and provide just the three main colors: `background`, `foreground`, and `accent-color`.
...
- <ApiLink kind="sass" module="themes" type="function-action-strip-theme" label="IgxActionStrip Theme" />
