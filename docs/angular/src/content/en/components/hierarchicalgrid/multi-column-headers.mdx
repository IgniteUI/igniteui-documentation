...
Every [`igx-column-group`]({environment:angularApiUrl}/classes/igxcolumngroupcomponent.html) supports [`moving`](/hierarchicalgrid/column-moving), [`pinning`](/hierarchicalgrid/column-pinning) and [`hiding`](/hierarchicalgrid/column-hiding).
<Aside type="note">
