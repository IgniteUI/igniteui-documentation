...
The default time picker mode is editable dropdown mode. To change the time picker mode to read-only dialog mode, set the <ApiLink type="TimePicker component" suffix={false} member="mode" label="mode" /> input to [`dialog`]({environment:angularApiUrl}/index.html#pickerinteractionmode):
