...
The Time Picker in Angular can be bound to either a Date object or time-only string value in `ISO 8601` format by setting the <ApiLink type="TimePicker" member="value" label="value" /> property or `ngModel`.
...
or set <ApiLink type="TimePicker" member="value" label="value" /> input in the template:
...
The <ApiLink type="TimePicker" /> supports action button customization. To achieve that, wrap the buttons in `ng-template` marked with the <ApiLink type="PickerActionsDirective" /> directive selector.
...
- Users can navigate the component's time portions via the keyboard <kbd>Up</kbd> and <kbd>Down</kbd> arrow keys or by scrolling in the input field and in the dropdown/dialog. Navigation in the input is possible regardless of the <ApiLink type="TimePicker" member="minValue" label="minValue" /> or <ApiLink type="TimePicker" member="maxValue" label="maxValue" />, while navigation in the dropdown/dialog will be restricted within the <ApiLink type="TimePicker" member="minValue" label="minValue" /> and <ApiLink type="TimePicker" member="maxValue" label="maxValue" /> range.
...
The default time picker mode is editable dropdown mode. To change the time picker mode to read-only dialog mode, set the <ApiLink type="TimePicker" member="mode" label="mode" /> input to <ApiLink kind="type" type="PickerInteractionMode" />:
...
or just change the <ApiLink type="TimePicker" member="mode" label="mode" /> in the time picker like this:
...
In dialog mode, the dialog header displays the currently selected time in the picker's <ApiLink type="TimePicker" member="inputformat" label="input format" />. You can change the header position by setting the <ApiLink type="TimePicker" member="headerOrientation" label="headerOrientation" /> property.

<Sample src="/scheduling/timepicker-sample-1" height={600} alt="Angular Time Picker Example" />

<hr/>

When the <ApiLink type="TimePicker" member="minValue" label="minValue" /> and <ApiLink type="TimePicker" member="maxValue" label="maxValue" /> are set the dialog displays the time within that range only. See the [Min max value](#min-max-value) example below, for more details.
...
Alternatively, if the <ApiLink type="TimePicker" member="inputFormat" label="inputFormat" /> property is not set, the input format will be inferred from the <ApiLink type="TimePicker" member="displayFormat" label="displayFormat" /> in case it can be parsed as containing numeric date-time parts only.
...
The time picker exposes public <ApiLink type="TimePicker" member="increment" label="increment" /> and <ApiLink type="TimePicker" member="decrement" label="decrement" /> methods, which accept two optional parameters: the `DatePart` to be modified and the `delta` by which it will be changed. If not specified the `DatePart` defaults to `Hours` and the `delta` defaults to <ApiLink type="TimePicker" member="itemsDelta" label="itemsDelta" />.
...
You can specify <ApiLink type="TimePicker" member="minValue" label="minValue" /> and <ApiLink type="TimePicker" member="maxValue" label="maxValue" /> to restrict the user input, in which case the dropdown/dialog will display the time within that range only. In dropdown mode however, it is still possible for the user to type in an invalid time. You can handle the <ApiLink type="TimePicker" member="validationFailed" label="validationFailed" /> event in order to notify the user if that happens.
...
<DocsAside type="info">
The displayed values for each time portion in the dropdown/dialog are calculated based on the items delta always starting from zero. If the <ApiLink type="TimePicker" member="minValue" label="minValue" /> and <ApiLink type="TimePicker" member="maxValue" label="maxValue" /> does not match the items delta, the displayed values will start/end from the next/last possible value that matches the threshold.
</DocsAside>
...
In reactive forms, we can handle the <ApiLink type="TimePicker" member="valueChange" label="valueChange" /> event of each component and update the value of the other.
...
Following the simplest approach, we create a new theme that extends the <ApiLink kind="sass" module="themes" type="function-dialog-theme" label="time-picker-theme" code={true} /> and accepts parameters that style the time picker.
...
<DocsAside type="info">
In order to style any additional components that are used as part of the time picker window's content (such as the [`IgxButton`](/components/inputs/button)), an additional theme should be created that is specific to the respective component and is placed under the dialog window's scope only (so it does not affect the rest of the application).
</DocsAside>
...
The items in our time picker **are not** descendants of our component `host` - they are currently being displayed in the default overlay outlet, at the end of the `document` body. Changing this is done by making use of the <ApiLink kind="interface" type="OverlaySettings" member="outlet" /> property in the `overlaySettings`. The `outlet` controls where the overlay container should be rendered.
...
- <ApiLink kind="sass" module="themes" type="function-time-picker-theme" label="IgxTimePickerComponent Styles" />
