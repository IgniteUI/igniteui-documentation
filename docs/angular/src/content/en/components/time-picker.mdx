...
The time picker component allows projecting child components - the same as in the <ApiLink type="InputGroup" />: [`igxLabel`](/label-input), [`IgxHint`](/input-group#hints), [`igxPrefix`](/input-group#prefix--suffix), [`igxSuffix`](/input-group#prefix--suffix), excluding <ApiLink type="InputDirective" suffix={false} />. More detailed information about this can be found in the [Label & Input](/label-input) topic.
...
The <ApiLink type="TimePicker" /> supports action button customization. To achieve that, wrap the buttons in `ng-template` marked with the <ApiLink type="PickerActionsDirective" suffix={false} /> directive selector.
...
The default time picker mode is editable dropdown mode. To change the time picker mode to read-only dialog mode, set the <ApiLink type="TimePicker" member="mode" label="mode" /> input to <ApiLink kind="variable" type="PickerInteractionMode" prefixed={false} />:
