...
The default time picker mode is editable dropdown mode. To change the time picker mode to read-only dialog mode, set the <ApiLink type="TimePicker" member="mode" label="mode" /> input to <ApiLink kind="enum" type="PickerInteractionMode" prefixed={false} />:
