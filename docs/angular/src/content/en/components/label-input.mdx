...
<Sample src="/data-entries/input-group-sample-1" height={100} alt="Angular Label & Input Example" />
...
To use the <ApiLink type="InputDirective" suffix={false} label="igxInput" /> and <ApiLink type="LabelDirective" suffix={false} label="igxLabel" />, you have to wrap them in an `<igx-input-group>` container:
...
We can validate an `input` using the <ApiLink type="InputDirective" suffix={false} member="required" label="required" /> attribute. This will add an asterisk next to the label, indicating that this field must be completed. The input will turn green/red depending on whether the validation passes/fails.
...
You can add logic to force `focus` on input elements using the <ApiLink type="igxFocus" suffix={false} /> directive.
...
<DocsAside type="note">
To use the <ApiLink type="igxFocus" suffix={false} /> directive, you have to import the <ApiLink type="FocusModule" suffix={false} />.
</DocsAside>

If you want the text in an input element, marked with `igxInput`, to be selected on focus, you have to enable the <ApiLink type="igxTextSelection" suffix={false} /> directive.
...
<DocsAside type="note">
To use the <ApiLink type="igxTextSelection" suffix={false} /> directive, you have to import the <ApiLink type="TextSelectionModule" suffix={false} />.
</DocsAside>
