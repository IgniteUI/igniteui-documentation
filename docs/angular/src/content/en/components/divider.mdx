...
By adding the <ApiLink type="DividerDirective" member="vertical" label="vertical" /> attribute and setting its value to `true`, you can change the direction of the divider from horizontal to vertical.
...
To change the default look simply use the <ApiLink type="DividerDirective" member="type" label="type" /> attribute of the divider and set its value to `dashed`.
...
To inset the divider, set the <ApiLink type="DividerDirective" member="middle" label="middle" /> attribute of the divider to `true` and provider the desired <ApiLink type="DividerDirective" member="inset" label="inset" /> value, the divider will start shrinking from both ends.
...
If the value of the <ApiLink type="DividerDirective" member="middle" label="middle" /> attribute is set to a false value, or if the attribute is omitted altogether, the divider will set in only on the left.
