...
For scenarios where more than three action items will be shown, it is best to use <ApiLink type="ActionStripMenuItemDirective" suffix={false} /> directive. Any item within the Action Strip marked with the `*igxActionStripMenuItem` structural directive will be shown in a dropdown, revealed upon toggling the more button i.e. the three dots representing the last action.
