...
<Sample src="/grid/grid-row-pinning" height={600} alt="Angular Grid Row Pinning Example" />
...
Row pinning is controlled through the `pinned` input of the <ApiLink kind="interface" type="RowType" suffix={false}  prefixed={false}/>. Pinned rows are rendered at the top of the Grid by default and stay fixed through vertical scrolling of the unpinned rows in the Grid body.

```typescript
this.grid.getRowByIndex(0).pinned = true;
```

You may also use the Grid's <ApiLink type="Grid" member="pinRow" label="pinRow" /> or <ApiLink type="Grid" member="unpinRow" label="unpinRow" /> methods of the <ApiLink type="Grid" /> to pin or unpin records by their ID:
...
Note that the row ID is the primary key value, defined by the <ApiLink type="Grid" member="primaryKey" label="primaryKey" /> of the grid, or the record instance itself. Both methods return a boolean value indicating whether their respective operation is successful or not. Usually the reason they fail is that the row is already in the desired state.

A row is pinned below the last pinned row. Changing the order of the pinned rows can be done by subscribing to the <ApiLink type="Grid" member="rowPinning" label="rowPinning" /> event and changing the <ApiLink kind="interface" prefixed={false} suffix={false} type="IPinRowEventArgs" member="insertAtIndex" /> property of the event arguments to the desired position index.
...
You can change the row pinning position via the <ApiLink type="Grid" member="pinning" label="pinning" /> configuration option. It allows you to set the pin area position to either Top or Bottom.
