...
The column selection feature can be enabled through the [`columnSelection`]({environment:angularApiUrl}/classes/igxgridcomponent.html#columnSelection) input, which takes [GridSelectionMode]({environment:angularApiUrl}/index.html#gridselectionmode) values.
...
> [!NOTE]
> The keyboard combinations are available only when the grid [`columnSelection`]({environment:angularApiUrl}/classes/igxgridcomponent.html#columnselection) input is set to `multiple`.
