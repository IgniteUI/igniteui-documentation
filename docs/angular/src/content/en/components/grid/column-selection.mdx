...
<Sample src="/grid/column-selection" height={570} alt="Angular Column Selection Example" />

<div class="divider--half"></div>

## Basic usage

The column selection feature can be enabled through the <ApiLink type="Grid" member="columnSelection" label="columnSelection" /> input, which takes [GridSelectionMode]({environment:angularApiUrl}/index.html#gridselectionmode) values.
...
<Aside type="note">
The keyboard combinations are available only when the grid <ApiLink type="Grid" member="columnselection" label="columnSelection" /> input is set to `multiple`.
</Aside>
