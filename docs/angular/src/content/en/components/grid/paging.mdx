...
Adding a [`igx-paginator`](/paginator) component will control whether the feature is present, you can enable/disable it by using a simple `*ngIf` with a toggle property. The <ApiLink type="Paginator" member="perPage" label="perPage" /> input controls the visible records per page. Let’s update our Grid to enable paging:
