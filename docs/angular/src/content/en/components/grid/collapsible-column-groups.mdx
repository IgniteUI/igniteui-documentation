...
The next step is to import the `IgxGridModule` in the app.module.ts file. Also, we strongly suggest that you take a brief look at [_multi-column groups_](/grid/multi-column-headers) topic, to see more detailed information on how to setup the column groups in your grid.
