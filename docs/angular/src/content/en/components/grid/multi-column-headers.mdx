...
Every [`igx-column-group`]({environment:angularApiUrl}/classes/igxcolumngroupcomponent.html) supports [`moving`](/grid/column-moving), [`pinning`](/grid/column-pinning) and [`hiding`](/grid/column-hiding).
> [!NOTE]
