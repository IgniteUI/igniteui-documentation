...
Then define a Grid with bound data source and <ApiLink type="Grid" member="rowEditable" label="rowEditable" /> set to true and an [Action Strip](/action-strip) component with editing actions enabled. The <ApiLink type="GridEditingActions" member="addRow" label="addRow" /> input controls the visibility of the button that spawns the row adding UI.
...
The internal <ApiLink type="BaseTransactionService" suffix={false} /> is automatically provided for Grid. It holds pending cell changes until the row state is submitted or cancelled.
...
The add row UI has the same behavior as the row editing one as they are designed to provide a consistent editing experience to end users. Please, refer to the [Grid Row Editing](/grid/row-editing) topic for more information.
...
- [Grid Row Editing](/grid/row-editing#styling)
