...
There is a convenient way to keep the state of the `igxToggle` directive and command it via the <ApiLink type="NavigationService" suffix={false} /> provider. We just need to set an identifier for our `igxToggle` element, which will be used to register the toggle with the service. If we would like to control its state automatically, we have to pass this identifier to the `igxToggleActionDirective`.
...
- <ApiLink type="NavigationService" suffix={false} />
