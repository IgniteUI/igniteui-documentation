...
Selection can be enabled by setting the <ApiLink type="Chip" member="selectable" label="selectable" /> input property to `true`. When selecting a chip, the <ApiLink type="Chip" member="selectedChanging" label="selectedChanging" /> event is fired. It provides the new <ApiLink kind="interface" prefixed={false} suffix={false} type="IChipSelectEventArgs" member="selected" /> value so you can get the new state and the original event in <ApiLink kind="interface" prefixed={false} suffix={false} type="IChipSelectEventArgs" member="originalEvent" /> that triggered the selection change. If this is not done through user interaction but instead is done by setting the <ApiLink kind="interface" prefixed={false} suffix={false} type="IChipSelectEventArgs" member="selected" /> property programmatically, the <ApiLink kind="interface" prefixed={false} suffix={false} type="IChipSelectEventArgs" member="originalEvent" /> argument has a value of `null`.
