...
Several pre-existing <ApiLink pkg="layouts" type="ToolAction" /> items and menus become available when the <ApiLink pkg="charts" type="DataChart" /> is linked with the Toolbar. Here is a list of the built-in Angular `DataChart` Tool Actions and their associated `OverlayId`:
...
<ApiLink pkg="charts" type="DataChart" /><br />
