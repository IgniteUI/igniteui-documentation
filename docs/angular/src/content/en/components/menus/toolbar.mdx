...
The Angular Toolbar contains a `Target` property. This is used to link a component, such as the `XamDataChart` as shown in the code below:
...
Several pre-existing `ToolAction` items and menus become available when the `XamDataChart` is linked with the Toolbar. Here is a list of the built-in Angular `XamDataChart` Tool Actions and their associated `OverlayId`:
