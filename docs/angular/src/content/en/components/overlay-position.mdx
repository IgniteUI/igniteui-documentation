...
<Sample src="/interactions/overlay-sample-main-1" height={350} alt="Angular Positioning Strategies Example" />
...
Positions the content, based on the directions passed in through <ApiLink kind="interface" prefixed={false} suffix={false} type="PositionSettings" />. These are Left/Center/Right for <ApiLink kind="interface" prefixed={false} suffix={false} type="PositionSettings" member="horizontalDirection" /> and Top/Middle/Bottom for <ApiLink kind="interface" prefixed={false} suffix={false} type="PositionSettings" member="verticalDirection" />. Defaults are:
...
Positions the element based on the start point from <ApiLink kind="interface" prefixed={false} suffix={false} type="OverlaySettings" /> and directions passed in through <ApiLink kind="interface" prefixed={false} suffix={false} type="PositionSettings" />. It is possible to either pass a start point (type <ApiLink type="Point" suffix={false} prefixed={false} />) or an `HTMLElement` as a positioning base. Defaults are:
...
Positions the element like the **Connected** positioning strategy and re-sizes the element to fit inside the view port (re-calculating width and/or height) in case the element is partially out of view. <ApiLink kind="interface" prefixed={false} suffix={false} type="PositionSettings" member="minSize" /> can be passed in <ApiLink kind="interface" prefixed={false} suffix={false} type="PositionSettings" /> to prevent resizing if it would put the element's dimensions below a certain threshold. Defaults are:
...
Then specify the positioning strategy to be used by the overlay. The position strategy is passed in as a property in the <ApiLink kind="interface" prefixed={false} suffix={false} type="OverlaySettings" /> parameter when the <ApiLink type="OverlayService" suffix={false} member="attach" label="overlay.attach()" /> method is called. In the example below we are changing the default `GlobalPositionStrategy` with `ConnectedPositionStrategy`:
...
You can also change the positioning strategy, used by the overlay, by overriding the <ApiLink kind="interface" prefixed={false} suffix={false} type="IPositionStrategy" /> property of the <ApiLink kind="interface" prefixed={false} suffix={false} type="OverlaySettings" /> object that is passed to the overlay:
