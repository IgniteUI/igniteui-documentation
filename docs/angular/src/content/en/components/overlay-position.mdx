...
Positions the element based on the start point from <ApiLink kind="interface" type="OverlaySettings" /> and directions passed in through <ApiLink kind="interface" type="PositionSettings" />. It is possible to either pass a start point (type <ApiLink pkg="core" type="Point" />) or an `HTMLElement` as a positioning base. Defaults are:
