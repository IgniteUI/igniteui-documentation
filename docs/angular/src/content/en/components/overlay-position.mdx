...
Positions the element based on the start point from [`overlaySettings`]({environment:angularApiUrl}/interfaces/overlaysettings.html) and directions passed in through [`positionSettings`]({environment:angularApiUrl}/interfaces/positionsettings.html). It is possible to either pass a start point (type <ApiLink type="Point" suffix={false} prefixed={false} />) or an `HTMLElement` as a positioning base. Defaults are:
