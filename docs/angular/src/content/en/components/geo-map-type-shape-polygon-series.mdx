...
In Angular map component, you can use the <ApiLink pkg="maps" type="GeographicShapeSeries" /> to display geo-spatial data using shape polygons in a geographic context. This type of geographic series is often used to render shapes of countries or regions defined by geographic locations.

## Angular Geographic Polygon Map Example


<Sample src="/maps/geo-map/type-shape-polygon-series" height={500} alt="Angular Using Shape Polygon Series Example" />



<div class="divider--half"></div>

The <ApiLink pkg="maps" type="GeographicShapeSeries" /> works a lot like the <ApiLink pkg="maps" type="GeographicPolylineSeries" /> except that geo-spatial data is rendered with polygons instead of polylines.

## Data Requirements
Similar to other types of geographic series in the map control, the <ApiLink pkg="maps" type="GeographicShapeSeries" /> has the `ItemsSource` property which can be bound to an array of objects. In addition, each data item in this object must have one data column that stores single/multiple shapes using an array of arrays of objects with x and y values representing geographic locations. This data column is then mapped to the <ApiLink pkg="maps" type="GeographicShapeSeriesBase" member="shapeMemberPath" /> property. The <ApiLink pkg="maps" type="GeographicShapeSeries" /> uses points of this mapped data column to plot polygons in the map control.

## Code Snippet
The following code demonstrates how to bind the <ApiLink pkg="maps" type="GeographicShapeSeries" /> to shapes of countries in the world loaded from a shape file using the <ApiLink pkg="maps" type="ShapefileConverter" />.
