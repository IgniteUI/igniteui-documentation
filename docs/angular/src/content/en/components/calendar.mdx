...
Let's add some `specialDates` to our `igxCalendar`. In order to do this, we have to create a <ApiLink type="DateRangeDescriptor" kind="interface" prefixed={false} /> item of type [`DateRangeType.Specific`]({environment:angularApiUrl}/enums/daterangetype.html#specific) and pass an array of dates as a [`dateRange`]({environment:angularApiUrl}/interfaces/daterangedescriptor.html#dateRange):
