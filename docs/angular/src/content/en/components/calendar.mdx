...
This section demonstrates the usage of <ApiLink type="Calendar" member="disabledDates" label="disabledDates" /> functionality. For this purpose, different single dates or ranges can be added to an array and then passed to the `disabledDates` descriptor.

The <ApiLink type="DateRangeType" kind="enum" prefixed={false} /> is used to specify a range that is going to be disabled.
...
The <ApiLink type="Calendar" member="specialDates" label="specialDates" /> feature is using almost the same configuration principles as the `disabledDates`. The ability to select and focus `specialDates` is what differs them from the `disabled` ones.

Let's add some `specialDates` to our `igxCalendar`. In order to do this, we have to create a <ApiLink type="DateRangeDescriptor" kind="interface" suffix={false} prefixed={false} /> item of type <ApiLink kind="enum" type="DateRangeType" prefixed={false} member="Specific" /> and pass an array of dates as a <ApiLink kind="interface" prefixed={false} suffix={false} type="DateRangeDescriptor" member="dateRange" />:
...
- <ApiLink type="DateRangeType" kind="enum" prefixed={false} />
- <ApiLink type="DateRangeDescriptor" kind="interface" suffix={false} prefixed={false} />
