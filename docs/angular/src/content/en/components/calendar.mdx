...
Let's add some `specialDates` to our `igxCalendar`. In order to do this, we have to create a <ApiLink type="DateRangeDescriptor" kind="interface" /> item of type <ApiLink kind="enum" type="DateRangeType" member="Specific" /> and pass an array of dates as a <ApiLink kind="interface" type="DateRangeDescriptor" member="dateRange" />:
