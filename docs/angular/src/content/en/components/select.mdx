...
You can see that we create a <ApiLink kind="interface" prefixed={false} suffix={false} type="PositionSettings" /> object that is directly passed to our <ApiLink type="_ConnectedPositioningStrategy_" suffix={false} prefixed={false} />, it is not required to do it, but since we want to define a custom positioning, we use them to override the strategy's default settings.
...
```html
<igx-select (selectionChanging)="onSelectionChanging($event)">
    <igx-select-item value="free">Free</igx-select-item>
    <igx-select-item value="restricted">Restricted</igx-select-item>
</igx-select>
```
