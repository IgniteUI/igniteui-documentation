...
<igx-checkbox [disabled]="true">Disabled</igx-checkbox>

{/* Displays state but blocks user changes */}
<igx-checkbox [readonly]="true" [(ngModel)]="readonlyValue">Read-only</igx-checkbox>
```
...
```html
<igx-checkbox (change)="onCheckboxChange($event)" value="item1">Item 1</igx-checkbox>
```
...
```html
<igx-checkbox value="option1" (change)="onCheckboxChange($event)">Option 1</igx-checkbox>
```
