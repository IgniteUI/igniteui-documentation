...
The Transaction is the main building block of the [Transaction service]({environment:angularApiUrl}/classes/igxtransactionservice.html). The Transaction is actually every operation that you execute on the data. The <ApiLink type="Transaction" kind="interface" prefixed={false} /> interface defines three properties: [`id`]({environment:angularApiUrl}/interfaces/transaction.html#id), [`newValue`]({environment:angularApiUrl}/interfaces/transaction.html#newValue) and [`type`]({environment:angularApiUrl}/interfaces/transaction.html#type).
...
Every time you add a Transaction, it is added to the transaction log and undo stack. All the changes in the transaction log are then accumulated per record. From that point, the service maintains an aggregated <ApiLink type="State" kind="interface" prefixed={false} />. The <ApiLink type="State" kind="interface" prefixed={false} /> consists of unique records and every record may be of one of the supported transaction types, mentioned above.
