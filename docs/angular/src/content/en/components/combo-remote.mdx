...
The sample below demonstrates remote binding using the <ApiLink type="Combo" member="dataPreLoad" label="dataPreLoad" /> property to load new chunk of remote data:

<Sample src="/lists/combo-remote" height={400} alt="Angular Combo Remote Binding Example" />

## Usage

To get started with the ComboBox component, import `IGX_COMBO_DIRECTIVES`. In this demo, a remote service is used for server requests, so `HttpClientModule` is also needed:
...
When binding a combobox to remote data, we need to have an available service that will load data on demand from a server. The combobox component exposes the <ApiLink type="Combo" member="virtualizationState" label="virtualizationState" /> property which gives the current state of a combobox - the first index and the number of items that need to be loaded. In order to show properly the scroll size, the <ApiLink type="Combo" member="totalItemCount" label="totalItemCount" /> property should have value that corresponds to the total items on the server.
