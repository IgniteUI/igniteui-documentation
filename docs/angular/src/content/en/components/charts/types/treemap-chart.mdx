...
- All data items must contain at least one data column (e.g. string) which should be mapped to the <ApiLink pkg="charts" type="Treemap" member="labelMemberPath" /> property.
- All data items must contain at least one numeric data column which should be mapped using the <ApiLink pkg="charts" type="Treemap" member="valueMemberPath" /> property.
- To categorize data into organized tiles you can optionally use <ApiLink pkg="charts" type="Treemap" member="parentIdMemberPath" /> and <ApiLink pkg="charts" type="Treemap" member="idMemberPath" />.

## Angular Treemap Configuration

In the following example, the treemap demonstrates the ability of changing it's algorithmic structure by modifying the <ApiLink pkg="charts" type="Treemap" member="layoutType" /> and <ApiLink pkg="charts" type="Treemap" member="layoutOrientation" /> properties.
...
<ApiLink pkg="charts" type="Treemap" member="layoutOrientation" /> property enables the user to set the direction in which the nodes of the hierarchy will be expanded.

Note that the <ApiLink pkg="charts" type="Treemap" member="layoutOrientation" /> property works with the layout types SliceAndDice and Strip.
...
In the following example, the treemap demonstrates the ability of node highlighting. There are two options for this feature. Each node can individually brighten, by decreasing its opacity, or cause all other nodes to trigger the same effect. To enable this feature, set <ApiLink pkg="charts" type="Treemap" member="highlightingMode" />to Brighten or FadeOthers.
...
- <ApiLink pkg="charts" type="Treemap" member="highlightedValueMemberPath" />: Specifies the name of the property in the datasource where the highlighted values are read.
- <ApiLink pkg="charts" type="Treemap" member="highlightedValueOpacity" />: Controls the opacity of the normal value behind the highlighted value.
- <ApiLink pkg="charts" type="Treemap" member="highlightedValuesDisplayMode" />: Enables or disables highlighted values.
