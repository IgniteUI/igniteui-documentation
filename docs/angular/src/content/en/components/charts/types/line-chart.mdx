...
- Different than an [area chart](/charts/types/area-chart), the line chart does not fill the area between the X-Axis (bottom axis) and the line.
- The Angular line chart is identical to the Angular [spline chart](/charts/types/spline-chart) in all aspects except that the line connecting data points does not have spline interpolation and smoothing for improved presentation of data.
