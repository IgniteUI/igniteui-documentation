...
The pie chart supports slice selection by mouse click as the default behavior. You can determine the selected slices by using the <ApiLink pkg="charts" type="PieChartBase" member="selectedItems" /> property. The selected slices are then highlighted.
