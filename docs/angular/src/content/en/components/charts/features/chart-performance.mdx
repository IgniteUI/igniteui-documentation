...
If you need to plot data sources with large number of data points (e.g. 10,000+), we recommend using Angular `XamDataChart` with one of the following type of series which where designed for specially for that purpose.
...
In addition to the general performance guidelines, the Angular `XamDataChart` control has the following unique features that affect performance.
...
<ApiRef pkg="charts" types={["CategoryChart","FinancialChart","XamDataChart"]} />
