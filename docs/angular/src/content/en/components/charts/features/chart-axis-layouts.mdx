...
The following example depicts the amount of renewable electricity produced since 2009, represented by a [Line Chart](/charts/types/line-chart). There is a drop-down that lets you configure the [`yAxisLabelLocation`]({environment:dvApiBaseUrl}/products/ignite-ui-angular/api/docs/typescript/latest/classes/igniteui_angular_charts.igxxychartcomponent.html#yAxisLabelLocation) so that you can visualize what the axes look like when the labels are placed on the left or right side on the inside or outside of the chart's plot area.
...
The following example depicts a stock price and trade volume chart with a [Stock Chart](/charts/types/stock-chart) and a [Column Chart](/charts/types/column-chart) plotted. In this case, the Y-Axis on the left is used by the [Column Chart](/charts/types/column-chart) and the Y-Axis on the right is used by the [Stock Chart](/charts/types/stock-chart), while the X-Axis is shared between the two.
...
The following example shows a Sin and Cos wave represented by a [Scatter Spline Chart](/charts/types/scatter-chart) with the X and Y axes crossing each other at the (0, 0) origin point.
...
- [Axis Gridlines](/charts/features/chart-axis-gridlines)
- [Axis Options](/charts/features/chart-axis-options)
