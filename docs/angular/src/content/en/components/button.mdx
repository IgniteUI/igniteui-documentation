...
We can allow the user to choose the size of the `igxButton` by using the `--ig-size` custom CSS property. To do this, import `IGX_BUTTON_GROUP_DIRECTIVES` and use the <ApiLink type="ButtonGroup" label="igxButtonGroup" /> component to display size values. This way whenever one gets selected, we will update the **--ig-size** CSS property.
