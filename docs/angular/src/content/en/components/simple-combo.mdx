...
The <ApiLink type="SimpleCombo" /> uses the <ApiLink type="DropDown" /> internally as an item container. It also includes the <ApiLink type="InputGroup" suffix={false} /> component. Creating new themes, that extend these components' themes, and scoping them under the respective classes will let's you change the simple combobox styles:
...
- The simple combobox does not have input for sizing its height. In the future, the <ApiLink type="InputGroup" suffix={false} /> component will expose an option that allows custom sizing, and then the <ApiLink type="SimpleCombo" suffix={false} /> will use the same functionality for proper styling and better consistency.
