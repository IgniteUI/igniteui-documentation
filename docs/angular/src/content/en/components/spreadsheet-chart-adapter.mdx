...
The Angular Spreadsheet component allows displaying charts in your <ApiLink pkg="spreadsheet" type="Spreadsheet" />.
...
Using <ApiLink pkg="spreadsheet" type="SpreadsheetChartAdapter" /> you can display the charts in the spreadsheet. The spreadsheet chart adapters creates and initializes chart elements for the spreadsheet based on a Infragistics.Documents.Excel.WorksheetChart instance.

In order to add a WorksheetChart to a worksheet, you must use the <ApiLink pkg="excel" type="WorksheetShapeCollection" member="addChart" /> method of the worksheet’s Shapes collection. You can find more detail of adding charts in Excel below.
...
In the following code snippet, an external [ExcelUtility](excel-utility.md) class is used to save and load a <ApiLink pkg="spreadsheet" type="Spreadsheet" member="workbook" />.
</DocsAside>

When setting up your Angular spreadsheet control to add charts, you will need to import the <ApiLink type="SpreadsheetChartAdapter" /> class like so:
...
<ApiLink type="SpreadsheetChartAdapter" pkg="spreadsheet" /><br />
