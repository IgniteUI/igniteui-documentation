...
<Sample src="/data-display/linear-progressbar" height={50} alt="Angular Linear Progress Example" />
...
You can set the type of your bar, using the <ApiLink type="LinearProgressBar" member="type" label="type" /> attribute. There are five types of linear progress bars - `default`, `error`, `success`, `info`, and `warning`.
...
<Sample src="/data-display/linear-progressbar-sample-1" height={200} alt="" />
...
The <ApiLink type="ProgressBarGradientDirective" suffix={false} member="animationDuration" label="animationDuration" /> input property is used to specify how long the animation cycle should take.
...
You can align the text, using the <ApiLink type="LinearProgressBar" member="textAlign" label="textAlign" /> property. Permitted values are `left`, `center`, and `right`.

To hide the text, use the <ApiLink type="LinearProgressBar" member="textVisibility" label="textVisibility" /> property and set its value to `false`.

Set the <ApiLink type="LinearProgressBar" member="textTop" label="textTop" /> property to `true` to move the text above the bar.

The <ApiLink type="LinearProgressBar" member="text" label="text" /> property can be used to customize the value of the text itself.
...
And do not forget to import the <ApiLink type="IgxTextAlign" kind="variable" prefixed={false} /> enumerator in your component if you're using the `textAlign` property.
...
<Sample src="/data-display/linear-progressbar-sample-2" height={200} alt="" />

<hr/>

<DocsAside type="info">
If the <ApiLink type="LinearProgressBar" member="step" label="step" /> input value is not defined, the progress update is **1% of the <ApiLink type="LinearProgressBar" member="max" label="max" /> value**. In case you want the progress to be faster, the <ApiLink type="LinearProgressBar" member="step" label="step" /> value should be greater than (**<ApiLink type="LinearProgressBar" member="max" label="max" /> * 1%**), respectfully for slower progress the <ApiLink type="LinearProgressBar" member="step" label="step" /> should be less than the default progress update.
</DocsAside>

<DocsAside type="note">
If the <ApiLink type="LinearProgressBar" member="step" label="step" /> value is defined greater than the <ApiLink type="LinearProgressBar" member="value" label="value" /> input, there is only one update, which gets **the value that is passed for progress update**.
</DocsAside>
...
<Sample src="/data-display/linear-dynamic-sample" height={200} alt="" />
...
Following the simplest approach, we create a new theme that extends the <ApiLink kind="sass" module="themes" type="function-progress-linear-theme" label="progress-linear-theme" code={true} /> and override the `$fill-color-default` and `$text-color` parameters.
...
<Sample src="/data-display/linear-progressbar-styling" height={50} alt="" />
...
- <ApiLink kind="sass" module="themes" type="function-progress-linear-theme" label="IgxLinearProgressBarComponent Styles" />
- <ApiLink type="IgxTextAlign" kind="variable" prefixed={false} />
