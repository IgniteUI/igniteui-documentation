...
Clicking on the (<ApiLink kind="enum" type="FilteringLogic" prefixed={false} member="And" /> or <ApiLink kind="enum" type="FilteringLogic" prefixed={false} member="Or" />) button placed above each group, will open a menu with options to change the group type or ungroup the conditions inside.
...
The search value of a condition can be templated using the <ApiLink type="QueryBuilderSearchValueTemplateDirective" suffix={false} label="igxQueryBuilderSearchValue" /> directive, applied to an `<ng-template>` inside of the `igx-query-builder`'s body:
...
- <ApiLink type="QueryBuilderSearchValueTemplateDirective" suffix={false} />
