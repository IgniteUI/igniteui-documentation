...
In the @@igComponent if you set rowEditable property to true, and editable property is not explicitly defined for any column, the editing will be enabled for all the columns except the _primary key_.
...
### Editing Templates
 ...
 If you want to use a data type specific _edit templates_, you should specify the column [`dataType`]({environment:angularApiUrl}/classes/igxcolumncomponent.html#dataType) property. So let's now see what are the default templates for each type:
