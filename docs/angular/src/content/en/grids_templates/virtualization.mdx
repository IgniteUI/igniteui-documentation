...
# Angular {ComponentTitle} Virtualization and Performance

In Ignite UI for Angular, the [{ComponentName}]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html) control now utilizes the [`igxForOf`]({environment:angularApiUrl}/classes/igxforofdirective.html) directive and virtualizes its content both vertically and horizontally.

<ComponentBlock for="Grid">


## Angular {ComponentTitle} Virtualization and Performance Example


<Sample src="/grid/grid-sample-2" height={550} alt="Angular {ComponentTitle} Virtualization and Performance Example" />
...
<Sample src="/hierarchical-grid/hierarchical-grid-lod" height={620} alt="Angular {ComponentTitle} Virtualization and Performance Example" />
...
By utilizing the [`igxForOf`]({environment:angularApiUrl}/classes/igxforofdirective.html) directive the {ComponentName} now optimizes DOM rendering and memory consumption by rendering only what is currently visible in the view port and swapping the displayed data while the user scrolls the data horizontally/vertically. [{ComponentName}]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html)'s [`width`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#width) and [`height`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#height) defaults to `100%` which will enable virtualization if the content displayed cannot fit inside the available space and scrollbars are required either vertically or horizontally. However, it is also possible to explicitly set the {ComponentTitle}'s [`width`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#width) and/or [`height`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#height) to `null` which means that the related dimension will be determined by the total size of the items inside. No scrollbar will then be shown and all items will be rendered along the respective dimension (columns if [`width`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#width) is `null` and rows if [`height`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#height) is `null`).

The size of the data chunks is determined by:

- The row height for the vertical (row) virtualization. This is determined by the [`rowHeight`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#rowheight) option and is 50(px) by default.
- The individual column widths in pixels for the horizontal (column) virtualization. They can be determined by either setting explicit width for each column component or setting the {ComponentTitle}'s [`columnWidth`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#columnWidth) option, which will be applied to all columns that don't have explicit width set.
...
The {ComponentTitle} supports remote virtualization, which is demonstrated in the [`{ComponentTitle} Remote Data Operations`](/{igPath}/remote-data-operations) topic.
</ComponentBlock>
