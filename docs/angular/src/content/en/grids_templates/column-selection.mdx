...
The column selection feature can be enabled through the <ApiLink type="{ComponentApiType}" member="columnSelection" label="columnSelection" /> input, which takes <ApiLink kind="variable" type="GridSelectionMode" prefixed={false} /> values.
...
<ApiLink type="ColumnGroup" /> properties:
