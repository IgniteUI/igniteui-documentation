...
The method returns a list of <ApiLink type="IgxSummaryResult" kind="interface" suffix={false} prefixed={false} />.
...
<DocsAside type="info">
In order to calculate the summary row height properly, the {ComponentTitle} needs the <ApiLink type="SummaryOperand" suffix={false} member="operate" label="operate" /> method to always return an array of <ApiLink type="IgxSummaryResult" kind="interface" suffix={false} prefixed={false} /> with the proper length even when the data is empty.
</DocsAside>
<ComponentBlock for="Grid">
