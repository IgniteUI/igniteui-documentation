...
All available column data types could be found in the official [Column types topic](column-types#default-template).
...
<Aside type="note">
In order to calculate the summary row height properly, the {ComponentTitle} needs the <ApiLink type="SummaryOperand" suffix={false} member="operate" label="operate" /> method to always return an array of <ApiLink type="IgxSummaryResult" kind="interface" prefixed={false} /> with the proper length even when the data is empty.
</Aside>
<ComponentBlock for="Grid">
...
- [{ComponentName}Summaries Styles]({environment:sassApiUrl}/themes#function-grid-summary-theme)
...
- [Selection](selection)
<ComponentBlock for="Grid">
* [Selection-based Aggregates]({environment:angularApiUrl}/grid/selection-based-aggregates.html)</ComponentBlock>
