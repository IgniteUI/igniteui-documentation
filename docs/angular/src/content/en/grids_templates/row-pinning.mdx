...
Row pinning is controlled through the `pinned` input of the <ApiLink kind="interface" type="RowType"/>. Pinned rows are rendered at the top of the {ComponentTitle} by default and stay fixed through vertical scrolling of the unpinned rows in the {ComponentTitle} body.
...
A row is pinned below the last pinned row. Changing the order of the pinned rows can be done by subscribing to the <ApiLink type="{ComponentApiType}" member="rowPinning" label="rowPinning" /> event and changing the <ApiLink kind="interface" type="IPinRowEventArgs" member="insertAtIndex" /> property of the event arguments to the desired position index.
