...
You may also use the {ComponentTitle}'s [`pinRow`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#pinRow) or [`unpinRow`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#unpinRow) methods of the [`{ComponentName}Component`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html) to pin or unpin records by their ID:
...
You can change the row pinning position via the [`pinning`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#pinning) configuration option. It allows you to set the pin area position to either Top or Bottom.
