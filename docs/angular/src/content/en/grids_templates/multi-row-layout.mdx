...
- <ApiLink type="Grid" member="gridKeydown" label="gridKeydown" /> is exposed. The event will emit <ApiLink type="IGridKeydownEventArgs" kind="interface" suffix={false} prefixed={false} />. This event is available only through the keyboard key combinations mentioned above, for all other key actions you can use `keydown` event `(keydown)="onKeydown($event)"`
