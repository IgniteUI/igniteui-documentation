...
Following the simplest approach, we create a new theme that extends the <ApiLink kind="sass" module="themes" type="function-paginator-theme" label="paginator-theme" code={true} /> and accepts the `$text-color`, `$background-color` and the `$border-color` parameters.
