...
license: commercial
llms:
  description: "The {ComponentTitle}'s editing exposes a built-in validation mechanism of user input when editing cells/rows."
---
...
- <ApiLink type="GridValidationService" member="valid" label="valid" /> - returns if the grid validation state is valid.
- <ApiLink type="GridValidationService" member="getInvalid" label="getInvalid" /> - returns records with invalid states.
- <ApiLink type="GridValidationService" member="clear" label="clear" /> - clears state for record by id or clears all state if no id is provided.
- <ApiLink type="GridValidationService" member="markAsTouched" label="markAsTouched" /> - marks the related record/field as touched.
...
- When using batch editing and the <ApiLink type="TransactionService" member="undo" label="undo" />/<ApiLink type="TransactionService" member="redo" label="redo" /> API of the transaction service.
...
Both events' arguments have a <ApiLink kind="interface" type="IGridEditEventArgs" member="valid" /> property and can be canceled accordingly. How it is used can be seen in the [Cross-field Validation example](#cross-field-example)
