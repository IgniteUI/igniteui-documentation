...
# Angular {ComponentTitle} Editing and Validation
...
The grid exposes a validation service via the <ApiLink type="Grid" member="validation" label="validation" /> property.
That service has the following public APIs:

- <ApiLink type="GridValidationService" suffix={false} member="valid" label="valid" /> - returns if the grid validation state is valid.
- <ApiLink type="GridValidationService" suffix={false} member="getInvalid" label="getInvalid" /> - returns records with invalid states.
- <ApiLink type="GridValidationService" suffix={false} member="clear" label="clear" /> - clears state for record by id or clears all state if no id is provided.
- <ApiLink type="GridValidationService" suffix={false} member="markAsTouched" label="markAsTouched" /> - marks the related record/field as touched.
...
- While editing via the cell editor based on the grid's <ApiLink type="Grid" member="validationTrigger" label="validationTrigger" />. Either on `change` while typing in the editor, or on `blur` when the editor loses focus or closes.
- When updating cells/rows via the API - <ApiLink type="Grid" member="updateRow" label="updateRow" />, <ApiLink type="Grid" member="updateCell" label="updateCell" /> etc..
- When using batch editing and the <ApiLink type="TransactionService" suffix={false} member="undo" label="undo" />/<ApiLink type="TransactionService" suffix={false} member="redo" label="redo" /> API of the transaction service.
...
## Angular {ComponentTitle} Validation Customization Options
...
In that scenarios you can use the <ApiLink type="Grid" member="cellEdit" label="cellEdit" /> or <ApiLink type="Grid" member="rowEdit" label="rowEdit" /> events and cancel the event in case the new value is invalid.
Both events' arguments have a <ApiLink kind="interface" prefixed={false} suffix={false} type="IGridEditEventArgs" member="valid" /> property and can be canceled accordingly. How it is used can be seen in the [Cross-field Validation example](#cross-field-example)
...
The cross-field validator can be added to the `formGroup` of the row from <ApiLink type="Grid" member="formGroupCreated" label="formGroupCreated" /> event, which returns the new `formGroup` for each row when entering edit mode:
...
<ComponentBlock for="HierarchicalGrid">
  ...
  Cross-field validators can be added to the formGroup on the <ApiLink type="Grid" member="formGroupCreated" label="formGroupCreated" /> event. In them multiple fields can be compared for validity.
...
<Sample src="/grid/grid-cross-field-validator-service" height={620} alt="Angular {ComponentTitle} Cross-field Validation Example" />
</ComponentBlock>
...
<Sample src="/hierarchical-grid/hierarchical-grid-cross-field-validation" height={620} alt="Angular {ComponentTitle} Cross-field Validation Example" />
</ComponentBlock>
...
<Sample src="/tree-grid/tree-grid-cross-field-validator-service" height={620} alt="Angular {ComponentTitle} Cross-field Validation Example" />
</ComponentBlock>
...
Using the [Ignite UI for Angular Theme Library](../themes/index), we can alter the default validation styles while editing.
...
- [{ComponentTitle} Transactions](batch-editing)
