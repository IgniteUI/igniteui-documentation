...
Once we've defined our drop-area in the template, we have to declare our handlers for the `igxDrop`'s <ApiLink type="DropDirective" suffix={false} member="enter" label="enter" />, <ApiLink type="DropDirective" suffix={false} member="leave" label="leave" /> and <ApiLink type="DropDirective" suffix={false} member="dropped" label="dropped" /> events in our component's `.ts` file.
...
<ComponentBlock for="HierarchicalGrid">
Once the row is dropped, we just call the row's <ApiLink suffix={false} type="{ComponentApiType}Row" label="delete()" /> method
</ComponentBlock>
...
Finally, we create a method that will be used to handle the <ApiLink type="DragDirective" suffix={false} member="dragMove" label="IgxDragDirective.dragMove" /> event (emitted for the dragged row). The method will change the value of the property used in the `igxRowDragGhost` template and force a rerender.
...
In order to track the position of the cursor, we bind to the `dragMove` event of the <ApiLink type="DragDirective" suffix={false} member="dragMove" label="IgxDragDirective" /> when we start dragging a row.
