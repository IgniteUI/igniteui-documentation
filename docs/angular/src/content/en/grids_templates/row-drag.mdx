...
Once the row is dropped, we just call the row's <ApiLink type="{ComponentApiType}Row" member="delete" label="delete()" /> method
</ComponentBlock>
...
Finally, we create a method that will be used to handle the <ApiLink type="DragDirective.dragMove" suffix={false} member="dragMove" label="IgxDragDirective.dragMove" /> event (emitted for the dragged row). The method will change the value of the property used in the `igxRowDragGhost` template and force a rerender.
