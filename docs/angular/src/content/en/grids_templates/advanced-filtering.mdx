...
license: commercial
llms:
  description: "The Advanced filtering provides a dialog which allows the creation of groups with filtering conditions across all columns for any Angular table like the {ComponentTitle}."
---
...
In order to open the advanced filtering dialog, the **Advanced Filtering** button in the grid toolbar should be clicked. The dialog is using the <ApiLink type="QueryBuilder" /> component to generate,display and edit the filtering logic. You can have a look at the [`Query Builder topic`](../query-builder.mdx#getting-started-with-ignite-ui-for-angular-query-builder) for details on the interaction process.
...
The advanced filtering generates a <ApiLink type="FilteringExpressionsTree" /> which is stored in the <ApiLink type="{ComponentApiType}" member="advancedFilteringExpressionsTree" label="advancedFilteringExpressionsTree" /> input property. You could use this property to set an initial state of the advanced filtering.
...
The advanced filtering in the `IgxHierarchicalGrid` can be used to filter root grid data based on child grids data using the _IN / NOT-IN_ operators. This way, subqueries can be created to define more complex filtering logic. More information about this functionality can be found in [`Query Builder's Using Sub-Queries section`](../query-builder-model.mdx#using-sub-queries). Here's a sample <ApiLink type="FilteringExpressionsTree" /> with a subquery:
...
If remote data is used, the <ApiLink type="HierarchicalGrid" member="schema" label="schema" /> property of the `IgxHierarchicalGrid` should be set. Please refer to [`Load on Demand`](../hierarchicalgrid/load-on-demand.mdx) topic for detailed guidance.
</ComponentBlock>
...
Since the Advanced Filtering dialog uses the <ApiLink type="QueryBuilder" /> component, you can use the <ApiLink kind="sass" module="themes" type="query-builder-theme" label="query-builder-theme" code={true} /> to style it. To style the header title, you can create a custom theme that extends the <ApiLink kind="sass" module="themes" type="query-builder-theme" label="query-builder-theme" code={true} /> and set the `$header-foreground` parameter.
