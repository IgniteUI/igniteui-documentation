...
over Group Row - collapses the group
 ...
 <ComponentBlock for="HierarchicalGrid">
collapses the row island</ComponentBlock> 
<ComponentBlock for="TreeGrid">
...
over Group Row - expands the group
 ...
 <ComponentBlock for="HierarchicalGrid">
expands the row island </ComponentBlock> 
<ComponentBlock for="TreeGrid">
...
- <kbd>Alt + Arrow Right</kbd> or <kbd>Alt + Arrow Down</kbd> - over Master Detail Row - expands the details view</ComponentBlock>
 <ComponentBlock for="Grid">
- <kbd>Space</kbd> - over Group Row - selects all rows in the group, if <kbd>rowSelection</kbd> property is set to multiple</ComponentBlock>
...
|---------|-------------|-----------|
| <ApiLink type="Grid" member="gridKeydown" label="gridKeydown" /> | An event that is emitted when any of key press/combinations described above is performed. Can be canceled. For any other key press/combination, use the default `onkeydown` event. | <ApiLink type="IGridKeydownEventArgs" kind="interface" prefixed={false} /> |
| <ApiLink type="Grid" member="activeNodeChange" label="activeNodeChange" /> | An event that is emitted when the active node is changed. You can use it to determine the Active focus position (header, tbody etc.), column index, row index or nested level. | <ApiLink type="IActiveNodeChangeEventArgs" kind="interface" prefixed={false} /> |
...
| API | Description | Arguments |
|---------|-------------|-----------|
| <ApiLink type="TreeGrid" member="gridKeydown" label="gridKeydown" /> | An event that is emitted when any of key press/combinations described above is performed. Can be canceled. For any other key press/combination, use the default `onkeydown` event. | <ApiLink type="IGridKeydownEventArgs" kind="interface" prefixed={false} /> |
...
| <ApiLink type="TreeGrid" member="getNextCell" label="getNextCell" />| returns <ApiLink type="ICellPosition" kind="interface" prefixed={false} /> object, which defines the next cell by `rowIndex` and `visibleColumnIndex`. A callback function can be passed as a third parameter of <ApiLink type="Grid" member="getnextcell" label="getNextCell" /> method. The callback function accepts `IgxColumnComponent` as a param and returns a `boolean` value indication if a given criteria is met: <br />_const nextEditableCell = grid.getNextCell(0, 4, (col) => col.editable);_ | `currentRowIndex`: number, `currentVisibleColumnIndex`: number, `callback`: (`IgxColumnComponent`) => boolean |
...
| API | Description | Arguments |
|---------|-------------|-----------|
| <ApiLink type="HierarchicalGrid" member="gridKeydown" label="gridKeydown" /> | An event that is emitted when any of key press/combinations described above is performed. Can be canceled. For any other key press/combination, use the default `onkeydown` event. | <ApiLink type="IGridKeydownEventArgs" kind="interface" prefixed={false} /> |
...
<ComponentBlock for="HierarchicalGrid">
_[Grid Keyboard Navigation](../grid/keyboard-navigation)</ComponentBlock>
<ComponentBlock for="HierarchicalGrid">
_ [Tree Grid Keyboard Navigation](../treegrid/keyboard-navigation)</ComponentBlock>
<ComponentBlock for="TreeGrid">
_[Hierarchical Grid Keyboard Navigation](../hierarchicalgrid/keyboard-navigation)</ComponentBlock>
<ComponentBlock for="TreeGrid">
_ [Grid Keyboard Navigation](../grid/keyboard-navigation)</ComponentBlock>
<ComponentBlock for="Grid">
_[Hierarchical Grid Keyboard Navigation](../hierarchicalgrid/keyboard-navigation)</ComponentBlock>
