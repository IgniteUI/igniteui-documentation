...
> Note: As seen in the code snippet above, the predefined `actions` UI components are wrapped in the <ApiLink type="GridToolbarActions" label="<igx-grid-toolbar-actions>` container" />. This way, the toolbar title is aligned to the left of the toolbar and the actions are aligned to the right of the toolbar.
