...
The exporting component is using the respective service for the target data format (<ApiLink type="ExcelExporterService" suffix={false} label="Excel" />, <ApiLink type="CsvExporterService" suffix={false} label="CSV" />, <ApiLink type="PdfExporterService" suffix={false} label="PDF" />). That means if the respective service is not provided through the dependency injection chain, the component
