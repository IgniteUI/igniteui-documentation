...
license: commercial
llms:
  description: "With Ignite UI for Angular {ComponentTitle} you can easily select data by using variety of events, rich API or with simple mouse interactions like single select."
---
...
IgniteUI for Angular {ComponentTitle} component provides three different selection modes - [Row selection](row-selection), [Cell selection](cell-selection) and [Column selection](column-selection). By default only **Multi-cell selection** mode is enabled in the {ComponentTitle}. In order to change/enable selection mode you can use <ApiLink type="Grid" member="rowSelection" label="rowSelection" />, <ApiLink type="Grid" member="cellSelection" label="cellSelection" /> or <ApiLink type="Column" member="selectable" label="selectable" /> properties.
