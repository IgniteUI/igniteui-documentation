...
Then define a {ComponentTitle} with bound data source and [`rowEditable`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#rowEditable) set to true and an [Action Strip](../action-strip) component with editing actions enabled. The [`addRow`]({environment:angularApiUrl}/classes/igxgrideditingactionscomponent.html#addRow) input controls the visibility of the button that spawns the row adding UI.
...
The internal [`IgxBaseTransactionService`]({environment:angularApiUrl}/classes/igxbasetransactionservice.html) is automatically provided for {ComponentTitle}. It holds pending cell changes until the row state is submitted or cancelled.
