...
# Angular {ComponentTitle} Row Editing

The {ComponentTitle} provides a convenient way to perform data manipulations through inline editing and a powerful API for Angular CRUD operations. Click on a row and press **Enter key** or simply double click with the mouse on the row that needs to be modified.

## Angular {ComponentTitle} Row Editing Example
...
<Sample src="/grid/grid-row-editing" height={550} alt="Angular {ComponentTitle} Row Editing Example" />
...
<Sample src="/tree-grid/treegrid-row-edit" height={590} alt="Angular {ComponentTitle} Row Editing Example" />
...
<Sample src="/hierarchical-grid/hierarchical-grid-row-editing" height={510} alt="Angular {ComponentTitle} Row Editing Example" />
...
Then define a {ComponentTitle} with bound data source and [`rowEditable`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#rowEditable) set to true:
<ComponentBlock for="Grid">
...
<Aside type="note">
It's not needed to enable editing for individual columns. Using the [`rowEditable`]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#rowEditable) property in the {ComponentTitle}, will mean that all rows, with defined `field` property, excluding primary one, will be editable. If you want to disable editing for specific column, then you set the [`editable`]({environment:angularApiUrl}/classes/igxcolumncomponent.html#editable) column's input to `false`.
</Aside>
...
<Aside type="note">
The {ComponentTitle} uses internally a provider [`IgxBaseTransactionService`]({environment:angularApiUrl}/classes/igxbasetransactionservice.html) that holds pending cell changes, until row state submitted or cancelled.
</Aside>
...
We will also style the current cell's editor and background to make it more distinct. You can learn more about cell styling in the [Cell Styling section](cell-editing#styling).
...
<Aside type="note">
If the component is using an [`Emulated`](../themes/sass/component-themes#view-encapsulation) ViewEncapsulation, it is necessary to penetrate this encapsulation using `::ng-deep` in order to style the grid Row Editing Overlay.
</Aside>
...
After we've defined our custom buttons, we can make use of the [`flat-icon-button-theme`]({environment:sassApiUrl}/themes#function-flat-icon-button-theme) to style them. You can learn more about `igx-icon-button` styling in the [Icon Button Styling documentation](../icon-button#icon-button-styling). We can create a custom theme for our `Done` and `Cancel`:
...
After styling the banner and buttons, we also define a custom style for [the cell in edit mode](cell-editing#styling). The result of all the combined styles can be seen below:
...
- [onRowEditEnter]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#onRowEditEnter)
- [onRowEdit]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#onRowEdit)
- [rowEditDone]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#rowEditDone)
- [onRowEditCancel]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#onRowEditCancel)
- [endEdit]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#endEdit)
...
- [primaryKey]({environment:angularApiUrl}/classes/{ComponentTypeDoc}.html#primaryKey)
