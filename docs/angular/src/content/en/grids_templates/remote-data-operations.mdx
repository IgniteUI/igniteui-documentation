...
When requesting data, you need to utilize the <ApiLink type="IForOfState" kind="interface" prefixed={false} /> interface, which provides the [`startIndex`]({environment:angularApiUrl}/interfaces/iforofstate.html#startIndex) and [`chunkSize`]({environment:angularApiUrl}/interfaces/iforofstate.html#chunkSize) properties.
