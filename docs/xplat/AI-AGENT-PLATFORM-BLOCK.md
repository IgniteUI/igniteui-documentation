...
### Grouping platforms with different code but shared note
...
<PlatformBlock for="Blazor">
```razor
this.GridRef.UpdateCell(newValue, rowID, "Age");
```
</PlatformBlock>
```
...
Every ` ``` ` code snippet that is **platform-specific must be inside** a `<PlatformBlock>`. The check: if the code uses any of these tokens, it is platform-specific and requires a PlatformBlock:
...
Every `<PlatformBlock for="...">` must have exactly one `</PlatformBlock>` closing tag. A common mistake is moving or removing a `</PlatformBlock>` when extending a block to cover more content:
...
## Checking Balance — Quick Verification

To verify all PlatformBlocks are balanced in a file, count open vs close tags:
...
If the counts differ, find the unmatched tag by reading through the file and tracking nesting depth.
