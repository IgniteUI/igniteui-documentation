---
title: "{Platform} {ComponentTitle} Tamaño - {ProductName}"
description: Aprenda a aplicar diferentes capacidades de tamaño al componente {ComponentTitle}. Puede usar un conjunto de opciones de vista compacta en {ProductName}.
keywords: tamaño de material, {Platform}, {ProductName}, Infragistics
license: commercial
mentionedTypes:
...
llms:
  description: La característica de tamaño de {ProductName} en {Platform} {ComponentTitle} permite a los usuarios controlar el espaciado y el diseño de los datos dentro de {ComponentName}.
---
...
# {Platform} {ComponentTitle} Tamaño

La característica Tamaño de {ProductName} en {Platform} {ComponentTitle} permite a los usuarios controlar el espaciado y el diseño de los datos dentro de <ApiLink type="{ComponentName}" />. Al cambiar `--ig-size`, puede mejorar significativamente la experiencia del usuario al interactuar con grandes cantidades de contenido. Pueden elegir entre tres opciones de tamaño:

- `--ig-size-large`
- `--ig-size-medium`
- `--ig-size-small`

## Ejemplo de tamaño de {Platform} {ComponentTitle}

<Sample src="/{ComponentSample}/layout-display-density" height={620} alt="{Platform} {ComponentTitle} Size Example" />

## Uso

Como puede ver en la demostración anterior, <ApiLink type="{ComponentName}" /> proporciona tres opciones de tamaño: **small**, **medium** y **large**. El fragmento de código siguiente muestra cómo establecer `--ig-size` en línea o como parte de una clase CSS:
...
Y ahora veamos en detalle cómo se refleja cada opción en el componente <ApiLink type="{ComponentName}" />. Al cambiar entre las distintas opciones de tamaño, se modificará la altura de cada elemento <ApiLink type="{ComponentName}" /> y los rellenos correspondientes. Además, si desea aplicar un <ApiLink type="{ComponentName}" member="width" /> de columna personalizado, tenga en cuenta que debe ser mayor que la suma de los rellenos izquierdo y derecho:

- **large**: este es el tamaño predeterminado de <ApiLink type="{ComponentName}" /> con la menor intensidad y una altura de fila igual a `50px`. Los rellenos izquierdo y derecho son `24px`; el <ApiLink type="{ComponentName}" member="width" /> de columna mínimo es `80px`;
- **medium**: este es el tamaño de intensidad intermedia con una altura de fila de `40px`. Los rellenos izquierdo y derecho son `16px`; el <ApiLink type="{ComponentName}" member="width" /> de columna mínimo es `64px`;
- **small**: este es el tamaño con la mayor intensidad y una altura de fila de `32px`. Los rellenos izquierdo y derecho son `12px`; el <ApiLink type="{ComponentName}" member="width" /> de columna mínimo es `56px`;

<DocsAside type="info">

Tenga en cuenta que actualmente **no puede** sobrescribir ninguno de los tamaños.

</DocsAside>

Continuemos ahora con nuestro ejemplo y veamos en acción cómo se aplica `--ig-size`. Primero agreguemos un botón que nos ayudará a cambiar entre cada tamaño:
...
Por último, proporcionemos la lógica necesaria para aplicar realmente el tamaño:
...
Otra opción que <ApiLink type="{ComponentName}" /> le proporciona, para poder cambiar la altura de las filas en <ApiLink type="{ComponentName}" />, es la propiedad <ApiLink type="{ComponentName}" member="rowHeight" />. Veamos entonces en acción cómo esta propiedad afecta al diseño de <ApiLink type="{ComponentName}" /> junto con `--ig-size`.

Tenga en cuenta lo siguiente:

- La variable CSS `--ig-size` no tendrá ningún impacto en la altura de la fila **si se especifica <ApiLink type="{ComponentName}" member="rowHeight" />**.
- `--ig-size` **afectará a todos los demás elementos en {ComponentTitle}**, como se ha descrito anteriormente.

Ahora podemos ampliar nuestro ejemplo y agregar la propiedad <ApiLink type="{ComponentName}" member="rowHeight" /> a <ApiLink type="{ComponentName}" />:
