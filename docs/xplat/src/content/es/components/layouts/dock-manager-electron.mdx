...
llms:
  description: "The Infragistics {Platform} Dock Manager component can be used in a multi-window Electron desktop application to manage the layout of each window, drag panes outside of a window in order to create a new window and drag/drop panes from one window to another."
---
...
In the `PaneHeaderDragStart` function we set the <ApiLink type="DockManager" member="draggedPane" label="DraggedPane" /> property of the Dock Manager component which will notify it that a drag operation has been started.
...
We have subscribed to the `DragOver` and `drop` events of the `document` element. In the `DragOver` listener we notify the Dock Manager that the mouse is dragged over it by setting its <ApiLink type="DockManager" member="dropPosition" label="DropPosition" /> property. This forces the Dock Manager to display its docking indicators.
...
When a pane is dropped out of its current window, we need to remove the <ApiLink type="DockManager" member="draggedPane" label="DraggedPane" /> from its Dock Manager component and update the layout.
