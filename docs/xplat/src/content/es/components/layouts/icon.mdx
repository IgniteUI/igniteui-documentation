...
mentionedTypes: ["Icon"]
llms:
  description: "The {Platform} Icon component allows you to easily display font or choose from a large set of predefined SVG icons, but it also gives you the ability to create custom font icons for your project."
---
...
<Sample src="/layouts/icon/sizing" height={60} alt="{Platform} Icon Example" />
...
Before using the <ApiLink pkg="core" type="Icon" />, you need to register it as follows:
...
You will then need to import the <ApiLink pkg="core" type="Icon" />, its necessary CSS, and register its module, like so:
...
You will also need to link an additional CSS file to apply the styling to the <ApiLink pkg="core" type="Icon" /> component. The following needs to be placed in the **wwwroot/index.html** file in a **Blazor Web Assembly** project or the **Pages/_Host.cshtml** file in a **Blazor Server** project:
...
The <ApiLink pkg="core" type="Icon" /> doesn't contain any icons on its own. It's a conduit for displaying any _registered_ SVG images.
...
The <ApiLink type="registerIcon" /> function allows you to register an SVG image as an icon from an external file:
...
To register an image as an icon, all you need to do is call one of the 2 "register" methods on a single <ApiLink pkg="core" type="Icon" /> element that allow you to add icons to an icon collection on your page.

The <ApiLink pkg="core" type="Icon" member="registerIcon" label="RegisterIcon" /> method allows you to register an SVG image as an icon from an external file:
...
In order to use the newly registered icon, all you have to do is to pass the name and collection to the <ApiLink pkg="core" type="Icon" /> element:
...
The second method for registering icons is by passing an SVG string to the <ApiLink type="Toolbar" member="registerIconFromText" label="RegisterIconFromText" /> method:
...
<Sample src="/layouts/icon/sizing" height={60} alt="{Platform} Icon Sizing" />
...
The icon component can be styled by applying styles directly to the <ApiLink pkg="core" type="Icon" /> element;
...
<Sample src="/layouts/icon/styling" height={70} alt="{Platform} Icon Sizing" />
