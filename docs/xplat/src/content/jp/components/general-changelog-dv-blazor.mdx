...
- <ApiLink pkg="inputs" type="Tooltip" /> - `DisableArrow` は非推奨です。矢印インジケーターをレンダリングするには、`WithArrow` を使用してください。
...
- `AzureMapsMapImagery` は <ApiLink pkg="maps" type="AzureMapsImagery" /> に名前が変更されました。
- <ApiLink pkg="maps" type="AzureMapsImagery" /> は `AzureMapsImageryStyle.Satellite` に名前が変更されました。
...
- `AzureMapsMapImagery` was renamed to <ApiLink pkg="maps" type="AzureMapsImagery" />
...
- <ApiLink pkg="inputs" type="Button" />: `Form`
...
- <ApiLink pkg="inputs" type="Combo" />: `ValueKey`、`DisplayKey`、`GroupKey`
...
- <ApiLink pkg="inputs" type="Dropdown" />: `SelectedItem`
- <ApiLink pkg="inputs" type="Input" />: `Pattern`、`MinLength`、`MaxLength`、<ApiLink pkg="inputs" type="DatePicker" member="min" label="Min" />、<ApiLink pkg="inputs" type="DatePicker" member="max" label="Max" />、<ApiLink pkg="inputs" type="Step" />
- <ApiLink pkg="inputs" type="DatePicker" member="select" label="Select" />: <ApiLink pkg="inputs" type="DatePicker" member="value" label="Value" />、`SelectedItem`
- <ApiLink pkg="inputs" type="Tile" />: `ColStart`、`RowStart`
- <ApiLink pkg="inputs" type="TileManager" suffix={false} />: `MinColumnWidth`、`MinRowHeight`、`Gap`
...
- <label>NEW</label> <ApiLink pkg="inputs" type="Tooltip" /> は、特定の要素のツールチップを表示する方法を提供します。使用するには、必要に応じてコンテンツを設定し、`Anchor` プロパティを介してターゲット要素の ID にリンクします。
...
- いくつかの列挙は名前が変更され、他の列挙と統合されました。名前の変更 (影響を受けるコンポーネントを含む):
  ...
  - `TickLabelRotation` (<ApiLink pkg="inputs" type="Slider" /> と <ApiLink pkg="inputs" type="RangeSlider" />) の名前が `SliderTickLabelRotation` に変更されました。
...
### List
- <ApiLink pkg="inputs" type="ListItem" suffix={false} /> に新しいプロパティ `Selected` を追加しました。
...
- <ApiLink pkg="inputs" type="Stepper" />
  - `ActiveStepChangingArgsEventArgs` has been renamed to <ApiLink pkg="inputs" type="ActiveStepChangingEventArgs" suffix={false} />
  - `ActiveStepChangedArgsEventArgs` has been renamed to <ApiLink pkg="inputs" type="ActiveStepChangedEventArgs" suffix={false} />
  - `StepperTitlePosition` now defaults to `Auto` to correctly reflect the default behavior
- <ApiLink pkg="inputs" type="Tree" />
  - `TreeSelectionChangeEventArgs` has been renamed to <ApiLink pkg="inputs" type="TreeSelectionEventArgs" suffix={false} />
...
- Added new property on <ApiLink pkg="inputs" type="ListItem" suffix={false} /> called `Selected`
...
- <ApiLink pkg="inputs" type="Combo" />
  ...
  - Removed <ApiLink pkg="grids" kind="interface" type="PositionStrategy" suffix={false} />, `Flip`, `SameWidth` properties.
- **IgbSelectComponent**
  - Removed <ApiLink pkg="grids" kind="interface" type="PositionStrategy" suffix={false} />, `Flip`, `SameWidth` properties.
...
- <ApiLink pkg="inputs" type="Dropdown" />
  - Removed <ApiLink pkg="grids" kind="interface" type="PositionStrategy" suffix={false} /> property.
- <ApiLink pkg="inputs" type="Input" />
  - Removed old named `Maxlength` and `Minlength` properties. Use `MaxLength` and `MinLength`.
  - Removed old named `Readonly` and `Inputmode` properties. Use <ApiLink pkg="inputs" type="DatePicker" member="readOnly" label="ReadOnly" /> and `InputMode`.
...
The type of Values from <ApiLink pkg="grids" kind="interface" type="PivotConfiguration" /> option is now array of IgbPivotValue - `IgbPivotValue[]`, it was `IgbPivotValueCollection` previously.
