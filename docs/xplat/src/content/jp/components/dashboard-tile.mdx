...
視覚化または視覚化のプロパティも、コントロールの上部にある <ApiLink pkg="core" type="Toolbar" /> を使用して構成できます。この <ApiLink pkg="layouts" type="Toolbar" /> には、現在の視覚化の既定のツールに加えて、以下で強調表示されている 4 つの Dashboard Tile 固有のツールが含まれています。
