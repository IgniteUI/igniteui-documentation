...
デフォルトでは、列はラベル テキストに <ApiLink pkg="grids" type="Column" member="field" label="Field" /> プロパティを使用します。ラベルをカスタマイズするには、<ApiLink pkg="grids" type="Column" member="header" label="Header" /> プロパティをより人間が読みやすい形式に設定します。
