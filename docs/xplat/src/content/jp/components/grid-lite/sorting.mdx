...
{GridLiteTitle} は単一および複数列ソートの両方をサポートします。複数列はデフォルトで有効で、グリッドの <ApiLink pkg="grids" type="SortingOptions" suffix={false} /> プロパティを通じて設定可能です。`Mode` プロパティは、値として `GridLiteSortingMode.Single` または `GridLiteSortingMode.Multiple` を受け入れます。
