...
グリッドは、`AutoGenerate` が true に設定されている場合、提供されたデータ ソースに基づいて列の構成を推測することをサポートします。データ内のレコードに基づいて、適切な <ApiLink type="Column" member="field" label="Field" /> および <ApiLink type="Column" member="dataType" label="DataType" /> プロパティを推測しようとします。
