...
_language: ja
llms:
  description: "列は、グリッド内の列子コンポーネントを使用して宣言的に定義されます。"
apiTerms: none
---
...
列は、グリッド内の列子コンポーネントを使用して宣言的に定義されます。`Column.field` プロパティは、列識別子として機能するため、列で唯一の必須プロパティです。また、グリッド行内の関連データをマッピングしてレンダリングするために使用されるプロパティでもあります。
...
列は、グリッド内の `<IgbGridLiteColumn>` 子要素を使用して宣言的に定義されます。<ApiLink type="Column" member="field" label="Field" /> プロパティは、列識別子として機能するため、列で唯一の必須プロパティです。また、グリッド行内の関連データをマッピングしてレンダリングするために使用されるプロパティでもあります。
</PlatformBlock>
...
グリッドは、`AutoGenerate` が true に設定されている場合、提供されたデータ ソースに基づいて列の構成を推論することをサポートします。データ内のレコードに基づいて、適切な <ApiLink type="Column" member="field" label="Field" /> および <ApiLink type="Column" member="dataType" label="DataType" /> プロパティを推論しようとします。
</PlatformBlock>
...
グリッドは、`autoGenerate` が true に設定されている場合、提供されたデータ ソースに基づいて列の構成を推論することをサポートします。データ内のレコードに基づいて、適切な `Column.field` および `Column.dataType` を推論しようとします。
</PlatformBlock>
...
列の幅を変更するには、列の `Column.width` プロパティを使用します。
...
列の幅を変更するには、<ApiLink type="IgbGridLiteColumn" /> コンポーネントの <ApiLink type="Column" member="width" label="Width" /> パラメーターを使用します。
...
このプロパティは<a href="https://developer.mozilla.org/ja/docs/Learn/CSS/Building_blocks/Values_and_units#numbers_lengths_and_percentages" target="_blank">有効な CSS 長さ単位</a>を受け入れます。

### 列の非表示

<PlatformBlock for="React, WebComponents">

列の `Column.hidden` プロパティを設定することで、列を非表示/表示できます。
...
<ApiLink type="IgbGridLiteColumn" /> コンポーネントの <ApiLink type="Column" member="hidden" label="Hidden" /> パラメーターを設定することで、列を非表示/表示できます。
...
{GridLiteTitle} の各列は、列要素の `Column.resizable` プロパティを設定することで、サイズ変更可能に構成できます。
...
{GridLiteTitle} の各列は、<ApiLink type="IgbGridLiteColumn" /> コンポーネントの <ApiLink type="Column" member="resizable" label="Resizable" /> パラメーターを設定することで、サイズ変更可能に構成できます。
...
列がサイズ変更可能に設定されている場合、列ヘッダーの右端をドラッグして幅を増減できます。リサイズ領域をダブルクリックすると、自動調整がトリガーされ、セルやヘッダーの最大コンテンツに合わせて幅が設定されます。
...
<Sample src="/{GridLiteSample}/column-config-dynamic" height={600} alt="{Platform} {GridLiteTitle} 列のダイナミック構成" />
