...
{ProductName} マップ コントロールには、Shape ファイルをタイル シリーズにロードして地理空間データをロードすることにより、<ApiLink pkg="geo-core" type="ShapefileRecord" suffix={false} /> によって生成される `ShapeFileRecord` を使用して、ヒートマップ画像を表示する機能があります。
...
<PlatformBlock for="React,Blazor">
  <ApiLink type="TileGeneratorMapImagery" pkg="maps" member="tileGenerator" label="TileGenerator" />
</PlatformBlock>
