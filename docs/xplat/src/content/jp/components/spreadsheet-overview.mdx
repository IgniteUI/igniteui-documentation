...
<ApiLink pkg="spreadsheet" type="Spreadsheet" suffix={false} /> を作成するには、以下のモジュールが必要です。
...
次のコード スニペットでは、外部の [ExcelUtility](excel-utility.md) クラスを使用して <ApiLink pkg="spreadsheet" type="Spreadsheet" member="workbook" label="Workbook" suffix={false} /> を保存およびロードしています。
</DocsAside>
