...
次の例に示すように、データを `dataSource` プロパティにバインドし、`chartType` プロパティを `PolarLineSeries.line` 列挙型に設定することで、`CategoryChart`コントロールに {Platform} 折れ線チャートを作成できます。
...
`CategoryChart` コントロールでこのチャート タイプを作成するには、以下の例のように、データをバインドし、`chartType` プロパティを `PolarLineSeries.line` に設定します:
...
| ------------------|--------------------|----------------------- |
| 折れ線              | `CategoryChart` | `chartType` = `PolarLineSeries.line` |
...
`PolarLineSeries`<br />
`RadialLineSeries`<br />
`StackedLineSeries`<br />
`Stacked100LineSeries`<br />
