...
次の例では、ツリーマップは、`NodeStyling` イベントを介してスタイル設定することによって実現されるノードのルック アンド フィールを変更する機能を示しています。

<PlatformBlock for="Blazor">

{Platform} プラットフォームではスタイル設定イベントを同期的に実行する必要があるため、代わりに `NodeStylingScript` イベント バリアントを使用し、JavaScript ベースのハンドラーを提供します。
...
- `highlightedDataSource`: ハイライト表示された値を読み取るデータ ソースを指定します。null の場合、ハイライト表示された値は ItemsSource プロパティから読み取られます。
