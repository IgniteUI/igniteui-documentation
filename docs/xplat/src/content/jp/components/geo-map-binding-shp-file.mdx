...
{ProductName} Map コンポーネントの <ApiLink pkg="maps" type="ShapefileConverter" /> クラスは、形状ファイルから地理空間データ (ポイント/位置、ポリライン、ポリゴン) を読み込み、それを <ApiLink pkg="maps" type="ShapefileRecord" /> オブジェクトのコレクションに変換します。
