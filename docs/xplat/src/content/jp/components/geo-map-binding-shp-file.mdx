...
{ProductName} Map コンポーネントの <ApiLink pkg="geo-core" type="ShapefileRecord" /> クラスは、形状ファイルから地理空間データ (ポイント/位置、ポリライン、ポリゴン) を読み込み、それを <ApiLink pkg="geo-core" type="ShapefileRecord" /> オブジェクトのコレクションに変換します。
