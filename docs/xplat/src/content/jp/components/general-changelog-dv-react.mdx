...
- `Dockmanager` の `IgrSplitPane` に新しい `useFixedSize` プロパティが追加され、新しいサイズ変更動作が可能になりました。

## 機能拡張

### Toolbar
- <ApiLink pkg="grids" type="Grid" member="toolbar" label="Toolbar" /> と `IgrToolPanel` に新しい `groupHeaderTextStyle` プロパティを追加しました。設定されている場合、すべての `IgrToolActionGroupHeader` アクションに適用されます。
