...
テーブル列からソート条件へアクセスする以外に <ApiLink pkg="excel" prefixed={false} type="WorksheetTable" /> の `SortSettings` プロパティの `SortConditions` コレクションからも公開されます。これは、列/ソート条件のペアの順番に並べられたコレクションです。このコレクション内の順序はソートの優先順位です。
...
また <ApiLink pkg="excel" prefixed={false} type="WorksheetTable" /> の `SortSettings` の `CaseSensitive` プロパティは、文字列が大文字と小文字を区別してソートできるかどうかを開発者が設定できます。
