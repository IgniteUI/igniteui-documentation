...
次に、以下のように、{Platform} <ApiLink type="Toast" /> と必要な CSS をインポートする必要があります:
...
{Platform} <ApiLink type="Toast" /> を使用する前に、次のように登録する必要があります:
...
Toast コンポーネントを表示する最も簡単な方法は、<ApiLink type="Toast" member="show" label="Show" /> メソッドを使用して、ボタン クリックで呼び出すことです。
...
<ApiLink type="Toast" member="displayTime" label="DisplayTime" /> プロパティを使用して、Toast コンポーネントが表示される期間を構成します。デフォルトでは、4000 ミリ秒に設定されています。

デフォルトでは、Toast コンポーネントは、<ApiLink type="Toast" member="displayTime" label="DisplayTime" /> で指定された期間が経過すると自動的に非表示になります。<ApiLink type="Toast" member="keepOpen" label="KeepOpen" /> プロパティを使用して、この動作を変更できます。このようにして、Toast は表示されたままになります。
...
タグ セレクターを使用して {Platform} <ApiLink type="Toast" /> 通知を直接スタイル設定できます。
