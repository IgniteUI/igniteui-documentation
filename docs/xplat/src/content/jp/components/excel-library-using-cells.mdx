...
この機能では、`Style` プロパティが <ApiLink pkg="excel" prefixed={false} type="CellFormat" /> オブジェクトに追加されています。これは書式の親スタイルを表す、<ApiLink pkg="excel" prefixed={false} type="WorkbookStyle" /> インターフェイスへの参照です。スタイルの書式では、このプロパティは常に null です。スタイルが親スタイルを持つことができないためです。行、列およびセル書式には、`Style` プロパティが常にデフォルトで `NormalStyle` スタイルを返します。
