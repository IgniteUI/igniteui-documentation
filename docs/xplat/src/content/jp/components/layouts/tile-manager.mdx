...
- <ApiLink pkg="inputs" type="Tile" /> - このコンポーネントは、タイル マネージャー内に表示される個々のタイルを表します。
...
また、<ApiLink type="TileManager" /> コンポーネントにスタイルを適用するために、追加の CSS ファイルをリンクする必要があります。以下は、**Blazor WebAssembly** プロジェクトの **wwwroot/index.html** ファイルまたは **Blazor Server** プロジェクトの **Pages/_Host.cshtml** ファイルに配置する必要があります:
...
<ApiLink type="TileManager" /> と <ApiLink type="Tile" /> の 2 つのコンポーネントの外観をカスタマイズすることもできます。
<ApiLink type="TileManager" /> は、Tile Manager のベース ラッパーのスタイル設定に使用できる CSS プロパティ `base` のみを公開します。
<ApiLink type="Tile" /> は、使用できるいくつかの CSS プロパティを公開します。
...
<ApiLink type="TileManager" pkg="core" /><br />
<ApiLink type="Tile" pkg="core" /><br />
