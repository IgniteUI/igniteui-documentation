...
次に、<ApiLink type="Splitter" /> と必要な CSS をインポートする必要があります:
...
また、<ApiLink pkg="core" type="Splitter" /> コンポーネントにスタイルを適用するために、追加の CSS ファイルをリンクする必要があります。以下は **Blazor Web Assembly** プロジェクトの **wwwroot/index.html** ファイル、または **Blazor Server** プロジェクトの **Pages/_Host.cshtml** ファイルに配置する必要があります:
