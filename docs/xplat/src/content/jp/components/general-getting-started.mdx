...
{/* TODO: the English section notes that the Blazor section above carries a screenshot at each step.
     Add the equivalent WinUI ones, or drop the images from this section deliberately. */}
...
{/* TODO: confirm the licensed feed URL. The package IDs are the nuspecs on gmurray/winui, which is
     where the WinUI packaging currently lives -- this branch's Source/Nuget carries only seven of the
     twelve, without Maps, Dashboards, Sparkline, TreeMap or SkiaSharp. nuget.org has the public ones
     only at 21.2.36-alpha. Revisit once that branch merges. */}
...
{/* TODO: a short first-control walkthrough, in the shape the Blazor section uses. The data grid overview
     topic already carries a complete XAML block that could be reused here. */}

</PlatformBlock>

<PlatformBlock for="Uno">
{/* TODO: Uno needs its own section, as on the English page. The namespace declarations are identical --
     Uno uses the same `using:` form and the same Infragistics.Controls.* namespaces -- but project
     creation differs, and its packages come from dotnet pack rather than the nuspecs
     (Infragistics.Uno.Core / .Charts / .DataGrid / .DataVisualization / .Gauges, plus the
     Infragistics.Uno.Wasm.* variants). */}
</PlatformBlock>
