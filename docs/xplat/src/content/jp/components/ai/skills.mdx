...
`ai-config` コマンドは、MCP サーバーを構成し、フレームワーク固有のスキル ファイルを各エージェントのスキル ディレクトリにコピーし、指示ファイルを設定します — 1 つのステップで完了します。`--assistants` を使用して、どのコーディング アシスタントが MCP 構成を受け取るかを選択し、`--agents` を使用して、どのエージェントがスキル ファイルを受け取るかを選択します。既存のファイルは、コンテンツが変更された場合にのみ更新されます。パラメーターが指定されていない場合、コマンドはインタラクティブ モードになり、アシスタントとエージェントを選択するよう求めます。利用可能なオプションについては、以下のテーブルを参照してください。

```bash
ig ai-config --assistants generic --agents claude
```

複数の値で `--agents` を使用して、複数のエージェントを同時にターゲットにします:
...
| フラグ | 値 | デフォルト |
|------|--------|---------|
| `--assistants` | `generic`、`vscode`、`cursor`、`gemini`、`junie`、`none` | インタラクティブに確認 |
| `--agents` | `generic`、`claude`、`copilot`、`cursor`、`codex`、`windsurf`、`gemini`、`junie`、`none` | インタラクティブに確認 |

<PlatformBlock for="Blazor">
<DocsAside type="info">
Blazor の場合、`ai-config` は `.csproj` または `.sln` からプロジェクトを検出します。`IgniteUI.Blazor` NuGet パッケージをインストールする必要はありません。プロジェクト ファイルが見つからない場合、コマンドはフレームワークを選択するよう求めます。
</DocsAside>
</PlatformBlock>
...
これにより、Ignite UI サーバーと一緒に `@angular/cli` MCP サーバーも登録されます。
...
<DocsAside type="info">
{ProductName} を手動でインストールし、`ai-config` を実行せずにスキルをコピーする場合、スキル ファイルは `node_modules` の下でも利用できます。それらをプロジェクトにコピーする (例: `.agents/skills/` の下での使用) には、次のコマンドを実行します:
</DocsAside>
...
GitHub CLI を使用して、{ProductName} リポジトリから直接スキル ファイルをダウンロードできます。インストール プロセスを開始するには、プロジェクト ルートで次のコマンドを実行します:
...
インストールするスキルとプロジェクト内のスキル ファイルのターゲット エージェントを選択するよう求められます。CLI は選択したスキルをダウンロードし、選択したエージェントに従って配置します。

スキルを後で更新するには、次のコマンドを実行します:
...
**Theming スキル**には、`igniteui-theming` MCP サーバーのセットアップ手順が含まれています。これにより、AI アシスタントがパレット生成やコンポーネント テーマのひな形作成などのライブ テーマ ツールにアクセスできます。VS Code、Cursor、Claude Desktop、JetBrains IDE の構成手順については、[Theming スキル ファイル]({GithubLink}/blob/master/skills/igniteui-angular-theming/SKILL.mdx)を参照してください。
