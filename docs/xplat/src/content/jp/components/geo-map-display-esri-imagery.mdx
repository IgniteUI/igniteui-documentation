...
## コード スニペット
以下のコード スニペットは、<ApiLink pkg="maps" type="ArcGISOnlineMapImagery" suffix={false} /> クラスを使用して <ApiLink pkg="maps" type="ArcGISOnlineMapImagery" member="geographicMap" label="GeographicMap" /> で Esri 画像サーバーからの {Platform} 地理的画像タイルを表示する方法を示します。
