...
<ApiLink pkg="maps" type="ArcGISOnlineMapImagery" suffix={false} /> は、<a href="https://www.esri.com/" target="_blank">Esri</a> によって作成された無料の地理的画像マッピング サービスです。世界の地理的画像タイルの 40 スタイル以上を提供します。この地理的画像サービスは、<a href="https://services.arcgisonline.com/ArcGIS/rest/services" target="_blank">www.arcgisonline.com</a> に直接アクセスして利用できます。
...
## コード スニペット
以下のコード スニペットは、<ApiLink pkg="maps" type="ArcGISOnlineMapImagery" suffix={false} /> クラスを使用して <PlatformBlock for="Angular,WebComponents"><ApiLink type="ArcGISOnlineMapImagery" pkg="maps" member="geographicMap" label="GeographicMap" suffix={false} /></PlatformBlock><PlatformBlock for="React,Blazor"><ApiLink type="ArcGISOnlineMapImagery" pkg="maps" member="geographicMap" label="GeographicMap" /></PlatformBlock> で Esri 画像サーバーからの {Platform} 地理的画像タイルを表示する方法を示します。
...
<ApiLink type="ArcGISOnlineMapImagery" pkg="maps" suffix={false} /><br />
