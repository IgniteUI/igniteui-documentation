...
_language: ja
llms:
  description: "{ProductName} コンポーネントを使用すると、ユーザーは CSS 変数と CSS パーツを公開することで、スタイル シートのさまざまなパーツを変更できます。"
---
...
<ApiLink pkg="core" type="Avatar" /> は、アバターのタイプに応じて、`base`、`icon`、`image`、`initials` の 4 つのパーツを公開することに気付くでしょう。これらのパーツを使用すると、スコープ内で定義されたスタイルを上書きできます。
