...
{Platform} Radial Gauge コンポーネントは、針、目盛り、範囲、ラベルなどの視覚要素をサポートし、定義済みの図形やスケールを表示できます。`RadialGauge` には、アニメーション化されたトランジションのサポートも組み込まれています。アニメーションは、<ApiLink pkg="gauges" type="RadialGauge" member="transitionDuration" label="TransitionDuration" /> プロパティの設定で簡単にカスタマイズできます。
