...
_language: ja
llms:
  description: "{Platform} Spreadsheet コンポーネントを使用して Spreadsheet にチャートを表示できます。"
---
...
以下のコード スニペットでは、外部の [ExcelUtility](./excel-utility.mdx) クラスを使用して <ApiLink type="Spreadsheet" member="workbook" label="Workbook" /> を保存およびロードしています。
</DocsAside>

ハイパーリンクを使用するように {Platform} スプレッドシート コントロールを設定するときは、<ApiLink type="SpreadsheetChartAdapter" /> クラスをインポートする必要があります。
