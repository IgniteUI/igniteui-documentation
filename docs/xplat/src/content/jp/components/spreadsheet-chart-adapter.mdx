...
<ApiLink pkg="spreadsheet" type="Spreadsheet" member="chartAdapter" label="ChartAdapter" suffix={false} exclude="Blazor,WebComponents" /> を使用すると、スプレッドシートにチャートを表示できます。スプレッドシート チャート アダプターは、Infragistics.Documents.Excel.WorksheetChart インスタンスに基づいてスプレッド シートのチャート要素を作成し、初期化します。
...
以下のコード スニペットでは、外部の [ExcelUtility](excel-utility.md) クラスを使用して <ApiLink pkg="spreadsheet" type="Spreadsheet" member="workbook" label="Workbook" suffix={false} exclude="Blazor,WebComponents" /> を保存およびロードしています。
</DocsAside>

ハイパーリンクを使用するように {Platform} スプレッドシート コントロールを設定するときは、<ApiLink pkg="spreadsheet" type="SpreadsheetChartAdapter" suffix={false} /> クラスをインポートする必要があります。
