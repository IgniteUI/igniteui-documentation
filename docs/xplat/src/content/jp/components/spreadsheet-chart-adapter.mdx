...
{Platform} Spreadsheet コンポーネントを使用して <ApiLink pkg="spreadsheet" type="Spreadsheet" exclude="Angular,Blazor,WebComponents" /> にチャートを表示できます。

## {Platform} Spreadsheet チャート アダプターの例

<Sample src="/excel/spreadsheet/adapter-chart" height={500} alt="{Platform} Spreadsheet チャート アダプターの例" />
...
ハイパーリンクを使用するように {Platform} スプレッドシート コントロールを設定するときは、<ApiLink pkg="spreadsheet" type="SpreadsheetChartAdapter" suffix={false} exclude="Angular,Blazor,React,WebComponents" /> クラスをインポートする必要があります。
