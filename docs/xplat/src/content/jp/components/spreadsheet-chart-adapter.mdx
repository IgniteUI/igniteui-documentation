...
{Platform} Spreadsheet コンポーネントを使用して <ApiLink type="Spreadsheet" /> にチャートを表示できます。
...
ハイパーリンクを使用するように {Platform} スプレッドシート コントロールを設定するときは、<ApiLink type="SpreadsheetChartAdapter" pkg="spreadsheet" suffix={false} exclude="Angular,Blazor,React,WebComponents" /> クラスをインポートする必要があります。
