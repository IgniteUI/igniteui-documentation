...
以下のコード スニペットは、<ApiLink pkg="maps" type="AzureMapsImagery" suffix={false} /> クラスを使用して {Platform} <ApiLink pkg="maps" type="AzureMapsImagery" member="geographicMap" label="GeographicMap" excludeSuffixFor="WebComponents" /> で Azure Maps からの地理的画像タイルを表示する方法を示します。
...
次のコード スニペットは、<ApiLink pkg="maps" type="AzureMapsImagery" suffix={false} /> クラスと <ApiLink pkg="maps" type="GeographicTileSeries" /> クラスを使用して、{Platform} <ApiLink pkg="maps" type="AzureMapsImagery" member="geographicMap" label="GeographicMap" excludeSuffixFor="WebComponents" /> の交通情報と濃い灰色のマップを結合した背景画像の上に地理画像タイルを表示する方法を示しています。
