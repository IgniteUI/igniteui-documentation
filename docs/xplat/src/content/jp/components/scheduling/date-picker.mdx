...
これらの詳細については、<ApiLink type="IgcDateTimeInput" /> の形式セクションを参照してください。

<Sample src="/scheduling/date-picker/format" height={500} alt="{Platform} Date Picker 入力形式の例" />

### 増加および減少

<ApiLink pkg="core" type="DatePicker" /> は、<ApiLink pkg="core" type="DatePicker" member="stepUp" label="StepUp" /> メソッドと <ApiLink pkg="core" type="DatePicker" member="stepDown" label="StepDown" /> メソッドを公開します。これらは両方とも <ApiLink type="IgcDateTimeInput" /> から取得され、現在設定されている日付の特定の <ApiLink pkg="core" type="DatePart" /> を増加および減少するために使用できます。
