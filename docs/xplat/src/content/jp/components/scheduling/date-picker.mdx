...
<ApiLink pkg="inputs" type="DatePicker" member="inputFormat" label="InputFormat" /> および <ApiLink pkg="inputs" type="DatePicker" member="displayFormat" label="DisplayFormat" /> は、ピッカーのエディターが指定された形式に従うように設定できるプロパティです。<ApiLink pkg="inputs" type="DatePicker" member="inputFormat" label="InputFormat" /> はロケール ベースであるため、何も指定されていない場合、ピッカーはデフォルトでブラウザーで使用されるものになります。
...
<ApiLink pkg="core" type="DatePicker" /> は、<ApiLink pkg="inputs" type="DatePicker" member="stepUp" label="StepUp" /> メソッドと <ApiLink pkg="inputs" type="DatePicker" member="stepDown" label="StepDown" /> メソッドを公開します。これらは両方とも `IgcDateTimeInput` から取得され、現在設定されている日付の特定の `DatePart` を増加および減少するために使用できます。
...
<ApiLink pkg="core" type="DatePicker" /> はフォーム要素で使用できます。コンポーネントの <ApiLink pkg="inputs" type="DatePicker" member="min" label="Min" /> プロパティと <ApiLink pkg="inputs" type="DatePicker" member="max" label="Max" /> プロパティはフォーム検証機能として機能します。
...
<ApiLink pkg="core" type="DatePicker" /> は、Date Picker が公開するプロパティを通じて、カレンダーの設定の一部を変更できます。これらの一部には、ピッカーが展開されたときに複数のカレンダーを表示できる <ApiLink pkg="inputs" type="DatePicker" member="visibleMonths" label="VisibleMonths" />、週の開始日を決定する <ApiLink pkg="inputs" type="DatePicker" member="weekStart" label="WeekStart" />、年の各週の番号を表示する <ApiLink pkg="inputs" type="DatePicker" member="showWeekNumbers" label="ShowWeekNumbers" /> などが含まれます。

## インターナショナリゼーション

<ApiLink pkg="core" type="DatePicker" /> のローカライズは、<ApiLink pkg="inputs" type="DatePicker" member="locale" label="Locale" /> 入力で制御できます。

以下は日本ロケール定義を持つ <ApiLink pkg="inputs" type="DatePicker" /> です。
