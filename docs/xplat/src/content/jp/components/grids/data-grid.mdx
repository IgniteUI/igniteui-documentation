...
テンプレートで使用可能なプロパティの詳細については、<ApiLink pkg="grids" type="Cell" /> の API を参照してください。
...
フラット データ バインディングのアプローチは既に説明したものと似ていますが、**セル値**の代わりに、`GridRow` の <ApiLink pkg="grids" kind="interface" type="GridRowComponent" member="data" label="Data" /> プロパティを使用します。
...
<ApiLink type="Cell" pkg="grids" />
<ApiLink type="GridToolbar" pkg="grids" />
<ApiLink type="Paginator" pkg="grids" />

<ApiLink type="GridRowComponent" pkg="grids" kind="interface" />
<ApiLink type="CellTemplateContext" pkg="grids" kind="interface" />
<ApiLink type="CellType" pkg="grids" kind="interface" />
<ApiLink type="ColumnPipeArgs" pkg="grids" kind="interface" />
