...
対応するスタイルも参照する必要があります。[テーマ](../themes/overview.md)のライトまたはダーク オプションを選択し、プロジェクトの構成に基づいてインポートできます。
...
適切に実装されると、セル編集テンプレートは、セルの <ApiLink type="CellType" member="editValue" label="EditValue" /> がグリッド[編集イベント サイクル](grid/editing.md#イベントの引数とシーケンス)を正しく渡します。
