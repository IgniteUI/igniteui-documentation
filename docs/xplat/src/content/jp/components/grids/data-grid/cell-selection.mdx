...
<Sample src="/grids/data-grid/cell-selection" height={600} alt="{Platform} グリッド選択の例" />
...
{Platform} データ グリッドでの選択は行およびセル レベルで有効になり、{Platform} グリッドの <ApiLink pkg="grids" type="DataGrid" member="selectionMode" label="SelectionMode" /> オプションを使用して構成できます。このプロパティには、以下にリストされている 5 つの異なるオプションがあります。
...
次の例は、グリッド内のすべての行を選択または選択解除する方法を示しています。<ApiLink pkg="grids" type="DataGrid" member="selectionMode" label="SelectionMode" /> は MultipleRow に設定する必要があることに注意してください。

<Sample src="/grids/data-grid/row-selection" height={600} alt="{Platform} グリッド選択の例" />
