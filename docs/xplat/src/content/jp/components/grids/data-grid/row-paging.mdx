...
行のページングは、<ApiLink pkg="grids" type="DataGrid" member="isPagerVisible" label="IsPagerVisible" /> プロパティを設定することにより、{ProductName} データ テーブル / データ グリッド内で有効になります。さらに、<ApiLink pkg="grids" type="DataGrid" member="pageSize" label="PageSize" /> を設定することで、表示される行の最大数を制限できます。

## {Platform} 行のページングの例

<Sample src="/grids/data-grid/pager" height={600} alt="{Platform} 行のページングの例" />
