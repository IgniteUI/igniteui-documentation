...
データ グリッドには、列オプションに関連付けられた文字列を割り当てるための <ApiLink pkg="grids" type="DataGrid" member="setCustomizedStringAsync" label="SetCustomizedStringAsync" /> が含まれています。グリッドの <ApiLink pkg="grids" type="DataGrid" member="actualDataSource" label="ActualDataSource" /> プロパティには、集計に必要なリソース文字列を割り当てるためのこのメソッドもあります。
...
<Sample src="/grids/data-grid/localization" height={600} alt="{Platform} グリッド ローカリゼーションの例" />
