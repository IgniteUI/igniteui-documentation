...
次に <ApiLink type="{ComponentName}" member="rowEditable" pkg="grids" prefixed={false} /> をバインドしたデータソースで定義し、`RowEditable` を true に設定してバインドします。
...
<DocsAside type="info" title="情報">
個々の列の編集を有効にする必要はありません。<ApiLink type="{ComponentName}" member="rowEditable" pkg="grids" prefixed={false} /> の `RowEditable` プロパティを使用すると、定義された `Field` プロパティを持つすべての行 (主行を除く) が編集可能になります。特定の列の編集を無効にしたい場合は、その列の `Editable` 入力を **false** に設定するだけです。
</DocsAside>
