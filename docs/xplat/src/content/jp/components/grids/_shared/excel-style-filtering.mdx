...
<ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> コンポーネントの Excel スタイル フィルタリングをオンにするには、2 つの入力を設定します。`AllowFiltering` を **true** に設定し、<ApiLink type="FilterMode" /> を `ExcelStyleFilter` に設定してください。
