...
{Platform} {ComponentTitle} の {ProductName} 集計機能は、グループ フッターとして列ごとのレベルで機能します。{Platform} {ComponentName} 集計は、列内のデータ タイプに応じて、あるいは <ApiLink type="{ComponentName}" pkg="grids" /> にカスタム テンプレートを実装することによって、定義済みのデフォルト集計項目を使用して別のコンテナの列情報を表示できます。
...
列のグループがある場合、<ApiLink type="{ComponentName}" pkg="grids" /> は <ApiLink type="{ComponentName}" member="summaryCalculationMode" label="SummaryCalculationMode" pkg="grids" /> と <ApiLink type="{ComponentName}" member="summaryPosition" label="SummaryPosition" pkg="grids" /> を使用して集計配置の変更やモードの計算をします。これら 2 つのプロパティに加えて、<ApiLink type="{ComponentName}" pkg="grids" /> は、参照するグループ行が縮小されたときに集計行が表示されたままであるかどうかを決定できる <ApiLink type="{ComponentName}" member="showSummaryOnCollapse" label="ShowSummaryOnCollapse" pkg="grids" /> プロパティを公開します。

以下は使用できる <ApiLink type="{ComponentName}" member="summaryCalculationMode" label="SummaryCalculationMode" pkg="grids" /> プロパティの値です:
...
以下は使用できる <ApiLink type="{ComponentName}" member="summaryPosition" label="SummaryPosition" pkg="grids" /> プロパティの値です。
...
<ApiLink type="{ComponentName}" member="showSummaryOnCollapse" label="ShowSummaryOnCollapse" pkg="grids" /> プロパティはブール値です。デフォルト値は **false** に設定されています。これは、親行が縮小されたときに集計行が非表示になることを意味します。プロパティが **true** に設定されている場合、グループ行が縮小されたときに、集計行は表示されたままになります。

<DocsAside type="info" title="情報">
<ApiLink type="{ComponentName}" member="summaryPosition" label="SummaryPosition" pkg="grids" /> プロパティは子レベルの集計のみに適用します。ルート レベルの集計は、<ApiLink type="{ComponentName}" pkg="grids" /> の下に常に固定されます。
</DocsAside>
...
<ApiLink type="{ComponentName}" pkg="grids" /> はルート ノードの集計と各ネストされた子ノード レベルの区別をサポートします。集計は <ApiLink type="{ComponentName}" member="summaryCalculationMode" label="SummaryCalculationMode" pkg="grids" /> プロパティを使用して設定できます。子レベル集計は、<ApiLink type="{ComponentName}" member="summaryPosition" label="SummaryPosition" pkg="grids" /> を使用して子ノードの前または後に表示できます。これら 2 つのプロパティに加えて、<ApiLink type="{ComponentName}" pkg="grids" /> は、参照するグループ行が縮小されたときに集計行が表示されたままであるかどうかを決定できる <ApiLink type="{ComponentName}" member="showSummaryOnCollapse" label="ShowSummaryOnCollapse" pkg="grids" /> プロパティを公開します。
...
<ApiLink type="{ComponentName}" member="showSummaryOnCollapse" label="ShowSummaryOnCollapse" pkg="grids" /> プロパティはブール値です。デフォルト値は **false** に設定されています。これは、親行が縮小されたときに集計行が非表示になることを意味します。プロパティが **true** に設定されている場合、親行が縮小されたときに、集計行は表示されたままになります。
