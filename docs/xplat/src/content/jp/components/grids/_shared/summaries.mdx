...
カスタム列集計内のすべての {ComponentTitle} データにアクセスできます。<PlatformBlock for="Angular,WebComponents,React">SummaryOperand <ApiLink pkg="grids" type="SummaryOperand" member="operate" label="Operate" suffix={false} /></PlatformBlock><PlatformBlock for="Blazor">`SummaryOperand` `Operate`</PlatformBlock> メソッドには、2 つの追加のオプション パラメーターが導入されています。
