...
{Platform} {ComponentTitle} の {ProductName} の縮小可能な列グループ機能を使用すると、<ApiLink type="{ComponentName}" pkg="grids" /> 内の複数レベルのネストされた列と列グループをグループ化し、これらのグループを縮小したり展開したりしてデータの視覚化とナビゲーションを向上させるオプションを提供することで、それらを整理および管理できます。
...
<ApiLink type="{ComponentName}" pkg="grids" /> および**縮小可能な複数列ヘッダー**を使用するには、まず以下のコマンドを入力して、{ProductName} をインストールする必要があります。
...
<ApiLink type="{ComponentName}" pkg="grids" /> のデフォルトの展開インジケーターは次のとおりです。

<Image src={expandIndicator} alt="expand_indicator" class="responsive-img" style="width: 450px; height: 130px" />

<ApiLink type="{ComponentName}" pkg="grids" /> のデフォルトの縮小インジケーターは次のとおりです。
