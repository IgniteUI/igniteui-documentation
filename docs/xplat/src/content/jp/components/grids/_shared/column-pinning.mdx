...
列のピン固定は <ApiLink pkg="grids" type="Column" member="pinned" /> の <ApiLink pkg="grids" type="Column" member="pinned" label="Pinned" /> プロパティによって制御されます。デフォルトでピン固定列は <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> の左側に固定して描画され、<ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> 本体のピン固定されていない列は水平スクロールされます。
...
次に、<ApiLink kind="sass" module="index.html" type="function-grid-theme" label="grid-theme" /> を拡張し、必要に応じてピン固定をカスタマイズするために必要なパラメーターを受け入れる新しいテーマを作成します。
</ComponentBlock>
