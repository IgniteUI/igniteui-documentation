...
{Platform} {ComponentTitle} の {ProductName} セル編集機能を使用すると、レコードの作成、更新、削除などのデータ操作を簡単に実行できます。<ApiLink type="{ComponentName}" pkg="grids" /> は、これらの操作をカスタマイズできる強力なパブリック API を提供します。データ操作のフェーズは次のとおりです。
...
有効にする編集モードを指定するために、<ApiLink type="{ComponentName}" pkg="grids" /> は <ApiLink type="Column" member="editable" label="Editable" /> および `RowEditable` ブール値プロパティを公開します。
...
>列が編集できない場合でも、<ApiLink type="{ComponentName}" pkg="grids" /> によって公開されるパブリック API を介して値を変更できます。
...
<ApiLink type="{ComponentName}" pkg="grids" /> で `RowEditable` プロパティを true に設定し、<ApiLink type="Column" member="editable" label="Editable" /> プロパティがどの列にも明示的に定義されていない場合、編集は*主キー*以外のすべての列で有効になります。
