...
リモート ページングは、データ取得を担当するサービスと、<ApiLink type="{ComponentName}" /> の構築とデータ サブスクリプションを担当するコンポーネントを宣言することで実現できます。詳細については、[リモート データ操作](remote-data-operations.md#リモート-ページング)トピックをご覧ください。
...
<DocsAside type="info" title="情報">
<ApiLink pkg="core" type="Color" /> および `Palette` は、色を生成および取得するための重要な機能です。使い方の詳細については[パレット](../themes/sass/palettes.md)のトピックを参照してください。
</DocsAside>
