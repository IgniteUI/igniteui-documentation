...
{ProductName} <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> コンポーネントは、[行選択](row-selection.md)、[セル選択](cell-selection.md)、[列選択](column-selection.md)の 3 つの選択モードを提供します。デフォルトでは、<ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> で**複数セル選択**モードのみが有効になっています。選択モードの変更または有効化は、`RowSelection`、`CellSelection` または `Selectable` プロパティを使用します。
...
`Selectable` プロパティを使用して、<ApiLink pkg="grids" type="Column" member="selectable" /> ごとに以下のオプションを指定できます。このプロパティが true または false に設定されている場合、対応する列の選択がそれぞれ有効または無効になります。
