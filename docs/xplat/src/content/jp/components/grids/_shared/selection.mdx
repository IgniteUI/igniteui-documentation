...
{ProductName} <ApiLink type="{ComponentName}" /> コンポーネントは、[行選択](row-selection.md)、[セル選択](cell-selection.md)、[列選択](column-selection.md)の 3 つの選択モードを提供します。デフォルトでは、<ApiLink type="{ComponentName}" /> で**複数セル選択**モードのみが有効になっています。選択モードの変更または有効化は、`RowSelection`、`CellSelection` または <ApiLink type="Column" member="selectable" label="Selectable" /> プロパティを使用します。
