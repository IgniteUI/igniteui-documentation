...
## 設定
<ApiLink pkg="grids" type="{ComponentName}" prefixed={false} />で項目の選択を設定するには、その `RowSelection` プロパティを設定する必要があります。このプロパティは、<ApiLink pkg="grids" type="GridSelectionMode" kind="interface" prefixed={false} /> 列挙を受け取ります。
