...
_language: ja
llms:
  description: "{ComponentTitle} のレンダリング機能を拡張します。"
---
...
<ApiLink type="ColumnState" member="colStart" /> は各セルの位置と範囲を決めるために 4 つの <ApiLink pkg="core" type="Input" /> プロパティを公開します。
...
<ApiLink type="{ComponentName}" /> を使用すると、[{ProductName} テーマ ライブラリ](../themes/styles.mdx)でスタイルを設定できます。<ApiLink type="Theme" /> は、グリッドのすべての機能をカスタマイズできるさまざまなプロパティを公開します。
...
上記で説明したアプローチでは、色の値がハード コーディングされていました。または `Palette` および <ApiLink pkg="core" type="Color" /> 関数を使用して、柔軟性を高めることができます。
...
カスタム パレットが生成された後、<ApiLink pkg="core" type="Color" /> 関数を使用して、さまざまな種類の原色と二次色を取得できます。
