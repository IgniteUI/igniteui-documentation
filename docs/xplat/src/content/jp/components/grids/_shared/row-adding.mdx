...
次に、バインドしたデータ ソースに <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> を定義をして `RowEditable` を true に設定し、編集アクションを有効にした <ApiLink pkg="grids" type="ActionStrip" /> コンポーネントを定義します。`AddRow` 入力は、行追加 UI を生成するボタンの表示状態を制御します。
