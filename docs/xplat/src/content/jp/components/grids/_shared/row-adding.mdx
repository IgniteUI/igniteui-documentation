...
<ComponentBlock for="Grid, HierarchicalGrid">
> **注**:
> [行の追加] のボタンの表示状態を制御する <ApiLink pkg="grids" type="GridEditingActions" /> 入力は、アクション ストリップ コンテキスト (タイプ <PlatformBlock for="Angular,React,WebComponents"><ApiLink pkg="grids" kind="interface" type="RowType" suffix={false} /></PlatformBlock><PlatformBlock for="Blazor"><ApiLink pkg="grids" kind="class" type="RowType" suffix={false} /></PlatformBlock> を使用して、ボタンが表示するレコードを調整できます。
