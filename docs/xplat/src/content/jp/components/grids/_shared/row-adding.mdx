...
<ComponentBlock for="Grid, HierarchicalGrid">
> **注**:
> [行の追加] のボタンの表示状態を制御する <ApiLink pkg="grids" type="GridEditingActions" /> 入力は、アクション ストリップ コンテキスト (タイプ <ApiLink pkg="grids" kind="interface" type="RowType" prefixed={false} /> を使用して、ボタンが表示するレコードを調整できます。
