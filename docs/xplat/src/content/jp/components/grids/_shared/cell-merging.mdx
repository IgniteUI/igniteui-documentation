...
グリッドは、<ApiLink pkg="grids" type="GridCellMergeMode" kind="type" prefixed={false} exclude="Blazor" /><PlatformBlock for="Blazor"><ApiLink pkg="grids" type="GridCellMergeMode" kind="enum" prefixed={false} suffix={false} /></PlatformBlock> 列挙型の値を受け入れる `cellMergeMode` プロパティを公開します。
