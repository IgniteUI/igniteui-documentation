...
グリッドは、<PlatformBlock for="Angular,React,WebComponents"><ApiLink pkg="grids" type="GridCellMergeMode" kind="type" prefixed={false} /></PlatformBlock><PlatformBlock for="Blazor"><ApiLink pkg="grids" type="GridCellMergeMode" kind="enum" prefixed={false} suffix={false} /></PlatformBlock> 列挙型の値を受け入れる `cellMergeMode` プロパティを公開します。
...
### 列結合のトグル
列レベルでは、<ApiLink pkg="grids" type="ColumnGroup" member="merge" label="merge" /> プロパティで結合の有効または無効を切り替えます。
...
### 結合ストラテジ クラス
カスタム結合ストラテジは <ApiLink pkg="grids" type="GridMergeStrategy" kind="interface" suffix={false} /> クラスを実装する必要があります:
...
### デフォルトのストラテジを拡張
一部の動作 (例: comparer ロジック) のみをカスタマイズしたい場合は、組み込みの <ApiLink pkg="grids" type="DefaultMergeStrategy" suffix={false} /> を拡張し、必要なメソッドのみをオーバーライドできます。
<PlatformBlock for="React">
```tsx
...
## デフォルトのストラテジを拡張
一部の動作 (例: comparer ロジック) のみをカスタマイズしたい場合は、組み込みの <ApiLink pkg="grids" type="DefaultTreeGridMergeStrategy" suffix={false} /> または <ApiLink pkg="grids" type="ByLevelTreeGridMergeStrategy" suffix={false} /> のいずれかを拡張し、必要なメソッドのみをオーバーライドできます。
<ComponentBlock for="Grid">
