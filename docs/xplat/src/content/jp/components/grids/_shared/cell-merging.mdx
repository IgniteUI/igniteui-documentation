...
グリッドは、<ApiLink pkg="grids" type="GridCellMergeMode" kind="type" prefixed={false} /> 列挙型の値を受け入れる `cellMergeMode` プロパティを公開します。
...
### 結合ストラテジ クラス
カスタム結合ストラテジは <ApiLink pkg="grids" type="GridMergeStrategy" /> クラスを実装する必要があります:
