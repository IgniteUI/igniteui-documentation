...
グリッドは、<PlatformBlock for="Angular,React,WebComponents"><ApiLink pkg="grids" type="GridCellMergeMode" kind="type" prefixed={false} /></PlatformBlock><PlatformBlock for="Blazor"><ApiLink pkg="grids" type="GridCellMergeMode" kind="enum" prefixed={false} /></PlatformBlock> 列挙型の値を受け入れる `cellMergeMode` プロパティを公開します。
