...
行のピン固定は `Row` の <ApiLink pkg="grids" type="GridRow" member="pinned" label="Pinned" exclude="React,WebComponents" /> 入力によって制御されます。デフォルトでピン固定行は <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> の上側に固定して描画され、<ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> 本体のピン固定されていない行は垂直スクロールされます。
