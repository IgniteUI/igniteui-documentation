...
行のピン固定は `Row` の <ApiLink type="GridRow" member="pinned" label="Pinned" /> 入力によって制御されます。デフォルトでピン固定行は <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> の上側に固定して描画され、<ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> 本体のピン固定されていない行は垂直スクロールされます。
...
<ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> の <ApiLink type="{ComponentName}" member="pinRow" label="PinRow" pkg="grids" prefixed={false} /> または <ApiLink type="{ComponentName}" member="unpinRow" label="UnpinRow" pkg="grids" prefixed={false} /> メソッドを使用して ID によって行をピン固定またはピン固定解除できます。
...
`{ComponentName}` を使用すると、[{ProductName} テーマ ライブラリ](../themes/styles.md)を介してスタイルを設定できます。{ComponentTitle} の <ApiLink type="function-grid-theme" label="theme" kind="sass" module="index.html" /> は、{ComponentTitle} のすべての機能をカスタマイズできるさまざまなプロパティを公開します。
