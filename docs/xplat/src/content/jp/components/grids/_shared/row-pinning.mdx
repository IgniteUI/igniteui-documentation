...
行のピン固定は `Row` の <ApiLink type="GridRow" member="pinned" label="Pinned" /> 入力によって制御されます。デフォルトでピン固定行は <ApiLink type="{ComponentName}" pkg="grids" /> の上側に固定して描画され、<ApiLink type="{ComponentName}" pkg="grids" /> 本体のピン固定されていない行は垂直スクロールされます。
...
<ApiLink type="{ComponentName}" pkg="grids" /> の <ApiLink type="{ComponentName}" member="pinRow" label="PinRow" pkg="grids" /> または <ApiLink type="{ComponentName}" member="unpinRow" label="UnpinRow" pkg="grids" /> メソッドを使用して ID によって行をピン固定またはピン固定解除できます。
