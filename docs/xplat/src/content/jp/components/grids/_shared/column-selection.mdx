...
[複数列ヘッダー](multi-column-headers.md)機能は <ApiLink type="Column" member="selectable" label="Selectable" /> 入力に反映されません。その子の 1 つ以上で選択動作が有効な場合、<ApiLink type="ColumnGroupComponent" /> は <ApiLink type="Column" member="selectable" label="Selectable" /> です。さらに、すべての <ApiLink type="Column" member="selectable" label="Selectable" /> 子孫が <ApiLink type="Column" member="selected" label="Selected" /> である場合、コンポーネントは <ApiLink type="Column" member="selected" label="Selected" /> としてマークされます。
</DocsAside>
