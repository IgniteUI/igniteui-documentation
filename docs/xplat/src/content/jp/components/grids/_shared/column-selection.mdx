...
列選択機能は、<ApiLink pkg="grids" kind="type" type="GridSelectionMode" prefixed={false} suffix={false} exclude="Blazor" /><PlatformBlock for="Blazor"><ApiLink pkg="grids" kind="enum" type="GridSelectionMode" prefixed={false} suffix={false}/></PlatformBlock> 値を受け取る `ColumnSelection` 入力によって有効にすることができます。
