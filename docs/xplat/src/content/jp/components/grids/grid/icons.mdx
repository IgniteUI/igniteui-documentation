...
{ProductName} <ApiLink pkg="grids" type="Grid" /> アイコンは、公開されている API メソッドを使用して、別のコレクション セットのカスタム アイコンを使用するようにカスタマイズできます。
...
<Sample src="/{GridSample}/change-icons-custom" height={600} alt="{Platform} {ComponentTitle} アイコンのカスタマイズの例" />
