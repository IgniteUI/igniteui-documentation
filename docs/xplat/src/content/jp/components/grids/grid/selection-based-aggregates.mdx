...
集計では、列のデータ タイプとニーズに応じて、<ApiLink type="SummaryOperand" />、<ApiLink type="NumberSummaryOperand" />.html、<ApiLink type="DateSummaryOperand" /> のいずれかの基本クラスを拡張することにより、基本的な集計機能をカスタマイズできます。
...
<ApiLink type="SummaryOperand" /> の派生クラスの `operate` メソッドでデータを処理します。データ タイプに基づいて、さまざまなカテゴリにデータを入れていきます。
...
データ タイプをグループ化した後、集計を開始できます。そのため、<ApiLink type="NumberSummaryOperand" /> および <ApiLink type="DateSummaryOperand" /> の既に公開されているメソッドを使用できます。
...
- [行の選択](../_shared/row-selection.mdx)
- [セルの選択](../_shared/cell-selection.mdx)
