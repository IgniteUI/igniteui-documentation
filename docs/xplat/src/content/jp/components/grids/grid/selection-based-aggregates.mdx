...
集計では、列のデータ タイプとニーズに応じて、<ApiLink pkg="grids" type="SummaryOperand" suffix={false} exclude="Blazor" />、<ApiLink pkg="grids" type="NumberSummaryOperand" suffix={false} exclude="Blazor" />.html、<ApiLink pkg="grids" type="DateSummaryOperand" suffix={false} exclude="Blazor" /> のいずれかの基本クラスを拡張することにより、基本的な集計機能をカスタマイズできます。
...
<ApiLink pkg="grids" type="SummaryOperand" suffix={false} exclude="Blazor" /> の派生クラスの `operate` メソッドでデータを処理します。データ タイプに基づいて、さまざまなカテゴリにデータを入れていきます。
...
データ タイプをグループ化した後、集計を開始できます。そのため、<ApiLink pkg="grids" type="NumberSummaryOperand" suffix={false} exclude="Blazor" /> および <ApiLink pkg="grids" type="DateSummaryOperand" suffix={false} exclude="Blazor" /> の既に公開されているメソッドを使用できます。
