...
次のコード スニペットでは、外部の [ExcelUtility](./excel-utility.mdx) クラスを使用して <ApiLink type="Worksheet" member="workbook" label="Workbook" /> を保存およびロードしています。
...
In the following code snippet, an external [ExcelUtility](./excel-utility.mdx) class is used to save and load a <ApiLink type="Workbook" />.
