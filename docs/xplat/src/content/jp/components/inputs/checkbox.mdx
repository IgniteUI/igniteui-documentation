...
チェックボックスの <ApiLink pkg="core" type="Checkbox" member="labelPosition" label="LabelPosition" /> 属性を設定することにより、チェックボックスの切り替えの前または後にラベルを配置するかどうかを指定できます。許可される値は、`before` と `after` (デフォルト) です。
...
`Form` でチェックボックスを使用する場合は、<ApiLink pkg="core" type="Checkbox" member="name" label="Name" /> と <ApiLink pkg="core" type="Checkbox" member="value" label="Value" /> の属性を使用できます。
