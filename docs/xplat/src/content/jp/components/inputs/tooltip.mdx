...
<ApiLink type="Tooltip" /> は、ターゲット要素を基準にして簡単に配置することもできます。必要なのは、<ApiLink type="Tooltip" member="placement" label="Placement" /> プロパティと <ApiLink type="PopoverPlacement" kind="enum" prefixed={false} /> のいずれかの配置オプションとともに使用するだけです。
