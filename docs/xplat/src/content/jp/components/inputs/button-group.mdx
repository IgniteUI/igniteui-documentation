...
<ApiLink pkg="core" type="ToggleButton" /> は、<ApiLink pkg="core" type="ToggleButton" member="selected" label="Selected" /> 属性または <ApiLink pkg="core" type="ButtonGroup" /> の <ApiLink type="PieChartBase" member="selectedItems" label="SelectedItems" /> 属性を通じて選択済みとしてマークできます。
...
<ApiLink pkg="core" type="ButtonGroup" /> の <ApiLink type="PieChartBase" member="selectedItems" label="SelectedItems" /> プロパティを使用するには、<ApiLink pkg="core" type="ToggleButton" /> <ApiLink pkg="core" type="ToggleButton" member="value" label="Value" /> 属性の設定が必須です。
</DocsAside>
