...
スイッチの <ApiLink type="DataLabel" member="labelPosition" label="LabelPosition" /> 属性を設定することにより、スイッチの切り替えの前または後にラベルを配置するかどうかを指定できます。許可される値は、`before` と `after` (デフォルト) です。
...
`Form` でスイッチを使用する場合は、<ApiLink type="Switch" member="name" label="name" /> と <ApiLink pkg="core" type="Switch" member="value" label="value" /> の属性を使用できます。
