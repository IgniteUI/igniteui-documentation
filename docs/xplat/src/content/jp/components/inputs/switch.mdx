...
次に、以下のように、<ApiLink type="Switch" /> と必要な CSS をインポートする必要があります:
...
<ApiLink type="Switch" /> の使用を開始する最も簡単な方法は次のとおりです:
...
<ApiLink type="Switch" /> コンポーネントは標準の `<form>` 要素では機能しません。代わりに `Form` を使用してください。
</DocsAside>
