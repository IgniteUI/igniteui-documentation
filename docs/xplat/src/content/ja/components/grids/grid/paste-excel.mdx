---
title: "{Platform} Excel 風 {ComponentTitle} - Infragistics"
description: "{Platform} グリッドを構成して、少ないコードで高機能かつ高パフォーマンスな API を使用して Excel からデータを貼り付け、充実した API を使用して選択したグリッド データを簡単にエクスポートできます。"
keywords: 選択項目のエクスポート, {Platform} 用 Ignite UI, {ComponentKeywords}, {ProductName}, Infragistics
license: commercial
mentionedTypes: ["Infragistics.Controls.Grid"]
llms:
  description: "{ProductName} グリッドは、クリップボードにコピーされた Excel データを読み取ることができます。"
---
...
# {Platform} Excel からグリッドへの貼り付け

{ProductName} <ApiLink type="Grid" /> は、クリップボードにコピーされた Excel データを読み取ることができます。このセクションでは、いくつかのカスタム コードを使用してこれを行う方法を示します。

## {Platform} Excel からの貼り付けの例

このサンプルは、Excel から <ApiLink type="Grid" /> Material UI テーブルへの貼り付けを実装する方法を示します。
サンプルを操作するには、任意の Excel スプレッドシートを開き、いくつかの行をコピーして、キーボード (<kbd>コントロール</kbd> + <kbd>ブイ</kbd>、<kbd>シフト</kbd> + <kbd>挿入</kbd>、<kbd>コマンド プロンプト</kbd> + <kbd>ブイ</kbd>) を使用してグリッドに貼り付けます。

上部には、2 つのオプションを持つドロップダウン ボタンがあります:

<ol>
<li>「データを新しい行として貼り付け」 – このモードでは、Excel からコピーされたデータは新しい行としてグリッドに追加されます</li>
<li>「アクティブ セルから貼り付け」 – このモードでは、グリッド内のデータが上書きされます。</li>
</ol>

貼り付け後の新しいデータは斜体で表示されます。
...
まず、グリッドの `rendered` イベントにバインドして、テキスト エリア要素を作成および管理する必要があります:
...
コードは、クリップボードから貼り付けられたデータを受け取るために使用される DOM textarea 要素を作成します。データが textarea に貼り付けられると、コードはそれを配列に解析します。
...
貼り付けられたデータは、ユーザーの選択に基づいて、新しいレコードとして追加したり、既存のレコードを更新するために使用したりできます。
参考として、addRecords メソッドと updateRecords メソッドを参照してください。
...
`paste-handler` ディレクティブ (そのコードは次のセクションにあります) を <ApiLink type="Grid" /> に追加し、その `onDataProcessed` イベントを処理する必要があります。`onDataProcessed` イベントには、Excel データに配列の形式でアクセスできる 1 つのパラメーターがあります。参考として、`addRecords` メソッドと `updateRecords` メソッドを参照してください。
...
これは `paste-handler` の実装です。コードは、クリップボードから貼り付けられたデータを受け取るために使用される DOM `textarea` 要素を作成します。データが `textarea` に貼り付けられると、ディレクティブはそれを配列に解析し、解析されたデータを渡してカスタム イベント `onDataProcessed` を発行します。
...
- [Excel エクスポーター](export-excel.md) - Excel Exporter サービスを使用して、グリッドから Excel にデータをエクスポートします。また、グリッドから選択したデータのみをエクスポートするオプションも提供します。エクスポート機能は ExcelExporterService クラスにカプセル化されており、データは MS Excel テーブル形式でエクスポートされます。この形式では、フィルタリング、並べ替えなどの機能を使用できます。これを行うには、ExcelExporterService の export メソッドを呼び出し、グリッド コンポーネントを最初の引数として渡す必要があります。
