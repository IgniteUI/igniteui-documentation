...
このサンプルでは、Excel から <ApiLink type="Grid" /> Material UI テーブルに貼り付ける方法を示します。
サンプルを操作するには、任意の Excel スプレッドシートを開き、いくつかの行をコピーして、キーボード (<kbd>CTRL</kbd> + <kbd>V</kbd>, <kbd>SHIFT</kbd> + <kbd>INSERT</kbd>,<kbd>CMD</kbd> + <kbd>V</kbd>) を使用してグリッドに貼り付けます。
...
このコードは、クリップボードから貼り付けられたデータを受け取るために使用される DOM textarea 要素を作成します。データが textarea に貼り付けられると、コードはそれを配列に解析します。
...
`paste-handler` ディレクティブ (そのコードは次のセクションにあります) を <ApiLink type="Grid" /> に追加し、その `onDataProcessed` イベントを処理する必要があります。`onDataProcessed` イベントには、Excel データに配列形式でアクセスできる 1 つのパラメーターがあります。参考として、`addRecords` メソッドと `updateRecords` メソッドを参照してください。
...
これは `paste-handler` の実装です。このコードは、クリップボードから貼り付けられたデータを受け取るために使用される DOM `textarea` 要素を作成します。データが `textarea` に貼り付けられると、ディレクティブはそれを配列に解析し、解析されたデータを渡すカスタム イベント `onDataProcessed` を発行します。
...
- [Excel Exporter](export-excel.md) - Excel Exporter サービスを使用して、Grid から Excel にデータをエクスポートします。また、Grid から選択したデータのみをエクスポートするオプションも提供します。エクスポート機能は ExcelExporterService クラスにカプセル化されており、データは MS Excel テーブル形式でエクスポートされます。この形式では、フィルタリング、並べ替えなどの機能を使用できます。これを行うには、ExcelExporterService の export メソッドを呼び出し、Grid コンポーネントを最初の引数として渡す必要があります。
