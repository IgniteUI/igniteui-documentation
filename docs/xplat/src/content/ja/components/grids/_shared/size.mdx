...
description: "{ComponentTitle} コンポーネントにさまざまなサイズ機能を適用する方法について説明します。{ProductName} で一連のコンパクト表示オプションを使用できます。"
keywords: マテリアル サイズ, {Platform}, {ProductName}, Infragistics
license: commercial
mentionedTypes:
...
llms:
  description: "{Platform} {ComponentTitle} の {ProductName} サイズ機能を使用すると、ユーザーは {ComponentName} 内のデータの間隔とレイアウトを制御できます。"
---
...
{Platform} {ComponentTitle} の {ProductName} サイズ機能を使用すると、ユーザーは <ApiLink type="{ComponentName}" /> 内のデータの間隔とレイアウトを制御できます。`--ig-size` を変更することで、大量のコンテンツを操作する際のユーザー エクスペリエンスを大幅に向上できます。次の 3 つのサイズ オプションから選択できます。
...
上記のデモで確認できるように、<ApiLink type="{ComponentName}" /> には **small**、**medium**、**large** の 3 つのサイズ オプションがあります。以下のコード スニペットは、`--ig-size` をインライン、または CSS クラスの一部として設定する方法を示しています。
...
次に、各オプションが <ApiLink type="{ComponentName}" /> コンポーネントにどのように反映されるかを詳しく見ていきます。異なるサイズ オプションを切り替えると、各 <ApiLink type="{ComponentName}" /> 要素の高さと対応するパディングが変更されます。また、カスタム列 <ApiLink type="{ComponentName}" member="width" /> を適用する場合は、それが左右のパディングの合計より大きい必要がある点に注意してください。

- **large** - これは既定の <ApiLink type="{ComponentName}" /> サイズで、密度が最も低く、行の高さは `50px` です。左右のパディングは `24px` です。最小列 <ApiLink type="{ComponentName}" member="width" /> は `80px` です。
- **medium** - これは中程度の密度のサイズで、行の高さは `40px` です。左右のパディングは `16px` です。最小列 <ApiLink type="{ComponentName}" member="width" /> は `64px` です。
- **small** - これは密度が最も高いサイズで、行の高さは `32px` です。左右のパディングは `12px` です。最小列 <ApiLink type="{ComponentName}" member="width" /> は `56px` です。

<DocsAside type="info">

現時点では、いずれのサイズもオーバーライド**できない**ことに注意してください。

</DocsAside>

それではサンプルを続けて、`--ig-size` がどのように適用されるかを実際に見てみましょう。まず、各サイズを切り替えるためのボタンを追加します。
...
最後に、サイズを実際に適用するために必要なロジックを提供します。
...
<ApiLink type="{ComponentName}" /> が提供するもう 1 つのオプションとして、<ApiLink type="{ComponentName}" /> 内の行の高さを変更できるプロパティ <ApiLink type="{ComponentName}" member="rowHeight" /> があります。それでは、このプロパティが `--ig-size` とともに <ApiLink type="{ComponentName}" /> レイアウトにどのように影響するかを実際に見てみましょう。

以下の点に注意してください。

- **<ApiLink type="{ComponentName}" member="rowHeight" /> が指定されている場合**、`--ig-size` CSS 変数は行の高さに影響しません。
- 上記で説明したように、`--ig-size` は **{ComponentTitle} 内の残りすべての要素に影響します**。

これでサンプルを拡張し、<ApiLink type="{ComponentName}" /> に <ApiLink type="{ComponentName}" member="rowHeight" /> プロパティを追加できます。
