---
title: "{Platform} {ComponentTitle} 条件付きセル スタイル - {ProductName}"
description: ユーザーが異なるセルをすばやく識別できるようにします。さまざまなセル スタイルを定義します。{Platform} {ComponentTitle} の条件付きセル スタイルを使用して、セルを目立たせます。
keywords: 条件付きスタイル設定, {Platform}, {ProductName}, Infragistics
license: commercial
mentionedTypes:
...
llms:
  description: "{Platform} {ComponentTitle} の {ProductName} 条件付きスタイル設定機能により、行またはセル レベルでのカスタム スタイル設定が可能になります。"
---
...
# {Platform} {ComponentTitle} 条件付きスタイリング

{Platform} {ComponentTitle} の {ProductName} 条件付きスタイリング機能では、行またはセル レベルでカスタム スタイルを適用できます。  <ApiLink type="{ComponentName}" /> 条件付きスタイリング機能は、特定の条件を満たすデータを視覚的に強調またはハイライトするために使用され、ユーザーがグリッド内の重要な情報や傾向を識別しやすくします。

## {ComponentTitle} 条件付き行スタイリング

{ProductName} の <ApiLink type="{ComponentName}" /> コンポーネントは、カスタム ルールに基づいて**行を条件付きでスタイル設定する** 2 つの方法を提供します。

- <ApiLink type="{ComponentName}" /> コンポーネントに <ApiLink type="{ComponentName}" member="rowClasses" /> 入力を設定する方法。
- <ApiLink type="{ComponentName}" /> コンポーネントに <ApiLink type="{ComponentName}" member="rowStyles" /> 入力を設定する方法。

このトピックの以降では、これらの両方について詳しく説明します。

### 行クラスの使用

<ApiLink type="{ComponentName}" member="rowClasses" /> 入力を設定し、カスタム ルールを定義することで、<ApiLink type="{ComponentName}" /> 行を条件付きでスタイル設定できます。
...
<ApiLink type="{ComponentName}" member="rowClasses" /> 入力はキーと値のペアを含むオブジェクト リテラルを受け取ります。キーは CSS クラスの名前で、値はブール値を返すコールバック関数またはブール値です。
...
カスタム スタイルを現在のコンポーネントとその子に強制的に適用するには、**::ng-deep** または **ViewEncapsulation.Non** を使用します。
...
<ApiLink type="{ComponentName}" /> コントロールは、データ行の条件付きスタイリングを可能にする <ApiLink type="{ComponentName}" member="rowStyles" /> プロパティを公開します。<ApiLink type="{ComponentName}" member="rowClasses" /> と同様に、キーがスタイル プロパティで値が評価用の式であるオブジェクト リテラルを受け取ります。また、通常のスタイリングも適用できます (条件なし)。

> <ApiLink type="{ComponentName}" member="rowStyles" /> と <ApiLink type="{ComponentName}" member="rowClasses" /> の両方のコールバック シグネチャは次のとおりです。
...
## {ComponentTitle} 条件付きセル スタイリング

## 概要

{ProductName} の <ApiLink type="{ComponentName}" /> コンポーネントは、カスタム ルールに基づいて**セルを条件付きでスタイル設定する** 2 つの方法を提供します。

- キーと値のペアを含むオブジェクト リテラルに <ApiLink type="Column" /> 入力 <ApiLink type="Column" member="cellClasses" /> を設定する方法。キーは CSS クラスの名前で、値はブール値を返すコールバック関数またはブール値です。その結果、セルに便利なマテリアル スタイリングが適用されます。

### セル クラスの使用

<ApiLink type="Column" /> <ApiLink type="Column" member="cellClasses" /> 入力を設定し、カスタム ルールを定義することで、<ApiLink type="{ComponentName}" /> セルを条件付きでスタイル設定できます。
...
<ApiLink type="Column" member="cellClasses" /> 入力はキーと値のペアを含むオブジェクト リテラルを受け取ります。キーは CSS クラスの名前で、値はブール値を返すコールバック関数またはブール値です。
...
カスタム スタイルを現在のコンポーネントとその子に強制的に適用するには、**::ng-deep** または **ViewEncapsulation.None** を使用します。
...
- キーがスタイル プロパティで値が評価用の式であるオブジェクト リテラルを受け取る <ApiLink type="Column" /> 入力 <ApiLink type="Column" member="cellStyles" />\` を使用する方法。

> `cellStyles` と `cellClasses` の両方のコールバック シグネチャは、現在次のように変更されています。
...
列は、列セルの条件付きスタイリングを可能にする <ApiLink type="Column" member="cellStyles" label="CellStyles" /> プロパティを公開します。<ApiLink type="Column" member="cellClasses" /> と同様に、キーがスタイル プロパティで値が評価用の式であるオブジェクト リテラルを受け取ります。また、通常のスタイリングも簡単に適用できます (条件なし)。
...
`ngOnInit` で、事前定義された `Columns` コレクションの各列に <ApiLink type="Column" member="cellStyles" label="CellStyles" /> 構成を追加します。このコレクションは、<ApiLink type="{ComponentName}" /> 列を動的に作成するために使用されます。
...
- 同じ条件にバインドされているセル (異なる列のセル) があり、1 つのセルが更新された場合、条件が満たされていても、他のセルは新しい値に基づいて更新されません。

<PlatformBlock for="Angular, WebComponents">

残りのセルに変更を適用するには、チェックを実行する必要があります。以下の例は、その方法を示しています。
