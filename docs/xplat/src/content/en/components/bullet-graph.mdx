...
The following sample demonstrates how setting multiple properties on the same <ApiLink pkg="gauges" type="XamBulletGraph" /> can transform it to completely different bullet graph.
...
The bullet graph supports one scale, one set of tick marks and one set of labels. The bullet graph component also has built-in support for animated transitions. This animation is easily customizable by setting the <ApiLink pkg="gauges" type="XamBulletGraph" member="transitionDuration" label="TransitionDuration" /> property.
...
The <ApiLink pkg="gauges" type="XamBulletGraph" /> requires the following modules:
...
<ApiRef types={["XamBulletGraph","XamLinearGraphRange"]} pkg="gauges" />
