...
The {Platform} Button Group component is used to organize <ApiLink type="ToggleButton" />'s into styled button groups with horizontal/vertical alignment, single/multiple selection and toggling.
...
You will then need to import the <ApiLink type="ButtonGroup" />, its necessary CSS, and register its module, like so:
...
You will then need to import the <ApiLink type="ButtonGroup" /> and its necessary CSS, like so:
...
Before using the <ApiLink type="ButtonGroup" />, you need to register it as follows:
...
You will also need to link an additional CSS file to apply the styling to the <ApiLink type="ButtonGroup" /> component. The following needs to be placed in the **wwwroot/index.html** file in a **Blazor Web Assembly** project or the **Pages/_Host.cshtml** file in a **Blazor Server** project:
...
Now that you have the {ProductName} Button Group imported, you can start with a basic configuration of the <ApiLink type="ButtonGroup" /> and its buttons.

Use the <ApiLink type="ButtonGroup" /> selector to wrap your <ApiLink type="ToggleButton" />s and display them into a button group. If you want a button to be selected by default, use the <ApiLink type="ToggleButton" member="selected" label="Selected" /> attribute:
...
### Alignment
Use the <ApiLink type="ButtonGroup" member="alignment" label="Alignment" /> property to set the orientation of the buttons in the button group.

<Sample src="/inputs/button-group/alignment" height={220} alt="{Platform} Button Group Alignment Example" />


### Selection
In order to configure the {ProductName} <ApiLink type="ButtonGroup" /> selection, you could use its <ApiLink type="ButtonGroup" member="selection" label="Selection" /> property. This property accepts the following three modes:
...
The sample below demonstrates the exposed <ApiLink type="ButtonGroup" /> selection modes:

<Sample src="/inputs/button-group/selection" height={170} alt="{Platform} Button Group Selection Example" />



A <ApiLink type="ToggleButton" /> could be marked as selected via its <ApiLink type="ToggleButton" member="selected" label="Selected" /> attribute or through the <ApiLink type="ButtonGroup" /> <ApiLink type="ButtonGroup" member="selectedItems" label="SelectedItems" /> attribute:
...
<DocsAside type="note">
Setting <ApiLink type="ToggleButton" /> <ApiLink type="ToggleButton" member="value" label="Value" /> attribute is mandatory for using the <ApiLink type="ButtonGroup" member="selectedItems" label="SelectedItems" /> property of the <ApiLink type="ButtonGroup" />.
</DocsAside>
...
The <ApiLink type="ButtonGroup" /> component exposes `group` CSS part that allows us to style the button group container.
Also, the <ApiLink type="ToggleButton" />s provide `toggle` CSS part that could be used to style the button element.
...
<ApiRef pkg="core" types={["ButtonGroup","ToggleButton","Ripple","Icon"]} />
