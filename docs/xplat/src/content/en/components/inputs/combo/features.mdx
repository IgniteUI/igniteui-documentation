...
The following demo shows some <ApiLink pkg="inputs" type="Combo" /> features that are enabled/disabled at runtime:
...
In our sample we are going to use the <ApiLink pkg="inputs" type="Switch" /> component, so we have to import them together with the combo:
...
Note that grouping is enabled/disabled by setting the <ApiLink pkg="inputs" type="Combo" member="groupKey" label="GroupKey" /> property to a corresponding data source field:
...
<DocsAside type="note">
The <ApiLink pkg="inputs" type="Combo" member="groupKey" label="GroupKey" /> property will only have effect if your data source consists of complex objects.
</DocsAside>
