...
The following demo shows some <ApiLink pkg="inputs" type="Combo" exclude="WebComponents" /> features that are enabled/disabled at runtime:
...
In our sample we are going to use the <ApiLink pkg="inputs" type="Switch" exclude="WebComponents" /> component, so we have to import them together with the combo:
...
Note that grouping is enabled/disabled by setting the <ApiLink pkg="inputs" type="Combo" member="groupKey" label="GroupKey" exclude="WebComponents" /> property to a corresponding data source field:
...
The {ProductName} <ApiLink type="Combo" exclude="WebComponents" /> exposes one more filtering property that allows passing configuration of both `FilterKey` and `CaseSensitive` options. The `FilterKey` indicates which data source field should be used for filtering the list of options. The `CaseSensitive` option indicates if the filtering should be case-sensitive or not.
...
Defining a <ApiLink type="Combo" member="groupKey" label="GroupKey" exclude="WebComponents" /> option will group the items, according to the provided key:
...
<DocsAside type="info">
The <ApiLink pkg="inputs" type="Combo" member="groupKey" label="GroupKey" exclude="WebComponents" /> property will only have effect if your data source consists of complex objects.
</DocsAside>
...
The <ApiLink type="Combo" exclude="WebComponents" /> label can be set easily using the <ApiLink type="Combo" member="label" label="Label" /> property:
