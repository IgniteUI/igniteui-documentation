...
The <ApiLink pkg="core" type="Select" /> component can be used inside a `Form` component, thus it exposes a <ApiLink type="Select" member="name" label="Name" /> property to be registered with. It also has a <ApiLink pkg="core" type="Select" member="label" label="Label" />, and <ApiLink pkg="core" type="Select" member="placeholder" label="Placeholder" /> properties. The <ApiLink pkg="core" type="Select" member="outlined" label="Outlined" /> property is used for styling purposes only when it comes to the Material theme. Except for the default slot, the component provides a few other slots including `header`, `footer`, `helper-text`, `prefix`, `suffix`, and `toggle-icon`. The component size can be changed using the `--ig-size` CSS variable.

### Item

The <ApiLink pkg="core" type="SelectItem" /> component allows the users to declaratively specify a list of options to be used by the <ApiLink pkg="core" type="Select" /> control. Each item provides a <ApiLink pkg="core" type="SelectItem" member="value" label="Value" /> property that represents the data it carries upon selection. The <ApiLink pkg="core" type="SelectItem" /> has a default slot which allows you to specify the text content of the item. This text content will be used as value in case the <ApiLink pkg="core" type="SelectItem" member="value" label="Value" /> property is not present on the <ApiLink pkg="core" type="SelectItem" />. You could also provide custom content to be rendered before or after the <ApiLink pkg="core" type="SelectItem" /> content using the `prefix` and `suffix` slots. You could predefine a selected item by setting the <ApiLink pkg="core" type="SelectItem" member="selected" label="Selected" /> property. You could also disable some or all items via the <ApiLink pkg="core" type="SelectItem" member="disabled" label="Disabled" /> property.
