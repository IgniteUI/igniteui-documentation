...
The <ApiLink type="Select" /> component can be used inside a `Form` component, thus it exposes a <ApiLink type="Select" member="name" label="Name" /> property to be registered with. 

It also has a <ApiLink type="Select" member="label" label="Label" />, and <ApiLink type="Select" member="placeholder" label="Placeholder" /> properties. The <ApiLink type="Select" member="outlined" label="Outlined" /> property is used for styling purposes only when it comes to the Material theme. 
...
The <ApiLink type="SelectItem" /> component allows the users to declaratively specify a list of options used by the <ApiLink type="Select" /> control. Each item provides a <ApiLink type="SelectItem" member="value" label="Value" /> property that represents the data it carries upon selection. The <ApiLink type="SelectItem" /> has a default slot that allows you to specify the text content of the item. This text content will be used as the value when the <ApiLink type="SelectItem" member="value" label="Value" /> property is not present on the <ApiLink type="SelectItem" />. 

You could also provide custom content to be rendered before or after the <ApiLink type="SelectItem" /> content using the **prefix** and **suffix** slots. As with the <ApiLink type="Select" /> component, we recommend using a `<span>` element when adding simple text, symbols, or emojis, and an [`<igc-icon>`](../layouts/icon.md) component when adding icons to the **prefix** and **suffix** slots. 

You can predefine a selected item by setting the <ApiLink type="SelectItem" member="selected" label="Selected" /> property. You can also disable some or all items via the <ApiLink type="SelectItem" member="disabled" label="Disabled" /> property.
...
Multiple <ApiLink type="SelectItem" /> components can be placed between the opening and closing brackets of an <ApiLink type="SelectGroup" /> component so that users can visually group them together. The <ApiLink type="SelectGroup" /> can be labelled via the **label** slot and disabled via the <ApiLink type="SelectGroup" member="disabled" label="Disabled" /> property. We recommend using a `<span>` element for the **label** slot.
...
## API References
<ApiLink type="Select" pkg="core" />
<ApiLink type="SelectItem" pkg="core" />
<ApiLink type="SelectHeader" pkg="core" />
<ApiLink type="SelectGroup" pkg="core" />
