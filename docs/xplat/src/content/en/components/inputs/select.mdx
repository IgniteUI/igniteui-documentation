...
You could also provide custom content to be rendered before or after the <ApiLink pkg="core" type="SelectItem" /> content using the `prefix` and `suffix` slots. As with the <ApiLink pkg="core" type="Select" /> component, we recommend using a `<span>` element when adding simple text, symbols, or emojis, and an [`<igc-icon>`](../layouts/icon.md) component when adding icons to the `prefix` and `suffix` slots.
