...
llms:
  description: "The {ProductName} Query Builder provides a rich UI that allows developers to build complex data filtering queries for a specified data set."
---
...
To start using the <ApiLink type="QueryBuilder" />, first, you need to install the `{ProductName}` package by running the following command:
...
The Query Builder has the <ApiLink type="QueryBuilder" member="expressionTree" label="ExpressionTree" /> property. You could use it to set an initial state of the control and access the user-specified filtering logic.
...
The <ApiLink type="ExpressionTree" /> is a bindable property which means you can subscribe to the `ExpressionTreeChange` event to receive notifications when the end-user changes the UI by creating, editing or removing conditions.
...
The <ApiLink type="ExpressionTree" /> is a bindable property which means you can use `ExpressionTreeChangeScript` to receive notifications when the end-user changes the UI by creating, editing or removing conditions.
...
```tsx
const queryBuilderRef = useRef<IgrQueryBuilderComponent>(null);
const [expressionTree, setExpressionTree] = useState<IgrFilteringExpressionsTree | null>(null);
...
The <ApiLink type="ExpressionTree" /> is stored in component state which means you can subscribe to the `ExpressionTreeChange` event to receive notifications when the end-user changes the UI by creating, editing or removing conditions. The event listener is attached in `useEffect` and cleaned up in the returned teardown function.
...
By default the <ApiLink type="QueryBuilder" /> header would not be displayed. In order to define such, the <ApiLink type="QueryBuilderHeader" /> component should be added inside the query builder.
...
The search value of a condition can be templated by setting the <ApiLink type="QueryBuilder" member="searchValueTemplate" label="SearchValueTemplate" /> property to a function that returns a lit-html template.
