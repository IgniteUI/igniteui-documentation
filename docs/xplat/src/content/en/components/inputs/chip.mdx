...
With the `Prefix` and `Suffix` parts of the <ApiLink type="Chip" /> component and their slots, you can add different content before and after the chip's main content. The component provides default select and remove icons, which can be customized using the **select** and **remove** slots. Additional content can also be added before or after the main content using the **start** and **end** slots.
