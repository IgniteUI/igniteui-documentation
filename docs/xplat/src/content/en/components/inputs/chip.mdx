...
We recommend using a `<span>` element when adding simple text, symbols, or emojis, and an <ApiLink pkg="core" type="Icon" /> component when adding icons to the `remove`, `select`, `start`, and `end` slots.
