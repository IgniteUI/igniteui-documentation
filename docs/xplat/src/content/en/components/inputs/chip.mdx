...
We recommend using a `<span>` element when adding simple text, symbols, or emojis, and an [`<igc-icon>`](../layouts/icon.mdx) component when adding icons to the `remove`, `select`, `start`, and `end` slots.
