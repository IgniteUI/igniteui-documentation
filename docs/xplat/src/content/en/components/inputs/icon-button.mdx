...
Before using the <ApiLink type="IconButton" />, you need to register it as follows:
...
You will then need to import the <ApiLink type="IconButton" />, its necessary CSS, and register its module, like so:
...
You will also need to link an additional CSS file to apply the styling to the <ApiLink type="IconButton" /> component. The following needs to be placed in the **wwwroot/index.html** file in a **Blazor Web Assembly** project or the **Pages/_Host.cshtml** file in a **Blazor Server** project:
...
The simplest way to start using the <ApiLink type="IconButton" /> is as follows:
...
Similar to the regular button components, the icon button supports several variants - `flat` (default), `contained`, and `outlined`; To change the icon button type set the <ApiLink type="IconButton" member="variant" label="variant" /> attribute of the icon button.
...
The icon button component will change its internal structure from `<button>` to an `<a>` type element when the <ApiLink type="IconButton" member="href" label="href" /> attribute is set. In that case the icon button can be thought of as a regular link. Setting the <ApiLink type="IconButton" member="href" label="href" /> attribute will allow you to also set the <ApiLink type="IconButton" member="rel" label="rel" />, <ApiLink type="IconButton" member="target" label="target" />, and <ApiLink type="IconButton" member="download" label="download" /> attributes of the icon button.
...
The <ApiLink type="IconButton" /> component exposes two CSS parts - `base` and `icon` that allow you to style the wrapping element (`<button>` or `<a>`) and the wrapped `<igc-icon>` element.
...
<ApiLink type="ButtonBase" pkg="core" /><br />
