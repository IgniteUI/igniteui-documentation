...
You will also need to link an additional CSS file to apply the styling to the <ApiLink pkg="core" type="CircularProgress" /> component. The following needs to be placed in the **wwwroot/index.html** file in a **Blazor Web Assembly** project or the **Pages/_Host.cshtml** file in a **Blazor Server** project:
...
The progress bar can be customized to use a color gradient instead of a solid color by using the `gradient` slot and the <ApiLink pkg="core" type="CircularGradient" /> component, which defines the gradient stops.
