...
You will then need to import the <ApiLink type="CircularProgress" />, its necessary CSS, and register its module, like so:
...
Before using the <ApiLink type="CircularProgress" />, you need to register it as follows:
...
The simplest way to start using the <ApiLink type="CircularProgress" /> is as follows:
...
You can set the type of your indicator, using the <ApiLink type="CircularProgress" member="variant" label="Variant" /> attribute. There are five types of circular progress indicators - **primary** (default), **error**, **success**, **info**, and **warning**.
