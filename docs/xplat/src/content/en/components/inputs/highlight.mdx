...
To use the <ApiLink type="Highlight" /> component, all you need to do is wrap its tags around the content you want to search. The component searches the content of all nested elements within the <PlatformBlock for="WebComponents">`<igc-highlight>`</PlatformBlock><PlatformBlock for="React">`<IgrHighlight>`</PlatformBlock><PlatformBlock for="Blazor">`<IgbHighlight>`</PlatformBlock> tags, and highlights all matches of the specified string.
...
Before using the <ApiLink pkg="core" type="Highlight" />, you need to register it as follows:
...
<PlatformBlock for="Blazor">
```razor
<IgbHighlight SearchText="dolor" CaseSensitive="true">
    <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit.</p>
</IgbHighlight>
```
</PlatformBlock>
...
The most common use case is binding the <ApiLink type="Highlight" /> component to a search <ApiLink pkg="core" type="Input" /> component, so that search matches are highlighted in real time as the user types.

To bind the two together, you can listen to the `igcInput` event of the <ApiLink pkg="core" type="Input" /> component and set the <ApiLink type="BaseSearchInfo" member="searchText" label="search-text" /> attribute of the <ApiLink type="Highlight" /> component to the input value every time the event is fired (you can also use the standard `input` event).
