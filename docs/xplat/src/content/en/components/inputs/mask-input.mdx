...
The <ApiLink type="MaskInput" /> exposes a <ApiLink pkg="inputs" type="MaskInput" member="valueMode" label="ValueMode" /> property that lets you choose between `raw` and `withFormatting` options to configure which input value (formatted or raw) to bind in your form when a specific mask is applied. By default, <ApiLink pkg="inputs" type="MaskInput" member="valueMode" label="ValueMode" /> is set to `raw`. Try it for yourself in the example below:
