...
Before using the <ApiLink type="MaskInput" />, you need to register it as follows:
...
You will then need to import the <ApiLink type="MaskInput" />, its necessary CSS, and register its module, like so:
...
You will also need to link an additional CSS file to apply the styling to the <ApiLink type="MaskInput" /> component. The following needs to be placed in the **wwwroot/index.html** file in a **Blazor Web Assembly** project or the **Pages/_Host.cshtml** file in a **Blazor Server** project:
...
These flags also participate in the component validation - i.e., the input becomes invalid if some but not all required positions are filled (no positions filled/empty value is still a responsibility of <ApiLink type="MaskInput" member="required" label="required" />). This applies to both stand-alone inputs and when included in a form.

### Applying Mask

Applying the mask is pretty straightforward. All you need to do is provide a predetermined pattern to the <ApiLink type="MaskInput" member="mask" label="Mask" /> property of the input.
...
Developers can customize the prompt symbol used for unfilled parts of the mask. To do this, simply provide any character to the <ApiLink type="MaskInput" member="prompt" label="Prompt" /> property:
...
Developers can also take advantage of the <ApiLink type="MaskInput" member="placeholder" label="Placeholder" /> property, which serves the purpose of the native input placeholder attribute. If no value is provided for the placeholder, the value of the mask is used as such.
...
The <ApiLink type="MaskInput" /> exposes a <ApiLink pkg="inputs" type="MaskInput" member="valueMode" label="ValueMode" exclude="WebComponents" /> property that lets you choose between `raw` and `withFormatting` options to configure which input value (formatted or raw) to bind in your form when a specific mask is applied. By default, <ApiLink pkg="inputs" type="MaskInput" member="valueMode" label="ValueMode" exclude="WebComponents" /> is set to `raw`. Try it for yourself in the example below:
...
The <ApiLink type="MaskInput" /> component exposes CSS parts for almost all of its inner elements. The following table lists all of the exposed CSS parts:
