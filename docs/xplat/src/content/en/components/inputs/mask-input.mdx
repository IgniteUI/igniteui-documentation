...
The <ApiLink pkg="core" type="MaskInput" /> exposes a <ApiLink type="ValueLayer" member="valueMode" label="ValueMode" /> property that lets you choose between `raw` and `withFormatting` options to configure which input value (formatted or raw) to bind in your form when a specific mask is applied. By default, <ApiLink type="ValueLayer" member="valueMode" label="ValueMode" /> is set to `raw`. Try it for yourself in the example below:
