...
- Add an `aria-label` when a textless Badge, including a `dot` Badge, communicates status that is not otherwise available to assistive technology.
...
- Add a meaningful `label` for a textless or `dot` Badge when its status is not otherwise available to assistive technology.
...
| -- | -- |
| [4.1.2 Name, Role, Value](https://www.w3.org/WAI/WCAG22/Understanding/name-role-value) | The Badge exposes the semantic `status` role. The xplat implementation also exposes a role description based on the current variant; Angular exposes an accessible label and a role description based on its type and content. |
