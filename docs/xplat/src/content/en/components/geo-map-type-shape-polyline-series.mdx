...
## Data Requirements
Similarly to other types of geographic series in the control, the <ApiLink type="GeographicPolylineSeries" /> has the <ApiLink type="Series" member="dataSource" label="ItemsSource" /> property which can be bound to an array of objects. In addition, each data item in this object must have one data column that stores single/multiple shapes using an array of arrays of objects with x and y values representing geographic locations. This data column is then mapped to the <ApiLink type="GeographicShapeSeriesBase" member="shapeMemberPath" label="ShapeMemberPath" /> property. The <ApiLink type="GeographicPolylineSeries" /> uses points of this mapped data column to plot polygons in the control.
