...
The {Platform} Toolbar contains a `Target` property. This is used to link a component, such as the <ApiLink pkg="charts" type="DataChart" /> as shown in the code below:
...
Several pre-existing `IgrToolAction` items and menus become available when the <ApiLink pkg="charts" type="DataChart" /> is linked with the Toolbar. Here is a list of the built-in {Platform} <ApiLink pkg="charts" type="DataChart" /> Tool Actions and their associated `OverlayId`:
...
- `HelpersHeader`: A sub section header.
  - `SeriesAvg`: A `IgrToolActionCheckbox` that adds or removes a <ApiLink pkg="charts" type="ValueLayer" /> to the chart's series collection using the <ApiLink pkg="charts" kind="enum" type="ValueLayerValueMode" prefixed={false} /> of type `Average`.
