...
The {Platform} Toolbar contains a `Target` property. This is used to link a component, such as the `DataChart` as shown in the code below:
...
Several pre-existing `ToolAction` items and menus become available when the `DataChart` is linked with the Toolbar. Here is a list of the built-in {Platform} `DataChart` Tool Actions and their associated `OverlayId`:
