...
The {Platform} Toolbar contains a <ApiLink pkg="layouts" type="Toolbar" member="target" label="Target" /> property. This is used to link a component, such as the <ApiLink pkg="charts" type="DataChart" /> as shown in the code below:
...
Several pre-existing <ApiLink pkg="layouts" type="ToolAction" /> items and menus become available when the <ApiLink pkg="charts" type="DataChart" /> is linked with the Toolbar. Here is a list of the built-in {Platform} `DataChart` Tool Actions and their associated `OverlayId`:
...
By default the {Platform} Toolbar is shown horizontally, but it also has the ability to shown vertically by setting the <ApiLink pkg="layouts" type="Toolbar" member="orientation" label="Orientation" /> property.
