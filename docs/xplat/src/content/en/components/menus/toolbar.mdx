...
Several pre-existing <ApiLink type="ToolAction" /> items and menus become available when the <ApiLink type="DataChart" /> is linked with the Toolbar. Here is a list of the built-in {Platform} `DataChart` Tool Actions and their associated `OverlayId`:
