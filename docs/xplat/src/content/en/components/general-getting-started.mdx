...
Uno Platform uses the `using:` form:
...
treemap have no Uno Platform package, so a topic covering those has no Uno Platform code to show.
...
Uno Platform uses the same `using:` form WinUI does, and the same namespaces:
