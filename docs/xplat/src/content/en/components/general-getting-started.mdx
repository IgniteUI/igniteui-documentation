...
description: "Install {ProductName} and render your first component. Covers CLI scaffolding, manual package setup, and AI tooling with Agent Skills and MCP servers."
keywords: "{ProductName}, Infragistics, Getting Started"
last_updated: "2025-04-06"
mentionedTypes: []
llms:
  description: "{ProductName} is a complete set of UI widgets, components, and Figma UI kits for {Platform} by Infragistics."
apiTerms: full
---
...
<DownloadCardGroup>
  <DownloadCard src={nodejs} alt="nodejs" href="https://nodejs.org/en/download/" label="DOWNLOAD NODE" />
  <DownloadCard src={vsCode} alt="vs-code" href="https://code.visualstudio.com/download" label="DOWNLOAD VS CODE" />
</DownloadCardGroup>
...
<PlatformBlock for="WebComponents">
Next you will need to import the components that you want to use in your `\index.ts` file. You could import one or more components using the <ApiLink type="defineComponents" /> function like this:
</PlatformBlock>
...
<PlatformBlock for="WebComponents">
You could also import all of the components using the <ApiLink type="defineAllComponents" /> function:
</PlatformBlock>
...
{/*<Image src={wcLiveServer} alt="wc-live-server" />*/}
...
{/*<Image src={geoMap} alt="geo-map" />*/}

<Sample src="/maps/geo-map/display-osm-imagery" height={750} alt="{Platform} Overview Example" />
</PlatformBlock>
...
This topic provides step-by-step instructions for creating {Platform} applications with {ProductName}
using Visual Studio.

## Prerequisites

- Visual Studio 2022 with the **Windows application development** workload
- The **Windows App SDK**, which the project template installs as the `Microsoft.WindowsAppSDK` package
- A project targeting `net9.0-windows10.0.19041.0` or later

## Create a New {Platform} Project

Start Visual Studio 2022, click **Create a new project**, select the **Blank App, Packaged (WinUI 3 in
Desktop)** template, and click **Next**. Provide a project name and location, and click **Create**.

## Install {ProductName}

{ProductName} is delivered via NuGet packages. In Visual Studio, open the NuGet package manager by selecting **Tools** → **NuGet Package Manager** →
**Manage NuGet Packages for Solution**, then install the packages for the controls you intend to use.
Every package needs **Infragistics.WinUI.Core**:

| Package | Controls | Namespace to declare |
| --- | --- | --- |
| `Infragistics.WinUI.Core` | required by all of the below | `Infragistics.Controls` |
| `Infragistics.WinUI.Charts` | charts and graphs | `Infragistics.Controls.Charts` |
| `Infragistics.WinUI.DataGrid` | the data grid | `Infragistics.Controls.Grids` |
| `Infragistics.WinUI.Gauges` | radial, linear and bullet gauges | `Infragistics.Controls.Gauges` |
| `Infragistics.WinUI.Maps` | the geographic map | `Infragistics.Controls.Maps` |
| `Infragistics.WinUI.Inputs` | inputs and editors | `Infragistics.Controls.Inputs` |
| `Infragistics.WinUI.Layouts` | toolbars and layout panels | `Infragistics.Controls.Layouts` |
| `Infragistics.WinUI.Dashboards` | the dashboard tile | `Infragistics.Controls.Dashboards` |
| `Infragistics.WinUI.Sparkline` | the sparkline | `Infragistics.Controls.Charts` |
| `Infragistics.WinUI.TreeMap` | the treemap | `Infragistics.Controls.Charts` |
| `Infragistics.WinUI.DataVisualization` | shared support for the visualization controls | — |
| `Infragistics.WinUI.SkiaSharp` | optional SkiaSharp rendering backend | — |

Licensed users take these from the Infragistics private NuGet feed. Add it as a package source in
Visual Studio under **Tools** → **NuGet Package Manager** → **Package Manager Settings**, with the
source set to `https://packages.infragistics.com/nuget/licensed/v3/index.json`.

## Declare the Namespaces

Each package of controls lives in its own namespace, and a XAML file declares the ones it uses. WinUI and
Uno use the `using:` form:
...
You only need the ones a file actually uses. Every code snippet in these topics is written with these
prefixes, so a snippet showing `igDataGrids:XamDataGrid` needs the `igDataGrids` declaration above.

## Add Your First Component

With the package installed and its namespace declared, a control is markup like any other. This adds a
radial gauge to a window, using the `igGauges` prefix declared above:
...
Run the project and the gauge draws its scale, tick marks and needle at 50. Each control's own topic
starts from a block like this one and adds the properties that topic is about.
...
{ProductName} runs on Uno Platform through its own set of packages. The controls, their properties and
their XAML are the same ones the WinUI topics show; what differs is how the project is created and which
packages exist.

## Prerequisites

Uno Platform has its own solution templates and its own prerequisite check rather than a single Visual
Studio workload, and both move faster than this topic can. Follow the Uno Platform
[Getting Started](https://platform.uno/docs/articles/get-started.html) guide to create a project and to
run `uno-check`, then come back here to add {ProductName}.

## Install {ProductName}

{ProductName} is delivered via NuGet packages. Every package needs **Infragistics.Uno.Core**:

| Package | Controls | Namespace to declare |
| --- | --- | --- |
| `Infragistics.Uno.Core` | required by all of the below | `Infragistics.Controls` |
| `Infragistics.Uno.Charts` | charts and graphs | `Infragistics.Controls.Charts` |
| `Infragistics.Uno.DataGrid` | the data grid | `Infragistics.Controls.Grids` |
| `Infragistics.Uno.Gauges` | radial, linear and bullet gauges | `Infragistics.Controls.Gauges` |
| `Infragistics.Uno.DataVisualization` | shared support for the visualization controls | — |

A WebAssembly head takes the `Infragistics.Uno.Wasm.*` packages instead: **Infragistics.Uno.Wasm.Core**,
**Infragistics.Uno.Wasm.Charts** and **Infragistics.Uno.Wasm.DataVisualization**.

This is a narrower set than WinUI's. The geographic map, dashboard tile, inputs, layouts, sparkline and
treemap have no Uno package, so a topic covering those has no Uno code to show.
...
Uno uses the same `using:` form WinUI does, and the same namespaces:
...
Declare only the ones a file uses. Every code snippet in these topics is written with these prefixes.
...
Run the project and the gauge draws its scale, tick marks and needle at 50.
</PlatformBlock>
