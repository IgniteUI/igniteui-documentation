...
{/* TODO: confirm the feed. The packages above are the IDs this repository packs
     (Source/Nuget/Infragistics.WinUI.*.nuspec); nuget.org has them only at 21.2.36-alpha, so a licensed
     feed URL probably belongs here too. And note there is no WinUI maps package in that set, while the
     geo-map topics do publish for WinUI -- worth resolving before this table is taken as complete. */}
