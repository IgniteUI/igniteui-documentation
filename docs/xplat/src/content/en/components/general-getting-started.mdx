...
This topic provides step-by-step instructions for creating WinUI applications with Ignite UI for WinUI
using Visual Studio.
...
## Install Ignite UI for WinUI

Ignite UI for WinUI is delivered via NuGet packages. In Visual Studio, open the NuGet package manager by
selecting **Tools** → **NuGet Package Manager** → **Manage NuGet Packages for Solution**, then search for
and install the packages for the controls you intend to use.

{/* TODO: name the shipping package IDs and the feed. Not written here because they could not be
     confirmed: nuget.org lists only Infragistics.WinUI.Core and Infragistics.WinUI.Charts at
     21.2.36-alpha, and neither docConfig.json nor the WinUI sample template names a customer-facing
     package -- the samples reference the product assemblies directly. Confirm against the release
