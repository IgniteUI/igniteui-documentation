...
{/* TODO: the Blazor section above carries screenshots at each step. Add the equivalent WinUI ones,
     or drop the images from this section deliberately. */}
...
{/* TODO: a short first-component walkthrough, in the shape the Blazor section uses. The data grid
     overview topic already carries a complete XAML block that could be reused here. */}

</PlatformBlock>

<PlatformBlock for="Uno">
{/* TODO: Uno needs its own section. The namespace declarations above are identical -- Uno uses the
     same `using:` form and the same Infragistics.Controls.* namespaces -- but project creation differs:
     Uno has its own solution templates rather than the WinUI "Blank App, Packaged" one. Written
     separately rather than folded into for="Xaml" so neither platform is given the other's steps. */}
</PlatformBlock>
