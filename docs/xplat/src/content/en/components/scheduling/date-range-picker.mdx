...
In two-input mode, you can use the `prefix-start`, `prefix-end`, `suffix-start`, and `suffix-end` slots instead to target the individual inputs. For all `prefix-` and `suffix-` type slots, we recommend using a `<span>` element for simple text, symbols, or emojis, and an [`<igc-icon>`](../layouts/icon.mdx) component for icons.

Another set of useful slots are `clear-icon` and `calendar-icon`, which allow you to customize the icons for the clear and calendar buttons in the input fields. For these slots we also recommend using a `<span>` element for symbols, or emojis and an [`<igc-icon>`](../layouts/icon.mdx) component for icons.
...
And for the `clear-icon-` and `calendar-icon-` type slots, we recommend using a `<span>` element for symbols, or emojis and an [`<igc-icon>`](../layouts/icon.mdx) component for icons.
