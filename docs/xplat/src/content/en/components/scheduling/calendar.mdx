...
In some cases you would want to have disabled dates in the Calendar which can't be selected by the end user. This functionality is achieved by using the <ApiLink pkg="core" type="Calendar" member="disabledDates" /> property. The <ApiLink pkg="core" type="Calendar" member="disabledDates" /> property is an array of `IgrDateRangeDescriptor` objects. Each descriptor has a `Type` and optionally a <ApiLink pkg="inputs" type="DateRangeDescriptor" member="dateRange" label="DateRange" suffix={false} exclude="React,WebComponents" /> which is an array of `Date` objects.
...
Let's add some special dates to our Calendar. In order to do this, we will create a `IgrDateRangeDescriptor` and pass the dates between the 3rd and the 8th of the current month:
