...
By default, the {ProductName} Calendar component renders a header area which contains information about the selected dates. You could hide the header by setting the <ApiLink type="Calendar" member="hasHeader" /> property to **false**. You could also configure `vertical` or `horizontal` orientation of the header using the <ApiLink type="Calendar" member="headerOrientation" /> property.
