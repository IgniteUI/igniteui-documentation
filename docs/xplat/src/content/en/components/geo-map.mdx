...
The following sample demonstrates how display data in <ApiLink pkg="maps" type="GeographicMap" /> using <ApiLink pkg="maps" type="GeographicProportionalSymbolSeries" /> also known as Bubble Series.
...
The map's Series property is used to support rendering an unlimited number of geographic series. This property is a collection of geographic series objects and any type of geographic series can be added to it. For example, <ApiLink pkg="maps" type="GeographicSymbolSeries" /> can be added for plotting geographic locations such as cities and the <ApiLink pkg="maps" type="GeographicPolylineSeries" /> for plotting connections (e.g. roads) between these geographic locations.
...
The <ApiLink pkg="maps" type="GeographicMap" /> requires the following modules, however the DataChartInteractivityModule is only required for mouse interactions, such as panning and zooming the map content.
