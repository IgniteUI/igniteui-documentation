...
The following sample demonstrates how display data in `GeographicMap` using `GeographicProportionalSymbolSeries` also known as Bubble Series.
...
The `GeographicMap` requires the following modules, however the DataChartInteractivityModule is only required for mouse interactions, such as panning and zooming the map content.
