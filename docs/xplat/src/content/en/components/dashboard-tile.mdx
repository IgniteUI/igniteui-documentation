...
The data visualization that is chosen by default is mainly dependent on the schema and the count of the <ApiLink type="DataSource" /> that you have bound. For example, if you bind a single numeric value, you will get a `XamRadialGauge`, but if you bind a collection of value-label pairs that are easy to distinguish from each other, you will likely get a `DataPieChart`. If you bind an <ApiLink type="DataSource" /> that has more value paths, you will receive a `XamDataChart` with multiple column series or line series, depending mainly on the count of the collection bound. You can also bind to a `ShapefileConverter` or data the appears to contain geographic points to receive a `XamGeographicMap`.
