...
In addition to accessing sort conditions from the table columns, they are also exposed off the <ApiLink pkg="excel" prefixed={false} type="WorksheetTable" />'s <ApiLink pkg="excel" prefixed={false} type="SortSettings" /> property's `SortConditions` collection. This is an ordered collection of columns/sort condition pairs. The order of this collection is the precedence of the sorting.
...
There is also a `CaseSensitive` property on the <ApiLink pkg="excel" prefixed={false} type="SortSettings" /> of the <ApiLink pkg="excel" prefixed={false} type="WorksheetTable" /> to determine whether strings should be sorted case sensitively or not.

The following code snippet demonstrates how to apply an `OrderedSortCondition` to a <ApiLink pkg="excel" prefixed={false} type="WorksheetTable" />:
...
<ApiRef pkg="excel" types={["WorksheetTableStyle","WorksheetTable"]} />
