...
## Data Requirements
Similarly to other types of geographic series in the map component, the <ApiLink type="GeographicSymbolSeries" /> has the <ApiLink type="Series" member="dataSource" label="ItemsSource" /> property which can be bound to an array of objects. In addition, each data item in this object must have two numeric data columns that store a geographic location (longitude and latitude). These data columns are then mapped to the `LatitudeMemberPath` and `LongitudeMemberPath` properties. The <ApiLink type="GeographicSymbolSeries" /> uses values of these mapped data columns to plot symbol elements in the geographic map component.
