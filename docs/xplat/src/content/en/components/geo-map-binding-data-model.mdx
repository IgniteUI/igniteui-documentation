...
## Code Snippet
The following code shows how to bind the `GeographicSymbolSeries` to a custom data model that contains geographic locations of some cities of the world stored using longitude and latitude coordinates. Also, we use the `GeographicPolylineSeries` to plot shortest geographic path between these locations using the great circle maths of `\MapGeodesics`, shown with the code below and written up on its own in [WorldUtility](./geo-map-resources-world-util.mdx)
