...
There is a new property called `UseInsetOutlines` to control how outlines on the <ApiLink pkg="charts" type="RadialPieSeries" /> are rendered. Setting this value to **true** will inset the outlines within the slice shape, whereas a **false** (default) value will place the outlines half-in half-out along the edge of the slice shape.
...
- <ApiLink pkg="grids" type="PivotGrid" />
  - Added `sortable` property for a <ApiLink pkg="grids" kind="interface" type="PivotDimension" />.
  - Added horizontal layout. Can be enabled inside the new `pivotUI` property as `rowLayout` `horizontal`.
  - Added row dimension summaries for horizontal layout only. Can be enabled for each <ApiLink pkg="grids" kind="interface" type="PivotDimension" /> by setting `horizontalSummary` to **true**.
...
- New `FavorLabellingScaleEnd`, determines whether the axis should favor emitting a label at the end of the scale. Only compatible with numeric axes (e.g. <ApiLink pkg="charts" type="NumericXAxis" />, <ApiLink pkg="charts" type="NumericYAxis" />, `PercentChangeAxis`).
...
#### Added
- <ApiLink pkg="inputs" type="ButtonGroup" /> - now allows resetting the selection state via the `SelectedItems` property.
- <ApiLink pkg="inputs" type="Input" />, <ApiLink pkg="inputs" type="Textarea" /> - exposed `ValidateOnly` to enable validation rules being enforced without restricting user input.
...
#### Added
- <ApiLink pkg="inputs" type="Combo" /> can now set `GroupSorting` to none which shows the groups in the order of the provided data.
...
#### Added
- <ApiLink pkg="inputs" type="Tree" /> - Added `ToggleNodeOnClick` property that determines whether clicking over a node will change its expanded state or not. Defaults to `false`.
...
- <ApiLink pkg="inputs" type="TreeItem" suffix={false} /> disabled styles.
- <ApiLink pkg="inputs" type="Snackbar" /> removed unnecessary styles.
- <ApiLink pkg="inputs" type="TreeItem" suffix={false} /> hover state visual design.
