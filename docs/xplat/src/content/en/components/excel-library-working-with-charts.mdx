...
llms:
  description: "The Infragistics {Platform} Excel Engine's WorksheetChart functionality allows you to add visual charting representations of data trends across regions of cells in a worksheet."
...
## Usage
In order to add a chart to a worksheet, you must use the <ApiLink type="WorksheetShapeCollection" member="addChart" label="AddChart" /> method of the worksheet's shapes collection. In this method, you can specify the chart type that you wish to use, the top-left cell, the bottom-right cell, and the percentages of those cells that you wish for the chart to take up.

The <ApiLink type="WorksheetShapeCollection" member="addChart" label="AddChart" /> method returns the worksheet chart element to be added to the worksheet. Once you have this, you can use the <ApiLink type="WorksheetChart" member="setSourceData" label="SetSourceData" /> method on the chart to set a cell address of the region of worksheet cells that you wish to use as a data source, as well as whether or not you want to switch the mapping of columns and rows to the X and Y axis.

There are over 70 supported chart types, including `Line`, `Area`, <ApiLink type="Column" />, and `Pie`.
