...
llms:
  description: "The {ProductName} Treemap chart displays hierarchical (tree-structured) data as a set of nested nodes."
---
...
In the following example, the <ApiLink type="Treemap" /> demonstrates the 30 largest countries in the world by total area.
...
- All data items must contain at least one data column (e.g. string) which should be mapped to the <ApiLink type="Treemap" member="labelMemberPath" label="LabelMemberPath" /> property.
- All data items must contain at least one numeric data column which should be mapped using the <ApiLink type="Treemap" member="valueMemberPath" label="ValueMemberPath" /> property.
- To categorize data into organized tiles you can optionally use <ApiLink type="Treemap" member="parentIdMemberPath" label="ParentIdMemberPath" /> and <ApiLink type="Treemap" member="idMemberPath" label="IdMemberPath" />.

## {Platform} Treemap Configuration

In the following example, the treemap demonstrates the ability of changing it's algorithmic structure by modifying the <ApiLink type="Treemap" member="layoutType" label="LayoutType" /> and <ApiLink type="Treemap" member="layoutOrientation" label="LayoutOrientation" /> properties.
...
<ApiLink type="Treemap" member="layoutOrientation" label="LayoutOrientation" /> property enables the user to set the direction in which the nodes of the hierarchy will be expanded.

Note that the <ApiLink type="Treemap" member="layoutOrientation" label="LayoutOrientation" /> property works with the layout types SliceAndDice and Strip.
...
- <ApiLink type="Treemap" member="highlightedDataSource" label="HighlightedItemsSource" />: Specifies the datasource to read highlighted values from. If null, then highlighted values are read from the ItemsSource property.
- <ApiLink type="Treemap" member="highlightedValueMemberPath" label="HighlightedValueMemberPath" />: Specifies the name of the property in the datasource where the highlighted values are read.
- <ApiLink type="Treemap" member="highlightedValueOpacity" label="HighlightedValueOpacity" />: Controls the opacity of the normal value behind the highlighted value.
- <ApiLink type="Treemap" member="highlightedValuesDisplayMode" label="HighlightedValuesDisplayMode" />: Enables or disables highlighted values.
