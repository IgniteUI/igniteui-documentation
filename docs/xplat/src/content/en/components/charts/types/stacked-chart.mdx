...
The {Platform} `XamDataChart` control renders every one of these types from the same data, and which one you get is the series you bind it to. The following sections demonstrate the individual types of {ProductName} Stacked Charts, each with the series that produces it.
