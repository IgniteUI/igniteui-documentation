...
|`UserStripAnnotation.label`|`string`|This property gets or sets the label to be shown in the user annotation.|
...
Once the user annotation has been added to the chart, it will appear in the `Series` collection as a `UserAnnotationLayer`. The `UserAnnotationLayer` has an `UserAnnotationLayer.annotations` collection that can store `UserSliceAnnotation`, `UserStripAnnotation` and `UserPointAnnotation` elements depending on the type of annotations added to the plot area.
