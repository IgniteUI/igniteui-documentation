...
Whether or not zooming is on by default depends on the chart you are using. If you are using <ApiLink type="CategoryChart" />, it is on by default, but it is not in the <ApiLink type="DataChart" />. In order to enable or disable navigation in the UI, you need to set either the `isHorizontalZoomEnabled` and/or the `isVerticalZoomEnabled` properties of the chart, depending on the direction that you wish to enable or disable zooming.
...
- `windowRect`: A <ApiLink type="Rect" /> object representing a rectangle that represents the portion of the chart that is currently in view. For example, a `windowRect` of "0, 0, 1, 1" would be the entirety of the data chart.
