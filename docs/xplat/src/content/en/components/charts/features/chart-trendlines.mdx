...
To change the color of the trendline that is drawn, you can set its `TrendLineLayer.brush` property. Alternatively, you can also set the `useIndex` property to `true`, which will pull from the chart's `DomainChart.brushes` palette based on the index in which the `TrendLineLayer` is placed in the chart's `Series` collection.
