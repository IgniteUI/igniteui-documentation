...
The linear gauge can be modified to show a second needle. This will make the main needle's `LinearGauge.value` appear with a lower opacity. To enable this first set `highlightValueDisplayMode` to Overlay and then apply a `highlightValue`.
