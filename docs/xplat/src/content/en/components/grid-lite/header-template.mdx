...
By default the column uses the <ApiLink pkg="grids" type="Column" member="field" label="Field" /> property for label text. To customize the label, set the <ApiLink pkg="grids" type="Column" member="header" label="Header" /> property to a more human readable format.
