...
The {GridLiteTitle} supports both single and multi-column sorting. Multi-column is enabled by default and can be configured through the <ApiLink pkg="grids" type="SortingOptions" suffix={false} /> property of the grid. The `Mode` property accepts `GridLiteSortingMode.Single` or `GridLiteSortingMode.Multiple` as values.
