...
The grid supports inferring the column configuration based on the provided data source when `AutoGenerate` is set to true. It tries to infer the appropriate <ApiLink pkg="grids" type="Column" member="field" label="Field" /> and <ApiLink pkg="grids" type="Column" member="dataType" label="DataType" /> properties based on records in the data.
...
To change the width of column, use the <ApiLink pkg="grids" type="Column" member="width" label="Width" /> parameter of the `IgbGridLiteColumn` component.
...
Columns can be hidden/shown by setting the <ApiLink pkg="grids" type="Column" member="hidden" label="Hidden" /> parameter on the `IgbGridLiteColumn` component.
...
Each column of the {GridLiteTitle} can be configured to be resizable by setting the <ApiLink pkg="grids" type="Column" member="resizable" label="Resizable" /> parameter on the `IgbGridLiteColumn` component.
