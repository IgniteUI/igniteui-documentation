...
In the {ProductName} map, you can add multiple geographic series objects to overlay a few shapefiles with geo-spacial data. For example, <ApiLink pkg="maps" type="GeographicSymbolSeries" /> for plotting geographic locations of ports, the <ApiLink pkg="maps" type="GeographicPolylineSeries" /> for plotting routes between ports, and the <ApiLink pkg="maps" type="GeographicShapeSeries" /> for plotting shapes of countries.
