...
llms:
  description: "The {Platform} Toast is a super lightweight and small pop-up component that is used for displaying a message content, notifying end-users about the status of a changed record."
---
...
You will then need to import the {Platform} <ApiLink pkg="core" type="Toast" /> and its necessary CSS, like so:
...
Before using the {Platform} <ApiLink pkg="core" type="Toast" />, you need to register it as follows:
...
The simplest way to display the toast component is to use its <ApiLink pkg="core" type="Toast" member="show" label="Show" /> method and call it on a button click.
...
Use the <ApiLink pkg="core" type="Toast" member="displayTime" label="DisplayTime" /> property to configure how long the toast component is visible. By default, it's set to 4000 milliseconds.

By default, the toast component is hidden automatically after a period specified by the <ApiLink pkg="core" type="Toast" member="displayTime" label="DisplayTime" />. You can use <ApiLink pkg="core" type="Toast" member="keepOpen" label="KeepOpen" /> property to change this behavior. In this way, the toast will remain visible.
...
You can style the {Platform} <ApiLink pkg="core" type="Toast" /> notifications directly using its tag selector:
