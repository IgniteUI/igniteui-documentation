...
Before using the <ApiLink type="Icon" />, you need to register it as follows:
...
You will then need to import the <ApiLink type="Icon" />, its necessary CSS, and register its module, like so:
...
You will also need to link an additional CSS file to apply the styling to the <ApiLink type="Icon" /> component. The following needs to be placed in the **wwwroot/index.html** file in a **Blazor Web Assembly** project or the **Pages/_Host.cshtml** file in a **Blazor Server** project:
...
The <ApiLink type="Icon" /> doesn't contain any icons on its own. It's a conduit for displaying any _registered_ SVG images.
...
To register an image as an icon, all you need to do is call one of the 2 "register" methods on a single <ApiLink type="Icon" /> element that allow you to add icons to an icon collection on your page.

The <ApiLink type="Icon" member="registerIcon" label="RegisterIcon" /> method allows you to register an SVG image as an icon from an external file:
...
In order to use the newly registered icon, all you have to do is to pass the name and collection to the <ApiLink type="Icon" /> element:
...
The icon component can be styled by applying styles directly to the <ApiLink type="Icon" /> element;
