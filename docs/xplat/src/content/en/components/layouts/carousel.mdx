...
If you want a slide to be active by default, use the <ApiLink type="CarouselSlide" member="active" label="Active" /> attribute:
...
- **Attributes**
  ...
  - [aria-live](https://www.w3.org/TR/wai-aria-1.1/#aria-live) - used to set the priority with which screen reader should treat updates to live regions - the possible settings are: **off** and **polite**. The default setting is **polite** and is set to the element that serves as the container for the set of slides. When the <ApiLink type="Carousel" member="interval" label="Interval" /> option is set and the carousel is in playing state, the **aria-live** attribute would be set to **off**.
...
<ApiLink type="CarouselSlide" />
<ApiLink type="CarouselIndicator" />
<ApiLink type="Icon" />
<ApiLink type="Input" />
<ApiLink type="Button" />
