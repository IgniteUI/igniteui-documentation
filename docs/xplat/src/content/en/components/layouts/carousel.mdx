...
The {ProductName} Carousel component allows users to use different elements for the active and inactive states of a single indicator. It is mandatory to provide two elements for each indicator slot (**empty** and **active**) when declaring an indicator, even if they are the same. We recommend using a `<span>` element for symbol or emoji indicators, and an [`<igc-icon>`](../layouts/icon.mdx) component for icon indicators.
...
Just like with the custom indicators, we recommend using a `<span>` element for symbol or emoji navigation buttons, and an [`<igc-icon>`](../layouts/icon.mdx) component for icon navigation buttons.
