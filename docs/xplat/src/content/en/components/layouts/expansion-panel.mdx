...
The `indicator` slot is intended for icon-like content. We recommend using the [`<igc-icon>`](../layouts/icon.mdx) component for this purpose. Alternatively, if you don't want to use an icon for the indicator, a `<span>` element can be used to display text, symbols, or emojis.
