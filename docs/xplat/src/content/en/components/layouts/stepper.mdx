...
For the `indicator` slot, we also recommend using a `<span>` element for symbol or emoji indicators, and an <ApiLink pkg="core" type="Icon" /> component for icon indicators.
...
The user can configure the title position using the <ApiLink pkg="core" type="Stepper" member="titlePosition" label="TitlePosition" /> property. It takes the following values:
...
The <ApiLink pkg="core" type="Stepper" /> component also gives you the ability to configure the duration of the transition between the steps. This could be achieved through the <ApiLink pkg="core" type="Stepper" member="animationDuration" label="AnimationDuration" /> property, which takes a number as an argument and it is common to both orientations. The default value is set to 320ms.
