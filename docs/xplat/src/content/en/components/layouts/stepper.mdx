...
<igc-stepper>
    ...
    <igc-step .disabled=${step.disabled}>
        <igc-icon slot="indicator" .iconName=${step.indicator}></igc-icon>
...
You can customize the stepper orientation through the exposed <ApiLink type="Stepper" member="orientation" label="Orientation" /> property. It could be set to **horizontal** or **vertical**.

**Horizontal Stepper Orientation**

**horizontal** is the default value for the <ApiLink type="Stepper" /> orientation property.
When the {Platform} stepper is horizontally orientated you have the opportunity to determine whether the step content would be displayed above or below the step headers. This could be achieved by setting the <ApiLink type="Stepper" /> <ApiLink type="Stepper" member="contentTop" label="ContentTop" /> boolean property, which default value is **false**. In case it is enabled the step content would be displayed above the step headers.
