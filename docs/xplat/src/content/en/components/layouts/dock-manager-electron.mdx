...
In the `paneHeaderDragStart` function we set the <ApiLink type="DockManager" member="draggedPane" /> property of the Dock Manager component which will notify it that a drag operation has been started.
...
We have subscribed to the `dragover` and `drop` events of the `document` element. In the `dragover` listener we notify the Dock Manager that the mouse is dragged over it by setting its <ApiLink type="DockManager" member="dropPosition" /> property. This forces the Dock Manager to display its docking indicators.
...
When a pane is dropped out of its current window, we need to remove the <ApiLink type="DockManager" member="draggedPane" /> from its Dock Manager component and update the layout.
