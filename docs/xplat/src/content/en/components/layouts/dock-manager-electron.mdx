...
In order to support dragging panes outside the application window we have replaced the built-in drag/drop which creates in-application floating panes with a custom implementation based on the [HTML Drag and Drop API](https://developer.mozilla.org/en-US/docs/Web/API/HTML_Drag_and_Drop_API). We have subscribed to the <ApiLink type="DockManager" member="paneHeaderConnected" label="PaneHeaderConnected" /> and <ApiLink type="DockManager" member="tabHeaderConnected" label="TabHeaderConnected" /> events which are fired when a header element is connected to the DOM. When a header element is connected we reset the built-in `DragService` and attach `DragStart` and `DragEnd` event listeners.
