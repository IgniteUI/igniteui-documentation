...
In order to support dragging panes outside the application window we have replaced the built-in drag/drop which creates in-application floating panes with a custom implementation based on the [HTML Drag and Drop API](https://developer.mozilla.org/en-US/docs/Web/API/HTML_Drag_and_Drop_API). We have subscribed to the <ApiLink pkg="dockmanager" type="DockManager" member="paneHeaderConnected" label="PaneHeaderConnected" /> and <ApiLink pkg="dockmanager" type="DockManager" member="tabHeaderConnected" label="TabHeaderConnected" /> events which are fired when a header element is connected to the DOM. When a header element is connected we reset the built-in `DragService` and attach `DragStart` and `DragEnd` event listeners.
...
In the `PaneHeaderDragStart` function we set the <ApiLink pkg="dockmanager" type="DockManager" member="draggedPane" label="DraggedPane" /> property of the Dock Manager component which will notify it that a drag operation has been started.
...
We have subscribed to the `DragOver` and `drop` events of the `document` element. In the `DragOver` listener we notify the Dock Manager that the mouse is dragged over it by setting its <ApiLink pkg="dockmanager" type="DockManager" member="dropPosition" label="DropPosition" /> property. This forces the Dock Manager to display its docking indicators.
...
When a pane is dropped out of its current window, we need to remove the <ApiLink pkg="dockmanager" type="DockManager" member="draggedPane" label="DraggedPane" /> from its Dock Manager component and update the layout.
