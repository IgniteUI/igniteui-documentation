...
You will then need to import the <ApiLink type="Avatar" /> and its necessary CSS, like so:
...
Before using the <ApiLink type="Avatar" />, you need to register it as follows:
...
You will also need to link an additional CSS file to apply the styling to the <ApiLink type="Avatar" /> component. The following needs to be placed in the **wwwroot/index.html** file in a **Blazor Web Assembly** project or the **Pages/_Host.cshtml** file in a **Blazor Server** project:
...
The <ApiLink type="Avatar" /> is capable of displaying images, initials, or any other content, including icons. Declaring an <ApiLink type="Avatar" /> is as simple as:
...
If the <ApiLink type="Avatar" member="initials" label="Initials" /> attribute is set all children elements of the avatar will be ignored and the string passed to this attribute will be displayed.
...
The avatar can also display an image when the <ApiLink type="Avatar" member="src" label="Src" /> attribute is assigned a valid URL to a static asset. In that case the <ApiLink type="Avatar" member="initials" label="Initials" /> value will be ignored and children elements will not be rendered.
...
The <ApiLink type="Avatar" /> component exposes several CSS parts, giving you full control over its style:
