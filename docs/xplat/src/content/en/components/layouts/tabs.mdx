...
When using the `label` slot, we recommend using a `<span>` element when you want to provide text and an <ApiLink pkg="core" type="Icon" /> or <ApiLink pkg="core" type="IconButton" /> component when you want to display icons.
...
Besides the `label` slot, which can be used to display an icon, text, or both, each tab also provides `prefix` and `suffix` slots for displaying additional content at the beginning and/or the end. When slotting content into the `prefix` and `suffix` slots, we recommend using a `<span>` element for symbols or emojis, and an <ApiLink pkg="core" type="Icon" /> or <ApiLink pkg="core" type="IconButton" /> component for icons.
