...
Tab text can be set either as simple string using the <ApiLink type="Tab" member="label" label="Label" /> property or by assigning children to the `label` slot. Any remaining children in the default slot are rendered as the tab content.
...
A tab is disabled by setting the <ApiLink type="Tab" member="disabled" label="Disabled" /> attribute:
...
The <ApiLink type="Tabs" member="alignment" label="Alignment" /> property controls how {Platform} tabs are positioned. It accepts the following values:
...
|Keys|Description|
|----|-----------|
| <kbd>←</kbd> | Selects previous (next in Right-to-Left mode) tab. If <ApiLink type="Tabs" member="activation" label="Activation" /> is set to `Manual` only focuses the tab. Scrolls to end if on first tab.  |
| <kbd>→</kbd> | Selects next (previous in Right-to-Left mode) tab. If <ApiLink type="Tabs" member="activation" label="Activation" /> is set to `Manual` only focuses the tab. Scrolls to start if on last tab. |
| <kbd>HOME</kbd> | Selects the first tab. |
| <kbd>END</kbd> | Selects the last tab. |
| <kbd>ENTER</kbd> / <kbd>SPACE</kbd> | Selects the focused tab when <ApiLink type="Tabs" member="activation" label="Activation" /> is `Manual` |
