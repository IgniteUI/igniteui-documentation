...
When using the `fullscreen-action`, `maximize-action`, `actions`, `side-adorner`, `corner-adorner` and `bottom-adorner` slots, we recommend using an <ApiLink pkg="core" type="Icon" /> or <ApiLink pkg="core" type="IconButton" /> component for custom icons.
