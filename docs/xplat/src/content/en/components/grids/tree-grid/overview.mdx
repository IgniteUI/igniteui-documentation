...
llms:
  description: "The {Platform} Tree Grid is a UI component that combines the functionality of a data grid (table) with a tree view, allowing hierarchical data to be easily displayed in a tabular format."
---
...
For more details on how to customize the appearance of the tree grid, you may have a look at the [styling](./overview.mdx#web-components-tree-grid-styling-configuration) section.
</PlatformBlock>
...
In order to achieve this, the tree grid provides us with a couple of ways to define the relations among our data objects - by using a [child collection](./overview.mdx#child-collection) for every data object or by using [primary and foreign keys](./overview.mdx#primary-and-foreign-keys) for every data object.
...
Now let's start by importing our <ApiLink type="TreeGrid" member="data" label="Data" /> collection and binding it to our tree grid.
...
In order for the tree grid to build the hierarchy, we will have to set its <ApiLink type="TreeGrid" member="childDataKey" label="ChildDataKey" /> property to the name of the child collection that is used in each of our data objects. In our case that will be the **Employees** collection.
...
We can now enable the row selection and paging features of the tree grid by using the <ApiLink type="GridBaseDirective" member="rowSelection" label="RowSelection" /> and add the <ApiLink type="Paginator" /> element.
...
Finally, we can enable the toolbar of our tree grid, along with the column hiding, column pinning and exporting features by using the <ApiLink type="GridToolbar" />, <ApiLink type="GridToolbarHiding" />, <ApiLink type="GridToolbarPinning" /> and <ApiLink type="GridToolbarExporter" /> respectively.
...
You can see the result of the code from above at the beginning of this article in the [Tree Grid Example](./overview.mdx#example) section.
...
In the sample data above, all records have an ID, a ParentID and some additional properties like Name, JobTitle and Age. As mentioned previously, the ID of the records must be unique as it will be our <ApiLink type="GridBaseDirective" member="primaryKey" label="PrimaryKey" />. The ParentID contains the ID of the parent node and could be set as a <ApiLink type="TreeGrid" member="foreignKey" label="ForeignKey" />. If a row has a ParentID that does not match any row in the tree grid, then that means this row is a root row.

The parent-child relation is configured using the tree grid's <ApiLink type="GridBaseDirective" member="primaryKey" label="PrimaryKey" /> and <ApiLink type="TreeGrid" member="foreignKey" label="ForeignKey" /> properties.
...
- The first column (the one that has a <ApiLink type="Column" member="visibleIndex" label="VisibleIndex" /> of 0) is always the tree column.
- The column that ends up with a <ApiLink type="Column" member="visibleIndex" label="VisibleIndex" /> of 0 after operations like column pinning, column hiding and column moving becomes the tree column.
