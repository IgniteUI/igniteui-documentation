...
In order to specify which edit mode should be enabled, the `{ComponentName}` exposes the following boolean properties - <ApiLink pkg="grids" type="{ComponentName}" member="editable" prefixed={false} /> and <ApiLink pkg="grids" type="{ComponentName}" member="rowEditable" prefixed={false} />.
...
In the `{ComponentName}`, if you set <ApiLink pkg="grids" type="{ComponentName}" member="rowEditable" prefixed={false} /> property to true, and the <ApiLink pkg="grids" type="{ComponentName}" member="editable" prefixed={false} /> property is not explicitly defined for any column, the editing will be enabled for all the columns except the **primary key**.
