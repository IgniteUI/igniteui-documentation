...
Setting a title for the toolbar in your grid is achieved by using the <ApiLink pkg="grids" type="GridToolbarTitle" />.
...
The <ApiLink pkg="grids" type="GridToolbarActions" /> is where users can place actions/interactions in relation to the parent grid.
...
<PlatformBlock for="Angular">
Each action now exposes a way to change the overlay settings of the actions dialog by using the <ApiLink pkg="grids" type="GridToolbarPinning" member="overlaySettings" label="OverlaySettings" /> input. For example:
...
The <ApiLink pkg="grids" type="GridToolbarPinning" /> component provides the default UI for interacting with column pinning in the grid.
...
The <ApiLink pkg="grids" type="GridToolbarHiding" /> provides the default UI for interacting with column hiding. Exposes the same input properties for customizing the UI, such as the component
...
<DocsAside type="info">
When exporting the <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> or any of its child grids down the hierarchy, the exported data will be a flat collection of rows
