...
The sample below demonstrates how <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> works with nested Cascading <ApiLink type="Combo" /> components.
...
In order enable column editing, make sure <ApiLink pkg="grids" type="{ComponentName}" member="editable" prefixed={false} /> property is set to `true`.

Once the column editing is enabled, you can start by adding your <ApiLink type="Combo" />. Please note that here in order to have only one single selection available, you will need to use set the <ApiLink pkg="grids" type="{ComponentName}" member="singleSelect" prefixed={false} /> property.
...
- <ApiLink pkg="grids" type="{ComponentName}" member="displayKey" prefixed={false} /> - Required for object arrays - Specifies which property will be used for the items' text. If no value is specified for <ApiLink pkg="grids" type="{ComponentName}" member="displayKey" prefixed={false} />, the  combo will use the specified <ApiLink pkg="grids" type="{ComponentName}" member="valueKey" prefixed={false} /> (if any).
...
<ApiRef pkg="grids" types={["Column","Combo","LinearProgress"]} />
