...
Then define a `{ComponentName}` with bound data source, <ApiLink pkg="grids" type="{ComponentName}" member="rowEditable" prefixed={false} /> set to true and an <ApiLink pkg="grids" type="ActionStrip" /> component with editing actions enabled. The `AddRow` input controls the visibility of the button that spawns the row adding UI.
...
The internal <ApiLink pkg="grids" type="BaseTransactionService" /> is automatically provided for `{ComponentName}`. It holds pending cell changes until the row state is submitted or cancelled.
