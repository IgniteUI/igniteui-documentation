...
The {ProductName} Size feature in {Platform} {ComponentTitle} allows users to control the spacing and layout of data within the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} />. By changing `--ig-size`, you can significantly improve the user experience when interacting with large amounts of content.  They can choose from three size options:
- `--ig-size-large`
- `--ig-size-medium`
- `--ig-size-small`
...
As you can see in the demo above, the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> provides three size options:  **small**, **medium** and **large**. The code snippet below shows how to set `--ig-size` either inline or part of a CSS class:
...
And now let's see in details how each option reflects on the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> component. When you switch between different size options the height of each <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> element and the corresponding paddings will be changed. Also if you want to apply custom column <ApiLink type="{ComponentName}" member="width" pkg="grids" prefixed={false} />, please consider the fact that it must be bigger than the sum of left and right padding:

- **large** - this is the default <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> size with the lowest intense and row height equal to `50px`. Left and Right paddings are `24px`; Minimal column <ApiLink type="{ComponentName}" member="width" pkg="grids" prefixed={false} /> is `80px`;
- **medium** - this is the middle intense size with `40px` row height. Left and Right paddings are `16px`; Minimal column <ApiLink type="{ComponentName}" member="width" pkg="grids" prefixed={false} /> is `64px`;
- **small** - this is the size with highest intense and `32px` row height. Left and Right paddings are `12px`; Minimal column <ApiLink type="{ComponentName}" member="width" pkg="grids" prefixed={false} /> is `56px`;
...
Another option that <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> provides for you, in order to be able to change the height of the rows in the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} />, is the property <ApiLink type="{ComponentName}" member="rowHeight" pkg="grids" prefixed={false} />. So let's see in action how this property affects the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> layout along with the `--ig-size`.

Please keep in mind the following:

- `--ig-size` CSS variable will have no impact on row height **if there is <ApiLink type="{ComponentName}" member="rowHeight" pkg="grids" prefixed={false} /> specified**.
- `--ig-size` will **affect all of the rest elements in the {ComponentTitle}**, as it has been described above.

We can now extend our sample and add <ApiLink type="{ComponentName}" member="rowHeight" pkg="grids" prefixed={false} /> property to the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} />:
