...
<PlatformBlock for="WebComponents">
Now let's create our search input!  By getting the input element we can get its current value. This will allow us to use the `{ComponentName}`'s `FindNext` and `FindPrev` methods to highlight all the occurrences of the <ApiLink pkg="grids" type="{ComponentName}" member="findNext" prefixed={false} /> and scroll to the next/previous one (depending on which method we have invoked).
</PlatformBlock>

<PlatformBlock for="Angular">
Now let's create our search input! By binding our <ApiLink pkg="grids" type="{ComponentName}" member="findNext" prefixed={false} /> as ngModel to our newly created input and subscribe to the ngModelChange event, we can detect every single <ApiLink pkg="grids" type="{ComponentName}" member="findNext" prefixed={false} /> modification by the user. This will allow us to use the `{ComponentName}`'s `FindNext` and `FindPrev` methods to highlight all the occurrences of the <ApiLink pkg="grids" type="{ComponentName}" member="findNext" prefixed={false} /> and scroll to the next/previous one (depending on which method we have invoked).
</PlatformBlock>

<PlatformBlock for="Blazor">
Now let's create our search input! By binding our <ApiLink pkg="grids" type="{ComponentName}" member="findNext" prefixed={false} /> to the `Value` property to our newly created input and subscribe to the `ValueChanging` event, we can detect every single <ApiLink pkg="grids" type="{ComponentName}" member="findNext" prefixed={false} /> modification by the user. This will allow us to use the `{ComponentName}`'s `FindNext` and `FindPrev` methods to highlight all the occurrences of the <ApiLink pkg="grids" type="{ComponentName}" member="findNext" prefixed={false} /> and scroll to the next/previous one (depending on which method we have invoked).
</PlatformBlock>
...
What if we would like to filter and sort our `{ComponentName}` or even to add and remove records? After such operations, the highlights of our current search automatically update and persist over any text that matches the <ApiLink pkg="grids" type="{ComponentName}" member="findNext" prefixed={false} />! Furthermore, the search will work with paging and will persist the highlights through changes of the `{ComponentName}`'s `PerPage` property.
...
<PlatformBlock for="Angular">
We will wrap all of our components inside an [InputGroup](../input-group.md). On the left we will toggle between a search and a delete/clear icon (depending on whether the search input is empty or not). In the center, we will position the input itself. In addition, whenever the delete icon is clicked, we will update our <ApiLink pkg="grids" type="{ComponentName}" member="findNext" prefixed={false} /> and invoke the `{ComponentName}`'s `ClearSearch` method to clear the highlights.
