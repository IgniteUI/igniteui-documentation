...
- (optional) <ApiLink pkg="grids" type="{ComponentName}" member="caseSensitive" prefixed={false} />: **boolean** (should the search be case sensitive or not, default value is false)
- (optional) <ApiLink pkg="grids" type="{ComponentName}" member="exactMatch" prefixed={false} />: **boolean** (should the search be by an exact match or not, default value is false)
...
<PlatformBlock for="Angular">
Now let's allow the user to choose whether the search should be case sensitive and/or by an exact match. For this purpose we can use simple checkbox inputs by binding our <ApiLink pkg="grids" type="{ComponentName}" member="caseSensitive" prefixed={false} /> and <ApiLink pkg="grids" type="{ComponentName}" member="exactMatch" prefixed={false} /> properties to the inputs' `Checked` properties respectively and handle their `Change` events by toggling our properties and invoking the `FindNext` method.
...
- For displaying a couple of chips that toggle the <ApiLink pkg="grids" type="{ComponentName}" member="caseSensitive" prefixed={false} /> and the <ApiLink pkg="grids" type="{ComponentName}" member="exactMatch" prefixed={false} /> properties. We have replaced the checkboxes with two stylish chips. Whenever a chip is clicked, we invoke its respective handler.
