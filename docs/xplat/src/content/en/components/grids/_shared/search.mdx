...
The {ProductName} Search Filter feature in {Platform} {ComponentTitle} enables the process of finding values in the collection of data. We make it easier to set up this functionality and it can be implemented with a search input box, buttons, keyboard navigation and other useful features for an even better user experience. While browsers natively provide content search functionality, most of the time the <ApiLink type="{ComponentName}" pkg="grids" /> virtualizes its columns and rows that are out of view. In these cases, the native browser search is unable to search data in the virtualized cells, since they are not part of the DOM. We have extended the {Platform} Material table-based grid with a **search API** that allows you to search through the **virtualized content** of the <ApiLink type="{ComponentName}" pkg="grids" />.

## {Platform} Search Example

The following example represents <ApiLink type="{ComponentName}" pkg="grids" /> with search input box that allows searching in all columns and rows, as well as specific filtering options for each column.
...
Great, and now let's prepare for the search API of our <ApiLink type="{ComponentName}" pkg="grids" />! We can create a few properties, which can be used for storing the currently searched text and whether the search is case sensitive and/or by an exact match.
...
<PlatformBlock for="WebComponents">
Now let's create our search input!  By getting the input element we can get its current value. This will allow us to use the <ApiLink type="{ComponentName}" pkg="grids" />'s `FindNext` and `FindPrev` methods to highlight all the occurrences of the <ApiLink type="{ComponentName}" member="findNext" pkg="grids" /> and scroll to the next/previous one (depending on which method we have invoked).
</PlatformBlock>

<PlatformBlock for="Angular">
Now let's create our search input! By binding our <ApiLink type="{ComponentName}" member="findNext" pkg="grids" /> as ngModel to our newly created input and subscribe to the ngModelChange event, we can detect every single <ApiLink type="{ComponentName}" member="findNext" pkg="grids" /> modification by the user. This will allow us to use the <ApiLink type="{ComponentName}" pkg="grids" />'s `FindNext` and `FindPrev` methods to highlight all the occurrences of the <ApiLink type="{ComponentName}" member="findNext" pkg="grids" /> and scroll to the next/previous one (depending on which method we have invoked).
</PlatformBlock>

<PlatformBlock for="Blazor">
Now let's create our search input! By binding our <ApiLink type="{ComponentName}" member="findNext" pkg="grids" /> to the `Value` property to our newly created input and subscribe to the `ValueChanging` event, we can detect every single <ApiLink type="{ComponentName}" member="findNext" pkg="grids" /> modification by the user. This will allow us to use the <ApiLink type="{ComponentName}" pkg="grids" />'s `FindNext` and `FindPrev` methods to highlight all the occurrences of the <ApiLink type="{ComponentName}" member="findNext" pkg="grids" /> and scroll to the next/previous one (depending on which method we have invoked).
</PlatformBlock>

<PlatformBlock for="React">
Now let's create our search input! By binding our `searchText` to the `value` property to our newly created input and subscribe to the `inputOccured` event, we can detect every single `searchText` modification by the user. This will allow us to use the <ApiLink type="{ComponentName}" pkg="grids" />'s `findNext` and `findPrev` methods to highlight all the occurrences of the `searchText` and scroll to the next/previous one (depending on which method we have invoked).
</PlatformBlock>

Both the `FindNext` and the `FindPrev` methods have three arguments:

- `Text`: **string** (the text we are searching for)
- (optional) <PlatformBlock for="Angular"><ApiLink type="ISearchInfo" member="caseSensitive" pkg="grids" kind="interface" /></PlatformBlock><PlatformBlock for="React,WebComponents"><ApiLink type="SearchInfo" member="caseSensitive" /></PlatformBlock><PlatformBlock for="Blazor"><ApiLink type="SearchInfo" member="caseSensitive" /></PlatformBlock>: **boolean** (should the search be case sensitive or not, default value is false)
- (optional) <PlatformBlock for="Angular"><ApiLink type="ISearchInfo" member="exactMatch" pkg="grids" kind="interface" /></PlatformBlock><PlatformBlock for="React,WebComponents"><ApiLink type="SearchInfo" member="exactMatch" /></PlatformBlock><PlatformBlock for="Blazor"><ApiLink type="SearchInfo" member="exactMatch" /></PlatformBlock>: **boolean** (should the search be by an exact match or not, default value is false)

When searching by an exact match, the search API will highlight as results only the cell values that match entirely the <ApiLink type="{ComponentName}" member="findNext" pkg="grids" /> by taking the case sensitivity into account as well. For example the strings '_software_' and '_Software_' are an exact match with a disregard for the case sensitivity.

The methods from above return a **number** value (the number of times the <ApiLink type="{ComponentName}" pkg="grids" /> contains the given string).
...
Let's also display the position of the current occurrence, along with the total results count! We can do this by using the grid's <ApiLink type="{ComponentName}" member="lastSearchInfo" pkg="grids" /> property. This property is automatically updated when using the **find** methods.
...
<PlatformBlock for="Angular">
Now let's allow the user to choose whether the search should be case sensitive and/or by an exact match. For this purpose we can use simple checkbox inputs by binding our <ApiLink type="ISearchInfo" member="caseSensitive" pkg="grids" kind="interface" /> and <ApiLink type="ISearchInfo" member="exactMatch" pkg="grids" kind="interface" /> properties to the inputs' `Checked` properties respectively and handle their `Change` events by toggling our properties and invoking the `FindNext` method.
...
What if we would like to filter and sort our <ApiLink type="{ComponentName}" pkg="grids" /> or even to add and remove records? After such operations, the highlights of our current search automatically update and persist over any text that matches the <ApiLink type="{ComponentName}" member="findNext" pkg="grids" />! Furthermore, the search will work with paging and will persist the highlights through changes of the <ApiLink type="{ComponentName}" pkg="grids" />'s `PerPage` property.
...
<PlatformBlock for="Angular">
We will wrap all of our components inside an [InputGroup](../input-group.md). On the left we will toggle between a search and a delete/clear icon (depending on whether the search input is empty or not). In the center, we will position the input itself. In addition, whenever the delete icon is clicked, we will update our <ApiLink type="{ComponentName}" member="findNext" pkg="grids" /> and invoke the <ApiLink type="{ComponentName}" pkg="grids" />'s `ClearSearch` method to clear the highlights.
...
- For displaying a couple of chips that toggle the <PlatformBlock for="Angular"><ApiLink type="ISearchInfo" member="caseSensitive" pkg="grids" kind="interface" /></PlatformBlock><PlatformBlock for="React,WebComponents"><ApiLink type="SearchInfo" member="caseSensitive" /></PlatformBlock><PlatformBlock for="Blazor"><ApiLink type="SearchInfo" member="caseSensitive" /></PlatformBlock> and the <PlatformBlock for="Angular"><ApiLink type="ISearchInfo" member="exactMatch" pkg="grids" kind="interface" /></PlatformBlock><PlatformBlock for="React,WebComponents"><ApiLink type="SearchInfo" member="exactMatch" /></PlatformBlock><PlatformBlock for="Blazor"><ApiLink type="SearchInfo" member="exactMatch" /></PlatformBlock> properties. We have replaced the checkboxes with two stylish chips. Whenever a chip is clicked, we invoke its respective handler.
