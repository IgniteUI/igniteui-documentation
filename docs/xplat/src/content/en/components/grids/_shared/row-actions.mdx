...
- <ApiLink pkg="grids" type="GridEditingActions" /> - includes functionality and UI specifically designed for the `{ComponentName}` editing. It allows you to quickly toggle edit mode for cells or rows, depending on the <ApiLink pkg="grids" type="{ComponentName}" member="rowEditable" prefixed={false} /> option and row deletion of the `{ComponentName}`.

- <ApiLink pkg="grids" type="GridPinningActions" /> - includes functionality and UI specifically designed for the `{ComponentName}` row pinning. It allows you to quickly pin rows and navigate between pinned rows and their disabled counterparts.

They are added inside the `{ComponentName}` and this is all needed to have an <ApiLink pkg="grids" type="ActionStrip" /> providing default interactions.
