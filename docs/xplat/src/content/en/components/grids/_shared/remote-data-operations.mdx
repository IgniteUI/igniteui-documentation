...
The developer can manually generate the necessary unique column values based on the information, that is provided by the `Column` and the <ApiLink pkg="grids" type="FilteringExpressionsTree" /> arguments and then invoke the `Done` callback.
...
When we define a custom paginator content we need to define the content in a way to get the data only for the requested page and to pass the correct **skip** and **top** parameters to the remote service according to the selected page and items `PerPage`. We are going to use the <ApiLink pkg="grids" type="Paginator" /> in order to ease our example configuration, along with the `PageSizeSelectorComponent` and `PageNavigationComponent` that were introduced - `PageSize` will add the per page dropdown and label and `PageNav` will add the navigation action buttons and labels.
