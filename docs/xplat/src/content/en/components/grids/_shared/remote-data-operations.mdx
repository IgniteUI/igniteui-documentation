...
llms:
  description: "The {ProductName} Remote Data Operations feature in {Platform} {ComponentTitle} supports operations such as remote virtualization, remote sorting, remote filtering and others."
apiTerms: none
---
...
By default, the <ApiLink type="{ComponentName}" /> uses its own logic for performing data operations.

You can perform these tasks remotely and feed the resulting data to the <ApiLink type="{ComponentName}" /> by taking advantage of certain inputs and events, which are exposed by the <ApiLink type="{ComponentName}" />.

<PlatformBlock for="Angular, Blazor, React, WebComponents">
<ComponentBlock for="Grid">
## Remote Virtualization

The <ApiLink type="{ComponentName}" /> supports the scenario in which the data chunks are requested from a remote service, exposing the behavior implemented in the `ForOf` directive it uses internally.

To utilize this feature, you need to subscribe to the <ApiLink type="{ComponentName}" member="dataPreLoad" /> output so that you make the appropriate request based on the arguments received, as well as set the public <ApiLink type="{ComponentName}" /> property <ApiLink type="{ComponentName}" member="totalItemCount" /> with the respective information coming from the service.
...
<DocsAside type="info">
The first <ApiLink type="{ComponentName}" member="chunkSize" /> will always be 0 and should be determined by you based on the specific application scenario.
</DocsAside>
...
The first thing to do is fetch the first chunk of the data. Setting the <ApiLink type="{ComponentName}" member="totalItemCount" /> property is important, as it allows the grid to size its scrollbar correctly.
...
To provide remote sorting and filtering, you need to subscribe to the <ApiLink type="{ComponentName}" member="dataPreLoad" />, <ApiLink type="{ComponentName}" member="sortingExpressionsChange" /> and <ApiLink type="{ComponentName}" member="filteringExpressionsTreeChange" /> outputs, so that you make the appropriate request based on the arguments received, as well as set the public <ApiLink type="{ComponentName}" /> property <ApiLink type="{ComponentName}" member="totalItemCount" /> with the respective information coming from the service.
...
When remote sorting and filtering are provided, usually we do not need the built-in sorting and filtering of the grid. We can disable them by setting the <ApiLink type="{ComponentName}" member="sortStrategy" /> and the <ApiLink type="{ComponentName}" member="filterStrategy" /> inputs of the grid to the <ApiLink type="NoopSortingStrategy" /> and the <ApiLink type="NoopFilteringStrategy" /> respective instances.
...
To provide remote filtering, you need to subscribe to the <ApiLink type="{ComponentName}" member="filteringExpressionsTreeChange" /> output so that you make the appropriate request based on the arguments received. Let's use a flat collection as a data source for our Tree Grid by providing a <ApiLink type="{ComponentName}" member="primaryKey" /> and a `ForeignKey`.
...
When remote filtering is provided, usually we do not need the built-in filtering of the Tree Grid. We can disable it by setting the <ApiLink type="{ComponentName}" member="filterStrategy" /> input of the Tree Grid to the <ApiLink type="NoopFilteringStrategy" /> instance.
...
The list items inside the Excel Style Filtering dialog represent the unique values for the respective column. The <ApiLink type="{ComponentName}" /> generates these values based on its data source by default. In case of remote filtering, the grid data does not contain all the data from the server. In order to provide the unique values manually and load them on demand, we can take advantage of the <ApiLink type="{ComponentName}" />'s <ApiLink type="{ComponentName}" member="uniqueColumnValuesStrategy" label="UniqueColumnValuesStrategy" /> input. This input is actually a method that provides three arguments:
...
After declaring the service, we need to create a component, which will be responsible for the <ApiLink type="{ComponentName}" /> construction and data subscription.
...
With the examples so far we clarified how to set up the <ApiLink type="{ComponentName}" /> with remote data. Now, let's focus on enabling batch editing for the grid by following the [Batch Editing topic/guide](batch-editing.mdx).
