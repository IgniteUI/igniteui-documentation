...
- By setting the `Column` input <ApiLink pkg="grids" type="Column" member="cellClasses" /> to an object literal containing key-value pairs. The key is the name of the CSS class, while the value is either a callback function that returns a boolean, or boolean value. The result is a convenient material styling of the cell.

### Using Cell Classes
You can conditionally style the <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> cells by setting the `Column` <ApiLink pkg="grids" type="Column" member="cellClasses" /> input and define custom rules.
...
- By using the <ApiLink pkg="grids" type="ColumnComponent" /> input <ApiLink pkg="grids" type="ColumnComponent" member="cellStyles" />` which accepts an object literal where the keys are style properties and the values are expressions for evaluation.
...
<PlatformBlock for="Angular">
On `ngOnInit` we will add the `CellStyles` configuration for each column of the predefined `Columns` collection, which is used to create the <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> columns dynamically.
