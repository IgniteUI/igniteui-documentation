...
In order to define a column group as collapsible, you need to set the <ApiLink pkg="grids" type="{ComponentName}" member="collapsible" prefixed={false} /> property on the <ApiLink pkg="grids" type="ColumnGroup" /> to **true**.

You need to define the property <ApiLink pkg="grids" type="{ComponentName}" member="visibleWhenCollapsed" prefixed={false} /> to at least two child columns. At least one column must be visible when the group is collapsed (<ApiLink pkg="grids" type="{ComponentName}" member="visibleWhenCollapsed" prefixed={false} /> set to **true**) and at least one column must be hidden when the group is expanded (<ApiLink pkg="grids" type="{ComponentName}" member="visibleWhenCollapsed" prefixed={false} /> set to `false`), otherwise the **collapsible functionality will be disabled**. If <ApiLink pkg="grids" type="{ComponentName}" member="visibleWhenCollapsed" prefixed={false} /> is not specified for some of the child columns, then this column will be always visible regardless of whether the parent state is expanded or collapsed.
...
The initial state of the column group which is specified as collapsible is <ApiLink pkg="grids" type="{ComponentName}" member="expanded" prefixed={false} /> set to **true**, but you can easily change this behavior by setting it to **false**.
...
You can define custom expand/collapse template and provide it to each of the collapsible column groups using <ApiLink pkg="grids" type="{ComponentName}" member="collapsibleIndicatorTemplate" prefixed={false} /> property. Check the markup below:
