...
> When single cell is selected <ApiLink pkg="grids" type="{ComponentName}" member="selected" prefixed={false} /> event is emitted, no matter if the **selection mode** is **single** or **multiple**. In multi-cell selection mode when you select a range of cells <ApiLink pkg="grids" type="{ComponentName}" member="rangeSelected" prefixed={false} /> event is emitted.
