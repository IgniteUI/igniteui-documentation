...
> [!Note]
> <ApiLink pkg="grids" type="{ComponentName}" member="selectedCells" prefixed={false} /> will return the correct result even if the cell is not visible in grids view port. <ApiLink pkg="grids" type="{ComponentName}" member="getSelectedData" prefixed={false} /> will also return the selected cell data.
> <ApiLink pkg="grids" type="{ComponentName}" member="getSelectedRanges" prefixed={false} /> will return the current selected ranges in the grid from both keyboard and pointer interactions. The type is **GridSelectionRange[]**.
</PlatformBlock>
