...
Column pinning is controlled through the <ApiLink pkg="grids" type="{ComponentName}" member="pinned" prefixed={false} /> property of the `Column`. Pinned columns are rendered on the left side of the `{ComponentName}` by default and stay fixed through horizontal scrolling of the unpinned columns in the `{ComponentName}` body.
