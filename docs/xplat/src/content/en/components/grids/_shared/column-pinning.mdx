...
This example demonstrates how you can pin a column or multiple columns to the left or right side of the <ApiLink type="{ComponentName}" pkg="grids" />.
...
You may also use the <ApiLink type="{ComponentName}" member="pinColumn" pkg="grids" /> or <ApiLink type="{ComponentName}" member="unpinColumn" pkg="grids" /> methods of the <ApiLink type="{ComponentName}" pkg="grids" /> to pin or unpin columns by their field name:
...
<PlatformBlock for="Angular, WebComponents">
A column is pinned to the right of the rightmost pinned column. Changing the order of the pinned columns can be done by subscribing to the <ApiLink type="{ComponentName}" member="columnPin" pkg="grids" /> event and changing the <ApiLink type="PinColumnEventArgs" member="insertAtIndex" /> property of the event arguments to the desired position index.

</PlatformBlock>

<PlatformBlock for="React">
A column is pinned to the right of the rightmost pinned column. Changing the order of the pinned columns can be done by subscribing to the <ApiLink type="{ComponentName}" member="columnPin" pkg="grids" /> event and changing the <ApiLink type="PinColumnEventArgs" label="insertAtIndex" /> property of the event arguments to the desired position index.
...
- Setting column widths in percentage (%) explicitly makes the <ApiLink type="{ComponentName}" pkg="grids" /> body and header content to be misaligned when there are pinned columns. For column pinning to function correctly the column widths should be in pixels (px) or auto-assigned by the <ApiLink type="{ComponentName}" pkg="grids" />.

<PlatformBlock for="Angular">
<ComponentBlock for="Grid">
## Styling

The <ApiLink type="{ComponentName}" pkg="grids" /> allows styling through the [{ProductName} Theme Library](../themes/styles.md). The grid's <ApiLink type="function-grid-theme" label="theme" kind="sass" module="index.html" /> exposes a wide variety of properties, which allow the customization of all the features of the grid.
