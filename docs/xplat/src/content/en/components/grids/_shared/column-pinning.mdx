...
<PlatformBlock for="Blazor">
A column is pinned to the right of the rightmost pinned column. Changing the order of the pinned columns can be done by subscribing to the `ColumnPinScript` event and providing a JavaScript function for changing the <ApiLink pkg="grids" kind="interface" type="PinColumnEventArgs" member="insertAtIndex" /> property of the event arguments to the desired position index.
