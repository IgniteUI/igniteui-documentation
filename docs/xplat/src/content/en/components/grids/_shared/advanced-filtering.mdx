...
The advanced filtering generates a <ApiLink pkg="grids" type="FilteringExpressionsTree" /> which is stored in the `AdvancedFilteringExpressionsTree` input property. You could use the `AdvancedFilteringExpressionsTree` property to set an initial state of the advanced filtering.
