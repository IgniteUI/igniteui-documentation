...
You can export <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> with defined [multi-column headers](multi-column-headers.md). All headers are reflected in the exported Excel file as they are displayed in the <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} />. If you want to exclude the defined multi-column headers from the exported data, set the `ExporterOption` <ApiLink pkg="grids" type="{ComponentName}" member="ignoreMultiColumnHeaders" prefixed={false} /> to `true`.
