...
The appearance of the date portions will be set (e.g. day, month, year) based on <ApiLink pkg="inputs" type="DatePicker" member="locale" label="Locale" /> format or <ApiLink pkg="grids" type="Column" member="pipeArgs" /> input. The pipe arguments can be used to specify a custom date format or timezone:
