...
{ProductName} `{ComponentName}` provides an <ApiLink pkg="grids" type="ColumnActionsComponent" /> with an `ColumnHidingDirective` which allows users to perform column hiding directly through the user interface or by using the {Platform} component.
</PlatformBlock>
...
For this purpose all we have to do is set both the <ApiLink pkg="grids" type="GridToolbarActions" /> and the <ApiLink pkg="grids" type="GridToolbarHiding" /> inside of the `{ComponentName}`.
