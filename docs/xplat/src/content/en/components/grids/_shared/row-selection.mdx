...
## Setup
In order to setup row selection in the <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} />, you just need to set the <ApiLink pkg="grids" type="{ComponentName}" member="rowSelection" prefixed={false} /> property. This property accepts <ApiLink pkg="grids" type="GridSelectionMode" kind="enum" prefixed={false} /> enumeration.

<ApiLink pkg="grids" type="GridSelectionMode" kind="enum" prefixed={false} /> exposes the following modes:
...
The <ApiLink pkg="grids" type="{ComponentName}" member="selected" prefixed={false} /> property shows whether the current row is selected or not while the `Index` property can be used to access the row index.
