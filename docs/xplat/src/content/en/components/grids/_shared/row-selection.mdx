...
## Setup
In order to setup row selection in the `{ComponentName}`, you just need to set the <ApiLink pkg="grids" type="{ComponentName}" member="rowSelection" prefixed={false} /> property. This property accepts <ApiLink pkg="grids" type="GridSelectionMode" kind="enum" prefixed={false} /> enumeration.
...
In the `{ComponentName}` by default row selection is disabled (<ApiLink pkg="grids" type="{ComponentName}" member="rowSelection" prefixed={false} /> is None). So you can **not** select or deselect a row through interaction with the `{ComponentName}` UI, the only way to complete these actions is to use the provided API methods.
...
<ComponentBlock for="Grid, HierarchicalGrid">
<PlatformBlock for="Angular">
- In order to have proper row selection and cell selection, while `{ComponentName}` has remote virtualization, a <ApiLink pkg="grids" type="{ComponentName}" member="primaryKey" prefixed={false} /> should be provided.
