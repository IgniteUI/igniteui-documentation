...
## Setup
In order to setup row selection in the `{ComponentName}`, you just need to set the <ApiLink pkg="grids" type="{ComponentName}" member="rowSelection" prefixed={false} /> property. This property accepts <ApiLink pkg="grids" type="GridSelectionMode" /> enumeration.

<ApiLink pkg="grids" type="GridSelectionMode" /> exposes the following modes:
