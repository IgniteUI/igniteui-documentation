...
Sorting, pinning and hiding features can be removed from the filter menu using the corresponding inputs: <ApiLink pkg="grids" type="{ComponentName}" member="sortable" prefixed={false} />, <ApiLink pkg="grids" type="{ComponentName}" member="selected" prefixed={false} />, <ApiLink pkg="grids" type="{ComponentName}" member="disablePinning" prefixed={false} />, <ApiLink pkg="grids" type="{ComponentName}" member="disableHiding" prefixed={false} />.
...
In the sample below **Product Name** and **Discontinued** columns have all four features enabled, **Quantity Per Unit** have all three disabled, **Unit Price** has only sorting and **Order Date** has only pinning and hiding and all are <ApiLink pkg="grids" type="{ComponentName}" member="selectable" prefixed={false} />.
