...
The {ProductName} <ApiLink type="{ComponentName}" /> component provides three different selection modes - [Row selection](row-selection.md), [Cell selection](cell-selection.md) and [Column selection](column-selection.md). By default only **Multi-cell selection** mode is enabled in the <ApiLink type="{ComponentName}" />. In order to change/enable selection mode you can use <ApiLink type="{ComponentName}" member="rowSelection" />, <ApiLink type="{ComponentName}" member="cellSelection" /> or <ApiLink type="Column" member="selectable" /> properties.
