...
With the <ApiLink type="{ComponentName}" member="rowSelection" pkg="grids" prefixed={false} /> property, you can specify:
...
The sample below demonstrates three types of **cell selection** behaviors in the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} />. Use the buttons below to enable each of the available selection modes.
...
The {ProductName} <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> component provides three different selection modes - [Row selection](row-selection.md), [Cell selection](cell-selection.md) and [Column selection](column-selection.md). By default only **Multi-cell selection** mode is enabled in the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} />. In order to change/enable selection mode you can use <ApiLink type="{ComponentName}" member="rowSelection" pkg="grids" prefixed={false} />, <ApiLink type="{ComponentName}" member="cellSelection" pkg="grids" prefixed={false} /> or <ApiLink type="Column" member="selectable" /> properties.

</ComponentBlock>

### {Platform} {ComponentTitle} Row Selection

Property <ApiLink type="{ComponentName}" member="rowSelection" pkg="grids" prefixed={false} /> enables you to specify the following options:

- `None` - Row selection would be disabled for the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} />.
- `Single` - Selection of only one row within the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> would be available.
...
Property <ApiLink type="{ComponentName}" member="cellSelection" pkg="grids" prefixed={false} /> enables you to specify the following options:

- `None` - Cell selection would be disabled for the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} />.
- `Single` - Selection of only one cell within the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> would be available.
- `Multiple` - Currently, this is the default state of the selection in the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} />. Multi-cell selection is available by mouse dragging over the cells, after a left button mouse clicked continuously.
...
The <ApiLink type="Column" member="selectable" /> property enables you to specify the following options for each <ApiLink type="Column" />. The corresponding column selection will be enabled or disabled if this property is set to true or false, respectively.
...
Using the <ApiLink type="{ComponentName}" member="contextMenu" pkg="grids" prefixed={false} /> event you can add a custom context menu to facilitate your work with <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} />. With a **right click** on the grid's body, the event emits the cell on which it is triggered. The **context menu** will operate with the emitted cell.
...
The <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> will fetch the copied data and will paste it in a container element.
...
<PlatformBlock for="Angular">
- Using the <ApiLink type="{ComponentName}" pkg="grids" prefixed={false} /> with Selection enabled on IE11 requires the explicit import of the array polyfill in polyfill.ts of the {Platform} application. IE11 is no longer supported as of version 13.0.0.
