...
The {ProductName} `{ComponentName}` component provides three different selection modes - [Row selection](row-selection.md), [Cell selection](cell-selection.md) and [Column selection](column-selection.md). By default only **Multi-cell selection** mode is enabled in the `{ComponentName}`. In order to change/enable selection mode you can use <ApiLink pkg="grids" type="{ComponentName}" member="rowSelection" prefixed={false} />, <ApiLink pkg="grids" type="{ComponentName}" member="cellSelection" prefixed={false} /> or <ApiLink pkg="grids" type="{ComponentName}" member="selectable" prefixed={false} /> properties.
...
Using the <ApiLink pkg="grids" type="{ComponentName}" member="contextMenu" prefixed={false} /> event you can add a custom context menu to facilitate your work with `{ComponentName}`. With a **right click** on the grid's body, the event emits the cell on which it is triggered. The **context menu** will operate with the emitted cell.
