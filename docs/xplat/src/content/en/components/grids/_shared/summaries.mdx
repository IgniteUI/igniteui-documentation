...
> [!Note]
> In order to calculate the summary row height properly, the {ComponentTitle} needs the <ApiLink pkg="grids" type="SummaryOperand" member="operate" label="Operate" prefixed={false} suffix={false} /> method to always return an array of <ApiLink pkg="grids" type="SummaryResult" kind="interface" /> with the proper length even when the data is empty.
