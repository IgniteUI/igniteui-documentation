...
By default, summary results, produced by the built-in summary operands, are localized and formatted according to the grid `Locale` and column <ApiLink pkg="grids" type="Column" member="pipeArgs" />. When using custom operands, the `Locale` and <ApiLink pkg="grids" type="Column" member="pipeArgs" /> are not applied. If you want to change the default appearance of the summary results, you may format them using the <ApiLink pkg="grids" type="Column" member="summaryFormatter" label="SummaryFormatter" /> property.
