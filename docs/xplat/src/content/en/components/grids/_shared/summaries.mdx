...
The other way to enable/disable summaries for a specific column or a list of columns is to use the public method <ApiLink pkg="grids" type="{ComponentName}" member="enableSummaries" prefixed={false} />/<ApiLink pkg="grids" type="{ComponentName}" member="disableSummaries" prefixed={false} /> of the `{ComponentName}`.
