...
**Column resizing** is also enabled per-column level, meaning that the `{ComponentName}` can have a mix of resizable and non-resizable columns. This is done via the <ApiLink pkg="grids" type="{ComponentName}" member="resizable" prefixed={false} /> input of the `Column`.
...
You can subscribe to the <ApiLink pkg="grids" type="{ComponentName}" member="columnResized" prefixed={false} /> event of the `{ComponentName}` to implement some custom logic when a column is resized. Both, previous and new column widths, as well as the `Column` object, are exposed through the event arguments.
