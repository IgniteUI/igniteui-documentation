...
llms:
  description: "The {ProductName} Column Resizing feature in {Platform} {ComponentTitle} allows users to easily adjust the width of the columns of the {ComponentName}."
---
...
**Column resizing** is also enabled per-column level, meaning that the <ApiLink type="{ComponentName}" /> can have a mix of resizable and non-resizable columns. This is done via the <ApiLink type="Column" member="resizable" /> input of the <ApiLink type="Column" />.
...
You can subscribe to the <PlatformBlock for="Angular,WebComponents"><ApiLink type="GridComponentEventMap" member="columnResized" /></PlatformBlock><PlatformBlock for="React"><ApiLink type="Grid" member="onColumnResized" /></PlatformBlock><PlatformBlock for="Blazor"><ApiLink type="Grid" member="columnResized" label="columnResized" /></PlatformBlock> event of the <ApiLink type="{ComponentName}" /> to implement some custom logic when a column is resized. Both, previous and new column widths, as well as the <ApiLink type="Column" /> object, are exposed through the event arguments.
...
Each column can be set to auto-size on initialization by setting <ApiLink type="{ComponentName}" member="width" /> to 'auto':
...
To get started with the styling of the <ApiLink type="{ComponentName}" /> column resize line, we need to import the index file, where all the theme functions and component mixins live:
