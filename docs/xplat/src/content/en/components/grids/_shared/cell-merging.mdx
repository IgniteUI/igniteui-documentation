...
The grid exposes a <ApiLink pkg="grids" type="{ComponentName}" member="cellMergeMode" prefixed={false} /> property that accepts values from the <ApiLink pkg="grids" type="GridCellMergeMode" kind="type" prefixed={false} /> enum:
...
A custom merge strategy must implement the <ApiLink pkg="grids" type="GridMergeStrategy" /> class:
...
If you only want to customize part of the behavior (for example, the comparer logic), you can extend the built-in <ApiLink pkg="grids" type="DefaultMergeStrategy" /> and override the relevant methods.
