...
The grid exposes a <ApiLink pkg="grids" type="{ComponentName}" member="cellMergeMode" prefixed={false} /> property that accepts values from the <ApiLink pkg="grids" type="GridCellMergeMode" kind="type" prefixed={false} exclude="Blazor" /><PlatformBlock for="Blazor"><ApiLink pkg="grids" type="GridCellMergeMode" kind="enum" prefixed={false} /></PlatformBlock> enum:
