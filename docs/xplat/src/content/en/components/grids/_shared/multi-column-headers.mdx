...
llms:
  description: "The {ProductName} Multi-Column Headers feature in {Platform} {ComponentTitle} allows you to group columns by placing them under a common multi-header."
---
...
The {ProductName} Multi-Column Headers feature in {Platform} {ComponentTitle} allows you to group columns by placing them under a common multi-header. Each multi-column headers group in the <ApiLink type="{ComponentName}" /> could be a representation of combinations between other groups or columns. This feature is particularly useful when dealing with large datasets where scrolling horizontally might be cumbersome.
...
The declaration of multi-column headers is achieved by wrapping a set of columns into an <ApiLink type="ColumnGroup" /> component with <ApiLink type="Column" member="header" label="Header" /> title information passed.
...
For achieving `n-th` level of nested headers, the declaration above should be followed. So by nesting <ApiLink type="ColumnGroup" /> leads to the desired result.
...
Every <ApiLink type="ColumnGroup" /> supports [moving](column-moving.mdx), [pinning](column-pinning.mdx) and [hiding](column-hiding.mdx).
<DocsAside type="info">
...
<PlatformBlock for="Blazor">
Each of the column groups of the grid can be templated separately. The column group expects `RenderFragment` for the <ApiLink type="Column" member="headerTemplate" label="HeaderTemplate" /> property.
...
<PlatformBlock for="WebComponents">
Each of the column groups of the grid can be templated separately. The following code snippet demonstrates how to use the <ApiLink type="ColumnGroup" member="headerTemplate" label="HeaderTemplate" /> of a column group:
...
Following the simplest approach, we create a new theme that extends the <ApiLink type="function-igx-grid-theme" label="igx-grid-theme" kind="sass" module="index.html" /> and accepts the `$header-background`, `$header-text-color`, `$header-border-width`, `$header-border-style` and `$header-border-color` parameters.
