...
The {Platform} {ComponentTitle} Column Selection feature in {ProductName} offers a simplified and Excel-like way to select and highlight an entire column with a single click. It can be enabled through the <ApiLink type="{ComponentName}" member="columnSelection" pkg="grids" prefixed={false} /> input. Thanks to the rich API, the feature allows for easy manipulation of the selection state, data extraction from the selected fractions, data analysis operations, and visualizations.
...
The column selection feature can be enabled through the <ApiLink type="{ComponentName}" member="columnSelection" pkg="grids" prefixed={false} /> input, which takes <ApiLink type="GridSelectionMode" /> values.
...
<DocsAside type="info">
The [Multi Column Headers](multi-column-headers.md) feature does not reflect on the <ApiLink type="{ComponentName}" member="selectable" pkg="grids" prefixed={false} /> input. The <ApiLink type="ColumnGroupComponent" /> is <ApiLink type="{ComponentName}" member="selectable" pkg="grids" prefixed={false} />, if at least one of its children has the selection behavior enabled. In addition, the component is marked as <ApiLink type="{ComponentName}" member="selected" pkg="grids" prefixed={false} /> if all of its <ApiLink type="{ComponentName}" member="selectable" pkg="grids" prefixed={false} /> descendants are <ApiLink type="{ComponentName}" member="selected" pkg="grids" prefixed={false} />.
</DocsAside>
...
<DocsAside type="info">
The keyboard combinations are available only when the grid <ApiLink type="{ComponentName}" member="columnSelection" pkg="grids" prefixed={false} /> input is set to `multiple`.
</DocsAside>
...
<DocsAside type="info">
The above statement also applies to the <ApiLink type="ColumnGroupComponent" />, except that when the <ApiLink type="{ComponentName}" member="selected" pkg="grids" prefixed={false} /> property is changed it changes the state of its descendants.
</DocsAside>
