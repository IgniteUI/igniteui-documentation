...
The column selection feature can be enabled through the <ApiLink pkg="grids" type="{ComponentName}" member="columnSelection" prefixed={false} /> input, which takes <ApiLink pkg="grids" kind="type" type="GridSelectionMode" prefixed={false} suffix={false} exclude="Blazor" /><PlatformBlock for="Blazor"><ApiLink pkg="grids" kind="enum" type="GridSelectionMode" prefixed={false} suffix={false}/></PlatformBlock> values.
