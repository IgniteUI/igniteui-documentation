...
The column selection feature can be enabled through the <ApiLink pkg="grids" type="{ComponentName}" member="columnSelection" prefixed={false} /> input, which takes <PlatformBlock for="Angular,React,WebComponents"><ApiLink pkg="grids" kind="type" type="GridSelectionMode" prefixed={false} suffix={false} /></PlatformBlock><PlatformBlock for="Blazor"><ApiLink pkg="grids" kind="enum" type="GridSelectionMode" prefixed={false} suffix={false} /></PlatformBlock> values.
...
<ApiLink kind="sass" module="index.html" type="function-grid-theme" label="grid-theme" /> accepts several parameters but those are the five responsible for changing the appearance of all selected columns:
