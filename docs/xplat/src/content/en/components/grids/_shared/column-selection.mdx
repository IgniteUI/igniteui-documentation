...
> [!Note]
> The [Multi Column Headers](multi-column-headers.md) feature does not reflect on the <ApiLink pkg="grids" type="Column" member="selectable" /> input. The `ColumnGroupComponent` is <ApiLink pkg="grids" type="Column" member="selectable" />, if at least one of its children has the selection behavior enabled. In addition, the component is marked as <ApiLink pkg="grids" type="Column" member="selected" /> if all of its <ApiLink pkg="grids" type="Column" member="selectable" /> descendants are <ApiLink pkg="grids" type="Column" member="selected" />.
...
> [!Note]
> The above statement also applies to the `ColumnGroupComponent`, except that when the <ApiLink pkg="grids" type="Column" member="selected" /> property is changed it changes the state of its descendants.
