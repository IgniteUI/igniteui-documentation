...
To set the initial filtering state of the `{ComponentName}`, set the `{ComponentName}` <ApiLink pkg="grids" type="FilteringExpressionsTree" /> property to an array of <ApiLink pkg="grids" type="FilteringExpressionsTree" /> for each column to be filtered.
...
The <ApiLink pkg="grids" type="FilteringLogic" kind="enum" prefixed={false} /> property of the `{ComponentName}` controls how filtering multiple columns will resolve in the `{ComponentName}`. You can change it at any time through the `{ComponentName}` API, or through the `{ComponentName}` input property.
...
- `{ComponentName}` `filteringExpressions` property is removed. Use <ApiLink pkg="grids" type="FilteringExpressionsTree" /> instead.
