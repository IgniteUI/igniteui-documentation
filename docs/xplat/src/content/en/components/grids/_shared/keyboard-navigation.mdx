...
|---------|-------------|-----------|
| <ApiLink pkg="grids" type="{ComponentName}" member="gridKeydown" prefixed={false} /> | An event that is emitted when any of key press/combinations described above is performed. Can be canceled. For any other key press/combination, use the default `onkeydown` event. | <ApiLink pkg="grids" type="GridKeydownEventArgs" /> |
| <ApiLink pkg="grids" type="{ComponentName}" member="activeNodeChange" prefixed={false} /> | An event that is emitted when the active node is changed. You can use it to determine the Active focus position (header, tbody etc.), column index, row index or nested level. | <ApiLink pkg="grids" type="ActiveNodeChangeEventArgs" /> |
</PlatformBlock>
