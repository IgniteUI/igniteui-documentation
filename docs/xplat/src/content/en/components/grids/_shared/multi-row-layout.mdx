...
llms:
  description: "The Multi-row Layout in the {ProductName} extends the rendering capabilities of the {Platform} {ComponentTitle}."
---
...
The <ApiLink type="Column" /> component exposes four <ApiLink pkg="core" type="Input" /> properties to determine the location and span of each cell:
...
- Filtering - only Excel Style Filtering is supported. Setting <ApiLink type="{ComponentName}" member="filterMode" /> explicitly to <ApiLink type="FilterMode" member="quickFilter" label="FilterMode.quickFilter" /> has no effect.
- Paging - works on records, not visual rows.
- Group By - <ApiLink type="{ComponentName}" member="hideGroupedColumns" /> option has no effect in Multi-row Layout. The grouped columns are always visible.
...
<ApiLink type="{ComponentName}" /> with Multi-Row Layouts provides build-in keyboard navigation.
...
- <ApiLink type="{ComponentName}" member="gridKeydown" /> is exposed. The event will emit `IGridKeydownEventArgs`. This event is available only through the keyboard key combinations mentioned above, for all other key actions you can use `KeyDown` event.
...
- Get template output of the whole configuration ready to by placed inside an <ApiLink type="{ComponentName}" /> or the JSON representation that can also be used and parsed in your template using [NgForOf](https://angular.io/api/common/NgForOf) for example.
...
The <ApiLink type="{ComponentName}" /> allows styling through the [{ProductName} Theme Library](../themes/styles.mdx). The grid's <ApiLink type="Theme" /> exposes a wide variety of properties, which allow the customization of all the features of the grid.
...
In the approach, that was described above, the color values were hardcoded. Alternatively, you can achieve greater flexibility, using the `Palette` and <ApiLink pkg="core" type="Color" /> functions.
...
After a custom palette has been generated, the <ApiLink pkg="core" type="Color" /> function can be used to obtain different varieties of the primary and the secondary colors.
