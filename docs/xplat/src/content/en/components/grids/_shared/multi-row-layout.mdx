...
- Filtering - only Excel Style Filtering is supported. Setting <ApiLink type="{ComponentName}" member="filterMode" pkg="grids" /> explicitly to <ApiLink type="FilterMode" member="quickFilter" label="FilterMode.quickFilter" /> has no effect.
- Paging - works on records, not visual rows.
- Group By - <ApiLink type="{ComponentName}" member="hideGroupedColumns" pkg="grids" /> option has no effect in Multi-row Layout. The grouped columns are always visible.
...
- <ApiLink type="{ComponentName}" member="gridKeydown" pkg="grids" /> is exposed. The event will emit `IGridKeydownEventArgs`. This event is available only through the keyboard key combinations mentioned above, for all other key actions you can use `KeyDown` event.
...
- Get template output of the whole configuration ready to by placed inside an <ApiLink type="{ComponentName}" pkg="grids" /> or the JSON representation that can also be used and parsed in your template using [NgForOf](https://angular.io/api/common/NgForOf) for example.
...
The <ApiLink type="{ComponentName}" pkg="grids" /> allows styling through the [{ProductName} Theme Library](../themes/styles.md). The grid's <ApiLink type="Theme" /> exposes a wide variety of properties, which allow the customization of all the features of the grid.
