...
<PlatformBlock for="Angular">
If you want to provide a custom template which will be applied when a cell is in edit mode, you can make use of the `CellTemplateDirective`. To do this, you need to pass an **ng-template** marked with the <ApiLink pkg="grids" type="{ComponentName}" member="cellEditor" prefixed={false} /> directive and properly bind your custom control to the cell <ApiLink pkg="grids" type="{ComponentName}" member="editValue" prefixed={false} />:
