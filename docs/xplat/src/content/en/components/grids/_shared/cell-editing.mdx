...
> [!Note]
> Any changes made to the cell's <ApiLink pkg="grids" type="CellType" member="editValue" kind="interface" /> in edit mode, will trigger the appropriate [editing event](editing.md#event-arguments-and-sequence) on exit and apply to the transaction state if transactions are enabled.
