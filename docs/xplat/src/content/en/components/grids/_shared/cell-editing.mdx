...
<PlatformBlock for="Angular">
The <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> allows for its cells to be styled through the [{ProductName} Theme Library](../themes/styles.md). The grid's [theme]({environment:sassApiUrl}/index.html#function-grid-theme) exposes a wide range of properties, which allow users to style many different aspects of the grid.
...
We can now define the theme using our palette. The cells are styled by the [grid-theme]({environment:sassApiUrl}/index.html#function-grid-theme), so we can use that to generate a theme for our <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} />:
