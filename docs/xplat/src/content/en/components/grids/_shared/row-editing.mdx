...
llms:
  description: "The {ProductName} Row Editing feature in {Platform} {ComponentTitle} allows editing data directly within the {ComponentName}."
---
...
Define a `{ComponentName}` with bound data source and <ApiLink type="{ComponentName}" member="rowEditable" /> set to true:
...
<DocsAside type="info">
Enabling editing for individual columns is not necessary. Using the <ApiLink type="{ComponentName}" member="rowEditable" /> property in the `{ComponentName}`, all rows, with defined `Field` property (excluding the primary row) will be editable. If you want to disable editing for a specific column, simply set the <ApiLink type="{ComponentName}" member="editable" /> input of that column to `false`.
</DocsAside>
...
After we've defined our custom buttons, we can make use of the <ApiLink type="function-button-theme" label="button-theme" kind="sass" module="index.html" /> to style them. You can learn more about `igx-button` styling in this [topic](../button.mdx#styling). We can create a custom theme for our `Done` and `Cancel`:
