...
> [!Note]
> Enabling editing for individual columns is not necessary. Using the <ApiLink pkg="grids" type="{ComponentName}" member="rowEditable" prefixed={false} /> property in the `{ComponentName}`, all rows, with defined `Field` property (excluding the primary row) will be editable. If you want to disable editing for a specific column, simply set the <ApiLink pkg="grids" type="Column" member="editable" /> input of that column to `false`.
