...
To get started import the <ApiLink type="{ComponentModule}" prefixed={false} /> in the **app.module.ts** file:
...
Define a `{ComponentName}` with bound data source and <ApiLink pkg="grids" type="{ComponentName}" member="rowEditable" prefixed={false} /> set to true:
...
<DocsAside type="info">
Enabling editing for individual columns is not necessary. Using the <ApiLink pkg="grids" type="{ComponentName}" member="rowEditable" prefixed={false} /> property in the `{ComponentName}`, all rows, with defined `Field` property (excluding the primary row) will be editable. If you want to disable editing for a specific column, simply set the <ApiLink pkg="grids" type="{ComponentName}" member="editable" prefixed={false} /> input of that column to `false`.
</DocsAside>
...
We can now define a custom <ApiLink kind="sass" module="index.html" type="function-banner-theme" label="banner theme" /> that will affect our Row Editing background and make use of one of the predefined palettes namely `$purple-palette` :
...
After we've defined our custom buttons, we can make use of the <ApiLink kind="sass" module="index.html" type="function-button-theme" label="button-theme" /> to style them. You can learn more about `igx-button` styling in this [topic](../button.md#styling). We can create a custom theme for our `Done` and `Cancel`:
