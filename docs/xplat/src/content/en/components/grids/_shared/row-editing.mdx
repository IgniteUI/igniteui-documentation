...
<DocsAside type="note">
The `{ComponentName}` utilizes <ApiLink pkg="grids" type="BaseTransactionService" /> - an internal provider that holds pending cell changes until the row state is either submitted or cancelled.
</DocsAside>
