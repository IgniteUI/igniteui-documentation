...
Define a `{ComponentName}` with bound data source and <ApiLink type="{ComponentName}" member="rowEditable" pkg="grids" prefixed={false} /> set to true:
...
<DocsAside type="info">
Enabling editing for individual columns is not necessary. Using the <ApiLink type="{ComponentName}" member="rowEditable" pkg="grids" prefixed={false} /> property in the `{ComponentName}`, all rows, with defined `Field` property (excluding the primary row) will be editable. If you want to disable editing for a specific column, simply set the <ApiLink type="{ComponentName}" member="editable" pkg="grids" prefixed={false} /> input of that column to `false`.
</DocsAside>
