...
Row pinning is controlled through the <ApiLink pkg="grids" kind="interface" type="RowDirective" member="pinned" suffix="{false}"/> input of the `Row`. Pinned rows are rendered at the top of the <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> by default and stay fixed through vertical scrolling of the unpinned rows in the <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> body.
...
A row is pinned below the last pinned row. Changing the order of the pinned rows can be done by subscribing to the <ApiLink pkg="grids" type="{ComponentName}" member="rowPinning" prefixed={false} /> event and changing the <ApiLink pkg="grids" kind="interface" type="PinRowEventArgsDetail" member="insertAtIndex" suffix="{false}"/> property of the event arguments to the desired position index.
