...
Row pinning is controlled through the <ApiLink type="RowDirective" member="pinned" /> input of the `Row`. Pinned rows are rendered at the top of the <ApiLink type="{ComponentName}" pkg="grids" /> by default and stay fixed through vertical scrolling of the unpinned rows in the <ApiLink type="{ComponentName}" pkg="grids" /> body.
...
You may also use the <ApiLink type="{ComponentName}" pkg="grids" />'s <ApiLink type="{ComponentName}" member="pinRow" label="PinRow" pkg="grids" /> or <ApiLink type="{ComponentName}" member="unpinRow" label="UnpinRow" pkg="grids" /> methods of the to pin or unpin records by their ID:
...
Note that the row ID is the primary key value, defined by the <ApiLink type="{ComponentName}" member="primaryKey" pkg="grids" /> of the grid, or the record instance itself. Both methods return a boolean value indicating whether their respective operation is successful or not. Usually the reason they fail is that the row is already in the desired state.

A row is pinned below the last pinned row. Changing the order of the pinned rows can be done by subscribing to the <ApiLink type="{ComponentName}" member="rowPinning" pkg="grids" /> event and changing the <ApiLink type="PinRowEventArgs" /> property of the event arguments to the desired position index.
...
You can change the row pinning position via the <ApiLink type="{ComponentName}" member="pinning" pkg="grids" /> configuration option. It allows you to set the pin area position to either Top or Bottom.
