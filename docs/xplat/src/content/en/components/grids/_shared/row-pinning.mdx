...
A row is pinned below the last pinned row. Changing the order of the pinned rows can be done by subscribing to the <ApiLink pkg="grids" type="{ComponentName}" member="rowPinning" prefixed={false} /> event and changing the <PlatformBlock for="Angular,WebComponents">`IgrPinRowEventArgs`</PlatformBlock><PlatformBlock for="React">`IgrPinRowEventArgs`</PlatformBlock><PlatformBlock for="Blazor">`IgrPinRowEventArgs`</PlatformBlock> property of the event arguments to the desired position index.
