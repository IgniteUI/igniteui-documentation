...
Row pinning is controlled through the <ApiLink pkg="grids" type="{ComponentName}" member="pinned" prefixed={false} /> input of the `Row`. Pinned rows are rendered at the top of the `{ComponentName}` by default and stay fixed through vertical scrolling of the unpinned rows in the `{ComponentName}` body.
