...
This is done via the <ApiLink pkg="grids" type="{ComponentName}" member="sortable" prefixed={false} /> input. With the <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> sorting, you can also set the `SortingIgnoreCase` property to perform case sensitive sorting:
