...
> [!Note]
> Sorting is performed using our <ApiLink pkg="grids" type="DefaultSortingStrategy" /> algorithm. Any `Column` or `ISortingExpression` can use a custom implementation of the <ApiLink pkg="grids" type="ISortingStrategy" kind="interface" prefixed={false} /> as a substitute algorithm. This is useful when custom sorting needs to be defined for complex template columns, or image columns, for example.
