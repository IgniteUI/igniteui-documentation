...
> [!Note]
> Sorting is performed using our <ApiLink pkg="grids" type="DefaultSortingStrategy" suffix={false} /> algorithm. Any <ApiLink pkg="grids" type="Column" /> or <ApiLink pkg="grids" type="SortingExpression" kind="interface" /> can use a custom implementation of the <ApiLink pkg="grids" type="SortingStrategy" kind="interface" /> as a substitute algorithm. This is useful when custom sorting needs to be defined for complex template columns, or image columns, for example.
...
> [!Note]
> If values of type `string` are used by a column of <ApiLink pkg="grids" type="Column" member="dataType" /> `Date`, the <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> won't parse them to `Date` objects and using <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> <ApiLink pkg="grids" type="{ComponentName}" member="sort" prefixed={false} label="Sorting" /> won't work as expected. If you want to use `string` objects, additional logic should be implemented on an application level, in order to parse the values to `Date` objects.
