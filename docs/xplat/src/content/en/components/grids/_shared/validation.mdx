...
llms:
  description: "The {Platform} {ComponentName} provides editing that has a built-in Validation mechanism of user input when editing cells/rows."
apiTerms: none
---
...
We expose the <PlatformBlock for="Angular"><ApiLink type="IGridFormGroupCreatedEventArgs" member="formGroup" label="FormGroup" /></PlatformBlock><PlatformBlock for="WebComponents,Blazor"><ApiLink type="FormGroup" /></PlatformBlock> that will be used for validation when editing starts on a row/cell via a <ApiLink type="{ComponentName}" member="formGroupCreated" /> event. You can modify it by adding your own validators for the related fields:
...
The grid exposes a validation service via the <ApiLink type="{ComponentName}" member="validation" /> property.
...
- <ApiLink type="{ComponentName}" member="markAsTouched" /> - marks the related record/field as touched.
...
- When updating cells/rows via the API - <ApiLink type="{ComponentName}" member="updateRow" />, <ApiLink type="{ComponentName}" member="updateCell" />, etc.
- When using batch editing and the `Undo`/`Redo` API of the transaction service.

<DocsAside type="info">
Validation will not trigger for records that have not been edited via user input or via the editing API. Visual indicators on the cell will only shown if the related input is considered touched - either via user interaction or via the <ApiLink type="{ComponentName}" member="markAsTouched" /> API of the validation service.
</DocsAside>
...
In some cases you may want to disallow submitting an invalid value in the data. In that scenario, you can use the <ApiLink type="{ComponentName}" member="cellEdit" /> or <ApiLink type="{ComponentName}" member="rowEdit" /> events and cancel the event in case the new value is invalid.
...
In that case a custom validator can be used to compare the values in the record via their shared <PlatformBlock for="Angular"><ApiLink type="IGridFormGroupCreatedEventArgs" member="formGroup" label="FormGroup" /></PlatformBlock><PlatformBlock for="WebComponents,Blazor"><ApiLink type="FormGroup" /></PlatformBlock>.
...
The cross-field validator can be added to the <PlatformBlock for="Angular"><ApiLink type="IGridFormGroupCreatedEventArgs" member="formGroup" label="FormGroup" /></PlatformBlock><PlatformBlock for="WebComponents,Blazor"><ApiLink type="FormGroup" /></PlatformBlock> of the row from `FormGroupCreated` event, which returns the new <PlatformBlock for="Angular"><ApiLink type="IGridFormGroupCreatedEventArgs" member="formGroup" label="FormGroup" /></PlatformBlock><PlatformBlock for="WebComponents,Blazor"><ApiLink type="FormGroup" /></PlatformBlock> for each row when entering edit mode:
