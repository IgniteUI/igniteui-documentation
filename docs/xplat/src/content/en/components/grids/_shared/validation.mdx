...
We expose the <ApiLink pkg="grids" type="FormGroup" /> that will be used for validation when editing starts on a row/cell via a <ApiLink pkg="grids" type="{ComponentName}" member="formGroupCreated" prefixed={false} /> event. You can modify it by adding your own validators for the related fields:
...
- While editing via the cell editor based on the grid's <ApiLink pkg="grids" type="ValidationTrigger" kind="enum" prefixed={false} />. Either on `Change` while typing in the editor, or on `Blur` when the editor loses focus or closes.
...
In that case a custom validator can be used to compare the values in the record via their shared <ApiLink pkg="grids" type="FormGroup" />.
