...
- While editing via the cell editor based on the grid's <ApiLink pkg="grids" type="ValidationTrigger" />. Either on `Change` while typing in the editor, or on `Blur` when the editor loses focus or closes.
