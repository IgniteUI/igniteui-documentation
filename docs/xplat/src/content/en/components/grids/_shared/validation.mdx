...
We extend some of the {Platform} Forms validator to directly work with the <ApiLink type="Column" />. The same validators are available as attributes to be set declaratively in <ApiLink type="Column" />. The following validators are supported out-of-the-box:
...
We expose the <PlatformBlock for="Angular"><ApiLink type="IGridFormGroupCreatedEventArgs" member="formGroup" label="FormGroup" pkg="grids" kind="interface" /></PlatformBlock><PlatformBlock for="WebComponents,Blazor"><ApiLink type="FormGroup" pkg="grids" /></PlatformBlock> that will be used for validation when editing starts on a row/cell via a <ApiLink type="{ComponentName}" member="formGroupCreated" pkg="grids" /> event. You can modify it by adding your own validators for the related fields:
...
The grid exposes a validation service via the <ApiLink type="{ComponentName}" member="validation" pkg="grids" /> property.
...
- <ApiLink type="{ComponentName}" member="markAsTouched" pkg="grids" /> - marks the related record/field as touched.
...
- While editing via the cell editor based on the grid's <ApiLink type="GridValidationTrigger" />. Either on `Change` while typing in the editor, or on `Blur` when the editor loses focus or closes.
- When updating cells/rows via the API - <ApiLink type="{ComponentName}" member="updateRow" pkg="grids" />, <ApiLink type="{ComponentName}" member="updateCell" pkg="grids" />, etc.
- When using batch editing and the `Undo`/`Redo` API of the transaction service.

<DocsAside type="info">
Validation will not trigger for records that have not been edited via user input or via the editing API. Visual indicators on the cell will only shown if the related input is considered touched - either via user interaction or via the <ApiLink type="{ComponentName}" member="markAsTouched" pkg="grids" /> API of the validation service.
</DocsAside>
...
You can define your own validation directive to use on a <ApiLink type="Column" /> in the template.
...
In some cases you may want to disallow submitting an invalid value in the data. In that scenario, you can use the <ApiLink type="{ComponentName}" member="cellEdit" pkg="grids" /> or <ApiLink type="{ComponentName}" member="rowEdit" pkg="grids" /> events and cancel the event in case the new value is invalid.
...
In that case a custom validator can be used to compare the values in the record via their shared <PlatformBlock for="Angular"><ApiLink type="IGridFormGroupCreatedEventArgs" member="formGroup" label="FormGroup" pkg="grids" kind="interface" /></PlatformBlock><PlatformBlock for="WebComponents,Blazor"><ApiLink type="FormGroup" pkg="grids" /></PlatformBlock>.
...
The cross-field validator can be added to the <PlatformBlock for="Angular"><ApiLink type="IGridFormGroupCreatedEventArgs" member="formGroup" label="FormGroup" pkg="grids" kind="interface" /></PlatformBlock><PlatformBlock for="WebComponents,Blazor"><ApiLink type="FormGroup" pkg="grids" /></PlatformBlock> of the row from `FormGroupCreated` event, which returns the new <PlatformBlock for="Angular"><ApiLink type="IGridFormGroupCreatedEventArgs" member="formGroup" label="FormGroup" pkg="grids" kind="interface" /></PlatformBlock><PlatformBlock for="WebComponents,Blazor"><ApiLink type="FormGroup" pkg="grids" /></PlatformBlock> for each row when entering edit mode:
