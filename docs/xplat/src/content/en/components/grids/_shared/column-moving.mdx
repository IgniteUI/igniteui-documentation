...
Note that when using the column moving feature, the <ApiLink pkg="grids" type="GridComponentEventMap" kind="interface" member="columnMovingEnd"/> event will be emitted if the operation was successful. Also note that in comparison to the drag and drop functionality, using the column moving feature does not require setting the <ApiLink pkg="grids" type="{ComponentName}" member="moving" prefixed={false} /> property to true.

## Events

There are several events related to the column moving to provide a means for tapping into the columns' drag and drop operations. These are <ApiLink pkg="grids" type="GridComponentEventMap" kind="interface" member="columnMovingStart" />, `ColumnMoving` and <ApiLink pkg="grids" type="GridComponentEventMap" kind="interface" member="columnMovingEnd" />.

You can subscribe to the <ApiLink pkg="grids" type="GridComponentEventMap" kind="interface" member="columnMovingEnd" /> event of the <ApiLink pkg="grids" type="{ComponentName}" prefixed={false} /> to implement some custom logic when a column is dropped to a new position. For example, you can cancel dropping the **Category** column after the **Change On Year(%)** column in the following code snippet.
