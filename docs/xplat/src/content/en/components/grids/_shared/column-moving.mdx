...
**Column moving** feature is enabled on a per-grid level, meaning that the `{ComponentName}` could have either movable or immovable columns. This is done via the <ApiLink pkg="grids" type="{ComponentName}" member="moving" prefixed={false} /> input of the `{ComponentName}`.
...
You can subscribe to the <ApiLink pkg="grids" type="{ComponentName}" member="columnMovingEnd" prefixed={false} /> event of the `{ComponentName}` to implement some custom logic when a column is dropped to a new position. For example, you can cancel dropping the **Category** column after the **Change On Year(%)** column in the following code snippet.
