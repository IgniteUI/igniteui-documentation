...
The {ProductName} State Persistence in {Platform} {ComponentTitle} allows developers to easily save and restore the grid state. When the <ApiLink type="GridState" /> is applied on the {Platform} <ApiLink type="{ComponentName}" pkg="grids" />, it exposes the <ApiLink type="GridState" member="getState" /> and <ApiLink type="{ComponentName}" member="setState" pkg="grids" /> methods that developers can use to achieve state persistence in any scenario.
</PlatformBlock>
...
The {ProductName} State Persistence in {Platform} {ComponentTitle} allows developers to easily save and restore the grid state. When the <ApiLink type="GridState" /> is applied on the {Platform} <ApiLink type="{ComponentName}" pkg="grids" />, it exposes the `GetStateAsStringAsync` and `ApplyStateFromStringAsync` methods that developers can use to achieve state persistence in any scenario.
</PlatformBlock>
...
- <ApiLink type="{ComponentName}" member="cellSelection" pkg="grids" />
- <ApiLink type="{ComponentName}" member="columnSelection" pkg="grids" />
...
<PlatformBlock for="Angular">
<ApiLink type="{ComponentName}" member="setState" pkg="grids" /> - The <ApiLink type="{ComponentName}" member="setState" pkg="grids" /> method accepts the serialized JSON string or <ApiLink type="IGridState" pkg="grids" kind="interface" /> object as argument and will restore the state of each feature found in the object/JSON string.
</PlatformBlock>
...
<ApiLink type="GridState" /> will not persist pivot dimension functions, value formatters, etc. by default (see [limitations](state-persistence.md#limitations)). Restoring any of these can be achieved with code on application level. The <ApiLink type="{ComponentName}" pkg="grids" /> exposes two events which can be used to set back any custom functions you have in the configuration: `DimensionInit` and `ValueInit`. Let's show how to do this:
...
> The `DimensionInit` and `ValueInit` events are emitted for each value and dimension defined in the <ApiLink type="{ComponentName}" member="pivotConfiguration" pkg="grids" /> property.
...
> `StateParsed` is only emitted when we are using <ApiLink type="{ComponentName}" member="setState" pkg="grids" /> with string argument.
