...
To allow customizing the display format by these parameters, the <ApiLink pkg="grids" type="Column" member="pipeArgs" label="PipeArgs" /> input is exposed. A column will respect only the corresponding properties for its data type, if `PipeArgs` is set. Example:
