...
When properly implemented, the cell editing template also ensures that the cell's <ApiLink type="CellType" member="editValue" label="EditValue" /> will correctly pass through the grid [editing event cycle](grid/editing.md#event-arguments-and-sequence).
