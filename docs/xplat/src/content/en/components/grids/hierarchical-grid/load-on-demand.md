...
description: The {ProductName} Hierarchical Grid provides the necessary tools to load data on demand for each child grid that is expanded. That way the volume of data would be greatly reduced and can be retrieved only when the user needs it.
keywords: "{Platform} hierarchical grid, igniteui for {Platform}, infragistics"
license: commercial
