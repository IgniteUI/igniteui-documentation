...
If you have a way to provide information whether a row has children prior to its expanding, you could use the <ApiLink pkg="grids" kind="interface" type="HierarchicalGridBaseDirective" member="hasChildrenKey" label="HasChildrenKey" exclude="Blazor" /> input property. This way you could provide a boolean property from the data objects which indicates whether an expansion indicator should be displayed.
...
Note that setting the <ApiLink pkg="grids" kind="interface" type="HierarchicalGridBaseDirective" member="hasChildrenKey" label="HasChildrenKey" exclude="Blazor" /> property is not required. In case you don't provide it, expansion indicators will be displayed for each row.

Additionally if you wish to show/hide the header expand/collapse all indicator you can use the <ApiLink pkg="grids" kind="interface" type="HierarchicalGridBaseDirective" member="showExpandAll" label="ShowExpandAll" exclude="Blazor" /> property.
