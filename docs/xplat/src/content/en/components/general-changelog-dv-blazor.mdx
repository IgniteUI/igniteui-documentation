...
#### Deprecated
- <ApiLink pkg="inputs" type="Tooltip" /> - `DisableArrow` is deprecated. Use `WithArrow` to render an arrow indicator.
...
### General
- <label>NEW</label> <ApiLink pkg="inputs" type="Tooltip" /> component provides a way to display a tooltip for a specific element. To use, set content as desired and link via the `Anchor` property to the target element's id:
...
- A number of enumerations have been renamed and/or merged with others. Renames (with affected components):
  ...
  - `TickLabelRotation` (<ApiLink pkg="inputs" type="Slider" /> and <ApiLink pkg="inputs" type="RangeSlider" />) has been renamed to `SliderTickLabelRotation`
...
- <ApiLink pkg="grids" type="RowIsland" />
  - Added new parameter `ParentRowData` in <ApiLink pkg="grids" type="GridCreatedEventArgsDetail" /> args for `GridCreated` event
...
- <ApiLink pkg="grids" type="PivotGrid" /> - The type of Columns, Rows, Filters from <ApiLink pkg="grids" kind="interface" type="PivotConfiguration" /> option is now array of IgbPivotDimension - `IgbPivotDimension[]`, it was `IgbPivotDimensionCollection` previously.
