...
description: "Use the Ignite UI Theming MCP server to generate {ProductName} palettes, themes, typography, and design token overrides through your AI assistant."
...
The AI will call `create_custom_palette` with `mode: "explicit"` for the primary color and auto-generate the rest.<PlatformBlock for="Angular"> See [Palettes](https://www.infragistics.com/products/ignite-ui-angular/angular/components/themes/sass/palettes) for more detail on shade generation.</PlatformBlock>
