...
Run this command from the root directory of your existing Angular, React, Blazor, or Web Components project. The command automatically configures the required MCP (Model Context Protocol) servers, copies the appropriate framework-specific skill files into each selected agent's skills directory, and creates or updates the necessary instruction files. Use the --assistants option to specify which coding assistants (such as GitHub Copilot, Claude Code, or others) should receive MCP server configuration, and use the --agents option to specify which agents should receive the framework skill files. If neither option is provided, the command starts in interactive mode, guiding you through the available assistants and agents with prompts. During execution, the installer compares existing files with the generated versions and only updates files whose content has changed, preserving timestamps and avoiding unnecessary modifications. This allows you to safely rerun the command whenever you add new assistants or agents, upgrade the framework integration, or synchronize your project with the latest skill definitions.
```bash
...
<PlatformBlock for="Angular">
<DocsAside type="info">
The `npx igniteui-cli` great new addition for this command is the availability to use and the `ig` forms do not register the `@angular/cli` MCP server. Use the Angular Schematics command above if you want all three servers configured in a single step.
</DocsAside>
</PlatformBlock>
