...
keywords: "{Platform}, {ProductName}, Infragistics, MCP, Model Context Protocol, Ignite UI MCP server, Ignite UI Theming MCP, Agent Skills, AI, agent, Copilot, Cursor, Claude Code, ai-config"
license: MIT
_canonicalLink: "{environment:dvUrl}/components/ai-assisted-development-overview"
last_updated: "2026-05-03"
mentionedTypes: []
llms:
  description: "{ProductName} provides a complete AI toolchain - Agent Skills, the Ignite UI CLI MCP server, the Ignite UI Theming MCP server and the MAKER MCP server - that grounds AI coding assistants in correct component APIs, import paths, and design tokens."
---
...
Изпълнете тази команда от главната директория на Вашия съществуващ Angular, React, Blazor или Web Components проект. Командата автоматично конфигурира необходимите MCP (Model Context Protocol) сървъри, копира подходящите файлове с умения, специфични за рамката, в директорията с умения на всеки избран агент и създава или актуализира необходимите файлове с инструкции. Използвайте опцията --assistants, за да посочите кои асистенти за кодиране (като GitHub Copilot, Claude Code или други) трябва да получат конфигурация на MCP сървър, и използвайте опцията --agents, за да посочите кои агенти трябва да получат файловете с умения за рамката. Ако не е предоставена нито една от опциите, командата стартира в интерактивен режим, като Ви води през наличните асистенти и агенти чрез подкани. По време на изпълнение инсталаторът сравнява съществуващите файлове с генерираните версии и актуализира само файловете, чието съдържание е променено, като запазва времевите маркери и избягва ненужни модификации. Това Ви позволява безопасно да изпълнявате командата повторно, когато добавяте нови асистенти или агенти, надграждате интеграцията с рамката или синхронизирате проекта си с най-новите дефиниции на умения.
...
npx igniteui-cli@latest2 ai-config2
```
...
Голямото ново допълнение `npx igniteui-cli` за тази команда е възможността да използвате, а формите `ig` не регистрират `@angular/cli` MCP сървъра. Използвайте командата Angular Schematics по-горе, ако искате и трите сървъра да бъдат конфигурирани в една стъпка.
