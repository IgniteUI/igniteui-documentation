...
llms:
  description: "The {ProductName} ComboBox component exposes several features such as filtering and grouping."
---
...
The following demo shows some <ApiLink type="Combo" /> features that are enabled/disabled at runtime:
...
In our sample we are going to use the <ApiLink pkg="core" type="Switch" /> component, so we have to import them together with the combo:
...
Note that grouping is enabled/disabled by setting the <ApiLink type="Combo" member="groupKey" label="GroupKey" /> property to a corresponding data source field:
...
<DocsAside type="info">
The <ApiLink type="Combo" member="groupKey" label="GroupKey" /> property will only have effect if your data source consists of complex objects.
</DocsAside>
