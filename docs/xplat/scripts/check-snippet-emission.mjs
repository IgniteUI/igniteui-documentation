        ...
        for (const platform of PLATFORMS) {
            ...
            try {
                ...
                if (!channel || channel === 'markup') {
                    ...
                    root['$type'] = `+check:${channel}`;
                    if (parsed.onViewInit) parsed['$onViewInit'] = `+check:${channel}`;
                    if (parsed.onInit) parsed['$onInit'] = `+check:${channel}`;
                    api.emitSnippets(JSON.stringify(parsed), platform, {
                        ...opts, forceCodeBehind: channel === 'code' });
                }
            } catch (e) {
                failures++;
