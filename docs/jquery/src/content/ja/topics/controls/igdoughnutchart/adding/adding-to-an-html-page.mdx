...
- [必要なリソースの手動で追加する](/general-and-getting-started/adding-the-required-resources-for-igniteui-for-jquery): このトピックは、必要な JavaScript リソースを追加して &#123;environment:ProductName&#125; ライブラリからコントロールを使用する場合の全般的なガイダンスを提供します。
...
- [ASP.NET MVC アプリケーションへの *igDoughnutChart* の追加](/controls/igdoughnutchart/adding/adding-using-the-mvc-helper): このトピックでは、%ProductNameMVC%% を使用して ASP.NET MVC アプリケーションに `igDoughnutChart` のインスタンスを作成する方法を紹介します。

- [jQuery および MVC API リファレンス リンク (*igDoughnutChart*)](/controls/igdoughnutchart/api-links): このトピックでは、`igDoughnutChart` コントロールと %ProductNameMVC%% に関する API ドキュメントへのリンクを提供します。
