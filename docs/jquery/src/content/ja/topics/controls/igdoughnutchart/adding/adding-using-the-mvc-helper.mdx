...
```
	...
	**2.**すべてのデータ ビジュアライゼーション コントロールに対して結合したファイルに参照を追加します
	...
	ASP.NET MVC ビューの HEAD タグに以下のファイル参照を追加します (igLoader を使用する、または [HTML ページへの *igDoughnutChart* の追加](/controls/igdoughnutchart/adding/adding-to-an-html-page)のトピックで説明するように、すべての Doughnut チャート関係のファイルを明示的に含めます)。
...
- [*igDoughnutChart* の HTML ページへの追加](/controls/igdoughnutchart/adding/adding-to-an-html-page): このトピックは、`igDoughnutChart` を HTML ページに追加する方法を説明します。

- [jQuery および MVC API リファレンス リンク (*igDoughnutChart*)](/controls/igdoughnutchart/api-links): このトピックでは、`igDoughnutChart` コントロールと &#123;environment:ProductNameMVC&#125;　に関する API ドキュメントへのリンクを提供します。
