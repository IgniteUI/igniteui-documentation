...
以下の表で、`igQRCodeBarcode` コントロールの主な機能を簡単に説明しています。詳細は、[igQRCodeBarcode の構成](/controls/igqrcodebarcode/configuring/configuring)のトピック グループを参照してください。
...
>**注:** エンコードするデータのタイプによっては、追加のエンコーディング ファイルを読み込み、`eciNumber` を設定する必要があります (詳細は、[文字エンコードの構成 (igQRCodeBarcode)](/controls/igqrcodebarcode/configuring/configuring-the-character-encoding) を参照してください)。
...
- [igQRCodeBarcode のスタイル設定](/controls/igqrcodebarcode/styling): このトピックでは、`igQRCodeBarcode` コントロールのルック アンド フィール、バーコードの色、背景色、および境界線の色と太さを設定する方法を説明します。
