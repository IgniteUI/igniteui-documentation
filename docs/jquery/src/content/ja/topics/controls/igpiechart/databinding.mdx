...
- [igDataSource](/igdatasource-igdatasource): これは、`igDataSource`™ に関連するすべてのトピックへリンクする参照ページです。

- [igDataSource の概要](/igdatasource-igdatasource-overview): このトピックは、`igDataSource` コントロールについて説明し、その基本的な使用方法と機能を紹介します。

- [igPieChart の概要](/igpiechart-overview): Web ページに円チャートを表示する `igPieChart` コントロールについての基本情報が含まれています。

- [igPieChart の追加](/igpiechart-adding): `igPieChart` を Web ページに 追加する方法を順に説明します。
...
-   HTML5 Web ページ
-   Web サイトまたは Web アプリケーション プロジェクトに追加された、必要なすべての JavaScript および CSS ファイル。`igPieChart` のインスタンス化と構成の詳細情報は、[igPieChart を追加する](/igpiechart-adding)を参照してください。
...
-   HTML5 Web ページ
-   Web サイトまたは Web アプリケーション プロジェクトに追加された、必要なすべての JavaScript および CSS ファイル。`igPieChart` のインスタンス化と構成の詳細情報は、[igPieChart の追加](/igpiechart-adding)を参照してください。
...
```
	...
	2. [XML へのバインド](/igdatasource-binding-to-xml)は、データ フィールドを定義する <ApiLink pkg="ig" type="dataschema" label="igDataSchema" /> が必要です。このコンポーネントは、<ApiLink pkg="ig" type="datasource" label="igDataSource" /> の Array、JSON、および XML データ オブジェクトの変換を処理します。<ApiLink pkg="ig" type="dataschema" member="schema.searchField" section="options" label="searchField" /> オプションを使用して、データ ソースのレコードが検索された場所を指定します。すべてのフィールド定義に `xpath` プロパティを指定することによって、<ApiLink pkg="ig" type="dataschema" member="schema.fields" section="options" label="fields" /> がどのように定義されるかに注意してください。これは、XML データソースに特有です。このようにして、リストにあるすべての階層オブジェクトにおいて、バインドするプロパティへのパスをデータ ソースに伝えます。
...
```
	...
	3. 構成した XML データおよび `igDataSchema` からチャートで使用可能な [Infragistics データソース](/igdatasource-igdatasource-overview)を作成します。
