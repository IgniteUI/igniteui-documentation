...
すべての機能は、追加データの要求に対して同じ [`dataSourceUrl`](&#123;environment:jQueryApiUrl&#125;/ui.igtreegrid#options:dataSourceUrl) エンドポイント アドレス ([ロード オン デマンド](/igtreegrid-load-on-demand) でも使用) を共有します。複数のリモート機能のカスタム バックエンド実装がある場合、提供されたパラメーターを読み込み、要求の複数のタイプを処理する必要があります。
