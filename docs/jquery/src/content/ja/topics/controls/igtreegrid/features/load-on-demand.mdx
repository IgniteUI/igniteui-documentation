...
行が展開されると、子レコードのデータがAJAX 呼び出しによりサーバーに要求されます。この機能は、他の[リモート機能](/igtreegrid-remote-features)により共有される同じ [`dataSourceUrl`](&#123;environment:jQueryApiUrl&#125;/ui.igtreegrid#options:dataSourceUrl) アドレスを使用します。すなわち、複数の要求のスタイルを処理できるように、複数のリモート機能のバックエンド実装が必要です。
