...
`igCurrencyEditor` は `igNumericEditor` のオプションを継承しますが、いくつかの igCurrencyEditor のみの独自なプロパティがあります。たとえば、`currencySymbol` オプションを使用すると、表示された通貨記号を変更できます。すべての `igCurrencyEditor` オプションについては、[igCurrencyEditor jQuery API](&#123;environment:jQueryApiUrl&#125;/ui.igCurrencyEditor) を参照してください。
