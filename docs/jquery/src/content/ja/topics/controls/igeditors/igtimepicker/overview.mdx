...
1.  最初にアプリケーションに必要なローカライズ済みのリソースを含めます。組み込みリソースの詳細は、「[&#123;environment:ProductName&#125; で JavaScript リソースを使用](/deployment-guide-javascript-resources)」ヘルプ トピックを参照してください。
...
サポートされる書式設定は[日付および時間の書式設定](/formatting-dates-numbers-and-strings)のパターンを使用します。時間形式は、一般的な時間指定子を使用して定義することもできます。たとえば、"HH:mm" の形式は "16:35" のように時間を表示します。"HH" は 24 時間形式で時刻の数値を、"mm" は AM/PM を除く分の数値を表示します。
