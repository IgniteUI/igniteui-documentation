...
## igTextEditor の概要
\{environment:ProductName\}™ のテキスト エディターまたは `igTextEditor` は、単一行または複数行の入力用に書式設定可能な入力フィールドを描画するコントロールです。

`igTextEditor` コントロールは、任意のサーバー技術を使用する作業を構成できる豊富なクライアント側 API を公開します。\{environment:ProductName\}™ のコントロールはサーバー非依存ですが、Microsoft® ASP.NET MVC Framework 専用の \{environment:ProductNameMVC\} の一部として含まれるコントロールでは、希望する .NET™ 言語を使用して構成できます。
...
![](../../../images/images/igTextEditor_Overview.png)
...
## \{environment:ProductFamilyName\} CLI を使用して igTextEditor の追加

新しい igTextEditor を簡単にアプリケーションに追加するには、\{environment:ProductFamilyName\} CLI を使用します。新しいアプリケーションを作成した後、以下のコマンドを実行すると、テキスト エディターがプロジェクトに追加されます。
...
4.  前述のセットアップ完了後、テキスト エディターを初期化し、<ApiLink type="igTextEditor" member="width" section="options" label="width" />、<ApiLink type="igTextEditor" member="placeHolder" section="options" label="placeHolder" /> などの必要なオプションを設定します。 
