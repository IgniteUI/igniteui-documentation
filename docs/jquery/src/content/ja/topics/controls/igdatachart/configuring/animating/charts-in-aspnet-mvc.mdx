---
title: "ASP.NET MVC でのチャートのアニメーション化 (igDataChart)"
slug: animating-charts-in-asp.net-mvc
---
...
-	[igDataChart の概要](/controls/igdatachart/overview/overview): このトピックは、`igDataChart`™ コントロールについて、その主要機能、最低必須事項、ユーザー機能といった事項の概念的情報を提供します。

-	[igDataChart の追加](/controls/igdatachart/adding): このトピックでは、`igDataChart`™ コントロールをページに追加し、データにバインドする方法を紹介します。

-	[igDataSource の概要](/data-sources/igdatasource/igdatasource-overview): `igDataSource` コントロールを紹介します。

-	[チャートの Infragistics Motion Framework](/controls/igdatachart/configuring/animating/motion-framework) : チャート対応の Infragistics® Motion Framework を紹介します。
...
![](../../../../images/images/Using_Motion_Framework_in_MVC_with_AJAX_Updates_1.png)
...
3. MVC ビューを追加します。
 ...
 新しい空の MVC ビューを作成するには、コントローラーの `Index()` メソッドを右クリックします。コンテキスト メニューから、[ビューを追加] を選択します。[igDataChart の追加](/controls/igdatachart/adding)のトピックのステップに従って、チャートを組み込むスケルトン MVC ビューを作成します。この例では、最初のステップ[必要なリソースへの参照の追加](/controls/igdatachart/adding#add-references-to-required-resources)が必要です。
...
-	[HTML および JavaScript におけるチャートのアニメーション化](/controls/igdatachart/configuring/animating/animating-html) : 次の方法を紹介します。HTML ビューを作成する、JavaScript を使用して柱状チャートにデータを動的に追加する、&#123;environment:ProductName&#125; ライブラリでチャートの Motion Framework を使用してデータ変更をアニメーション化する。
...
-	[Motion Framework](/controls/igdatachart/configuring/animating/motion-framework#motion-framework-sample) : このサンプルでは、Motion Framework を使用すると、柱状シリーズのチャートにアニメーションを追加する方法を紹介します。
