...
- [igUpload の概要](/controls/igupload/overview): このトピックは `igUpload` コントロールおよびその機能について紹介します。このトピックでは、このコントロールを HTML ページへ追加する方法を示します。

- [HTTP ハンドラーとモジュールの使用](/controls/igupload/working-with-igupload/using-http-handler-and-modules): このトピックは、HTTP モジュールおよび HTTP ハンドラーを構成して、`igUpload` コントロールでアップロードされるデータを承諾するのに必要なサーバー イベントを処理する方法をデモします。
...
| 構成可能な要素 | 詳細 | プロパティ |
| --- | --- | --- |
| [ファイル選択モード (単一/複数)](#config-selection-mode) | ユーザーによるファイル選択が可能かどうか、いつファイルを選択するか、また一度に複数のファイルを選択できるかどうかを構成できます。 | [mode](environment:jQueryApiUrl/ui.igupload#options:mode) [multipleFiles](environment:jQueryApiUrl/ui.igupload#options:multipleFiles) |
| [トリガーのアップロード (手動/自動)](#config-upload-trigger) | ユーザーが `igUpload` パネルにファイルを追加すると、アップロードが自動的に開始または手動で開始 (ユーザーの Upload ボタン押下によって) するかどうかを構成します。 | [autostartupload](environment:jQueryApiUrl/ui.igupload#options:autostartupload) |
| [許可されるファイル タイプ](#config-allowed-files) | ユーザーがアップロードできるファイルのタイプを構成できます。 | [allowedExtensions](environment:jQueryApiUrl/ui.igupload#options:allowedExtensions) |
| [アップロードするファイルの最大数](#max-number-files) | ページの更新ごとにアップロードできる最大ファイル数を構成できます。 | [maxUploadedFiles](environment:jQueryApiUrl/ui.igupload#options:maxUploadedFiles) |
| [同時にアップロードするファイルの最大数の構成](#simultaneously-uploaded-files) | この設定は、同時ファイル アップロード数のしきい値を構成します。 | [maxSimultaneousFilesUploads](environment:jQueryApiUrl/ui.igupload#options:maxSimultaneousFilesUploads) |
| [複数ファイルのアップロードで単一の要求を使用](#singleRequest) | この設定はコントロールを単一の HTTP 要求を使用して複数のファイルのアップロードを構成します。 | [useSingleRequest](environment:jQueryApiUrl/ui.igupload#options:useSingleRequest) |
| [ファイル表示モード](#file-display-mode) | `igUpload` コントロールのパネルに表示するファイル数を構成できます。 | [mode](environment:jQueryApiUrl/ui.igupload#options:mode) |
...
ファイル選択モードは、[mode](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:mode) および [multipleFiles](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:multipleFiles) オプションによって管理されます。[mode](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:mode) が multiple の場合のみ [multipleFiles](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:multipleFiles) を true に設定する意味があります。[mode](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:mode) が single の場合、[multipleFiles](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:multipleFiles) は効果がありません。
...
目的:|使用するプロパティ:|設定の選択肢:
---|---|---
自動アップロードを有効にする|[autostartupload](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:autostartupload)|true
自動アップロードを無効にする|[autostartupload](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:autostartupload)|false
...
ユーザーがアップロードできるファイルのタイプを構成できます。この機能は、許容されるファイル タイプの一覧で使用可能なファイル タイプのファイル名拡張子を明示的にリストして処理されます。このリストは、[allowedExtensions](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:allowedExtensions) プロパティの値として使用可能なファイル拡張子を表す文字列の配列として設定されます。
...
目的:|使用するプロパティ:|設定の選択肢:
---|---|---
許可する拡張子を定義します。|[allowedExtensions](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:allowedExtensions)|許可されるファイル拡張子を表す文字列の配列
...
目的:|使用するプロパティ:|設定の選択肢:
---|---|---
最大アップロード ファイル数を設定|[maxUploadedFiles](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:maxUploadedFiles)|必要な整数値
...
同時アップロード可能な最大ファイル数は [maxSimultaneousFilesUploads](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:maxSimultaneousFilesUploads) プロパティで管理されます。
...
目的:|使用するプロパティ:|設定の選択肢:
---|---|---
同時ファイル アップロードの最大数を設定します。|[maxSimultaneousFilesUploads](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:maxSimultaneousFilesUploads)|必要な整数値
...
目的:| 使用するプロパティ:| 設定の選択肢:
---|---|---
複数ファイルをファイルごとの要求でアップロードします。| [useSingleRequest](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:useSingleRequest)| false (デフォルト)
複数のファイルを単一の要求でアップロードします。| [useSingleRequest](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:useSingleRequest)| true
...
複数ファイルモードで表示されるファイル数は、[maxUploadedFiles](&#123;environment:jQueryApiUrl&#125;/ui.igupload#options:maxUploadedFiles) で設定された数を超えることはできません。
...
- [ファイルをストリームとして保存](/controls/igupload/working-with-igupload/saving-files-as-stream): このトピックは、アップロード ファイルをファイルまたはメモリストリームとして処理し、保存する方法を説明します。詳細な手順は、各プロセスでメモリストリームとしてファイルを保存トピックをご参照ください。
