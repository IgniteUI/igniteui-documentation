...
任意のレイアウトを選択するには、`igLayoutManager` コントロールの [layoutMode](&#123;environment:jQueryApiUrl&#125;/ui.iglayoutmanager#options) プロパティを設定します。デフォルトのレイアウトは、[**列レイアウト**](#column-layout)です。レイアウトの構成やカスタマイズの詳細は、[igLayoutManager の追加](/iglayoutmanager-adding)および[igLayoutManager の構成](/iglayoutmanager-configuring-layouts)のトピックを参照してください。
...
以下の表に、`igLayoutManager` コントロールのレイアウトの概要を示します。各レイアウトの詳細を概要表の後に各レイアウトの詳細を説明します。また、[igLayoutManager の構成](/iglayoutmanager-configuring-layouts)のトピックも参照してください。
...
完全な要件の一覧については、[igLayoutManager の追加](/iglayoutmanager-adding)のトピックを参照してください。
