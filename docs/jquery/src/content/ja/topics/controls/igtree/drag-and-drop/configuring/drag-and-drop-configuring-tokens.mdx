...
- [カスタム ドロップ検証の構成 (igTree)](/controls/igtree/walkthroughs/drag-and-drop-configuring-custom-drop-validation): このトピックでは、コード例を使用して、 Javascript および MVC の両方で `igTree` コントロールのドラッグ アンド ドロップ機能のカスタム ドロップ検証を構成にする方法を紹介します。
