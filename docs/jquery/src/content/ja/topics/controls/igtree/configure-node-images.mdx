...
| --- | --- | --- |
| ノード イメージを構成する | 画像は `igTree` コントロールの各ノードの次を表示するようバインディング、CSS、または URL で構成できます。 | <ApiLink type="igTree" member="parentNodeImageUrl" section="options" label="parentNodeImageUrl" /> <ApiLink type="igTree" member="parentNodeImageClass" section="options" label="parentNodeImageClass" /> <ApiLink type="igTree" member="leafNodeImageUrl" section="options" label="leafNodeImageUrl" /> <ApiLink type="igTree" member="leafNodeImageClass" section="options" label="leafNodeImageClass" /> <ApiLink type="igTree" member="bindings.imageUrlKey" section="options" label="imageUrlKey" /> <ApiLink type="igTree" member="bindings.imageUrlXPath" section="options" label="imageUrlXPath" /> |
...
![](../../images/images/igTree_Configure_Node_Images_Pic01.png)
...
---|---|---
[URL を使用して親ノードおよびリーフ ノードの画像を構成する](#example_configure_using_url)|<ApiLink type="igTree" member="parentNodeImageUrl" section="options" label="parentNodeImageUrl" /><br/><ApiLink type="igTree" member="leafNodeImageUrl" section="options" label="leafNodeImageUrl" />|string URL<br/>string URL
[CSS を使用して親ノードおよびリーフ ノードの画像を構成する](#example_configure_using_css)|<ApiLink type="igTree" member="parentNodeImageClass" section="options" label="parentNodeImageClass" /><br/><ApiLink type="igTree" member="leafNodeImageClass" section="options" label="leafNodeImageClass" />|string CSS クラス名<br/>string CSS クラス名
[バインディングにより個々のデータ項目のノード イメージを構成する](#example_configure_through_binding)|<ApiLink type="igTree" member="bindings.imageUrlKey" section="options" label="imageUrlKey" /><br/>(XML の場合)<br/><ApiLink type="igTree" member="bindings.imageUrlXPath" section="options" label="imageUrlXPath" />|画像への URL がある string 型データ メンバー <br/> XML にバインドされた場合の画像 URL への string 型 XPath
...
<table class="table tale-bordered">
	...
	<tbody>
        <tr>
            ...
            <td rowspan="2">![](../../images/images/igTree_Configure_Node_Images_Pic02.png)</td>
</tr>
...
オプション|設定|プレビュー
---|---|---
<ApiLink type="igTree" member="bindings.imageUrlKey" section="options" label="imageUrlKey" /> | ImageUrl|![](../../images/images/igTree_Configure_Node_Images_Pic03.png)
...
-   [igTree でノードを構成する](/igtree-configure-nodes)
