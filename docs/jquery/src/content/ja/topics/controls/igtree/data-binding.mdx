...
データ ソース|バインディング
---|---
[igDataSource](/data-sources/igdatasource/igdatasource-overview)|`igDataSource` は、コントロールのデータ操作を管理するために `igTree` コントロールによって内部的に使用されます。このデータ ソースは、さまざまなタイプのローカル データやリモート データを受け入れます。
...
-   [igTree のリモート ロード オン デマンドを構成する](/controls/igtree/optimize-performance)
