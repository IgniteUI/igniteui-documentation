...
視覚要素|プロパティ
---|---
ボディ|[contentTemplate](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:contentTemplate)
ヘッダー|[headerTemplate.title](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:headerTemplate.title)
閉じるボタン|[headerTemplate.closeButton](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:headerTemplate.closeButton)
...
[**シングル/マルチ ターゲット**](#single-multiple-targets)|ポップオーバーをマルチ ターゲットに表示されるように構成できます。オプションの [selectors](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:selectors) を使用して、ポップオーバーを表示する要素を指定できます。
...
ポップオーバーが表示されるターゲットの側面を「direction」と呼び、[direction](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:direction) オプションを設定して構成します。

ポップオーバーがターゲット要素よりも小さい場合は、常にターゲットである DOM 要素の表示領域の中央に表示されます。

ポップオーバーがターゲットよりも大きい場合は、ターゲットである DOM 要素に沿った位置に構成することもできます。これは [position](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:position) オプションで実行できます。

さらに、ポップオーバーを表示できる領域を制限する、境界線が機能するコンテナー (DIV 要素など) で、位置を制限することもできます。ポップオーバーを表示できる制限領域は、ポップオーバーの [containment](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:containment) で構成できます。
...
-   [igPopover の構成](/controls/igpopover/configuring-igpopover)
...
ポップオーバーを表示するイベントを制御できます。[showOn](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:showOn) オプションで制御します。`igPopover` でアクティブ化できるユーザー操作は以下のとおりです。
...
ポップオーバーをマルチ ターゲットに表示されるように構成できます。[selectors](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:selectors) オプションを使用して、ポップオーバーを表示する要素を指定できます。
...
<table class="table table-bordered">
	...
	<tbody>
        ...
        <tr>
            ...
            <td>デフォルトでは、`igPopover` はシングル要素で初期化されます。[selectors](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:selectors) オプションに設定すると、マルチ ターゲットを構成できます。 option.</td>
...
ポップオーバーが表示するターゲット要素に対する [direction](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:direction) と [position](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:position) も構成できます。
...
- [igPopover の構成](/controls/igpopover/configuring-igpopover): このトピックでは、`igPopover` コントロールの機能および動作を構成する方法を説明します。
