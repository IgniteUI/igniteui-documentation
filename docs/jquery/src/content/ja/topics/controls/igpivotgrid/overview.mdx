...
`igPivotGrid` コントロールのコンテンツが `igGrid` コントロールにより描画されます。グリッドのオプションが <ApiLink type="igPivotGrid" member="gridOptions" section="options" label="gridOptions" /> で設定できます。
igGrid の以下の機能は gridOptions.<ApiLink type="igPivotGrid" member="gridOptions.features" section="options" label="features" /> オプションで有効にできます:
- サイズ変更
- ツールチップ
...
並べ替えの適用|並べ替えボタン。ユーザーは 1 つ以上の列の値を並べ替えしたり、特定のレベルのメンバーヘッダーを並べ替えできます。|ユーザーの並べ替えの他、特定のレベルに対する最初の並べ替え方向は <ApiLink type="igPivotGrid" label="igPivotGrid プロパティ" />を介して設定できます。|![](../../images/images/positive.png)<ul><li> [並べ替え(サンプル)](\{environment:SamplesUrl\}/pivot-grid/sorting)</li></ul>
...
`igPivotGrid` コントロールを使用した必要なリソースの詳細なリストについては、「[igPivotView の HTML ページへの追加](/igpivotdataselector-adding-to-html-page)」を参照してください。
