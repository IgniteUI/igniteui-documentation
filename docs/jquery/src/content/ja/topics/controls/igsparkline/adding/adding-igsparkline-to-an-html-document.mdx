...
このトピックは、[`igSparkline`](&#123;environment:jQueryApiUrl&#125;/ui.igSparkline.html)™ を HTML ページに追加し JavaScript 配列へバインドする方法を示します。
...
- [igSparkline の概要](/controls/igsparkline/overview): このトピックは、`igSparkline` コントロールの概要、その利点、およびサポートされるチャート タイプを提供します。

- [igSparkline の追加の概要](/controls/igsparkline/adding/adding-igsparkline-overview): このトピックでは、`igSparkline`™ をアプリケーションに追加する各種方法の概要について説明します。

- [必要なリソースの手動で追加する](/general-and-getting-started/adding-the-required-resources-for-igniteui-for-jquery): このトピックでは、&#123;environment:ProductName&#125;®での JavaScript リソースの構成について説明します。
...
- [igSparkline を ASP.NET MVC ビューに追加](/controls/igsparkline/adding/adding-igsparkline-to-an-aspnet-mvc-view): このトピックでは、ASP.NET MVC ビューでの `igSparkline` インスタンス作成とオブジェクトの .NET コレクションへのバインドを見てみます。

- [jQuery と MVC API リンク (igSparkline)](/controls/igsparkline/jquery-and-aspnet-mvc-api): このトピックでは、`igSparkline` コントロールのための jQuery と ASP.NET MVC ヘルパー クラスのAPIドキュメントへのリンクを提供します。
