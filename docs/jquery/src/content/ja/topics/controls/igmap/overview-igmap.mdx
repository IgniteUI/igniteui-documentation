...
| 地理記号シリーズ | 地理座標に基づいて、マップ上のマーカーを視覚化します。, 関連トピック: [地理シンボル シリーズの構成](/controls/igmap/configuring/series/configuring-geographic-symbol-series) | geographicSymbol | [latitudeMemberPath](environment:jQueryApiUrl/ui.igMap#options:series.latitudeMemberPath) [longitudeMemberPath](environment:jQueryApiUrl/ui.igMap#options:series.longitudeMemberPath) |
| 地理図形シリーズ | シェープ (SHP) ファイルまたはカスタム形状データ ソースが提供する形状データに基づいて、国や地域などの、マップ上の囲まれた領域を視覚化します。, 関連トピック: [地理図形シリーズの構成](/controls/igmap/configuring/series/configuring-geographic-shapes) | geographicShape | [shapeDataSource](environment:jQueryApiUrl/ui.igMap#options:series.shapeDataSource) [databaseSource](environment:jQueryApiUrl/ui.igMap#options:series.databaseSource) [shapeMemberPath](environment:jQueryApiUrl/ui.igMap#options:series.shapeMemberPath) |
| 地理的ポリライン シリーズ | シェープ (SHP) ファイルまたはカスタム形状データ ソースが提供する形状データに基づいて、道路、川、または毎日の供給路などの、マップ上の開いたパスを視覚化します。, 関連トピック: [地理ポリライン シリーズの構成](/controls/igmap/configuring/series/configuring-geographic-polyline-series) | geographicPolyline | [shapeDataSource](environment:jQueryApiUrl/ui.igMap#options:series.shapeDataSource) [databaseSource](environment:jQueryApiUrl/ui.igMap#options:series.databaseSource) [shapeMemberPath](environment:jQueryApiUrl/ui.igMap#options:series.shapeMemberPath) |
| 地理散布エリアシリーズ | 三角測量 (ITF) ファイルまたはその他のカスタム三角測量データ ソースで定義される不規則三角網 (TIN) に基づいて、色分けされた地理領域を表示します。地理座標および値メンバーも必要です。これは地形高度、人口密度、平均/現在の気温などを示すために使用されます。, 関連トピック: [地理散布エリア シリーズの構成](/controls/igmap/configuring/series/configuring-geographic-scatter-area-series) | geographicScatterArea | [triangulationDataSource](environment:jQueryApiUrl/ui.igMap#options:series.triangulationDataSource) オプション: [triangleVertexMemberPath1](environment:jQueryApiUrl/ui.igMap#options:series.triangleVertexMemberPath1) [triangleVertexMemberPath2](environment:jQueryApiUrl/ui.igMap#options:series.triangleVertexMemberPath2) [triangleVertexMemberPath3](environment:jQueryApiUrl/ui.igMap#options:series.triangleVertexMemberPath3) [longitudeMemberPath](environment:jQueryApiUrl/ui.igMap#options:series.longitudeMemberPath) [latitudeMemberPath](environment:jQueryApiUrl/ui.igMap#options:series.latitudeMemberPath) [colorMemberPath](environment:jQueryApiUrl/ui.igMap#options:series.colorMemberPath) |
| 地理等高線シリーズ | 三角測量 (ITF) ファイルまたはその他のカスタム三角測量データ ソースで定義される不規則三角網 (TIN) に基づいて、色付けされた閉じたパスを視覚化します。地理座標および値メンバーも必要です。これは地形高度、人口密度、平均/現在の気温などを示すために使用されます。, 関連トピック: [地理等高線シリーズの構成](/controls/igmap/configuring/series/configuring-geographic-contour-line-series) | geographicContourLine | [triangulationDataSource](environment:jQueryApiUrl/ui.igMap#options:series.triangulationDataSource) オプション: [triangleVertexMemberPath1](environment:jQueryApiUrl/ui.igMap#options:series.triangleVertexMemberPath1) [triangleVertexMemberPath2](environment:jQueryApiUrl/ui.igMap#options:series.triangleVertexMemberPath2) [triangleVertexMemberPath3](environment:jQueryApiUrl/ui.igMap#options:series.triangleVertexMemberPath3) [longitudeMemberPath](environment:jQueryApiUrl/ui.igMap#options:series.longitudeMemberPath) [latitudeMemberPath](environment:jQueryApiUrl/ui.igMap#options:series.latitudeMemberPath) [valueMemberPath](environment:jQueryApiUrl/ui.igMap#options:series.valueMemberPath) |
...
>**注:** `igLoader` コントロールを使用して JavaScript および CSS リソースを読み込むことが推奨されます。`igLoader` コントロールを `igMap` と共に使用する方法の例は、[igMap の追加](/controls/igmap/adding-igmap)のトピックおよび[地理シンボル シリーズ シリーズ](&#123;environment:SamplesUrl&#125;/map/geo-symbol-series)のサンプルを参照してください。
...
ズーム機能によって、ズームインして特定のマップ領域を公開したり、ズームアウトしてよりスケールの大きいビューを表示したりすることができます。ズームインとズームアウトは、キーボードおよび/またはマウスを使って、タッチ デバイスでタッチ、ホールド、ドラッグして、あるいは Overview Plus Detail (OPD) 画面の支援によって、ユーザーが直接実行できます。ズームは、`igMap` コントロールの [windowRect](&#123;environment:jQueryApiUrl&#125;/ui.igMap#options:windowRect) オプションを使用して、コードによって実行することもできます。
...
-   [ナビゲーション機能の構成](/controls/igmap/configuring/features/configuring-navigation-features)

###<a id="panning"></a> パンニング

パンによって、現在表示されているウィンドウをマップから North、South、East、または West に移動させることができます。パンは、キーボードおよび/またはマウスを使って、タッチ デバイスでタッチ、ホールド、ドラッグして、あるいは Overview Plus Detail (OPD) 画面の支援によって、ユーザーが直接実行できます。ズームは、`igMap` コントロールの [windowRect](&#123;environment:jQueryApiUrl&#125;/ui.igMap#options:windowRect) オプションを使用して、コードによって実行することもできます。
...
-   [ビジュアル機能の構成](/controls/igmap/configuring/features/configuring-visual-features)
...
-   [スタイル設定 (igMap)](/controls/igmap/styling-igmap)
...
| ズーム | マウス ドラッグ Ctrl/Alt/Shift + マウス ドラッグ マウス スクロール デュアル タッチ ドラッグ Page Up/Page Down | ズームインすると、地域の詳細表示が可能になります。ズームアウトすると、領域の広範なビューが表示されます。 | [ナビゲーション機能の構成](/controls/igmap/configuring/features/configuring-navigation-features) |
| パン | マウス ドラッグ Ctrl/Alt/Shift + マウス ドラッグ タッチ アンド ドラッグ 矢印キー | ビューを水平または垂直に移行し、隣接した領域を表示します。 | [ナビゲーション機能の構成](/controls/igmap/configuring/features/configuring-navigation-features) |
| 移動する | OPD パネル | マップの現在表示されている部分の相対位置を示します。ズーム用のボタンとスライダーを含み、現在表示されているウィンドウをパンできます。 | [ナビゲーション機能の構成](/controls/igmap/configuring/features/configuring-navigation-features) |
| ビュー ツールチップ | マウス ホバー タッチ アンド ホールド | ユーザーにツールチップを表示します。 | [ビジュアル機能の構成](/controls/igmap/configuring/features/configuring-visual-features) |
...
-   [データ バインディング (igMap)](/controls/igmap/data-binding-igmap)
...
-	[マップのスタイル設定 (igMap)](/controls/igmap/styling-igmap): このトピックでは、テーマを使用して `igMap`™ コントロールのルック アンド フィールをカスタマイズする方法を説明します。

-	[機能の構成 (igMap)](/controls/igmap/configuring/features/configuring-features): このグループのトピックでは、`igMap`™ コントロールのさまざまな機能の構成方法を説明します。対応する機能としては、特定の地理領域へのナビゲーション、Overview Plus Detail パネルの有効または無効化、表示中の領域をマップ上で取得、パンとズームに関するユーザー インタラクションの構成、ツールチップ テンプレートの構成、カスタム マーカーの設定などがあります。
...
-	[マップのツールチップ](/controls/igmap/configuring/features/configuring-visual-features#map-tooltips-sample): このサンプルでは、`igMap` コントロールのマップ ツールチップを設定する方法を紹介します。
