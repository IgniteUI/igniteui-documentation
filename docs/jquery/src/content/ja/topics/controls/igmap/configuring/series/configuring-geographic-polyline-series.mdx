...
ポリラインの色は、シリーズ オブジェクトの CSS スタイルやオプションを使用して管理できます。詳細については、トピック[igMap のスタイル設定](/controls/igmap/styling-igmap) を参照してください。
...
| 構成可能な項目 | 詳細 | プロパティ |
| --- | --- | --- |
| シリーズ タイプの設定 | 当該の必須設定値は、地理ポリラインのマップ シリーズのタイプを構成し、シリーズ名を設定します。 | **JavaScript の場合:** [series.type](environment:jQueryApiUrl/ui.igMap#options:series.type) [series.name](environment:jQueryApiUrl/ui.igMap#options:series.name) 値:, **series.type: “geographicPolyline”** **series.type: “seriesName”**, ASP.NET MVC の場合:, [MapSeriesBuilder クラス](Infragistics.Web.Mvc~Infragistics.Web.Mvc.MapSeriesBuilder`1.html) [.GeographicPolyline()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.MapSeriesBuilder`1~GeographicPolyline.html) 値:, **series.GeographicPolyline(“seriesName”)** |
| データ バインドのオプション | 当該の必須設定値は、シェイプ ファイル、データベース ファイル、またはカスタム データ ソースに収められる URL を構成します。 | JavaScript の場合: [series.shapeDataSource](environment:jQueryApiUrl/ui.igMap#options:series.shapeDataSource) [series.databaseSource](environment:jQueryApiUrl/ui.igMap#options:series.databaseSource) ASP.NET MVC の場合: [GeographicPolylineSeries クラス](Infragistics.Web.Mvc~Infragistics.Web.Mvc.GeographicPolylineSeries`1.html) [.ShapeDataSource()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.GeographicShapeSeriesBase`3~ShapeDataSource.html) [.DatabaseSource()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.GeographicShapeSeriesBase`3~DatabaseSource.html) |
| ポリラインの色 | この項目はポリラインの色を構成します。輪郭線の既定色は白になっています。 | JavaScript の場合: [series.outline](environment:jQueryApiUrl/ui.igMap#options:series.outline) [series.shapeStyle.stroke](environment:jQueryApiUrl/ui.igMap#options:series.shapeStyle) ASP.NET MVC の場合: [GeographicPolylineSeries クラス](Infragistics.Web.Mvc~Infragistics.Web.Mvc.GeographicPolylineSeries`1.html) [.Outline()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.Series`3~Outline.html) [.ShapeStyle()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.GeographicShapeSeriesBase`3~ShapeStyle.html) |
| ポリラインの太さ | この項目はポリラインの太さを構成します。太さの既定値は 0 になっています。 | JavaScript の場合: [series.thickness](environment:jQueryApiUrl/ui.igMap#options:series.thickness) [series.shapeStyle.thickness](environment:jQueryApiUrl/ui.igMap#options:series.shapeStyle) ASP.NET MVC の場合: [GeographicPolylineSeries クラス](Infragistics.Web.Mvc~Infragistics.Web.Mvc.GeographicPolylineSeries`1.html) [.Thickness()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.Series`3~Thickness.html) [.ShapeStyle()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.GeographicShapeSeriesBase`3~ShapeStyle.html) |
...
-	[機能の構成 (igMap)](/controls/igmap/configuring/features/configuring-features): このグループのトピックでは、`igMap` コントロールのさまざまな機能を構成する方法を説明します。対応する機能としては、特定の地理領域へのナビゲーション、Overview Plus Detail パネルの有効または無効化、表示中の領域をマップ上で取得、パンとズームに関するユーザー インタラクションの構成、ツールチップ テンプレートの構成、カスタム マーカーの設定などがあります。
