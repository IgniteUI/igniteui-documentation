...
指定した時間範囲のすべての予定のコレクションを [getAppointmentsInRange](&#123;environment:jQueryApiUrl&#125;/ui.igscheduler#methods:getAppointmentsInRange) メソッドによって取得できます。 
...
予定は、予定オブジェクトを引数として渡して [createAppointment](&#123;environment:jQueryApiUrl&#125;/ui.igscheduler#methods:createAppointment) メソッドで追加できます。
...
予定は、予定オブジェクトおよび変更するプロパティを含むオブジェクトを引数として渡して [editAppointment](&#123;environment:jQueryApiUrl&#125;/ui.igscheduler#methods:editAppointment) メソッドで編集できます。
...
予定は、予定オブジェクトを引数として渡して [deleteAppointment](&#123;environment:jQueryApiUrl&#125;/ui.igscheduler#methods:deleteAppointment) メソッドで削除できます。
