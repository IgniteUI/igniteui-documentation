...
4. 上記のセットアップが完了したら、*ID*、*columns*、*dataSource* などの**オプションを設定**します。
    ...
    1.  <ApiLink type="iggrid" member="columns" section="options" label="columns" /> - `igGrid` の列オブジェクト定義。
    ...
    2.  <ApiLink type="iggrid" member="dataSource" section="options" label="dataSource" /> - `igGrid` がデータを表示しているデータ。次のようなオプションがあります。
	    -   JSON オブジェクト
	    -   JavaScript 配列
	    -   XML
	    -   リモート データ
