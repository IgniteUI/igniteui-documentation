...
---|---|---
[デフォルト設定でツールチップを表示する](#default-settings)|デフォルト設定でツールチップを表示するには、ツールチップ機能を呼び出します。|[name](&#123;environment:jQueryApiUrl&#125;/ui.iggridtooltips#options:name)
[ツールチップのないセル列を構成する](#disable)|たとえば、列のセルに画像が含まれている場合に、その列のツールチップを無効にすると便利です。この場合、列ごとに個々にツールチップのビヘイビアーを構成します。|[visibility](&#123;environment:jQueryApiUrl&#125;/ui.iggridtooltips#options:visibility) [columnSettings](&#123;environment:jQueryApiUrl&#125;/ui.iggridtooltips#options:columnSettings)
[カスタム ツールチップ位置を構成する](#custom)|ツールチップを表示させる位置を指定する。マウス ポインターからツールチップのオフセットを指定することでこれを構成します。|[cursorLeftOffset](&#123;environment:jQueryApiUrl&#125;/ui.iggridtooltips#options:cursorLeftOffset) [cursorTopOffset](&#123;environment:jQueryApiUrl&#125;/ui.iggridtooltips#options:cursorTopOffset)
...
> **注:** MVC で同様の操作を行う方法については、「[igGrid のツールチップを有効にする](/controls/iggrid/features/tooltips/enabling-tooltips)」トピックを参照してください。
...
- [igGrid のツールチップを有効にする](/controls/iggrid/features/tooltips/enabling-tooltips)
