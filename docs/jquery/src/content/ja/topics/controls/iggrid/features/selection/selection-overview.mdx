...
`igGridSelection` を有効にする場合、[`persist`](&#123;environment:jQueryApiUrl&#125;/ui.iggridselection#options:persist) モードにあります。つまり、選択した行およびセルは、グリッドの並べ替え、列フィルタリングなどのその他の機能と操作したか、データ バインドした後に選択は残ります。
...
ユーザーがグリッドと操作した後に選択をクリアする現在の動作に戻るには、[`persist`](&#123;environment:jQueryApiUrl&#125;/ui.iggridselection#options:persist) オプションで機能を無効にできます。以下はコード スニペットです。
...
igHierarchicalGrid の選択では、デフォルトで UP/DOWN/LEFT/RIGHT キーでの子グリッドへの移動を スキップします。([skipChildren](&#123;environment:jQueryApiUrl&#125;/ui.iggridselection#options:skipChildren) オプションを参照)。
