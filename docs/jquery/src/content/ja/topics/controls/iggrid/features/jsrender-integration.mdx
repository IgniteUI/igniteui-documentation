...
- [列テンプレートの構成 (igGrid、RWD モード)](/controls/iggrid/features/responsive/configure/responsive-web-design-mode-configuring-row-and-column-templates): このトピックは、コード例を用いて `igGrid`™ コントロールの各 Responsive Web Design (RWD) モード 構成に対して列テンプレートを定義する方法、およびアクティブな RWD モード構成の切り替え時のテンプレートの自動変更を構成する方法について説明します。
