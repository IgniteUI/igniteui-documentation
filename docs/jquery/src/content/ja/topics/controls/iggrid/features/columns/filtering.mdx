...
`igGridFiltering` を有効する場合、<ApiLink type="iggridfiltering" member="persist" section="options" label="persist" /> モードで使用されます。**dataBind()** への呼び出しの後、UI およびデータ ソース ビューで永続化が適用されます。フィルター エディターはクリアされなく、データ ソースのフィルターも残ります。
...
ユーザーが *igGrid* を再バインドした後にフィルターをクリアする以前の動作に戻るには、<ApiLink type="iggridfiltering" member="persist" section="options" label="persist" /> オプションで機能を無効できます。以下はコード スニペットです。
...
![](../../../../images/images/simple.png)

図 1: 簡易フィルタリング (モード:「簡易」)

![](../../../../images/images/advanced.png)

図 2: 詳細フィルタリング (モード:「詳細」)

![](../../../../images/images/dialog.png)

図 3: 詳細フィルタリング ダイアログ

![](../../../../images/images/tooltip.png)
...
リスト 6: &#123;environment:ProductNameMVC&#125; で使用する Razor または CSHTML マークアップ
...
&#123;environment:ProductNameMVC&#125; を使用して LINQ (IQueryable) 経由でサーバー側データをバインドする場合、URL にエンコードされたすべてのフィルタリング情報は自動的に LINQ 式の句 (Where 句) に変換されるため、データをフィルターするのに余分な作業は必要ありません。
...
プロパティ|説明
---|---
<ApiLink type="iggridfiltering" member="columnSettings.columnKey" section="options" label="columnKey" /> | この列設定が適用される列のキー。columnKey と columnIndex は相互に排他的です。
<ApiLink type="iggridfiltering" member="columnSettings.columnIndex" section="options" label="columnIndex" /> | この列設定が適用される列のインデックス。columnKey と columnIndex は相互に排他的です。
<ApiLink type="iggridfiltering" member="columnSettings.allowFiltering" section="options" label="allowFiltering" /> | 該当の列でフィルタリングが有効か無効かを指定します。デフォルトは true です。
<ApiLink type="iggridfiltering" member="columnSettings.condition" section="options" label="condition" /> | 列に使用される[デフォルト フィルタリング条件](#default-conditions-list)。
<ApiLink type="iggridfiltering" member="columnSettings.conditionList" section="options" label="conditionList" /> | 列で使用可能な条件を定義する文字列の配列。 未設定の場合、列のデータ型ごとのデフォルトの条件、そして `customConditions` で定義された条件が適用されます。
<ApiLink type="iggridfiltering" member="columnSettings.customConditions" section="options" label="customConditions" /> | [カスタムのフィルター条件](#customCond)を定義しているオブジェクト (スキーマは左記の通り:  &#123;labelText: "/*dataType='string'*/", expressionText: "/*dataType='string'*/", requireExpr: /*dataType='bool'*/, filterImgIcon: "/*dataType='string'*/", filterFunc: /*dataType='string or function'*/ &#125;)。 
<ApiLink type="iggridfiltering" member="columnSettings.defaultExpressions" section="options" label="defaultExpressions" /> | 初期フィルター式の配列。フィルター式オブジェクトのスキーマは次のように定義されます: &#123;fieldName: "/*dataType='string'*/", cond: "/*dataType='string'*/" expr: "/*dataType='string'*/"&#125;
...
| クライアント側イベント名 | 説明 | 引数 |
| --- | --- | --- |
| dataFiltering | フィルタリング要求を行う前に発生するイベント (ローカルまたはリモート)です | columnKey: フィルタリングのトリガー元となる列キー, columnIndex: 列のインデックス, owner: フィルタリング ウィジェット オブジェクトへの参照 |
| dataFiltered | データがフィルターされ、グリッドで描画された後に発生するイベント | dataFiltering と同じです **注:** 式の配列は、`dataFiltering` イベント関数 (ui.expressions) から取得できます。 |
| dropDownOpening | フィルタリング ドロップダウンを開く直前に発生するイベント | dropDown: フィルタリング ドロップ ダウンへの参照, owner: フィルタリング ウィジェット オブジェクトへの参照 |
| dropDownOpened | フィルタリング ドロップダウンを開いた後に発生するイベントです。 | dropDownOpening と同じです |
| dropDownClosing | フィルタリング ドロップダウンを閉じる直前に発生するイベント | dropDownOpening と同じです |
| dropDownClosed | フィルタリング ドロップダウンを閉じた後に発生するイベント | dropDownOpening と同じです |
| filterDialogOpening | 詳細フィルタリング ダイアログを開く前に発生するイベント | dialog: jQuery オブジェクト ラッパーにおけるフィルタリング要素への参照, owner: フィルター ウィジェット オブジェクトへの参照 |
| filterDialogOpened | 詳細フィルタリング ダイアログを開いた後に発生するイベント | filterDialogOpening と同じです |
| filterDialogMoving | 詳細フィルタリング ダイアログを移動しているときに発生するイベント | filterDialogOpening と同じです |
| filterDialogFilterAdding | 新しいフィルターが詳細フィルタリング ダイアログに追加される前に発生するイベント | filtersTableBody: jQuery オブジェクトでラップされた任意のフィルターを保持する TABLE 要素への参照, owner: フィルタリング ウィジェット オブジェクトへの参照 |
| filterDialogFilterAdded | 新しいフィルターが詳細フィルタリング ダイアログに追加された後に発生するイベント | filter: jQuery オブジェクト ラッパーにおける filterRow 要素への参照, owner: フィルタリング ウィジェット オブジェクトへの参照 |
| filterDialogClosing | 詳細フィルタリング ダイアログが閉じる直前に発生するイベント | owner: 呼び出しウィジェット (これ) への参照 |
| filterDialogClosed | 詳細フィルタリング ダイアログを閉じた後に発生するイベント | owner: 呼び出しウィジェット (これ) への参照 |
| filterDialogContentsRendering | コンテンツが詳細フィルタリング ダイアログに描画される前に発生するイベント | dialogElement: コンテンツが jQuery オブジェクト ラッパーで描画されるダイアログ要素, owner: フィルタリング ウィジェット オブジェクトへの参照 |
| filterDialogContentsRendered | コンテンツが詳細フィルタリング ダイアログに描画された後に発生するイベント | filterDialogContentsRendering と同じです |
| filterDialogFiltering | 詳細フィルタリング ダイアログで OK ボタンを押し、フィルタリングを開始するときに発生するイベント **注:** それでも dataFiltering イベントと dataFiltered イベントはこのイベントの後に発生します。 | filterDialogOpening と同じです |
...
条件は関連する列の列設定で <ApiLink type="iggridfiltering" member="columnSettings.customConditions" section="options" label="customConditions" /> オプションを使用して定義します。
...
**サンプル:**
<div class="embed-sample">
　  [igGrid フィルタリングのカスタム条件](&#123;environment:SamplesEmbedUrl&#125;/grid/custom-conditions-filtering)
</div>
...
-   [フィルタリング](&#123;environment:SamplesUrl&#125;/grid/simple-filtering)
