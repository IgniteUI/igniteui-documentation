...
| 列テンプレート | 各 RWD モード プロファイルに対して個別の列テンプレートを指定できます。列テンプレートは、各列に対して個別に定義されます。 **関連トピック** [列テンプレートを構成する](/iggrid-responsive-web-design-mode-configuring-row-and-column-templates) | [columnSettings](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings) [columnSettings.columnKey](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.columnKey) [columnSettings.configuration.desktop.template](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.configuration.desktop.template) [columnSettings.configuration.tablet.template](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.configuration.tablet.template) [columnSettings.configuration.phone.template](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.configuration.phone.template) columnSettings.configuration.&lt;custom_mode&gt;.template |
| RWD モードのプロファイル | 事前に定義されたプロファイルに加えてカスタム RWD モードを指定できます。 **関連トピック** [カスタム レスポンス Web デザイン (RWD) モード プロファイルの作成](/iggrid-responsive-web-design-mode-creating-custom-profile) | [responsiveModes](environment:jQueryApiUrl/ui.iggridresponsive#options:responsiveModes) responsiveModes.&lt;custom_mode&gt; |
