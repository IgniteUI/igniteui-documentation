---
title: "レスポンス Web デザイン (RWD) モードの概要 (igGrid)"
slug: iggrid-responsive-web-design-mode-overview
---
...
- トピック
    ...
    - [igGrid の機能](/controls/iggrid/features/features-landing-page): このトピックは、`igGrid` コントロールの機能について説明します。
- 外部リソース
...
-----------------------------|--------------------------------
![](../../../../images/images/ResponsiveDesignMode_Overview_1.png) | ![](../../../../images/images/ResponsiveDesignMode_Overview_2.png)
...
![](../../../../images/images/ResponsiveDesignMode_Overview_3.png)

**関連トピック**

-   [列テンプレートを構成する](/controls/iggrid/features/responsive/configure/responsive-web-design-mode-configuring-row-and-column-templates)
...
![](../../../../images/images/ResponsiveDesignMode_Overview_4.png)

タブレット プロファイル (768 x 1024 px)|携帯電話 プロファイル (320 x 480 px)
-----------------------------|--------------------------------
![](../../../../images/images/ResponsiveDesignMode_Overview_5.png) | ![](../../../../images/images/ResponsiveDesignMode_Overview_6.png)

RWD プロファイル テンプレートが構成されると、テンプレートはプロファイルのアクティブ化により自動的に切り替わります。

**関連トピック**

-   [個別の RWD モード プロファイル用のテンプレートの構成](/controls/iggrid/features/responsive/configure/responsive-web-design-mode-configuring-row-and-column-templates)
...
-   [カスタム レスポンス Web デザイン (RWD) モード プロファイルの作成](/controls/iggrid/features/responsive/configure/responsive-web-design-mode-creating-custom-profile)
...
-   [ブートストラップ サポートの構成](/controls/iggrid/features/responsive/configure/responsive-web-design-mode-configuring-bootstrap-support)
...
| --- | --- | --- |
| 列の非表示 | 各 RWD モード プロファイルにおいて、どの列を表示または非表示にするかを指定できます。 **関連トピック** [列の非表示の構成](/controls/iggrid/features/responsive/configure/responsive-web-design-mode-configuring-column-hiding) | CSS メディア クエリー ベースで構成する場合 (CSS クラスを使用): [columnSettings](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings) [columnSettings.columnKey](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.columnKey) [columnSettings.classes](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.classes) 列単位で構成する場合 (構成設定を使用): [columnSettings](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings) [columnSettings.configuration.desktop.hidden](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.configuration) [columnSettings.configuration.tablet.hidden](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.configuration) [columnSettings.configuration.phone.hidden](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.configuration) columnSettings.configuration.&lt;custom_mode&gt;.hidden |
| 列テンプレート | 各 RWD モード プロファイルに対して個別の列テンプレートを指定できます。列テンプレートは、各列に対して個別に定義されます。 **関連トピック** [列テンプレートを構成する](/controls/iggrid/features/responsive/configure/responsive-web-design-mode-configuring-row-and-column-templates) | [columnSettings](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings) [columnSettings.columnKey](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.columnKey) [columnSettings.configuration.desktop.template](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.configuration.desktop.template) [columnSettings.configuration.tablet.template](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.configuration.tablet.template) [columnSettings.configuration.phone.template](environment:jQueryApiUrl/ui.iggridresponsive#options:columnSettings.configuration.phone.template) columnSettings.configuration.&lt;custom_mode&gt;.template |
| RWD モードのプロファイル | 事前に定義されたプロファイルに加えてカスタム RWD モードを指定できます。 **関連トピック** [カスタム レスポンス Web デザイン (RWD) モード プロファイルの作成](/controls/iggrid/features/responsive/configure/responsive-web-design-mode-creating-custom-profile) | [responsiveModes](environment:jQueryApiUrl/ui.iggridresponsive#options:responsiveModes) responsiveModes.&lt;custom_mode&gt; |
| ブートストラップのサポート | Twitter Bootstrap の RWD クラスを使用して RWD モードを構成できます。 **関連トピック** [ブートストラップ サポートの構成](/controls/iggrid/features/responsive/configure/responsive-web-design-mode-configuring-bootstrap-support) | `.visible-phone` `.visible-tablet` `.visible-desktop` `.hidden-phone` `.hidden-tablet` `.hidden-desktop` |
...
デフォルトでは、RWD モードは RWD モード構成のプロバイダーとして `$.ig.InfragisticsMode` クラスを使用するよう構成されます。これは [`responsiveModes`](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:responsiveModes) オプションで実施されます。これには、プロファル切り替えのロジックを実装するクラス名を含む各プロファイルが文字列である RWD モード プロファイルが含まれます。以下の表は、各プロパティのデフォルト設定のリストです。

プロパティ|タイプ|デフォルト値|説明
---|---|---|---
[responsiveModes.desktop](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:responsiveModes.desktop) |string|“infragistics” |デスクトップ プロファイルは `$.ig.InfragisticsMode` クラスで提供されます。
[responsiveModes.tablet](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:responsiveModes.tablet) |string|“infragistics”|タブレット プロファイルは `$.ig.InfragisticsMode` クラスで提供されます。
[responsiveModes.phone](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:responsiveModes.phone) |string|“infragistics”|携帯電話プロファイルは `$.ig.InfragisticsMode` クラスで提供されます。
...
- [レスポンス Web デザイン (RWD) モードの構成 (igGrid)](/controls/iggrid/features/responsive/configure/configuring-responsive-web-design-mode-landingpage): このグループのトピックは、列非表示の構成、カスタムの行おより列のテンプレートの作成、カスタム RWD の構成の作成および Twitter Bootstrap で使用するための RWD モードの構成など、`igGrid` コントロールにおける レスポンス Web デザイン (RWD) モード関連の各種構成タスクを説明します。
