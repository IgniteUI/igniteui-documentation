...
- [igGrid の概要](/controls/iggrid/overview): このトピックでは、igGrid を Web ページに追加する方法について説明します。

- [igGrid/igDataSource アーキテクチャの概要](/controls/iggrid/igdatasource-architecture-overview): このトピックでは、`igGrid` 内部の働きと、各種データ ソースとのバインディングを可能にするために igGrid が` gDataSource` とどのように協調するかを説明します。
...
- [group](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.group): 列に `group` プロパティがある場合、この列はグループ列で、データ ソースのデータにバインドされていません。親列は表示されているだけで、グリッドの THEAD でのみ描画されます。

- [groupOptions](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.groupOptions): `group` のオプション。

- [groupOptions.expanded](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.groupOptions.expanded): `group` を展開または縮小します。このプロパティは、`allowGroupCollapsing` が有効な場合のみに適用されます。

- [groupOptions.allowGroupCollapsing](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.groupOptions.allowGroupCollapsing): `group` の縮小/展開を許可し、グループ ヘッダーで展開インジケーターを表示します。

- [groupOptions.hidden](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.groupOptions.hidden): グループが非表示にされる条件を制御します。このプロパティは、`allowGroupCollapsing` が有効な場合のみに適用されます。グループを非表示にする 4 つのオプションがあります: なし、常に、親グループの縮小時と親グループの展開時。

- [headerText](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.headerText): 列のキャプション。

- [key](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.key): グループ列は、複数列ヘッダー機能が他の機能と併せて使用されている場合に、値を ID として使用する key プロパティを承諾します。たとえば、グループ列の `key` を使用して、最初にグループを非表示にします。しかし、`key` プロパティの値はデータ ソースからの列キーであってはなりません。`key` が指定されていない場合は、自動的に生成されます。各 `key` 値は `columns` 配列で一意でなければなりません。

- [rowspan](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.rowspan): テーブル ヘッダー セルの `rowspan` 属性を変更できます。
...
- [hidden](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.hidden): `hidden` プロパティを使用して、最初はグループ列を非表示にできます。


## <a id="method-reference"></a> 複数列ヘッダーのメソッド参照

メソッド [`renderMultiColumnHeader`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#methods:renderMultiColumnHeader) を使用して、ランタイム時に複数列ヘッダーを変更できます。このメソッドは、新しい階層の列の配列を承諾します。`renderMultiColumnHeader` を実行する場合、グリッド全体が再バインドされ、再描画されます。これが `igGrid.options.columns` 配列を変更する唯一の方法です。
...
[getMultiColumnHeaders](&#123;environment:jQueryApiUrl&#125;/ui.iggridmulticolumnheaders#methods:getMultiColumnHeaders)|複数列ヘッダー配列を返します。複数列ヘッダーが定義されていない場合は、*undefined* が返されます。|なし
[expandGroup](&#123;environment:jQueryApiUrl&#125;/ui.iggridmulticolumnheaders#methods:expandGroup) |グループの展開|グループ キー、コールバック
[collapseGroup](&#123;environment:jQueryApiUrl&#125;/ui.iggridmulticolumnheaders#methods:collapseGroup) |グループの縮小|グループ キー、コールバック
[toggleGroup](&#123;environment:jQueryApiUrl&#125;/ui.iggridmulticolumnheaders#methods:toggleGroup) |グループの切り替え|グループ キー、コールバック
...
- [複数列ヘッダーの構成 (igGrid)](/controls/iggrid/features/columns/multi-headers/multicolumnheaders-configuring): このトピックでは、`igGrid` での複数列ヘッダーの構成について説明しています。

- [列の非表示を構成する](/controls/iggrid/features/columns/hiding/configure-column-hiding): このトピックでは、コードで `igGrid` コントロールの列を構成する方法について説明します。

- [列のサイズ変更 (igGrid)](/controls/iggrid/features/columns/column-resizing): このトピックでは、`igGrid` コントロールの列サイズ変更機能について説明します。
