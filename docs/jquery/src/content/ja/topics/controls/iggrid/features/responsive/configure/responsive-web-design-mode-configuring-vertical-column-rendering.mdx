...
-   [選択](/iggrid-selection)機能は、選択が行識別のためにインデックスを使用する場合のみに UI により動作します。[primaryKey](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:primaryKey) を指定しなく、選択の永続化を無効にします。イベント引数、selectedCells/selectedRows プロパティ、および API は誤った結果を返します。
