...
- [igGrid の概要](/controls/iggrid/overview): このトピックでは、`igGrid` を Web ページに追加する方法を説明します。

- [igGrid/igDataSource アーキテクチャの概要](/controls/iggrid/igdatasource-architecture-overview): このトピックでは、`igGrid` の内部機能および `igDataSource` とどのように相互作用して異なるデータ ソースへのバインディングを可能にしているかを説明します。

- [igGrid の機能](/controls/iggrid/features/features-landing-page): これは、`igGrid` の全機能を説明するトピックのグループです。
...
一度に 1 つの列のみを移動できます。`igGrid` からの列の移動方法に関する詳細は、「[ユーザー インタラクションと操作性](#user-interactions)」を参照してください。コードによる列の移動方法については、「[プログラムによる列の移動](/controls/iggrid/features/columns/moving/columnmoving-movingcolumnsprogrammatically)」トピックを参照してください。

列移動は、個々の列において、または列のグループにおいて実施できます。列移動は[機能セレクター](/controls/iggrid/features/feature-chooser)でレンダリングされ、[複数列ヘッダー](/controls/iggrid/features/columns/multi-headers/multicolumnheaders-landingpage)などその他のすべての `igGrid` 機能と互換性があります。

列移動機能は、グリッド内のデータのみを再レンダリングします。データ ソースを変更することはありません。

列移動は、`igGrid` の `features` 配列に新機能オブジェクトを追加し、その機能オブジェクトの `name` プロパティを *ColumnMoving* に設定することで有効になります。有効になったら、明示的に指定されない限り列移動はすべての列で許可されます。詳細は、「[列移動の有効化](/controls/iggrid/features/columns/moving/columnmoving-enabling) 」トピックを参照してください。
...
列移動タイプは、type プロパティを介して構成します。詳細は、「[列移動の構成](/controls/iggrid/features/columns/moving/columnmoving-configuring) 」トピックを参照してください。
...
<table class="table table-bordered">
	...
	<tbody>
        <tr>
            ...
            <td>ドラッグ アンド ドロップを以下の 2 つの操作モードのいずれかで構成できます。マウスがドラッグされているときにグリッドが継続的に更新するかどうか、またはマウス ボタンを放したときにのみ更新されるかどうかに基づいて、Immediate または Deferred になります。 <ul> <li> [列移動の構成](/controls/iggrid/features/columns/moving/columnmoving-configuring) </li> </ul></td>
</tr>
        ...
        <tr>
            ...
            <td>列移動インターフェイスは、有効 (デフォルト) または無効になります。有効である場合、メニューとダイアログの両方を使用できます。無効の場合はどちらも使用できません。 <ul> <li> [列移動の構成](/controls/iggrid/features/columns/moving/columnmoving-configuring) </li> </ul></td>
</tr>
...
列は列ごとにドラッグ アンド ドロップから無効にできます。詳細は、「[列移動の構成](/controls/iggrid/features/columns/moving/columnmoving-configuring) 」トピックを参照してください。
...
列移動ドロップダウン メニューは、グリッド内すべての列を有効または無効にできますが、個々の列を有効または無効にすることはできません。メニューはデフォルトで有効です。[`addMovingDropdown`](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#addMovingDropdown) プロパティを false に設定すると無効になります。

**関連トピック**

-   [列移動の構成](/controls/iggrid/features/columns/moving/columnmoving-configuring)
-   [プロパティ リファレンス](/controls/iggrid/features/columns/moving/columnmoving-propertyreference)
...
列移動機能の[movingDialogWidth](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#movingDialogWidth)、[movingDialogHeight](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#movingDialogHeight)、[movingDialogAnimationDuration](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#movingDialogAnimationDuration) プロパティでダイアログの幅、高さおよびドラッグ アニメーション期間を構成できます。
...
| 構成可能な項目 | 詳細 | プロパティ |
| --- | --- | --- |
| モード | デフォルトでは[列移動モード](#mode)は即時です。代わりに遅延モードを構成できます。 | [mode](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#mode) |
| タイプ | デフォルトでは[列移動タイプ](#type)は DOM 操作です。代わりにグリッドの再レンダリングをタイプに構成できます。タイプは、ブラウザーによって機能パフォーマンスに異なった影響を与えます。 | [moveType](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#moveType) |
| 列 | どの列の移動を許可するかを指定できます。 | [columnSettings](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#columnSettings) [columnSettings.columnKey](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#columnKey) [columnSettings.allowMoving](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#allowMoving) |
| インターフェイス | グリッドの列移動インターフェイスを有効または無効にします。 | [addMovingDropdown](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#addMovingDropdown) |
...
プロパティ|タイプ|デフォルト値|説明
---|---|---|---
[mode](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#mode)|string|"immediate"|デフォルトの[列移動モード](#mode)は Immediate に設定されています。
[moveType](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#moveType)|string|“dom“ |デフォルトの[列移動タイプ](#type)は DOM 操作に設定されています。
[addMovingDropdown](/controls/iggrid/features/columns/moving/columnmoving-propertyreference#addMovingDropdown)|bool|true|列移動インターフェイスはユーザーに対して表示され操作可能です。
