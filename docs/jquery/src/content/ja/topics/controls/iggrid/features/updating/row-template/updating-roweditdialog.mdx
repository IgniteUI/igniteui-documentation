...
| プロパティ | タイプ | 説明 | デフォルト値 |
| --- | --- | --- | --- |
| [editMode](environment:jQueryApiUrl/ui.iggridupdating#options:editMode) | “row\|cell\|**dialog**\|none\|null” | `editMode` プロパティには、新しい値 **dialog** を追加しました。 | row |
...
パブリック API メソッド [startEdit](&#123;environment:jQueryApiUrl&#125;/ui.igGridUpdating#methods:startEdit) と [endEdit](&#123;environment:jQueryApiUrl&#125;/ui.igGridUpdating#methods:endEdit) では、行編集ダイアログを開閉できます。
...
|イベント|説明|
|---|---|
|[rowEditDialogBeforeOpen](&#123;environment:jQueryApiUrl&#125;/ui.iggridupdating#events:rowEditDialogBeforeOpen)|このイベントは行編集ダイアログが開く前に発生します。キャンセルはできません。|
|[rowEditDialogAfterOpen](&#123;environment:jQueryApiUrl&#125;/ui.iggridupdating#events:rowEditDialogAfterOpen)|このイベントは行編集ダイアログが開いてから発生します。|
|[rowEditDialogContentsRendered](&#123;environment:jQueryApiUrl&#125;/ui.iggridupdating#events:rowEditDialogContentsRendered)|このイベントは行編集ダイアログのコンテンツのレンダリングのあとに発生します。|
|[rowEditDialogBeforeClose](&#123;environment:jQueryApiUrl&#125;/ui.iggridupdating#events:rowEditDialogBeforeClose)|このイベントは行編集ダイアログが閉じる前に発生します。キャンセルはできません。|
|[rowEditDialogAfterClose](&#123;environment:jQueryApiUrl&#125;/ui.iggridupdating#events:rowEditDialogAfterClose)|このイベントは行編集ダイアログが閉じた後にに発生します。|
