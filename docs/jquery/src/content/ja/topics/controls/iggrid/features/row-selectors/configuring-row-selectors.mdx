...
- トピック
	- まず「[行セレクターを有効にする](/controls/iggrid/features/row-selectors/enabling-row-selectors)」トピックを読む必要があります。
- 外部リソース 
...
選択チェックボックス|行セレクター列にチェックボックスを含むかどうかを判断します|[enableCheckBoxes](&#123;environment:jQueryApiUrl&#125;/ui.iggridrowselectors#options:enableCheckBoxes)
行の番号付け|プロパティが有効な場合、行セレクター列には行番号が入っています。|[enableRowNumbering](&#123;environment:jQueryApiUrl&#125;/ui.iggridrowselectors#options:enableRowNumbering)
チェックボックス状態変更イベントをキャンセルする|`checkBoxStateChanging` イベントにフックし、ある状態にあるそのイベントをキャンセルします。|-
行の番号付けシード|シードがデフォルト番号付けに追加されます。|[rowNumberingSeed](&#123;environment:jQueryApiUrl&#125;/ui.iggridrowselectors#options:rowNumberingSeed)
行セレクター列の幅。|行セレクター列の幅はプロパティを設定することで構成できます。|[rowSelectorColumnWidth](&#123;environment:jQueryApiUrl&#125;/ui.iggridrowselectors#options:rowSelectorColumnWidth)
構成可能イベント|これらのイベントの詳細情報は、プロパティ参照セクションのリストを参照してください。 [igGridRowSelectors イベント](/controls/iggrid/features/row-selectors/rowselectors-events) | 
必須選択|選択機能を有効にすることを必須にします。選択が有効でない場合、例外がスローされます。|[requireSelection](&#123;environment:jQueryApiUrl&#125;/ui.iggridrowselectors#options:requireSelection)
...
-   [行セレクターのイベント](/controls/iggrid/features/row-selectors/rowselectors-events)
