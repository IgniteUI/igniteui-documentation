...
## JavaScript でのコンボ エディター プロバイダーの構成
...
```
	...
	-	<ApiLink type="igCombo" member="dataSource" section="options" label="dataSource" /> – ドロップダウン リストに項目を作成するために、コンポで使用されるデータ ソース。
	-	<ApiLink type="igCombo" member="valueKey" section="options" label="valueKey" /> – データ ソース内のフィールド。ドロップダウン項目の実際の値として使用。
	-	<ApiLink type="igCombo" member="textKey" section="options" label="textKey" /> (オプション - 設定されない場合は、valueKey と同じ) - ドロップダウン項目の表示値として使用される、データ ソース内のフィールド。
	-	<ApiLink type="igCombo" member="mode" section="options" label="mode" /> (オプション - 設定されない場合は、デフォルトで「editable」に設定)
	...
	グリッドのセルに、関連付けられたドロップダウン項目の値ではなくテキストを表示したい場合は、追加の <ApiLink type="iggrid" member="columns.formatter" section="options" label="formatter" /> 関数を列に定義し、値を表示テキストと関連付ける必要があります。類似の例を、次の関連サンプル [コンボ エディターを使用するグリッド](&#123;environment:NewSamplesUrl&#125;/combo/grid-with-combo-editor)に示します。
...
```
	...
	前述の構成では、textKey と valueKey は同じフィールドを使用するように設定されています。したがって、<ApiLink type="iggrid" member="columns.formatter" section="options" label="formatter" /> 関数を指定する必要はありません。
...
-   <ApiLink type="igCombo" label="igCombo API マニュアル" />
