...
```
	...
	-	[dataSource](&#123;environment:jQueryApiUrl&#125;/ui.igCombo#options:dataSource) – ドロップダウン リストに項目を作成するために、コンポで使用されるデータ ソース。
	-	[valueKey](&#123;environment:jQueryApiUrl&#125;/ui.igCombo#options:valueKey) – データ ソース内のフィールド。ドロップダウン項目の実際の値として使用。
	-	[textKey](&#123;environment:jQueryApiUrl&#125;/ui.igCombo#options:textKey) (オプション - 設定されない場合は、valueKey と同じ) - ドロップダウン項目の表示値として使用される、データ ソース内のフィールド。
	-	[mode](&#123;environment:jQueryApiUrl&#125;/ui.igCombo#options:mode) (オプション - 設定されない場合は、デフォルトで「editable」に設定)
	...
	グリッドのセルに、関連付けられたドロップダウン項目の値ではなくテキストを表示したい場合は、追加の [`formatter`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.formatter) 関数を列に定義し、値を表示テキストと関連付ける必要があります。類似の例を、次の関連サンプル [コンボ エディターを使用するグリッド](&#123;environment:NewSamplesUrl&#125;/combo/grid-with-combo-editor)に示します。
...
3. 結果の検証
	...
	![](../../../../images/images/igGrid_Combo_Editor_Provider_1.png)
...
```
	...
	前述の構成では、textKey と valueKey は同じフィールドを使用するように設定されています。したがって、[`formatter`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.formatter) 関数を指定する必要はありません。
...
3. 結果の検証
	...
	![](../../../../images/images/igGrid_Combo_Editor_Provider_MultiSelection_1.png)

	セルの編集モードに入ると、コンマで区切られたリストからの関連値は、ドロップダウンで選択済みとマークされます。

	![](../../../../images/images/igGrid_Combo_Editor_Provider_MultiSelection_2.png)


## <a id="topics"></a> 関連トピック 
以下は、その他の役立つトピックです。

-	[igCombo の構成](/controls/igbulletgraph/configuring/configuring)
- 	[更新の概要 (igGrid)](/controls/iggrid/features/updating/updating)
-   [igGridUpdating API マニュアル](&#123;environment:jQueryApiUrl&#125;/ui.igGridUpdating)
-   [igCombo API マニュアル](&#123;environment:jQueryApiUrl&#125;/ui.igCombo)
 
## <a id="samples"></a> 関連サンプル

-	[基本編集](&#123;environment:NewSamplesUrl&#125;/grid/basic-editing)
- 	[コンボ エディターを含むグリッド](&#123;environment:NewSamplesUrl&#125;/combo/grid-with-combo-editor)
