...
機能は <ApiLink pkg="ig" type="datasource" label="$.ig.DataSource" /> のページング機能の一番上に構築されるため、`$.ig.DataSource` でサポートされるすべての範囲のデータ ソースをサポートします。しかし、この機能はページング機能自体と互換性がありません。統合シナリオのサポートについては、｢[機能マトリックス (igGrid)](/feature-compatibility-matrix(iggrid)).mdx)｣のトピックを参照してください。
...
-   機能の名称は「igGridLoadOnDemand」から <ApiLink type="igGridAppendRowsOnDemand" label="igGridAppendRowsOnDemand" /> に変更されています。ユーザーのすべての API 呼出しを、新しい機能名に置き換える必要があります。
-   igGrid <ApiLink type="iggrid" member="features" section="options" label="features" /> オプションの配列では、機能の名称が「LoadOnDemand」から「AppendRowsOnDemand」に変更されています。
...
- [機能マトリックス (igGrid)](/feature-compatibility-matrix(iggrid)).mdx)
