...
機能は [$.ig.DataSource](&#123;environment:jQueryApiUrl&#125;/ig.datasource) のページング機能の一番上に構築されるため、`$.ig.DataSource` でサポートされるすべての範囲のデータ ソースをサポートします。しかし、この機能はページング機能自体と互換性がありません。統合シナリオのサポートについては、｢[機能マトリックス (igGrid)](/feature-compatibility-matrix(iggrid)).mdx)｣のトピックを参照してください。
