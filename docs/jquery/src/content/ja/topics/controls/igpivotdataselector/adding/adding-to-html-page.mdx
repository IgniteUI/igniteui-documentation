...
- [igPivotDataSelector の概要](/controls/igpivotdataselector/overview): このトピックは、主要機能、最小要件およびユーザー機能性など、`igPivotDataSelector` コントロールに関する概念的な情報を提供します。
...
データ ソースは、`igPivotDataSelector` の [dataSource](&#123;environment:jQueryApiUrl&#125;/ui.igPivotDataSelector#options:dataSource) パラメーターまたは [dataSourceOptions](&#123;environment:jQueryApiUrl&#125;/ui.igPivotDataSelector#options:dataSourceOptions) プロパティのいずれかを通して指定します。データ ソースの設定は、`igPivotDataSelector` の初期化時に唯一設定しなければならない強制オプションです。
...
```
	...
	以下は、`igPivotDataSelector` の [dataSourceOptions](&#123;environment:jQueryApiUrl&#125;/ui.igPivotDataSelector#options:dataSourceOptions)プロパティを使用してデータ ソースを指定するオルタナティブな (直接の) 方法です。(「[](#adding-control-summary)[igPivotDataSelector 追加の概要](#adding-control-summary)」を参照)
...
- [igPivotDataSelector を ASP.NET MVC アプリケーションに追加](/controls/igpivotdataselector/adding/adding-using-the-mvc-helper): このトピックは、`igPivotDataSelector` は ASP.NETMVC アプリケーションを追加する方法をについて概念とコード例の両方で説明します。
