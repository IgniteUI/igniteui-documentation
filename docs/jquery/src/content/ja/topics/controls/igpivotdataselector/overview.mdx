...
`igPivotDataSelector` は、データが `igPivotGrid`™ で可視化されている場合にユーザーがデータ スライスを選択できるインタラクティブな UI コントロール (jQuery UI ウィジェット) です。 [データ ソース コンポーネント](/data-sources/olap/multidimensional-data-source-components)と連携する補完コントロールです。`igPivotDataSelector` コントロールは以下から構成されます。

-   データベース (1)、キューブ (2) およびメジャー グループ (3) ([igOlapXmlaDataSource](/data-sources/olap/xmla/igolapxmladatasource)™ を使用する場合のみ) を選択するためのドロップダウン
...
`igPivotDataSelector` コントロールを使用した必要なリソースの詳細なリストについては、「[igPivotDataSelector の HTML ページへの追加](/controls/igpivotdataselector/adding/adding-to-html-page)」を参照してください。
...
- [igPivotDataSelector の HTML ページへの追加](/controls/igpivotdataselector/adding/adding-to-html-page): このトピックは、`igPivotDataSelector` を HTML ページへ追加する方法を示します。

- [jQuery と MVC API リンク (igPivotDataSelector)](/controls/igpivotdataselector/api-links): このトピックは、`igPivotDataSelector` と ASP.NET MVC ヘルパーに関する API ドキュメントへのリンクの一覧を示します。
