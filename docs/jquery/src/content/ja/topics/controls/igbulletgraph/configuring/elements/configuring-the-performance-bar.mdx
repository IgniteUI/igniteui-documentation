...
パフォーマンス バーはスケール範囲の先頭から開始しなければなりません。終了位置のみ構成可能です。終了位置でバーの長さを効果的に構成できます。終了位置は <ApiLink type="igBulletGraph" member="value" section="options" label="value" /> プロパティで処理されます。

スケール全域のディメンションでの位置は、<ApiLink type="igBulletGraph" member="valueInnerExtent" section="options" label="valueInnerExtent" /> および <ApiLink type="igBulletGraph" member="valueOuterExtent" section="options" label="valueOuterExtent" /> プロパティにより、グラフ領域の端に対して構成できます。パフォーマンス バーの位置を構成すると幅も定義されます。

![](../../../../images/images/igBulletGraph_Configuring_the_Performance_Bar_1.png)

バーのルック アンド フィールは、各プロパティ (<ApiLink type="igBulletGraph" member="valueBrush" section="options" label="valueBrush" />、<ApiLink type="igBulletGraph" member="valueOutline" section="options" label="valueOutline" /> および <ApiLink type="igBulletGraph" member="valueStrokeThickness" section="options" label="valueStrokeThickness" />) を使用して塗りつぶし色、境界線の色および境界線の線幅をカスタマイズできます。
