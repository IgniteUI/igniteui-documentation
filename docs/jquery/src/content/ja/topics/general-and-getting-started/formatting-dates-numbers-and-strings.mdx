...
`infragistics.util` ファイルは、&#123;environment:ProductName&#125; コントロールで日付、数値、文字列を書式設定するための `$.ig.formatter` のユーティリティ関数を含みます。たとえば、igGrid の [format](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.format) オプションで使用されます。
