...
2. `igOlapXmlaDataSource` を構成します。
	...
	`igOlapXmlaDataSource` がリモート データ プロバイダーで機能するためには、<ApiLink pkg="ig" type="OlapXmlaDataSource" member="serverUrl" section="options" label="serverUrl" /> を手順 1 で定義したアクションの URL に設定し、<ApiLink pkg="ig" type="OlapXmlaDataSource" member="isRemote" section="options" label="isRemote" /> オプションを true に設定します。
