...
[フィールド](&#123;environment:jQueryApiUrl&#125;/ig.datasource#options:settings.fields)のマッパー オプションでデータ抽出関数を定義でき、フィールドに使用されるデータを指定できるようになります。サブフィールドまたは複合データ
...
`igDataSource` コンポーネントは、すべてのフィールドで特定の単語または句を検索する方法を提供します。検索には [filterByText](&#123;environment:jQueryApiUrl&#125;/ig.datasource#options:methods.filterByText) メソッドを使用します。
...
-   [DataSchema を使用したデータ変換の実行 (igDataSource)](/data-sources/igdatasource/using-dataschema)