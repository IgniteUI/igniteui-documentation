...
## 概要
&#123;environment:ProductName&#125;™ グリッドまたは `igGrid` では、`igDataSource` コントロールを介して既存のプレーンな HTML テーブルにバインドできます。HTML テーブルへのバインドには、考慮すべき点がいくつかあります。
...
## 既知の問題と制限
`igGrid` には、考慮が必要な[既知の制限](/controls/iggrid/known-issues)があります。
