...
前提条件タイプ|コンテンツ
---|---
トピック|[「igTree を使用した作業の開始」](igTree-Getting-Started.html)というトピックを最初にお読みください。
...
`igTree` コントロールは、オンにされたすべてのノード、オフにされたすべてのノード、および部分的に選択されたノードを取得する API を備えています。
        ...
        <tr>
            <td>
選択イベントとチェックボックス イベントを処理する
			</td>

            <td>
選択イベントをキャプチャし、発生している選択操作に対するロジックを実行します。
			</td>
...
ノードを選択/選択解除する
            ...
            <td>
これらのメソッドを使用して、コードからモードを選択および選択解除します。
			</td>
