...
- トピック
	- [Infragistics テンプレート エンジン](Infragistics-Templating-Engine.html): このセクションには、Infragistics® テンプレート エンジンの使用に関するトピックが含まれています。
- 外部リソース
...
デフォルトでは、igGrid コントロールは Infragistics テンプレート エンジンを使用します。したがって、jsRender を使用するには、[`templatingEngine`](%%jQueryApiUrl%%/ui.iggrid#options:templatingEngine) オプションを使用して明示的に構成する必要があります。これは、JavaScript と ASP.NET MVC では異なります。
...
この手順では、jsRender syntax ヘルパー関数を使用して、`igGrid` でカスタム列 [`template`](%%jQueryApiUrl%%/ui.iggrid#options:columns.template) を作成する方法を説明します。
...
- [基本的な列テンプレートの作成](Creating-a-Basic-Column-Template-in-the-igGrid.html): このトピックでは、`igGrid` の基本的な列テンプレートを作成する方法を紹介します。
