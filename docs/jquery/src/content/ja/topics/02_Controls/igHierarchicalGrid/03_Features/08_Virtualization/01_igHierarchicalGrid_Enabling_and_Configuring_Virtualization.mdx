...
#### 仮想化を有効にする方法の概要
- 仮想化は [`virtualization`](%%jQueryApiUrl%%/ui.iggrid_hg#options) プロパティを `true` に設定することで有効になります。仮想化のタイプは、[`virtualizationMode`](%%jQueryApiUrl%%/ui.iggrid_hg#options) プロパティを continuous に設定することで指定できます。
