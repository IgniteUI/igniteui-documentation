...
# デフォルト ステートメントを含む条件付きテンプレートの作成 (igTemplating)
...
-   [デフォルト ステートメントを使用した条件付きテンプレートの作成](#conditional-template-default-statement)
    -   [概要](#introduction)
    -   [プレビュー](#preview)
    -   [前提条件](#prerequisites)
    -   [手順](#steps)
...
## <a id="conditional-template-default-statement"></a>デフォルト ステートメントを含む条件付きテンプレートの作成
### <a id="introduction"></a>概要

この例では、デフォルトのステートメントがある条件付きテンプレートを作成し、結果を HTML テーブルに追加します。次の例では、年齢の値が 17 未満か、または 18 ～ 21 の間かを確認しています。その確認に基づき、2 つの異なるスタイルを適用しています。
...
以下の手順は、基本的な条件付きテンプレートを作成する方法を示します。
