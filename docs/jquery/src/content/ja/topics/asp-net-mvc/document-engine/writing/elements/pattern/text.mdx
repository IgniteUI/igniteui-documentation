...
Text 要素は高度にカスタマイズした段落コンテンツをレポートに追加します。Text 要素には、どのような場合であってもコンテンツを目立たせるいくつかのテキスト固有のプロパティを含みます。これらのプロパティのいくつかは、[「レポート グラフィックス」](/asp-net-mvc/document-engine/writing/graphics/report-graphics)のセクションで見つけることができますが、以下のようなテキスト固有のプロパティがいくつかあります。
