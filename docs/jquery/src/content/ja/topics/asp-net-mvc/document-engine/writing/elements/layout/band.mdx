...
この詳細なガイドのコードによって、このトピックの上部にあるスクリーンショットと非常に似た PDF レポートを作成することができます。後はレポートをパブリッシュするだけです。レポートのパブリッシュについての詳細は、[「レポートをパブリッシュ」](/asp-net-mvc/document-engine/writing/elements/layout/report/publish-a-report)を参照してください。
