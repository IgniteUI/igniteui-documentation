...
[jsRender](http://www.jsviews.com/) は強力で高速なテンプレート エンジンです。複合テンプレートを処理することができ、カスタム タグやヘルパー関数を拡張してカスタマイズ可能な高度なテンプレートを作成できます。グリッド内の [jsRender Integration](/iggrid-jsrender-integration) は [`templatingEngine`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:templatingEngine) オプションによって制御されます。
