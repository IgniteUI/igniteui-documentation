...
All options provided for a directive are intended for the &#123;environment:ProductName&#125; widget that it will create, therefore all applicable options can be found under the [&#123;environment:ProductName&#125; API reference](&#123;environment:jQueryApiUrl&#125;/). There are two mutually-exclusive ways to define options – declaratively in the View or as an object in the Scope.
...
```
The name of the parent tag must match the name of the complex option or array ([`columns`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns) in this case). For arrays the name of the child tag obviously doesn’t carry over and can arbitrary, but for complex object settings it must also match the child object. For example the [`restSettings`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:restSettings) have `create` and `remove` options that can be set like so:
...
```
>**Note:** Keep in mind there are many user interaction-related events in the &#123;environment:ProductName&#125; controls are not raised when the controls are manipulated through the API. The directives use API methods to achieve data binding. For example, the [`activeItemChanged`](&#123;environment:jQueryApiUrl&#125;/ui.igcombo#events:activeItemChanged) event of the `igCombo` control is not raised because a change was made to the `ngModel` to which it was bound.
...
## <a id="creating-angular-app"></a>Creating Angular app with &#123;environment:ProductName&#125;
