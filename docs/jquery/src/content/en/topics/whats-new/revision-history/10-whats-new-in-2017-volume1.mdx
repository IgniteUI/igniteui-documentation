...
Feature | Description
---|---
[Round Decimals](#roundDecimals)| The numeric editor introduces new option [`roundDecimals`](&#123;environment:jQueryApiUrl&#125;/ui.ignumericeditor#options:roundDecimals) that allows to round values with decimal point.
...
Feature | Description
---|---
[Zoom Enabling Options](#zoomEnablingProperties) | New options called [`isHorizontalZoomEnabled`](&#123;environment:jQueryApiUrl&#125;/ui.igDataChart#options:isHorizontalZoomEnabled) and [`isVerticalZoomEnabled`](&#123;environment:jQueryApiUrl&#125;/ui.igDataChart#options:isHorizontalZoomEnabled) have been added which control whether zooming is allowed on either the horizontal or vertical axis.

### igMap

Feature | Description
---|---
[OpenStreet Tile Path](#tilePathProperty) | The option called [`tilePath`](&#123;environment:jQueryApiUrl&#125;/ui.igMap#options:backgroundContent.tilePath) has been added to the [`backgroundContent`](&#123;environment:jQueryApiUrl&#125;/ui.igMap#options:backgroundContent) option for the OpenStreet tile source.
...
The igDataSource component provides a way to search for a specific words or phrases in all of its fields via the [`filterByText`](&#123;environment:jQueryApiUrl&#125;/ig.datasource#methods:filterByText) method.
...
The [`enableUTCDates`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:enableUTCDates) now have new function. It affects only the dates serialization. Dates are now serialized as [UTC ISO 8061](https://en.wikipedia.org/wiki/ISO_8601#UTC) string instead of using the local time and zone values.

In order to handle the displaying of the dates a new option [`dateDisplayType`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.dateDisplayType) is introduced in the grid column's definition and it works only for columns of type `date`. 
...
In previous versions of the product, if user sets or enters a value in a numeric editor that has more decimal places than the one defined in the `maxDecimals` option, then the value was truncated. E.g. If an editor with defined 'maxDecimals' to `3`, receives a value `123.4567`, then it will be truncated to `123.456`. With version 17.1 of the product, a new option [`roundDecimals`](&#123;environment:jQueryApiUrl&#125;/ui.ignumericeditor#options:roundDecimals) is introduced, which is enabled by default and rounds the numeric values, using the JavaScript `Math.round()` function. This means that the value of `123.4567` will be rounded and displayed in the editor as `123.457`. If the [`roundDecimals`](&#123;environment:jQueryApiUrl&#125;/ui.ignumericeditor#options:roundDecimals) is disabled, then it will truncate the value and will show it as `123.456`, like in the old versions.

## igDateEditor/igDatePicker

### <a id="dateHandling"></a> Date Handling

When the dates in the editors are transferred from the client to the server аnd vice versa, the options [`enableUTCDates`](&#123;environment:jQueryApiUrl&#125;/ui.igdateeditor#options:enableUTCDates) and [`displayTimeOffset`](&#123;environment:jQueryApiUrl&#125;/ui.igdateeditor#options:displayTimeOffset) can be used to configure the editоrs and to properly handle date transfer.
...
New options called [`isHorizontalZoomEnabled`](&#123;environment:jQueryApiUrl&#125;/ui.igDataChart#options:isHorizontalZoomEnabled) and [`isVerticalZoomEnabled`](&#123;environment:jQueryApiUrl&#125;/ui.igDataChart#options:isVerticalZoomEnabled) were added, deprecating the existing [`horizontalZoomable`](&#123;environment:jQueryApiUrl&#125;/ui.igDataChart#options:horizontalZoomable) and [`verticalZoomable`](&#123;environment:jQueryApiUrl&#125;/ui.igDataChart#options:verticalZoomable) options respectively.  The older options are being left as-is in this release for backwards compatibility with existing applications.

## igMap
### <a id="tilePathProperty"></a> OpenStreet Tile Path

Open Street Map can now accept custom tile source by re-purposing the [`tilePath`](&#123;environment:jQueryApiUrl&#125;/ui.igMap#options:backgroundContent.tilePath) option off of the [`backgroundContent`](&#123;environment:jQueryApiUrl&#125;/ui.igMap#options:backgroundContent) object.
...
Before this change [`tilePath`](&#123;environment:jQueryApiUrl&#125;/ui.igMap#options:backgroundContent.tilePath) was only relevant to the Bing Map. After the change it is applicable to the Open Street Map as well.
