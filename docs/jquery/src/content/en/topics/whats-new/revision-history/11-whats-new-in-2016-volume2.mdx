...
You can now specify the color of the labels depending on whether they are rendered inside or outside of the funnel chart using the [`textColor`](&#123;environment:jQueryApiUrl&#125;/ui.igfunnelchart#options:textColor) and [`outerLabelTextColor`](&#123;environment:jQueryApiUrl&#125;/ui.igfunnelchart#options:outerLabelTextColor) options. You can also style the text of both the inner and outer labels using the [`textStyle`](&#123;environment:jQueryApiUrl&#125;/ui.igfunnelchart#options:textStyle)   and [`outerLabelTextStyle`](&#123;environment:jQueryApiUrl&#125;/ui.igfunnelchart#options:outerLabelTextStyle) options. 
...
Also two new API methods are added to [`expand`](&#123;environment:jQueryApiUrl&#125;/ui.iggridgroupby#methods:expand) and [`collapse`](&#123;environment:jQueryApiUrl&#125;/ui.iggridgroupby#methods:collapse) group rows.
...
Updating feature now works in row and cell edit mode when Multi-Row Layout is configured. Use [`navigationIndex`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.navigationIndex) option to configure the tab order of the editors when in edit mode.
...
You can now select a pie slice on the igPieChart control. This feature is enabled by default. You can set the [`selectionMode`](&#123;environment:jQueryApiUrl&#125;/ui.igPieChart#options:selectionMode) option in order to support single or multiple slice selection and then check the [`selectedItem`](&#123;environment:jQueryApiUrl&#125;/ui.igPieChart#options:selectedItem)  or [`selectedItems`](&#123;environment:jQueryApiUrl&#125;/ui.igPieChart#options:selectedItems) options in order to see what data items are associated with those selected slices.
...
The igPieChart control now has a [`labelClick`](&#123;environment:jQueryApiUrl&#125;/ui.igPieChart#events:labelClick) that fires when the labels of the pie slices are clicked.
...
Two new options are added to the igPieChart control, these are intended to replace the existing [`valueMemberPath`](&#123;environment:jQueryApiUrl&#125;/ui.igPieChart#options:valueMemberPath)  and [`labelMemberPath`](&#123;environment:jQueryApiUrl&#125;/ui.igPieChart#options:labelMemberPath) options. The new options are [`dataValue`](&#123;environment:jQueryApiUrl&#125;/ui.igPieChart#options:dataValue) and [`dataLabel`](&#123;environment:jQueryApiUrl&#125;/ui.igPieChart#options:dataLabel). With the addition of these options, [`valueMemberPath`](&#123;environment:jQueryApiUrl&#125;/ui.igPieChart#options:valueMemberPath)  and [`labelMemberPath`](&#123;environment:jQueryApiUrl&#125;/ui.igPieChart#options:labelMemberPath) are now deprecated.
...
Adding [`creditCard`](&#123;environment:jQueryApiUrl&#125;/ui.igValidator#options:creditCard) option and validation rule.
