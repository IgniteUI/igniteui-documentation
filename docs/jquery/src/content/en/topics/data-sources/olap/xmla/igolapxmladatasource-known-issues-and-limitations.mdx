---
title: "Known Issues and Limitations (igOlapXmlaDataSource)"
...
### Authenticated cross-domain data access not working with the Mozilla Firefox browser
...
>**Workaround: ** Register a custom-managed module in IIS to capture the Firefox’s `OPTIONS` verb and complete the response with an authorization header. For details, refer to the [Configuring Authenticated Access for the Mozilla Firefox Browser (igOlapXmlaDataSource)](./02_Config/00_Data Provider/01_IIS/igOlapXmlaDataSource_Configuring_Authenticated_Access_for_Firefox.mdx) topic.

### Authenticated data access not working with Internet Explorer 8 and 9 for non-trusted domains
...
### Authentication popup not displayed in the Chrome browser for cross-domain requests
...
- [igOlapXmlaDataSource](/igolapxmladatasource.mdx): This is a group of topics explaining the  `igOlapXmlaDataSource` component and its use.
