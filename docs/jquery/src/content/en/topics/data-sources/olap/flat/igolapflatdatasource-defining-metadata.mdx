---
title: "Defining Metadata (igOlapFlatDataSource)"
...
- [Adding igOlapFlatDataSource](/adding/igolapflatdatasource-adding.mdx): This is a group of topics about adding of the `igOlapFlatDataSource` component to an HTML page and an ASP.NET MVC View.
...
| --- | --- | --- |
| [Metadata cube](#config-metadata-cube) | Defines the name and caption for the cube; it is also used for defining the measures’ dimension and all other dimensions. | `cube` `name` `caption` `measuresDimension` `measures` |
| [Measures’ dimensions](#config-measures-dimension) | Defines the name and caption for the measures’ dimension. Used for specifying the measures. | `measuresDimension` `name` `caption` `measures` |
| [Measures](#config-measures) | Defines the names, captions, and aggregator functions of measures. | `measures` `name` `caption` `displayFolder` `aggregator` |
| [Dimensions](#config-dimentions) | Defines the names, captions, and hierarchies for the dimensions. | `dimensions` `name` `caption` `hierarchies` |
| [Hierarchies](#config-hierarchies) | Defines the names, captions, and levels for the hierarchies of a measure. | `hierarchies` `name` `caption` `displayFolder` `levels` |
| [Levels](#config-levels) | Defines the names, captions, and member provider functions for the levels of a hierarchy. | `levels` `name` `caption` `memberProvider` |
...
## Configuring the Measures’ Dimension
### Overview
...
Configuring measures consists of defining of the names, captions, and aggregator the functions for the measures. Metadata for the measures is specified in the <ApiLink pkg="ig" type="OlapFlatDataSource" member="measures" section="options" label="measures" /> array of the <ApiLink pkg="ig" type="OlapFlatDataSource" member="measuresDimension" section="options" label="measuresDimension" />. The property settings must be applied to each of the `measureMetadata` objects in the measures array.
...
| Specify the position of the measure in the display hierarchy | displayFolder | string | A string in the following format: `“Folder\\Inner Folder”`. In an igPivotDataSelector such a measure will be displayed like this: ![](images/igOlapFlatDataSource_Defining_Metadata_2.png) | No |
| Specify a callback to be invoked when each cell result is evaluated | aggregator | function | A function returning a value to be displayed in a result cell. Returning null results in no cell being created in the result. The function accepts two parameters: `items` – an array of the data source objects that must participate in calculating the value for the cell `cellMetadata` – a `CellMetadata` object containing additional information about the cell For sample implementations of average, count and sum aggregators, refer to [Code Example: Defining Metadata for igOlapFlatDataSource](#code-example). | Yes |
...
## Code Example: Defining Metadata for igOlapFlatDataSource
### Description
...
- [Adding igPivotDataSelector to an ASP.NET MVC Application](///controls/igpivotdataselector/adding/adding-using-the-mvc-helper.mdx): This topic explains, in both conceptual and step-by-step form, how to add the `igPivotDataSelector` control to an ASP.NET MVC application using ASP.NET MVC helper.
