...
5. Initialize the data source.
	...
	Initialize the flat data source by calling the OlapFlatDataSource object’s `initialize()` method that returns a [promise](http://api.jquery.com/deferred.promise/) object that will be resolved with the root [OlapMetadataTreeItem](&#123;environment:jQueryApiUrl&#125;/ig.OlapMetadataTreeItem) (the root node of the metadata tree). This is because the initialization is an asynchronous process. The promise object is an object that encapsulates an asynchronous operation exposing methods that can initiate and execute callbacks when the operation either completes or fails such as [done](http://api.jquery.com/deferred.done/), [fail](http://api.jquery.com/deferred.fail/), etc..
