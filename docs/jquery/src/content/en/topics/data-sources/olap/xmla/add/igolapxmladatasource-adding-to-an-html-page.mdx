---
title: "Adding igOlapXmlaDataSource to an HTML Page"
...
[igOlapXmlaDataSource Overview](/igolapxmladatasource-overview.mdx)
...
The `igOlapXmlaDataSource` makes the OLAP data from a SSAS server available for use in a JavaScript client environment. In order for the component to work correctly, the [serverUrl](&#123;environment:jQueryApiUrl&#125;/ig.OlapXmlaDataSource#options:serverUrl) property has to be specified. Also before using the component, you must initialize it.

Normally this data source component is used together with one of the OLAP pivot UI controls available with &#123;environment:ProductName&#125;.
...
-   The required JavaScript files:
    -   The jQuery library
    -   The required &#123;environment:ProductName&#125;™ JavaScript files
...
3. Initialize the `igOlapXmlaDataSource`
	...
	Initialize the XMLA data source by calling the `OlapXmlaDataSource` object’s `initialize()` method that returns a [promise](http://api.jquery.com/deferred.promise/) object that will be resolved with the root [MetadataTreeItem](&#123;environment:jQueryApiUrl&#125;/ig.OlapMetadataTreeItem) (The root node of the metadata tree). This is because the initialization is an asynchronous process (a call to the servers is made). The promise is an object that encapsulates an asynchronous operation exposing methods that can initiate and execute callbacks when the operation either completes or fails such as [done](http://api.jquery.com/deferred.done/), [fail](http://api.jquery.com/deferred.fail/), etc.
...
- [Adding igOlapXmlaDataSource to an ASP.NET MVC Application](/igolapxmladatasource-adding-to-an-aspnetmvc-application.mdx): This topic demonstrates how to add the `igOlapXmlaDataSource` to an ASP.NET MVC View using the ASP.NET MVC helper.

- [Configuring Authenticated Access for the Mozilla Firefox Browser (igOlapXmlaDataSource)](../02_Config/00_Data Provider/01_IIS/igOlapXmlaDataSource_Configuring_Authenticated_Access_for_Firefox.mdx): This topic provides a workaround for configuring IIS for cross-domain authenticated access for the Mozilla® Firefox® browser.

- [Adding igPivotView to an HTML Page](////controls/igpivotview/adding/adding-to-html-page.mdx): This topic demonstrates how to add the `igPivotView`™ to an HTML page.

- [Adding igPivotDataSelector to an HTML Page](////controls/igpivotdataselector/adding/adding-to-html-page.mdx): This topic demonstrates how to add the `igPivotDataSelector`™ to an HTML page.

- [Adding igPivotGrid to an HTML Page](////controls/igpivotgrid/adding/adding-to-an-html-page.mdx): This topic demonstrates how to add the `igPivotGrid`™ to an HTML page.
