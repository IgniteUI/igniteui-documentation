---
title: "Data Provider Configuration Overview (igOlapXmlaDataSource)"
...
This topic provides an overview of the supported data providers for the `igOlapXmlaDataSource`™ component and conceptual-level information on how to configure them. For configuration specifics and concrete instructions, refer to [Configuring igOlapXmlaDataSource Through a Remote Provider](/igolapxmladatasource-configuring-through-a-remote-provider.mdx) and [Adding igOlapXmlaDataSource to an HTML Page](//add/igolapxmladatasource-adding-to-an-html-page.mdx).
...
In order to configure the `igOlapXmlaDataSource` for direct use, it needs to be supplied with the URL of the msmdpump data provider. For the configuration specifics and concrete instructions, refer to [Configuring IIS for MSMDPUMP for igOlapXmlaDataSource](/iis/igolapxmladatasource-configuring-iis-for-msmdpump.mdx) and [Adding igOlapXmlaDataSource to an HTML Page](//add/igolapxmladatasource-adding-to-an-html-page.mdx)
...
Avoiding some of the [configuring required for server-side configurations](/iis/igolapxmladatasource-configuring-iis-for-msmdpump.mdx).
...
For configuration specifics and concrete instructions, refer to [Configuring `igOlapXmlaDataSource` Through a Remote Provider](/igolapxmladatasource-configuring-through-a-remote-provider.mdx) and [Adding `igOlapXmlaDataSource` to an HTML Page](//add/igolapxmladatasource-adding-to-an-html-page.mdx).
...
- [Configuring `igOlapXmlaDataSource` Through a Remote Provider](/igolapxmladatasource-configuring-through-a-remote-provider.mdx): This topic explains how to configure the `igOlapXmlaDataSource` with a remote data provider.
...
- [Binding to XMLA Data Source](&#123;environment:SamplesUrl&#125;/pivot-grid/binding-to-xmla-data-source): This sample demonstrates how to bind the `igPivotGrid`™ to an `igOlapXmlaDataSource`.

- [Remote Xmla Provider](&#123;environment:SamplesUrl&#125;/pivot-grid/remote-xmla-provider): This sample demonstrates reducing the network traffic using a remote provider with the `igOlapXmlaDataSource` component.

- [ADOMD.NET Remote Data Provider](&#123;environment:SamplesUrl&#125;/pivot-grid/remote-adomd-provider): This sample demonstrates using the ADOMD.NET remote provider with the `igPivotGrid` control.
