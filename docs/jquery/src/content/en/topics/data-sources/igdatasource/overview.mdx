...
The [field's](&#123;environment:jQueryApiUrl&#125;/ig.datasource#options:settings.fields) mapper option allows defining a data extraction function, which will allow specifying the data to be used for the field - it could be a sub-field or combined data from the complex object.
...
The `igDataSource` component provides a way to search for a specific words or phrases in all of its fields. This is done by the [filterByText](&#123;environment:jQueryApiUrl&#125;/ig.datasource#options:methods.filterByText) method.
