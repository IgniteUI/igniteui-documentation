...
The <ApiLink pkg="ig" type="datasource" member="settings.fields" section="options" label="field's" /> mapper option allows defining a data extraction function, which will allow specifying the data to be used for the field - it could be a sub-field or combined data from the complex object.
...
The `igDataSource` component provides a way to search for a specific words or phrases in all of its fields. This is done by the <ApiLink pkg="ig" type="datasource" member="methods.filterByText" section="options" label="filterByText" /> method.
...
-   [Performing Data Transformations Using DataSchema (igDataSource)](/igdatasource-using-dataschema.mdx)
