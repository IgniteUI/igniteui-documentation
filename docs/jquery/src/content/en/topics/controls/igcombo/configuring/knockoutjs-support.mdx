...
| Configuring data source for the igCombo | Required | Sets a valid data source accepted by $.ig.DataSource, or an instance of an $.ig.DataSource itself. | [`dataSource`](&#123;environment:jQueryApiUrl&#125;/ui.igcombo#options:dataSource) |
| Configuring the data source field that holds the items' display text | Required | Sets the name of the data source field which contains the items' display text. The text for each item is displayed in the dropdown list and in the combo box when an item is selected. The same text is stored in the text option, too. | [`textKey`](&#123;environment:jQueryApiUrl&#125;/ui.igcombo#options:textKey) |
| Configuring the data source field that holds the items' values | Required | Sets the name of data source field (ID field) which contains the items' values. The igCombo items' values are mapped against this field. | [`valueKey`](&#123;environment:jQueryApiUrl&#125;/ui.igcombo#options:valueKey) |
