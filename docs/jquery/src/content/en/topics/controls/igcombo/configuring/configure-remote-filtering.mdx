...
| --- | --- | --- |
| Remote filtering with OData | OData provides a way to filter the combo when the data source has a very large amount of records. The igCombo control internally handles sending the proper requests for filtered results from the service. | [filteringType](&#123;environment:jQueryApiUrl&#125;/ui.igCombo#options:filteringType)[responseDataKey](&#123;environment:jQueryApiUrl&#125;/ui.igCombo#options:responseDataKey) |
| Remote filtering with the ASP.NET MVC | The &#123;environment:ProductNameMVC&#125; Combo provides the API to setup server-side filtering when binding to collections of business objects. | [filteringType](&#123;environment:jQueryApiUrl&#125;/ui.igCombo#options:filteringType)ComboDataSourceAction[filterExprUrlKey](&#123;environment:jQueryApiUrl&#125;/ui.igCombo#options:filterExprUrlKey) |
