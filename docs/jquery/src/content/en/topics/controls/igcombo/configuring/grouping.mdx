...
In order to enable combo `grouping` <ApiLink type="igcombo" member="grouping.key" section="options" label="key" /> property should be set. This property represents the `name of column` by which the records to be grouped. When combo is initialized all items will be grouped by that key in ascending order. `Sort direction` can be changed by using <ApiLink type="igcombo" member="grouping.dir" section="options" label="dir" /> property as swhown in the code snippet below:
**In JavaScript:**
...
-	<ApiLink type="igcombo" member="grouping" section="options" label="Grouping api help" />
