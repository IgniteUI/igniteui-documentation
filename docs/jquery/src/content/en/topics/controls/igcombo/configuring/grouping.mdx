...
In order to enable combo `grouping` [`key`](&#123;environment:jQueryApiUrl&#125;/ui.igcombo#options:grouping.key) property should be set. This property represents the `name of column` by which the records to be grouped. When combo is initialized all items will be grouped by that key in ascending order. `Sort direction` can be changed by using [`dir`](&#123;environment:jQueryApiUrl&#125;/ui.igcombo#options:grouping.dir) property as swhown in the code snippet below:
**In JavaScript:**
...
-	[Grouping api help](&#123;environment:jQueryApiUrl&#125;/ui.igcombo#options:grouping)
