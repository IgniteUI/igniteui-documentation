...
The data source is specified through either the [dataSource](&#123;environment:jQueryApiUrl&#125;/ui.igPivotDataSelector#options:dataSource) parameter or the [dataSourceOptions](&#123;environment:jQueryApiUrl&#125;/ui.igPivotDataSelector#options:dataSourceOptions) property of the `igPivotDataSelector`. The data source setting is the only mandatory option to set when initializing the `igPivotDataSelector`.
...
```
	...
	Following is the alternative (direct) way to specify a data source using the [dataSourceOptions](&#123;environment:jQueryApiUrl&#125;/ui.igPivotDataSelector#options:dataSourceOptions) property of the `igPivotDataSelector`. (See [](#adding-control-summary)[Adding igPivotDataSelector summary](#adding-control-summary).)
