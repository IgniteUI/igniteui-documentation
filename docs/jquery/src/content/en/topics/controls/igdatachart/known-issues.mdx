...
| [Monolith shadows do not allow for the blur effect to be applied to them](#monolith_shadow) | When the [useSingleShadow](&#123;environment:jQueryApiUrl&#125;/ui.igDataChart#options:useSingleShadow) property of the series is set to "true", the [shadowBlur](&#123;environment:jQueryApiUrl&#125;/ui.igDataChart#options:shadowBlur) setting is disregarded and no blur is applied to the shadow. | ![](../../images/images/positive.png) |
...
When the [`useSingleShadow`](&#123;environment:jQueryApiUrl&#125;/ui.igDataChart#options:useSingleShadow) property of the series is set to "true", the [`shadowBlur`](&#123;environment:jQueryApiUrl&#125;/ui.igDataChart#options:shadowBlur) setting is disregarded and no blur is applied to the shadow. This is a deliberate limitation due to a [Google® Chrome™ bug](https://code.google.com/p/chromium/issues/detail?id=100703) and extended to all major browsers thus ensuring consistent behavior across all of them. This behavior is subject to change once the aforementioned Chrome issue is resolved.
