...
```
	...
	In case static resource loading is preferred you can refer to the [igDataChart Overview: Minimum Requirements](./00_Overview/~igDataChart_Overview.mdx#min-requirements) to see which resource files must be linked to use charts.
...
3. Add data source. <a id="add-data-array"></a> 
	...
	**HTML example**
	...
	For the HTML example some JavaScript code must be added that defines an array with several data records for financial data. In order to get information on binding to other data sources you may refer to the [Binding igDataChart to Data](/igdatachart-databinding.mdx) topic. 
...
## Resources for Charts Reference
...
| Chart Series Type | Resource Name |
| --- | --- |
...
| Shape Files | igDataChart.igShapeDataSource |
...
| Date-time axis | igDateTimeAxis |
| TimeXAxis | igTimeXAxis |
| Chart legend | igChartLegend |
| Overview Plus Detail Pane | igOverviewPlusDetailPane |
