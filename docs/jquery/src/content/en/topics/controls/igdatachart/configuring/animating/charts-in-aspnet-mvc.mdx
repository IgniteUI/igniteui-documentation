---
title: "Animating Charts in ASP.NET MVC(igDataChart)"
...
-	[igDataSource Overview](////data-sources/igdatasource/igdatasource-overview.mdx): Introduces the `igDataSource` control.

-	[Infragistics Motion Framework for Charts](/igdatachart-motion-framework.mdx) : Introduces the Infragistics® Motion Framework™ for charts.
...
3. Add an MVC view.
 ...
 To create a new empty MVC view, **right click in the** `Index()` **method** in the controller. From the context menu, select Add View. Follow the steps in the [Adding igDataChart](//igdatachart-adding.mdx) topic to create a skeleton MVC view, which incorporates charts. In this example, only the first step, [Adding references to required resources](../../01_igDataChart_Adding.mdx#add-references-to-required-resources) is required.
...
-	[Motion Framework](./00_igDataChart_Motion_Framework.mdx#motion-framework-sample) : This sample demonstrates how column series charts are animated with the help of the Motion Framework.
