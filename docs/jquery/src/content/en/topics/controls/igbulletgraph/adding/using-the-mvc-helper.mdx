...
## Adding igBulletGraph to an ASP.NET MVC Application – Conceptual Overview
...
Apart from its <ApiLink type="igBulletGraph" member="ranges" section="options" label="startValue" /> and <ApiLink type="igBulletGraph" member="ranges" section="options" label="endValue" />, a <ApiLink type="igBulletGraph" member="ranges" section="options" label="name" /> must be assigned to the individual range, so that its extent is specified.
...
```
    ...
    Page](/igbulletgraph-adding-to-an-html-page.mdx) topic.
...
## Adding igBulletGraph to an ASP.NET MVC Application – Procedure
...
- [Adding *igBulletGraph* to an HTML Page](/igbulletgraph-adding-to-an-html-page.mdx): This topic demonstrates, with code examples, how to add the `igBulletGraph` control to an HTML page.
