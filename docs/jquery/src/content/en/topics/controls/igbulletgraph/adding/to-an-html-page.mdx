...
4. **Configure the scale.**
	...
	In order to customize the values of the scale, you need to set its [`minimumValue`](&#123;environment:jQueryApiUrl&#125;/ui.igBulletGraph#options:minimumValue) and [`maximumValue`](&#123;environment:jQueryApiUrl&#125;/ui.igBulletGraph#options:maximumValue) properties. In this example, the scale will start at 5 and end at 55.
...
5. ​**Add performance bar.**
	...
	The primary measure of the `igBulletGraph` is visualized by its performance bar. Its value is managed by the [`value`](&#123;environment:jQueryApiUrl&#125;/ui.igBulletGraph#options:value) property setting. For this example, set the value property to 35.
...
6. **Configure the comparative marker.**
	...
	To position the comparative measure marker on the scale is managed by the value of the [`targetValue`](&#123;environment:jQueryApiUrl&#125;/ui.igBulletGraph#options:targetValue) property. For this example, set the `targetValue` property to 43.
...
7. **Add comparative ranges.**
	...
	In order to compare the value displayed by the performance bar against some meaningful range(s) of values, these comparative ranges need to be displayed on the scale. Comparative ranges are managed by [`ranges`](&#123;environment:jQueryApiUrl&#125;/ui.igBulletGraph#options:ranges) property within which several individual ranges can be defined, each of which having its own starting and ending values ([`startValue`](&#123;environment:jQueryApiUrl&#125;/ui.igBulletGraph#options:startValue) and [`endValue`](&#123;environment:jQueryApiUrl&#125;/ui.igBulletGraph#options:endValue)) and color ([`brush`](&#123;environment:jQueryApiUrl&#125;/ui.igBulletGraph#options:brush)). 
