---
title: "Cell Merging Advanced Customization (igGrid)"
...
- [igGrid Cell Merging Overview](/iggrid-cellmerging-overview.mdx): This topic shows you how to enable the Cell Merging feature to be used inside `igGrid`.
...
Cell Merging provides two predefined merging strategies. The first one checks for duplicate cell values. The second one merges subsequent *null* values. In addition to the predefined methods, the <ApiLink type="iggridcellmerging" member="mergeStrategy" section="options" label="*mergeStrategy*" /> option can be used to define a custom strategy.
...
In order to specify which predefined method is used for cell merging, the <ApiLink type="iggridcellmerging" member="mergeStrategy" section="options" label="*mergeStrategy*" /> property needs to be set to either `"duplicate"` or `"null"` (defaults to `"duplicate"`).

### <a id="merge-duplicate"></a> Duplicate strategy

When the `mergeStrategy` option is set to *"duplicate"*, the way it determines if two cells should be merged is by comparing their values. If they pass a *`===`* check (which means they have both equal value and equal type), the two cells will be eligible for merging. After that depending on the <ApiLink type="iggridcellmerging" member="mergeType" section="options" label="*mergeType*" /> option cells will be merged accordingly. 
...
Only one cell for the column needs to carry the "Project Name" information while the rest of the cells may have a value of `null`. To ensure they remain `null` in the grid's data source, they should be of a "nullable" type, which for igDataSource is either `number`, `object` or `Date`. For this example the <ApiLink type="iggrid" member="columns.dataType" section="options" label="*dataType*" /> of the column is set to `"object"` so the `null` values are retained. You could also set <ApiLink type="iggrid" member="localSchemaTransform" section="options" label="*localSchemaTransform*" /> to false.
...
- [igGrid Cell Merging - Overview](/iggrid-cellmerging-overview.mdx)
- [igGrid Overview](//iggrid-overview.mdx)
- [igGrid Sorting](/columns/sorting/iggrid-sorting-overview.mdx)