---
title: "Configuring Column Summaries (igGrid)"
...
## Summaries Configuration Overview
...
------------------------------|-----------------------|-------------------------
Summary type | The type of the summary calculationis set through a property. | <ApiLink type="iggridsummaries" member="type" section="options" label="type" />
Automatic/manual calculation | Calculations can be performed either automatically (immediately after a value is entered) or after the user clicks a button. This functionality is delivered through two rendering modes. | <ApiLink type="iggridsummaries" member="calculateRenderMode" section="options" label="calculateRenderMode" />
Summary compacting | Summaries can be rendered compactly through a special rendering mode. | <ApiLink type="iggridsummaries" member="compactRenderingMode" section="options" label="compactRenderingMode" />
Summary button | A summary button in the header cell, configurable through a dedicated property. | <ApiLink type="iggridsummaries" member="showSummariesButton" section="options" label="showSummariesButton" />
Summary menu button | Summary menu button - A summary menu button in the footer cell, configurable through a dedicated property. | <ApiLink type="iggridsummaries" member="showDropDownButton" section="options" label="showDropDownButton" />
Canceling summary calculation | Handle `summaryCalculation` event and, on some condition, cancel it. | -
Configurable events | For detailed information about these events, refer to their listing in the property reference section: [Column Summaries Events (igGrid)](/iggrid-column-summaries-events.mdx) | 
...
[Summaries (Remote Calculation)](&#123;environment:SamplesUrl&#125;/grid/summaries-remote)

## Configuring the Calculation Mode(Automatic/Manual)
...
### Property settings for configuring the calculation mode
...
### Property settings for enabling summary compacting
...
### Property settings for enabling the summary button
...
## Configuring a Summary Menu Button
...
### Property settings for enabling the summary menu button
...
-   [Enabling Column Summaries (igGrid)](./00_igGrid_Enabling _Column_Summaries.mdx)
-   [Column Summaries Events (igGrid)](/iggrid-column-summaries-events.mdx)