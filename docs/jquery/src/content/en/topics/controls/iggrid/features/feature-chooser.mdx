...
<Sample src="/grid/feature-chooser/index.html" height={428} alt="Feature Chooser"/>
...
The visibility behavior of the gear icon is controlled by the grid's <ApiLink type="iggrid" member="featureChooserIconDisplay" section="options" label="featureChooserIconDisplay" /> option. The default value for this option is `desktopOnly` and behaves as described above. Should you want to make the gear icon available in all contexts, then set `featureChooserIconDisplay` equal to always.
...
- [igGrid Multiple Sorting Modal](/columns/sorting/iggrid-multiple-sorting-dialog.mdx)
- [igGrid Filtering](/columns/iggrid-filtering.mdx)
