...
Since v13.2 igGrid supports jsRender templating. It is controlled by the option [`templatingEngine`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:templatingEngine). The default value of the option is “infragistics” and in this case the `igGrid` is using the Infragistics templating engine for rendering the templates in the control. You need to set the property to “jsRender” in order to use it as templating engine.
...
By default, the igGrid control uses the Infragistics template engine. Consequently, in order to use jsRender you need to configure it explicitly using the [`templatingEngine`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:templatingEngine) option. This is done differently in JavaScript and ASP.NET MVC.
...
This procedure explains how to create a custom column [`template`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.template) in `igGrid` using the jsRender syntax, helper functions.
