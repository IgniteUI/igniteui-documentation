---
title: "jsRender Integration (igGrid)"
...
- Topics
    - [Infragistics templating engine](../../../06_Infragistics-Templating-Engine/~Infragistics Templating Engine.mdx): This section contains topics covering the use the Infragistics® Templating Engine.
- External Resources
...
Since v13.2 igGrid supports jsRender templating. It is controlled by the option <ApiLink type="iggrid" member="templatingEngine" section="options" label="templatingEngine" />. The default value of the option is “infragistics” and in this case the `igGrid` is using the Infragistics templating engine for rendering the templates in the control. You need to set the property to “jsRender” in order to use it as templating engine.
...
## Configuring jsRender templating – Conceptual Overview

By default, the igGrid control uses the Infragistics template engine. Consequently, in order to use jsRender you need to configure it explicitly using the <ApiLink type="iggrid" member="templatingEngine" section="options" label="templatingEngine" /> option. This is done differently in JavaScript and ASP.NET MVC.
...
## Code Example: Creating a custom column template using jsRender syntax in JavaScript

This procedure explains how to create a custom column <ApiLink type="iggrid" member="columns.template" section="options" label="template" /> in `igGrid` using the jsRender syntax, helper functions.
...
1.  The required &#123;environment:ProductName&#125; JavaScript and CSS files for version 13.2
...
- [Creating a Basic Column Template](./00_Columns/06_Template/00_Creating a Basic Column Template in the igGrid.mdx): This topic demonstrates how to create basic column template for the `igGrid`.

- [Configuring Column Templates (igGrid, RWD Mode)](/responsive/configure/iggrid-responsive-web-design-mode-configuring-row-and-column-templates.mdx): This topic explains, with code examples, how to define column templates for the individual Responsive Web Design (RWD) mode configurations of the `igGrid`™ control and how to configure automatic change of template when switching the active RWD mode configuration.
