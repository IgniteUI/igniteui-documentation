---
title: "Columns and Layout (igGrid)"
...
<Sample src="/grid/grid-layout/index.html" height={428} alt="Grid Layout"/>
...
-   The `format` column property corresponding to dataType=”date” (Date objects) can be “date”, “dateLong” , “dateLong” , “dateTime” , “timeLong” or explicit pattern like “MM-dd-yyyy h:mm:ss tt”. See [Formatting Dates](../../../01_General-and-Getting-Started/14_Formatting_Dates_Numbers_and_Strings.mdx#formatting-dates) for details on the valid custom date format specifiers.
-   The `format` column property corresponding to dateType=”number” (number objects) or for dataType=”string” can be “number”, “double” , “int” , “currency” , “percent”. See [Formatting Numbers](../../../01_General-and-Getting-Started/14_Formatting_Dates_Numbers_and_Strings.mdx#formatting-numbers) for details on the valid custom number format specifiers.
-   The `format` column property corresponding to dateType=”bool” (bool objects) can be “checkbox”.
-   If `dataType`=”number”, then the corresponding format also can be set to something like “0.0###”, “#.##”, “0.000” etc. In this case number of zeros after the decimal point define minimum decimal places and overall number of characters after decimal point defines number of maximum decimal places. See [Formatting Numbers](../../../01_General-and-Getting-Started/14_Formatting_Dates_Numbers_and_Strings.mdx#formatting-numbers) for details on the valid custom number format specifiers.
-   If `dataType` is not “date” or “number”, then the corresponding format can contain “&#123;0&#125;” flag. In this case that flag is replaced by the value of cell.For example, if format=”Name: &#123;0&#125;” and value is cell is “Bob”, then cell will be rendered as “Name: Bob” (see [Formatting Strings](../../../01_General-and-Getting-Started/14_Formatting_Dates_Numbers_and_Strings.mdx#formatting-strings) for details).
...
Column formatting (rendering) is affected by several `igGrid` options. These are column's [`formatter`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.formatter), [`format`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.format) and [`template`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.template). Additionally there's grid's [`autoFormat`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:autoFormat) option which affects how the regional settings are applied to the grid.

-  [`autoFormat`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:autoFormat) - is a string which identifies how the regional settings are applied globally on the grid for "date" and "number" columns. By default only "date" columns are formatted according the regional settings. This option is overridden by [`format`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.format) and [`formatter`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.formatter) options if available.
...
-  [`formatter`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.formatter) - is a function or a string name of a function bound to the global window object. It gives you full control on rendering the data source value. When defining formatter function it's up to you to control the format and regional representation of the value. There is a utility function [`$.ig.formatter(rawValue, dataType, formatPattern)`](///general-and-getting-started/formatting-dates-numbers-and-strings.mdx) which can be used for formatting values either using the regional settings or using custom format pattern.
...
```
 ...
 [`formatter`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.formatter) and [`format`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.format) options do not operate at the same time. When defined, [`formatter`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.formatter) function is considered with priority and 
 [`format`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.format) is not used. However value from the [`formatter`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.formatter) function is further decorated with a [`template`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.template).
...
<Sample src="/grid/column-format-function/index.html" height={428} alt="Column Format Function"/>

-  [`format`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.format) - is a string identifying a format patterns. Internally [`format`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.format) option uses the `$.ig.formatter(rawValue, dataType, formatPattern)` function. When set, [`format`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.format) overrides the setting of the [`autoFormat`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:autoFormat) option and also the default regional settings.

 Here is the flow of column rendering when [`format`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.format) is used:
...
<Sample src="/grid/column-formats/index.html" height={428} alt="igGrid Column Formats"/>

- [`template`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.template) - is a templated string (templating engine used is defined in the `templatingEngine` option). See [Templating Engine Overview](../../../06_Infragistics-Templating-Engine/01_igTemplating Overview.mdx) topic for details on template syntax. 
 
 Here is the flow of column rendering when [`template`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.template) is used:
...
- [`columnCssClass`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.columnCssClass) - is a space-separated list of CSS classes that are applied to the TD element of the cell.  
 When the [`template`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.template) option defines the rendering of the TD element the [`columnCssClass`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.columnCssClass) option is ignored (see the example below). 
 Here is the flow of column rendering when [`columnCssClass`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.columnCssClass) is used:
...
- [`headerCssClass`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.headerCssClass) - is a space-separated list of CSS classes that are applied to the TH element of the column header text configured by the [`headerText`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.headerText) option.   
 Here is the flow of column rendering when [`headerCssClass`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.headerCssClass) is used:
...
By default the cell text in igGrid is left aligned. To customize the cell text alignment use [`columnCssClass`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.columnCssClass) option. Just create custom CSS classes to align the text to the desired direction and then apply them to the column using the `columnCssClass`.
...
<Sample src="/grid/configure-text-alignment/index.html" height={428} alt="igGrid Configure Text Alignment"/>
...
```
The function is defined via the [`mapper`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.mapper) column option as shown in Listing 2. It allows specifying values per data record to be used for all data operations related to the specific column. 
...
<Sample src="/grid/handling-complex-objects/index.html" height={428} alt="Handling Complex Objects"/>
...
<Sample src="/grid/auto-generate-columns/index.html" height={428} alt="igGrid Auto-Generate Columns"/>
...
The second CSS is custom to &#123;environment:ProductName&#125; and contains layout-related rules that are not available in Theme Roller / jQuery UI. Therefore it is required so the control is ensured to function properly.
...
|  |  |
| --- | --- |
| ![](images/igGrid_Columns_and_Layout_01.png) | ![](images/igGrid_Columns_and_Layout_02.png) |
...
<Sample src="/grid/checkbox-column/index.html" height={428} alt="igGrid Checkbox Column"/>
...
-   [Formatting Dates, Numbers and Strings](///general-and-getting-started/formatting-dates-numbers-and-strings.mdx)
-   [Templating Engine Overview](../../../06_Infragistics-Templating-Engine/01_igTemplating Overview.mdx)
