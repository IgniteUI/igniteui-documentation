...
Column formatting (rendering) is affected by several `igGrid` options. These are column's <ApiLink type="iggrid" member="columns.formatter" section="options" label="formatter" />, <ApiLink type="iggrid" member="columns.format" section="options" label="format" /> and <ApiLink type="iggrid" member="columns.template" section="options" label="template" />. Additionally there's grid's <ApiLink type="iggrid" member="autoFormat" section="options" label="autoFormat" /> option which affects how the regional settings are applied to the grid.

-  <ApiLink type="iggrid" member="autoFormat" section="options" label="autoFormat" /> - is a string which identifies how the regional settings are applied globally on the grid for "date" and "number" columns. By default only "date" columns are formatted according the regional settings. This option is overridden by <ApiLink type="iggrid" member="columns.format" section="options" label="format" /> and <ApiLink type="iggrid" member="columns.formatter" section="options" label="formatter" /> options if available.
...
```
 ...
 <ApiLink type="iggrid" member="columns.formatter" section="options" label="formatter" /> and <ApiLink type="iggrid" member="columns.format" section="options" label="format" /> options do not operate at the same time. When defined, <ApiLink type="iggrid" member="columns.formatter" section="options" label="formatter" /> function is considered with priority and 
 <ApiLink type="iggrid" member="columns.format" section="options" label="format" /> is not used. However value from the <ApiLink type="iggrid" member="columns.formatter" section="options" label="formatter" /> function is further decorated with a <ApiLink type="iggrid" member="columns.template" section="options" label="template" />.
...
-  <ApiLink type="iggrid" member="columns.format" section="options" label="format" /> - is a string identifying a format patterns. Internally <ApiLink type="iggrid" member="columns.format" section="options" label="format" /> option uses the `$.ig.formatter(rawValue, dataType, formatPattern)` function. When set, <ApiLink type="iggrid" member="columns.format" section="options" label="format" /> overrides the setting of the <ApiLink type="iggrid" member="autoFormat" section="options" label="autoFormat" /> option and also the default regional settings.
 ...
 Here is the flow of column rendering when <ApiLink type="iggrid" member="columns.format" section="options" label="format" /> is used:
...
- <ApiLink type="iggrid" member="columns.template" section="options" label="template" /> - is a templated string (templating engine used is defined in the `templatingEngine` option). See [Templating Engine Overview](../../../06_Infragistics-Templating-Engine/01_igTemplating Overview.mdx) topic for details on template syntax. 
 
 Here is the flow of column rendering when <ApiLink type="iggrid" member="columns.template" section="options" label="template" /> is used:
...
- <ApiLink type="iggrid" member="columns.columnCssClass" section="options" label="columnCssClass" /> - is a space-separated list of CSS classes that are applied to the TD element of the cell.  
 When the <ApiLink type="iggrid" member="columns.template" section="options" label="template" /> option defines the rendering of the TD element the <ApiLink type="iggrid" member="columns.columnCssClass" section="options" label="columnCssClass" /> option is ignored (see the example below). 
 Here is the flow of column rendering when <ApiLink type="iggrid" member="columns.columnCssClass" section="options" label="columnCssClass" /> is used:
...
- <ApiLink type="iggrid" member="columns.headerCssClass" section="options" label="headerCssClass" /> - is a space-separated list of CSS classes that are applied to the TH element of the column header text configured by the <ApiLink type="iggrid" member="columns.headerText" section="options" label="headerText" /> option.   
 Here is the flow of column rendering when <ApiLink type="iggrid" member="columns.headerCssClass" section="options" label="headerCssClass" /> is used:
...
By default the cell text in igGrid is left aligned. To customize the cell text alignment use <ApiLink type="iggrid" member="columns.columnCssClass" section="options" label="columnCssClass" /> option. Just create custom CSS classes to align the text to the desired direction and then apply them to the column using the `columnCssClass`.
...
```
The function is defined via the <ApiLink type="iggrid" member="columns.mapper" section="options" label="mapper" /> column option as shown in Listing 2. It allows specifying values per data record to be used for all data operations related to the specific column. 
