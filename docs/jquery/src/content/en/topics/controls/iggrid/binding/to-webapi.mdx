...
- [REST Updating (igGrid)](/features/iggrid-rest-updating.mdx): This topic explains igGrid support for REST services.
- [igGrid Overview](/iggrid-overview.mdx): This topic provides conceptual information about the igGrid including information regarding features, binding to data sources, requirements, templates, and interaction.
...
Add a button and attach an event handler for it's click event. In it call the igGrid's <ApiLink type="igGrid" member="saveChanges" section="methods" label="saveChanges" /> method to send the pending changes to the server.
