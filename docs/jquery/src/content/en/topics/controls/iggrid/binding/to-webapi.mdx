...
Add a button and attach an event handler for it's click event. In it call the igGrid's [saveChanges](&#123;environment:jQueryApiUrl&#125;/ui.igGrid#methods:saveChanges) method to send the pending changes to the server.
