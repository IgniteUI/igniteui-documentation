---
title: "Responsive Web Design (RWD) Mode Overview (igGrid)"
...
- Topics
    ...
    - [igGrid Features](/iggrid-features-landing-page.mdx): This is a group of topics explaining the features of the `igGrid` control.
- External Resources
...
-   [Configuring Column Templates](/configure/iggrid-responsive-web-design-mode-configuring-row-and-column-templates.mdx)
...
-   [Configuring Templates for the Individual RWD Mode Profiles](/configure/iggrid-responsive-web-design-mode-configuring-row-and-column-templates.mdx)
...
-   [Creating Custom Responsive Web Design (RWD) Mode Profiles](/configure/iggrid-responsive-web-design-mode-creating-custom-profile.mdx)
...
-   [Configuring Bootstrap Support](/configure/iggrid-responsive-web-design-mode-configuring-bootstrap-support.mdx)
...
| Configurable aspect | Details | Properties/classes |
| --- | --- | --- |
| Column hiding | You can specify which columns to be visible and which – hidden in each respective RWD mode profile. **Related topics** [Configuring Column Hiding](/configure/iggrid-responsive-web-design-mode-configuring-column-hiding.mdx) | When configuring on CSS Media Queries basis (using CSS classes): [columnSettings](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:columnSettings) [columnSettings.columnKey](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:columnSettings.columnKey) [columnSettings.classes](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:columnSettings.classes) When configuring on a per-column basis(using configuration settings): [columnSettings](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:columnSettings) [columnSettings.configuration.desktop.hidden](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:columnSettings.configuration) [columnSettings.configuration.tablet.hidden](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:columnSettings.configuration) [columnSettings.configuration.phone.hidden](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:columnSettings.configuration) columnSettings.configuration.&lt;custom_mode&gt;.hidden |
| Column template | You can specify an individual column template for each RWD mode profile. Column templates are defined individually for each column. **Related topics** [Configuring Column Templates](/configure/iggrid-responsive-web-design-mode-configuring-row-and-column-templates.mdx) | [columnSettings](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:columnSettings) [columnSettings.columnKey](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:columnSettings.columnKey) [columnSettings.configuration.desktop.template](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:columnSettings.configuration.desktop.template) [columnSettings.configuration.tablet.template](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:columnSettings.configuration.tablet.template) [columnSettings.configuration.phone.template](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:columnSettings.configuration.phone.template) columnSettings.configuration.&lt;custom_mode&gt;.template |
| RWD mode profiles | You can specify custom RWD mode profiles in addition to the pre-defined profiles. **Related topics** [Creating Custom Responsive Web Design (RWD) Mode Profiles](/configure/iggrid-responsive-web-design-mode-creating-custom-profile.mdx) | [responsiveModes](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:responsiveModes) responsiveModes.&lt;custom_mode&gt; |
| Bootstrap support | You can configure RWD mode using Twitter Bootstrap’s RWD classes. **Related topics** [Configuring Bootstrap Support](/configure/iggrid-responsive-web-design-mode-configuring-bootstrap-support.mdx) | `.visible-phone` `.visible-tablet` `.visible-desktop` `.hidden-phone` `.hidden-tablet` `.hidden-desktop` |


### <a id="profiles-configuration"></a> RWD mode default profiles configuration

By default, RWD mode is configured to use `$.ig.InfragisticsMode` class as a provider for the RWD mode configuration. This is done in the [`responsiveModes`](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:responsiveModes) option, which contains the RWD mode profiles where each profile is a string containing the name of the class which implements the logic of profile switching. The following table lists the default settings of the respective properties.

Property | Type | Default Value | Description
---|---|---|---
[responsiveModes.desktop](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:responsiveModes.desktop) | string | “infragistics” | The desktop profile is provided by `$.ig.InfragisticsMode` class.
[responsiveModes.tablet](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:responsiveModes.tablet) | string | “infragistics” | The tablet profile is provided by `$.ig.InfragisticsMode` class.
[responsiveModes.phone](&#123;environment:jQueryApiUrl&#125;/ui.iggridresponsive#options:responsiveModes.phone) | string | “infragistics” | The phone profile is provided by `$.ig.InfragisticsMode` class.
