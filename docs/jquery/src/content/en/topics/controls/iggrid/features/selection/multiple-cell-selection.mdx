---
title: "Multiple Cell Selection (igGrid)"
...
- [Touch Support for &#123;environment:ProductName&#125; Control](////general-and-getting-started/touch-support-for-igniteui-for-jquery-controls.mdx): This topic introduces to user to the new updates that &#123;environment:ProductName&#125; controls has to support touch interactions.

- [igGrid Selection](/iggrid-selection-overview.mdx): This topic shows you how to enable and use `igGrid` Selection.
...
The `igGrid` Selection property that allows users to drag and select multiple cells under Desktop platforms is called <ApiLink type="iggridselection" member="mouseDragSelect" section="options" label="mouseDragSelect" /> and must be set to `true`.

The corresponding property for multiple cell selection via touch is <ApiLink type="iggridselection" member="touchDragSelect" section="options" label="touchDragSelect" /> and must also be set to `true`.
...
-   [igGrid Selection](&#123;environment:SamplesUrl&#125;/grid/selection)


### <a id="clicks-taps"></a> Multiple Cell Selection by multiple clicks/taps

Achieving selection of non-contiguous cells (random locations) is done by holding **CTRL** + cell click when selecting cells in a Desktop environment. Enabling the same functionality for touch environments requires the <ApiLink type="iggridselection" member="multipleCellSelectOnClick" section="options" label="multipleCellSelectOnClick" /> property to be set to `true`. The effect of setting this property is that it ensures anytime a user clicks a new cell the existing selection preserved as if **CTRL** is being pressed on the keyboard. Touch-enabled environments take advantage of this behavior in order to achieve non-contiguous selection.
...
---|---|---
<ApiLink type="iggridselection" member="multipleSelection" section="options" label="multipleSelection" /> | false | Enables/disables multiple selection.
<ApiLink type="iggridselection" member="multipleCellSelectOnClick" section="options" label="multipleCellSelectOnClick" /> | false | Enable/disables multiple cell selection as if the CTRL key is being held down when a cell is clicked or tapped.
<ApiLink type="iggridselection" member="mouseDragSelect" section="options" label="mouseDragSelect" /> | true | Enables/disables selection by dragging the mouse.
<ApiLink type="iggridselection" member="touchDragSelect" section="options" label="touchDragSelect" /> | true | Enables/disables selection by drag gesture.
...
- [Enabling igHierarchicalGrid Selection](///ighierarchicalgrid/features/selection/jquery-ighierarchical-grid-selection-overview.mdx): Describes how to configure Selection on the `igHierarchicalGrid` control.
...
- [Selection](&#123;environment:SamplesUrl&#125;/grid/selection): This sample demonstrates configuration of cell selection in the `igGrid` control.
