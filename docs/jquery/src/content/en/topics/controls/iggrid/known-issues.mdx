...
[Column captions not wrapping onto multiple lines](#column-caption-wrap) | Column captions ([`headerText`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.headerText)) do not wrap onto multiple lines. This is a breaking change from the previous versions’ functionality. | ![](../../images/images/positive.png)
...
[The showHeader option not working correctly](#showHeader) | When the [`showHeader`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:showHeader) option is set to false on grid initialization, setting it to true run-time using the API will not show the header. | ![](../../images/images/positive.png)
...
| [Unbound column values not updated when remote paging and *unboundValues* is used](#unboundValues-remote-paging) | The grid displays same values for the unbound column when [`unboundValues`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.unboundValues) is set on the client with remote paging enabled. | ![](../../images/images/negative.png) |
...
| `igGrid`’s [`getUnboundValues()`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#methods:getUnboundValues) method of grids does not return unbound values in certain situations | Using the `getUnboundValues()` client API method does not return unbound values if they are set through the [`dataBound`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#events:dataBound) client-side event. | ![](../../images/images/negative.png) |
...
| Grid and its columns (either explicitly or using the [defaultColumnWidth](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:defaultColumnWidth) option) widths are mandatory and should be defined in pixels units. | This is a third party issue in Chrome and Safari. In those browsers the thead element, which contains the fixed headers, stretches to the full width of the caption, which may lead to the unfix button going outside of the visible area of the header (in cases where the column width is smaller than the caption width). All other browsers retain the same column width in the fixed grid area and don't exhibit this behavior. | ![](../../images/images/negative.png) |
...
Issue | Description | Status
------|-------------|-------
Multi-column headers feature is not supported with [`columnVirtualization`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columnVirtualization) | When virtualization and multi-column headers features are enabled, only [`rowVirtualization`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:rowVirtualization) is integrated. Column virtualization is not supported in this scenario. | ![](../../images/images/negative.png)


## igGridFiltering
Issue | Description | Status
------|-------------|-------
[Simple Filtering does not work with column virtualization](#simple-filtering)| Filtering mode = "simple" does not work with column virtualization ([columnVirtualization](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columnVirtualization) = true) | ![](../../images/images/positive.png)
...
[Values in a column grouped but not displayed correctly](#groupby-incorrect-values) | When [`autoGenerateColumns`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:autoGenerateColumns) is set to true and a column is set to be grouped at initialization, the values in the column are grouped but not displayed correctly. | ![](../../images/images/positive.png)
...
[The *igGridRowSelectors* widget requires the Selection feature enabled](#row-selectors-selection) | The [`requireSelection`](&#123;environment:jQueryApiUrl&#125;/ui.iggridrowselectors#options:requireSelection) option of the `igGridRowSelectors` widget is set to true by default and will throw an exception indicating that the Selection feature is required for using row selectors. | ![](../../images/images/positive.png)
...
["Select All"](./03_Features/02_Row Selectors/01_igGrid_Configuring_Row_Selectors.mdx#select-all-pages) functionality does not work for remote Paging | The [`enableSelectAllForPaging`](&#123;environment:jQueryApiUrl&#125;/ui.iggridrowselectors#options:enableSelectAllForPaging) option is not working for remote Paging scenario. | ![](../../images/images/negative.png)
...
[Limitation when setting custom methods](#summaries-custom-methods) | When setting custom methods, it is highly recommended to set the order and [`summaryCalculator`](&#123;environment:jQueryApiUrl&#125;/ui.iggridsummaries#options:columnSettings.summaryOperands.summaryCalculator) options of the summary operands. | ![](../../images/images/positive.png)
...
> **Workaround 2** 
> 
> Use this workaround if you need to use a `<div>` element as the base element for the grid. The calls to the grid's features API should rely on the table to which they are actually attached: the `[gridElementId]_table`. If the grid is bound to a `<div>` element with `id` attribute set to "grid1", the easiest way to reference that table is to use either the grid's [`id()`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#methods:id) or [`widget()`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#methods:widget) API methods as shown below.
...
When the [`showHeader`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:showHeader) option is set to false on grid initialization, setting it to true run-time using the API will not show the header.
...
> **Workaround** 
> 
> In all cases, call the [`dataBind`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#methods:dataBind) method to render the rows in the correct order.
...
### <a id="SetUnboundValues"></a> The grid SetUnboundValues(&lt;Column key&gt;, &lt;Dictionary of values&gt;) method overload of the &#123;environment:ProductNameMVC&#125; Grid requires a primary key
...
> **Workaround** 
> 
> You can set height for the TD element in the rows and set [`syncRowHeights`](&#123;environment:jQueryApiUrl&#125;/ui.iggridcolumnfixing#options:syncRowHeights) to false (because of a performance issue)
...
>**Workaround**: Use advanced Filtering ([mode](&#123;environment:jQueryApiUrl&#125;/ui.iggridfiltering#options:mode) = "advanced") with column virtualization
...
When [`autoGenerateColumns`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:autoGenerateColumns) is set to true (explicitly or by default) and a column is set to be grouped at initialization, the values in the column are grouped but not displayed correctly.
...
The `ui.groupedColumns` input argument of the [`groupedColumnsChanged`](&#123;environment:jQueryApiUrl&#125;/ui.iggridgroupby#events:groupedColumnsChanged) event may be empty when a column has been grouped by dragging it into the Group By area.

> **Workaround** 
> 
> The `ui.groupedColumns` array shows only grouped columns on the root level (as if it is acting as a flat grid, as in any flat grid with Group By). If you want to get all grouped columns, use the [`groupByColumns()`](&#123;environment:jQueryApiUrl&#125;/ui.iggridgroupby#methods:groupByColumns) function from the API, which recursively iterates all child grids and adds up the grouped columns for any layout.
...
> **Workaround** 
> 
> Set [`modalDialogDropDownWidth`](&#123;environment:jQueryApiUrl&#125;/ui.iggridgroupby#options:modalDialogDropDownWidth) and [`modalDialogDropDownAreaWidth`](&#123;environment:jQueryApiUrl&#125;/ui.iggridgroupby#options:modalDialogDropDownAreaWidth) to values that allow showing the whole layouts tree. Another workaround would be to set all values to null.
...
> **Workaround** 
> 
> Hook to appropriate event (it’s recommended to hook to [`summariesCalculating`](&#123;environment:jQueryApiUrl&#125;/ui.iggridsummaries#events:summariesCalculating) event) and use your own logic to request the summaries from the server.

### <a id="summaries-custom-methods"></a> Limitation when setting custom methods

When setting custom methods, it is highly recommended to set the order and [`summaryCalculator`](&#123;environment:jQueryApiUrl&#125;/ui.iggridsummaries#options:summaryCalculator) options of the summary operands. If you do not define order, the summary calculator will probably not behave as you expect it – the order of summary operands in the drop-down and in the summaries may differ. When you set the [`summaryOperand`](&#123;environment:jQueryApiUrl&#125;/ui.iggridsummaries#options:summaryOperand) to custom type but do not set the [`customCalculator`](&#123;environment:jQueryApiUrl&#125;/ui.iggridsummaries#options:customCalculator), the `summaryOperand` will not be shown neither in the results in the summaries nor in the drop-down.
...
-   The [`fadeTimespan`](&#123;environment:jQueryApiUrl&#125;/ui.iggridtooltips#options:fadeTimespan) value exceeds either of the following values: [`showDelay`](&#123;environment:jQueryApiUrl&#125;/ui.iggridtooltips#options:showDelay) or [`hideDelay`](&#123;environment:jQueryApiUrl&#125;/ui.iggridtooltips#options:hideDelay)
-   The [`showDelay`](&#123;environment:jQueryApiUrl&#125;/ui.iggridtooltips#options:showDelay) and [`hideDelay`](&#123;environment:jQueryApiUrl&#125;/ui.iggridtooltips#options:hideDelay) values are too low (less than 200 ms).
...
### <a id="updating-edit-virtualization"></a> Updating rows/cells when the Updating and Continuous Virtualization features are enabled while [`autoCommit`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:autoCommit) is disabled is not supported.
Editing rows/cells with Updating when the [`autoCommit`](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:autoCommit) grid option is set to false and Continuous Virtualization is enable will throw exception.
