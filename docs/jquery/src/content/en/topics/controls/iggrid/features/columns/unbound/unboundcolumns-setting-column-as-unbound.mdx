---
title: "Setting a Column as Unbound (igGrid)"
slug: iggrid-unboundcolumns-setting-column-as-unbound
---
...
## Setting a Column as Unbound – Overview
...
## Setting a Column as Unbound in JavaScript (Code Example)

Populate unbound column values as part of grid initialization code by using the <ApiLink type="iggrid" member="columns.unboundValues" section="options" label="unboundValues" /> property. In this scenario, unbound column data should already be available at the time the grid is initializing. It is important to note that unbound values populate sequentially in the unbound column, so the order of the grid data and order of `unboundValues` data should match.

Note: There are other ways to set unbound column values on the client that are covered in [Populating Unbound Columns Locally (igGrid)](/working/populate/iggrid-unboundcolumns-populating-with-data-locally.mdx) topic.
...
## Setting a Column as Unbound in ASP.NET MVC (Code Example)
...
> **Note:** There are other ways to set unbound column values on the server covered in [Populating Unbound Columns Remotely (igGrid)](/working/populate/iggrid-unboundcolumns-populating-with-data-remotely.mdx) topic.
...
The View is strongly typed with model `IQueryable<UnboundColumns.Models.Employee>`. The &#123;environment:ProductNameMVC&#125; Grid uses this model to bind to data. The code configures the grid with one unbound column with key `FullName` and provides its values by the `ViewData` variable with key `EmployeeFullName`.
...
- [Working with Unbound Columns(igGrid)](/working/iggrid-unboundcolumns-working-with-landingpage.mdx): This group of topics explains how an unbound column can be populated with data on the client or on the server.
