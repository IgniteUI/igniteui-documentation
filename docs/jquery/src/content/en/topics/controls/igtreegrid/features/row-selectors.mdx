---
title: "Row Selectors (igTreeGrid)"
...
## Row Selector`s Numbering Modes
In the `igTreeGrid` there are two possible modes for the row selectors numbering<ApiLink type="igtreegridrowselectors" member="rowSelectorNumberingMode" section="options" label="mode" /> – sequential and hierarchical.
