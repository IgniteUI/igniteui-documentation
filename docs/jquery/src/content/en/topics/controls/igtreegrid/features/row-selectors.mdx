...
In the `igTreeGrid` there are two possible modes for the row selectors numbering[`mode`](&#123;environment:jQueryApiUrl&#125;/ui.igtreegridrowselectors#options:rowSelectorNumberingMode) – sequential and hierarchical.
