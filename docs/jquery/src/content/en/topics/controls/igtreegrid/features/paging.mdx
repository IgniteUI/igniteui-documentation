...
Paging <ApiLink type="igtreegridpaging" member="mode" section="options" label="mode" /> option controls that functionality and by default is set to operate only on root level records. In the default mode changing the visible rows by expanding or collapsing rows will not affect the page count.
...
For scenarios where the child data flows to the next page (when paging mode is set to "allLevels") and context needs to be introduced for the child level row, the TreeGridPaging feature introduces an additional <ApiLink type="igtreegridpaging" member="contextRowMode" section="options" label="contextRowMode" /> option that allows rendering of a context row below the grid headers that contains information on the parent row(s) of the current child level row.
There are three possible modes to choose from: "none", "parent" and "breadcrumb".

### No context row (default)
...
<ApiLink type="igtreegridpaging" member="breadcrumbKey" section="options" label="breadcrumbKey" /> - sets the column key of ancestor to be shown in the breadcrumb trail. By default uses the primary key column.

<ApiLink type="igtreegridpaging" member="breadcrumbKey" section="options" label="breadcrumbDelimiter" /> - sets the delimiter between ancestors in the breadcrumb trail. By default is ' &gt; '.

### Custom context row rendering

The whole context row rendering can be handled manually using the <ApiLink type="igtreegridpaging" member="renderContextRowFunc" section="options" label="renderContextRowFunc" /> option, which allows setting a custom function to handle the rendering of the context row.
...
### Topics
-	[Remote Features (igTreeGrid)](/igtreegrid-remote-features.mdx): This topic contains an overview and implementation details on performing remote operations with `igTreeGrid` features.
