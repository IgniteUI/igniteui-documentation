...
1.	Sorting against a column sorts the grid's data recursively at each level. To control the range of levels affected by the feature two additional properties are available - <ApiLink type="igtreegridsorting" member="fromLevel" section="options" label="fromLevel" /> and <ApiLink type="igtreegridsorting" member="toLevel" section="options" label="toLevel" /> defining the first and last bound levels in the hierarchy to be sorted.
...
The extended Updating feature adds support for hierarchy within a single grid, maintains functionality such as inline editing experience or row edit template and takes advantage of the to the underlying <ApiLink pkg="ig" type="treehierarchicaldatasource" label="igTreeHierarchicalDataSource" />.
