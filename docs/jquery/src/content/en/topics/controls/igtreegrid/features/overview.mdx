---
title: "Features Overview (igTreeGrid)"
...
1.	Sorting against a column sorts the grid's data recursively at each level. To control the range of levels affected by the feature two additional properties are available - [`fromLevel`](&#123;environment:jQueryApiUrl&#125;/ui.igtreegridsorting#options:fromLevel) and [`toLevel`](&#123;environment:jQueryApiUrl&#125;/ui.igtreegridsorting#options:toLevel) defining the first and last bound levels in the hierarchy to be sorted.
...
**Related topic:** [Paging (igTreeGrid)](/igtreegrid-paging.mdx)
...
**Related topic:** [Filtering (igTreeGrid)](/igtreegrid-filtering.mdx)


### <a id="updating"></a> Updating

The extended Updating feature adds support for hierarchy within a single grid, maintains functionality such as inline editing experience or row edit template and takes advantage of the to the underlying [`igTreeHierarchicalDataSource`](&#123;environment:jQueryApiUrl&#125;/ig.treehierarchicaldatasource).

**Related topic:** [Updating (igTreeGrid)](/igtreegrid-updating.mdx)
...
- [Load on Demand](&#123;environment:SamplesUrl&#125;/tree-grid/load-on-demand)
