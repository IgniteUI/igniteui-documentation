...
As you become acquainted with the `igTreeGrid` you'll need to be comfortable with a few terms which have specific meaning in the grid's [API](&#123;environment:jQueryApiUrl&#125;/ui.igtreegrid) and documentation.
...
The tree grid enjoys a disconnected architecture in the same manner as other &#123;environment:ProductName&#125; grids. Under the surface the `igTreeGrid` is powered by the `igTreeHierarchialDataSource` component. The data source component is responsible for implementing logic for features that directly affect the tree grid's source data before its ready to be presented to the user. For details about this specialized data source, see [igTreeHierarchicalDataSource](&#123;environment:jQueryApiUrl&#125;/ig.treehierarchicaldatasource).
...
The hierarchical data's relationship is managed by using an array of objects in the `employees` member. To initialize the tree grid with hierarchical data you need to use the [`childDataKey`](&#123;environment:jQueryApiUrl&#125;/ui.igtreegrid#options:childDataKey) option to establish the relationship:
...
> **Note:** Currently the `igTreeGrid` supports only **continuous** flavor of Virtualization, therefore the [`virtualizationMode`](&#123;environment:jQueryApiUrl&#125;/ui.igtreegrid#options:virtualizationMode) must always be set to the appropriate value:
...
Initializing the `igTreeGrid` requires that you pass an [`options`](&#123;environment:jQueryApiUrl&#125;/ui.igtreegrid#options) object that defines the characteristics of the grid. The following code snippet demonstrates how to create a grid bound to a flat data source that includes filtering, sorting and paging.
