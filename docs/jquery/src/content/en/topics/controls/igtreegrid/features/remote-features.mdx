---
title: "Remote Features (igTreeGrid)"
...
-   [Features Overview (igTreeGrid)](/igtreegrid-features-overview.mdx): This topic covers the basics around the modular features available for the `igTreeGrid` control. 
-   [Filtering (igTreeGrid)](/igtreegrid-filtering.mdx): This topic explains how the `igTreeGridFiltering` operates on hierarchical data.
...
All features share the same <ApiLink type="igtreegrid" member="dataSourceUrl" section="options" label="dataSourceUrl" /> endpoint address (also used for [Load on Demand](/igtreegrid-load-on-demand.mdx)) for requesting additional data. This means that if there are any custom back-end implementations for multiple remote features they need to be able to handle more than one style of request by reading the provided parameters.
...
### Binding to flat data in &#123;environment:ProductNameMVC&#125;
...
Enable the remote operation by setting the <ApiLink type="igtreegridfiltering" member="type" section="options" label="type" /> feature option to `'remote'`. Data requests for Filtering will supply at least one `filter(<property>)` style parameter. If Advanced <ApiLink type="igtreegridfiltering" member="mode" section="options" label="mode" /> is enabled a `filterLogic` conditional operator is also sent specifying if the criteria must all match ("AND") or any one is enough ("OR"). Another parameters in the request `fromLevel` and `toLevey` specify from which to which level the `igTreeGrid` is going to be filtered. It will look similar to the following:
...
**Related topic:** [Remote Filtering (igGrid)](../../igGrid/03_Features/00_Columns/07_igGrid_Filtering.mdx#remote)

### Remote Sorting

Enable the remote operation by setting the <ApiLink type="ui.igtreegridsorting" member="type" section="options" label="type" /> feature option to `'remote'`. Ajax requests have at least one `sort(<propertyName>)` style parameter (depending on the <ApiLink type="igtreegridsorting" member="mode" section="options" label="mode" />) that defines either ascending or descending sorting direction for each object property. There are two additional parameters defining from which(fromlevel) to which level(toLevel) sorting is performed:
...
**Related topic:** [Remote Sorting (igGrid)](../../igGrid/03_Features/00_Columns/09_Sorting/00_igGrid_Sorting_Overview.mdx#remote)

### Remote Paging

Enable the remote operation by setting the <ApiLink type="igtreegridpaging" member="type" section="options" label="type" /> feature option to `'remote'`. Remote Paging operations use following parameters - the `page` index the user is requesting, the respective row count per page set by `pageSize`, `listExpansionStates[2]` which keeps the information about the expansion state.  Every time a node is expanded or collapsed a request is sent in order to update this property and preserve this state across user interactions:
...
**Related topic:** [Remote Paging (igGrid)](../../igGrid/03_Features/06_igGrid_Paging.mdx#remote)
...
### Samples
- [igTreeGrid Remote Features](&#123;environment:SamplesUrl&#125;/tree-grid/remote-features)
