...
## Configuring igDialog with External Page
...
--- | --- | ---
Show header text – title of the site | <ApiLink type="igDialog" member="headerText" section="options" label="headerText" /> | “Infragistics”
Make the `igDialog` maximize- able | <ApiLink type="igDialog" member="showMaximizeButton" section="options" label="showMaximizeButton" /> | true
Set a temporary `IFRAME` URL source. | <ApiLink type="igDialog" member="temporaryUrl" section="options" label="temporaryUrl" /> | “Infragistics.com”


> **Note:** It is not obligatory to use the <ApiLink type="igDialog" member="temporaryUrl" section="options" label="temporaryUrl" /> property, but using it may prevent exceptions. When the `igDialog` for the external page is created, the target element is removed from its original container and inserted into dynamically created main element. This may trigger an exception if the `IFRAME` element does not have some temporary URL as a source. If an error appears while loading external page, you can use the <ApiLink type="igDialog" member="temporaryUrl" section="options" label="temporaryUrl" /> property, which will set impermanent source of the `IFRAME` element.
