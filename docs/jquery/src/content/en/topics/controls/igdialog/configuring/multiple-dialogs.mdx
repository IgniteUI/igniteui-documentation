...
## Configuring Multiple igDialogs
...
--- | --- | --- | ---
Get reference to the top modal dialog | <ApiLink type="igDialog" member="getTopModal" section="methods" label="getTopModal()" /> | None | object – reference to the `igDialog`
Check if dialog is modal and it is currently active | <ApiLink type="igDialog" member="isTopModal" section="methods" label="isTopModal()" /> | None | boolean
Moves not a modal dialog to the top | <ApiLink type="igDialog" member="moveToTop" section="methods" label="moveToTop(e)" /> | *e* – browser event | object – reference to the moved `igDialog`
