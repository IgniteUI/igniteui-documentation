...
--- | --- | ---
Opened | *In this state, igDialog is opened. This is the default state of the control.* | [Show and Hide](./02_Configuring/00_igDialog Show and Hide.mdx)
Closed | *In this state igDialog is closed.* | [Show and Hide](./02_Configuring/00_igDialog Show and Hide.mdx)
Minimized | *In this state igDialog is minimized.* | [Maximize and Minimize Window](./02_Configuring/01_igDialog Maximize and Minimize.mdx)
Maximized | *In this state igDialog is maximized.* | [Maximize and Minimize Window](./02_Configuring/01_igDialog Maximize and Minimize.mdx)
...
-   [Showing and hiding igDialog](./02_Configuring/00_igDialog Show and Hide.mdx)
...
-   [Maximize and Minimize igDialog](./02_Configuring/01_igDialog Maximize and Minimize.mdx)
...
-   [Pin igDialog](./02_Configuring/02_igDialog Pin.mdx)
...
You can drag and drop the `igDialog`. The only property that needs to be modified to support this behavior is <ApiLink type="igDialog" member="draggable" section="options" label="draggable" />, which is set to true as its default.
...
You can drag and drop the `igDialog`. The only property that needs to be modified to support this behavior is <ApiLink type="igDialog" member="resizable" section="options" label="resizable" />, which is set to true as its default.
...
-   [Positioning igDialog](./02_Configuring/03_igDialog Position.mdx)

**Related Samples:**

-   [API and Events](./03_API Reference/02_igDialog Event Reference.mdx#attaching-handlers-jquery)
...
-   [igDialog Header and Footer](./02_Configuring/04_igDialog Header and Footer.mdx)
...
You can close the `igDialog` using the “ESC” key. The only property that needs to be modified to support this behavior is <ApiLink type="igDialog" member="closeOnEscape" section="options" label="closeOnEscape" />, which is set to true by default.

**Related Topics:**

-   [Showing and hiding Dialog Window](./02_Configuring/00_igDialog Show and Hide.mdx)
...
-   [Modal igDialog](./02_Configuring/06_igDialog Modal State.mdx)
...
-   [Multiple igDialogs](./02_Configuring/07_igDialog Multiple Dialogs.mdx)
...
-   [External igDialogs](./02_Configuring/05_igDialog External Page.mdx)
...
-   [Animate igDialogs](./02_Configuring/08_igDialog Animations.mdx)

### Focus

Sets and retrieves the ability to process focus and blur events of the control itself, as well as its child elements located in the dialog. The only property that needs to be modified to support this behavior is <ApiLink type="igDialog" member="trackFocus" section="options" label="trackFocus" />, which is set to true by default.
...
- [Configuring *igDialog*](./02_Configuring/~Configuring igDialog.mdx): This topic references all of the main `igDialog` features, their configuration, and usage.

- [*igDialog* API Reference](./03_API Reference/~igDialog API Reference.mdx): This topic introduces `igDialog` API categories. You can find all of the references to the control properties, methods, events and CSS classes, as well as some specific cases when the API is used.

- [*igDialog* Known Issues and Limitations](./05_igDialog Known Issues.mdx): This topic demonstrates the known issues for the `igDialog` control.
