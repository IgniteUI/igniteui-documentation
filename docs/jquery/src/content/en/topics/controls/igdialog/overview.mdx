...
You can drag and drop the `igDialog`. The only property that needs to be modified to support this behavior is [`draggable`](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:draggable), which is set to true as its default.
...
You can drag and drop the `igDialog`. The only property that needs to be modified to support this behavior is [`resizable`](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:resizable), which is set to true as its default.
...
You can close the `igDialog` using the “ESC” key. The only property that needs to be modified to support this behavior is [`closeOnEscape`](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:closeOnEscape), which is set to true by default.
...
Sets and retrieves the ability to process focus and blur events of the control itself, as well as its child elements located in the dialog. The only property that needs to be modified to support this behavior is [`trackFocus`](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:trackFocus), which is set to true by default.
