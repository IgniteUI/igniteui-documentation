...
| Configuring pin/unpin | The properties that need to be configured to allow pin and unpin of the *igDialog* using control UI. | [showPinButton](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:showPinButton) [pinned](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:pinned) |
| Pin on minimized | The property that allows you to configure the *igDialog* so that when the state is changed to minimized, it becomes pinned to its parent. | [pinOnMinimized](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:pinOnMinimized) |
| Pin *igDialog* | Method from *igDialog* API that allows pinning. | [pin()](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#methods:pin) |
| Unpin *igDialog* | Method from *igDialog* API that allows unpinning. | [unpin()](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#methods:unpin) |


## <a id="pin-unpin"></a> Configuring Pin/Unpin

The table below demonstrates which properties need to be configured in order to pin the `igDialog` control. Setting [`showPinButton`](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:showPinButton) property will enable the pin icon in the header, while setting the [`pinned`](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:pinned) property will configure the initial state of the control.
...
--- | --- | ---
Show pin button | [showPinButton](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:showPinButton) | true
Pin the igDialog | [pinned](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options) | true
...
In order to: | Use this property: | And set it to:
--- | --- | ---
Pin on minimize | [pinOnMinimized](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:pinOnMinimized) | true
Minimize igDialog | [state](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:state) | “minimized”
...
As a result of the configuration from the previous paragraph, you will be able to pin the dialog window when the window is unpinned and by pressing the button at the top right corner of the header. If you have the [`showPinButton`](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:showPinButton) option disabled, then you can pin the control using its API.
...
As a result of the configuration from the previous paragraph, you will be able to unpin the dialog window, when the window is pinned and by pressing the button at the top right corner of the header. If you have the [`showPinButton`](&#123;environment:jQueryApiUrl&#125;/ui.igDialog#options:showPinButton) option disabled, then you can unpin the control using its API.
