...
`igTileManager` is bound to data in the same way as any other control of the &#123;environment:ProductName&#125;® library. Data is bound either by assigning a data source to the [dataSource](&#123;environment:jQueryApiUrl&#125;/ui.igtilemanager#options:dataSource) option or by providing a URL in the [dataSourceUrl](&#123;environment:jQueryApiUrl&#125;/ui.igtilemanager#options:dataSourceUrl) if data is provided by either a web service or Windows Communication Foundation (WCF) service. The `igTileManager` control creates and uses an [igDataSource](&#123;environment:jQueryApiUrl&#125;/ig.datasource) object to handle data.
...
[`igDataSource`](&#123;environment:jQueryApiUrl&#125;/ig.datasource) |The `igDataSource` component is used internally by the `igTileManager` control to manage data operations for the control. The data source accepts many different types of local and remote data.
