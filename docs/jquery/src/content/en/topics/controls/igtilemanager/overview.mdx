...
---|---|---|---
Maximize a tile |A mouse click on a minimized tile |Maximizing can be enabled (default) or disabled with the [preventMaximizingSelector](&#123;environment:jQueryApiUrl&#125;/ui.igtilemanager#options:preventMaximizingSelector) property.|![](../../images/images/positive.png)
...
Resize the panels|The splitter bar|The splitter can be disabled and hidden from view by setting the [showSplitter](&#123;environment:jQueryApiUrl&#125;/ui.igtilemanager#options:showSplitter) option to false.|![](../../images/images/positive.png)
Scroll the Minimized Tiles panel|The scroll bar|The scroll bar can be prevented from appearing by setting the [showRightPanelScroll](&#123;environment:jQueryApiUrl&#125;/ui.igtilemanager#options:showRightPanelScroll) option to false.|![](../../images/images/positive.png)
...
Animation can be disabled by setting the [animationDuration](&#123;environment:jQueryApiUrl&#125;/ui.igtilemanager#options:animationDuration) option to 0. In this case, the clicked minimized tile will just appear in the Maximized Tile panel as maximized.
...
It is possible to specify which tile to be initially maximized when the `igTileManager` control is rendered. This is done by providing the index number of the tile to be maximized as the value of the [maximizedTileIndex](&#123;environment:jQueryApiUrl&#125;/ui.igtilemanager#options:maximizedTileIndex) option.
...
The rows and columns of the internal grid in the Minimized Tiles panel can be pre-defined using the [rightPanelCols](&#123;environment:jQueryApiUrl&#125;/ui.igtilemanager#options:rightPanelCols) option. This option has effect only in Dual Panel mode.
...
The margins of the minimized tiles define the space around each tile in the grid. The top and the left margin of the minimized tiles are configurable thus forming an outset between the tiles. Tiles’ margins are configured with the [marginTop](&#123;environment:jQueryApiUrl&#125;/ui.igtilemanager#options:marginTop) and [marginLeft](&#123;environment:jQueryApiUrl&#125;/ui.igtilemanager#options:marginLeft) options.
