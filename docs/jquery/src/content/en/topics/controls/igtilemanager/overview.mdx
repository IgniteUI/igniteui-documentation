...
- [igLayoutManager Overview, Grid Layout section](/iglayoutmanager/overview.mdx): This section provides a conceptual overview of the Grid layout of the the `igLayoutManager`™ control.
...
Any &#123;environment:ProductName&#125;® control can be placed inside those tiles thus enabling moving and resizing of tiles, and changing the state of the tiles at run-time.
...
---|---|---|---
Maximize a tile |A mouse click on a minimized tile |Maximizing can be enabled (default) or disabled with the <ApiLink type="igtilemanager" member="preventMaximizingSelector" section="options" label="preventMaximizingSelector" /> property.|![](../../images/images/positive.png)
...
Resize the panels|The splitter bar|The splitter can be disabled and hidden from view by setting the <ApiLink type="igtilemanager" member="showSplitter" section="options" label="showSplitter" /> option to false.|![](../../images/images/positive.png)
Scroll the Minimized Tiles panel|The scroll bar|The scroll bar can be prevented from appearing by setting the <ApiLink type="igtilemanager" member="showRightPanelScroll" section="options" label="showRightPanelScroll" /> option to false.|![](../../images/images/positive.png)
...
When the `igTileManager` control in its Dual Panel operational mode, a certain number of columns must be explicitly reserved to accommodate the Maximized Tile panel. For details, see [Configurable number of rows and columns in the Minimized Tiles panel](./03_igTileManager_Configuring.mdx#config-rows-columns).
...
A tile displays different content depending on whether it is minimized and maximized. That is why each of these two states is configured individually using the <ApiLink type="igtilemanager" member="minimizedState" section="options" label="minimizedState" /> and `maximizedState` properties respectively. These properties take selectors as their value and the tile displays the content of the corresponding elements when in the respective state.
...
Animation can be disabled by setting the <ApiLink type="igtilemanager" member="animationDuration" section="options" label="animationDuration" /> option to 0. In this case, the clicked minimized tile will just appear in the Maximized Tile panel as maximized.

**Related topic:**

-   [Configuring igTileManager](/igtilemanager-configuring.mdx)

Scrollable Minimized Tiles panel

The scroll bar of the Minimized Tiles panel allows users to navigate to the minimized tiles that are not visible. Such scrolling can be disabled by setting the <ApiLink type="igtilemanager" member="showRightPanelScroll" section="options" label="showRightPanelScroll" /> option to false. If the scrollbar is disabled, users will only see only as much minimized tiles as the current size of the Minimized Tiles panel can accommodate. The only way to show more minimized tiles in this case would be to enlarge the Minimized Tiles panel using the splitter.

#### Configurable tile sizes and positions

The internal grid that contains the tiles can be configured to have a certain number of rows and columns using the <ApiLink type="igtilemanager" member="cols" section="options" label="cols" /> and <ApiLink type="igtilemanager" member="rows" section="options" label="rows" /> options. This way the size and the positions of the tiles will be automatically calculated for placement in the desired number of rows and columns.
...
It is possible to specify which tile to be initially maximized when the `igTileManager` control is rendered. This is done by providing the index number of the tile to be maximized as the value of the <ApiLink type="igtilemanager" member="maximizedTileIndex" section="options" label="maximizedTileIndex" /> option.
...
The rows and columns of the internal grid in the Minimized Tiles panel can be pre-defined using the <ApiLink type="igtilemanager" member="rightPanelCols" section="options" label="rightPanelCols" /> option. This option has effect only in Dual Panel mode.
...
For the full requirements listing, refer to the [Adding igTileManager](/igtilemanager-adding.mdx) topic.
...
<table class="table table-bordered">
	...
	<tbody>
        <tr>
            <td rowspan="2">[Number of rows and columns](./03_igTileManager_Configuring.mdx#config-rows-columns)</td>
        ...
        <tr>
            <td rowspan="4">[Individual columns](./03_igTileManager_Configuring.mdx#column-dimentions)</td>
            <td>Column height</td>
            <td>The width and height of each individual column in the tile grid are configurable.</td>
        ...
        <tr>
            <td rowspan="2">[Item position and span](./03_igTileManager_Configuring.mdx#config-positions-span)</td>
            <td>Item position</td>
        ...
        <tr>
            <td rowspan="2">[Tile states](#tile-states)</td>
        ...
        <tr>
            <td rowspan="2">[Tiles’ margins](./03_igTileManager_Configuring.mdx#config-tiles-margin)</td>
            <td>Top margin</td>
        ...
        <tr>
            <td rowspan="5">[Minimized Tiles panel](./03_igTileManager_Configuring.mdx#config-minimized-tile-panel)</td>
            <td>Number of columns</td>
        ...
        <tr>
            <td rowspan="2">[Animated transitions’ duration](./03_igTileManager_Configuring.mdx#config-animation-duration)</td>
        ...
        <tr>
            ...
            <td>You can specify elements of a minimized tile that will not trigger maximizing. By default, clicking on `<a>` and `<input>` tags does not trigger maximizing.</td>
...
- [Adding igTileManager](/igtilemanager-adding.mdx): This topic demonstrates, with code examples, how to add the `igTileManager` control to an HTML page in either JavaScript or ASP.NET MVC. This topic covers initializing `igTileManager` on the HTML markup.
...
-  [jQuery and MVC API Links (igTileManager)](/igtilemanager-jquery-and-aspnet-mvc-helper-api-links.mdx): This topic lists the links to the API reference documentation for jQuery and its ASP.NET MVC helper class for the `igTileManager` control.
