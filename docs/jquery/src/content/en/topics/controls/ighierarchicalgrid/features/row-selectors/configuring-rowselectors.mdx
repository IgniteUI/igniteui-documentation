---
title: "Configuring Row Selectors (igHieararchicalGrid)"
...
- [Enabling Row Selection in igHierarchicalGrid](/ighierarchicalgrid-enabling-rowselectors.mdx): Demonstrates how to enable the *igHierarchicalGrid* control’s row selection using jQuery and ASP.NET MVC.
...
-------------------- | ------- | -----------
Selection of multiple rows | By default multiple row selection is enabled. This is controlled through the igGridSelection control. | [multipleSelection](&#123;environment:jQueryApiUrl&#125;/ui.iggridselection_hg#options) (igGridSelection™)
Selection with checkboxes | Enables checkboxes in the Row Selectors column. | [enableCheckBoxes](&#123;environment:jQueryApiUrl&#125;/ui.iggridrowselectors_hg#options)
Row numbering | When enabled, allows row numbering of the Row Selectors column. | [enableRowNumbering](&#123;environment:jQueryApiUrl&#125;/ui.iggridselection_hg#options)
Row numbering starting value | Allows Row numbering to start from a custom value. | [rowNumberingSeed](&#123;environment:jQueryApiUrl&#125;/ui.iggridselection_hg#options)
Width of the row selectors column | Sets the width of the row selector column. | [rowSelectorColumnWidth](&#123;environment:jQueryApiUrl&#125;/ui.iggridselection_hg#options)
Require a Selection to configure | Causes the control to throw an exception if the Selection feature is disabled. | [requireSelection](&#123;environment:jQueryApiUrl&#125;/ui.iggridselection_hg#options)

> **Note**:Keep in mind that `RowSelectors` behavior is per layout. Only one layout can have selection at a time, so if you select/check a row in a different layout, the selected rows from the previous layout will become unselected. For more information see the [Known Issues and Limitations (igHierarchicalGrid)](//ighierarchicalgrid-known-issues.mdx) topic.
...
- [Enabling Row Selectors](/ighierarchicalgrid-enabling-rowselectors.mdx): Demonstrates, with code examples, how to enable the Row Selection feature in jQuery and in ASP.NET MVC.
- [Known Issues and Limitations (igHierarchicalGrid)](//ighierarchicalgrid-known-issues.mdx): This topic summarizes the known issues and limitations of the igHierarchicalGrid control.
