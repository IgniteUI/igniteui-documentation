...
In order to enable REST support for the igHierarchicalGrid you need to set the <ApiLink type="ighierarchicalgrid" label="rest" /> option to `true`. For a load-on-demand scenario (`initialDataBindDepth=-1`) when root layout REST settings `url` is defined, then child layouts do not inherit the Url from the <ApiLink type="iggrid" label="restSettings" />. Instead, the Url is generated from both the root `restSettings` and the child layout configuration.
...
For the child layouts, you can define <ApiLink type="iggrid" label="batch" /> and <ApiLink type="iggrid" label="template" /> options. If you want to use a template for the child layout then you should also define a template for the root `restSettings` because the child layout template is concatenated with the root template.
...
In the following screenshot you can see the REST request made by the igHierarchicalGrid when the <ApiLink type="ighierarchicalgrid" label="saveChanges" /> method is executed.
