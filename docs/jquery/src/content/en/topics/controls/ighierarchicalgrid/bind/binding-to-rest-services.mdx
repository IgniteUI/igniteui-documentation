---
title: "Binding igHierarchicalGrid to REST Services"
...
- [REST Updating (igGrid)](//iggrid/features/rest-updating.mdx):  This topic explains igGrid support for REST services.
...
- [Initializing igHierarchicalGrid](/ighierarchicalgrid-initializing.mdx): This topic demonstrates how to initialize the igHierarchicalGrid in both jQuery and MVC.
...
In order to enable REST support for the igHierarchicalGrid you need to set the [rest](&#123;environment:jQueryApiUrl&#125;/ui.ighierarchicalgrid#options) option to `true`. For a load-on-demand scenario (`initialDataBindDepth=-1`) when root layout REST settings `url` is defined, then child layouts do not inherit the Url from the [restSettings](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options). Instead, the Url is generated from both the root `restSettings` and the child layout configuration.
...
For the child layouts, you can define [batch](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options) and [template](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options) options. If you want to use a template for the child layout then you should also define a template for the root `restSettings` because the child layout template is concatenated with the root template.
...
In the following screenshot you can see the REST request made by the igHierarchicalGrid when the [`saveChanges`](&#123;environment:jQueryApiUrl&#125;/ui.ighierarchicalgrid#methods) method is executed.
...
-   &#123;environment:ProductName&#125; JavaScript and Theme Files
...
- [REST Updating (igGrid)](//iggrid/features/rest-updating.mdx): This topic explains *igGrid* support for REST services.
- [Binding to ASP.NET MVC Web API (igHierarchicalGrid)](/ighierarchicalgrid-binding-to-webapi.mdx): This topic explains how to bind the igHierarchicalGrid to a Web API Service.
