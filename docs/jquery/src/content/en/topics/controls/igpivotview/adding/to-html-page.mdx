...
The data source is specified through either the [dataSource](&#123;environment:jQueryApiUrl&#125;/ui.igPivotView#options:dataSource) parameter or the [dataSourceOptions](&#123;environment:jQueryApiUrl&#125;/ui.igPivotView#options:dataSourceOptions) property of the `igPivotView`. The data source setting is the only mandatory option to set when initializing the `igPivotView`.
...
```
	...
	Following is the alternative (direct) way to specify a data source using the [dataSourceOptions](&#123;environment:jQueryApiUrl&#125;/ui.igPivotView#options:dataSourceOptions) property of the igPivotView. (See [Adding igPivotView summary](#summary).)
