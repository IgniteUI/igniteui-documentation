---
title: "Using Server-Side Events in ASP.NET MVC  (igUpload)"
...
-   [Using the HTTP Handler and Module (igUpload)](/igupload-using-http-handler-and-modules.mdx)
...
| Event | Description | Arguments | Cancelable |
| --- | --- | --- | --- |
| UploadStarting | Triggered when file upload is starting. At this stage the file is yet to be uploaded and the information from the request header is available in the `UploadStartingEventArgs` argument. Having this information you can implement validation rules and decide whether or not to cancel the upload. | object - contains the UploadProgressManager instance which triggered the event. [UploadStartingEventArgs](Infragistics.Web.Mvc~Infragistics.Web.Mvc.UploadStartingEventArgs.html) - contains information for the file to be uploaded. | true |
| FileUploading | Triggered when each chunk of the file is uploaded on the server. In this event the file can be processed manually by reading its current chunk from the FileUploadingEventArgs.FileChunk property. Note: This event is used for [Saving Files as Stream](/igupload-saving-files-as-stream.mdx) scenario. This event is only triggered when FileSaveType="memorystream". | object - contains the UploadProgressManager instance which triggered the event. [FileUploadingEventArgs](Infragistics.Web.Mvc~Infragistics.Web.Mvc.FileUploadingEventArgs.html) - contains the current chunk of data of the uploading file. | true |
| UploadFinishing | Triggered when file upload is finishing. At this stage the file is already uploaded but it’s still has the temporary name. The `igUpload` has released the file and it can be freely modified. | object - contains the UploadProgressManager instance which triggered the event. [UploadFinishingEventArgs](Infragistics.Web.Mvc~Infragistics.Web.Mvc.UploadFinishingEventArgs.html) - contains information of the uploaded file. | true |
| UploadFinished | Triggered when file upload has finished. At this stage the file is uploaded and it is renamed with its original name. If there already is a file with the same name the old one is overwritten, only the last one will be available. | object - contains the UploadProgressManager instance which triggered the event. [UploadFinishedEventArgs](Infragistics.Web.Mvc~Infragistics.Web.Mvc.UploadFinishedEventArgs.html) - contains information about the uploaded file. | false |
...
```
This value can then be retrieved on the client-side on the related client-side events- <ApiLink type="igupload" member="fileUploading" section="events" label="fileUploading" /> and <ApiLink type="igupload" member="fileUploaded" section="events" label="fileUploaded" />. The  `uploadInfo` event argument contains the additional file information, including the serverMessage value send from the server.
...
In order to add additional data to the request you can use the <ApiLink type="igupload" member="onFormDataSubmit" section="events" label="onFormDataSubmit" /> client-side event to add additional data to the request. The <ApiLink type="igupload" member="addDataField" section="methods" label="addDataField" /> and <ApiLink type="igupload" member="addDataFields" section="methods" label="addDataFields" /> methods can be used to add the additional parameters.
...
### Requirements
To complete this procedure you need to follow the steps defined in the [igUpload Overview](/igupload-overview.mdx) topic:
