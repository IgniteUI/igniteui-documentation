---
title: "Handling Events (igPopover)"
...
- [igPopover Overview](/igpopover-overview.mdx): This topic provides conceptual information about the igPopover control including its main features and functionality.
...
Event|Description
---|---
<ApiLink type="igpopover" member="showing" section="events" label="showing" />|Fired before showing the popover.
<ApiLink type="igpopover" member="shown" section="events" label="shown" />|Fired after showing the popover is shown.
<ApiLink type="igpopover" member="hiding" section="events" label="hiding" />|Fired before hiding the popover.
<ApiLink type="igpopover" member="hidden" section="events" label="hidden" />|Fired after hiding the popover is hidden.
...
Example|Description
---|---
[Handling the Event Upon Initialization in jQuery](#jquery-handling)|This example assigns an event handling function to the <ApiLink type="igpopover" member="showing" section="events" label="showing" />event at initialization.
[Handling the Event at Run-Time in ASP.NET MVC](#mvc-handling)|This example assigns an event hander to the <ApiLink type="igpopover" member="showing" section="events" label="showing" /> event at run-time.
[Modifying the Content of igPopover in the Handler of the Event](#modify-content)|This example demonstrates how to change the content of igPopover’s container using the handler for the <ApiLink type="igpopover" member="showing" section="events" label="showing" /> event.
[Re-Positioning igPopover in the Handler of the Event](#re-positioning-igPopover)|This example demonstrates how to change the position of the popover container using the <ApiLink type="igpopover" member="shown" section="events" label="shown" /> event at run-time.
...
This example assigns an event handling function to the <ApiLink type="igpopover" member="showing" section="events" label="showing" /> event at initialization.
...
This example assigns an event handler for the <ApiLink type="igpopover" member="showing" section="events" label="showing" /> event at run-time.
...
This example demonstrates how to change the content of `igPopover`’s container using the handler for the <ApiLink type="igpopover" member="showing" section="events" label="showing" /> event.
...
container using the
<ApiLink type="igpopover" member="shown" section="events" label="shown" />
event at run-time.
...
- [Basic Usage](\{environment:SamplesUrl\}/popover/basic-popover): This sample demonstrates the `igPopover` control in an ASP.NET MVC scenario. The control is initialized in the View using chaining syntax.

- [ASP.NET MVC Basic Usage](\{environment:SamplesUrl\}/popover/aspnet-mvc-helper): This sample demonstrates the basic initialization scenarios (on a single target element and on multiple target elements) of `igPopover` in JavaScript.
