...
---|---
Body|[contentTemplate](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:contentTemplate)
Header|[headerTemplate.title](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:headerTemplate.title)
Close button|[headerTemplate.closeButton](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:headerTemplate.closeButton)
...
[**Single/multiple targets**](#single-multiple-targets)|You can configure the popover display over multiple targets. Using the option [selectors](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:selectors), can specify which elements should display popovers.
...
The side of the target on which the popover is displayed is called “direction” and is configured by setting the  [direction](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:direction) option.
...
If the popover is larger than the target you can further configure its position along of the target DOM element. This is done with the [position](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:position) option.

In addition to that you can further restrict the positioning by providing a container (DIV element for example) whose boundaries serve to restrict the area within which the popover can be shown. The restricting area where the popover should be shown is configurable through the [containment](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:containment) for the popover.
...
You can control the event on which the popover to be shown. This is managed with the [showOn](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:showOn) option. The user actions on which `igPopover` can be activated are:
...
You can configure the popover display over multiple targets. Using the [selectors](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:selectors) option you can specify on which elements the popovers should appear.
...
The [direction](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:direction) and [position](&#123;environment:jQueryApiUrl&#125;/ui.igpopover#options:position) in which the popover shows relative to the target element are also configurable.
