---
title: "Adding igPivotGrid to an HTML Page"
...
The data source is specified through either the <ApiLink type="igPivotGrid" member="dataSource" section="options" label="dataSource" /> parameter or the <ApiLink type="igPivotGrid" member="dataSourceOptions" section="options" label="dataSourceOptions" /> property of the `igPivotGrid`. The data source setting is the only mandatory option to set when initializing the `igPivotGrid`.
...
<br/>
		...
		&lt;tr&gt;
			...
			&lt;td&gt;The styles from the following CSS file are used for rendering various elements of the control: <ul> <li>`<IG CSS root>/structure/modules/infragistics.ui.shared.css`</li> <li>`<IG CSS root>/structure/modules/infragistics.ui.grid.css`</li> <li>`<IG CSS root>/structure/modules/infragistics.ui.pivot.css`</li> </ul>&lt;/td&gt;
...
```
	...
	Following is the alternative (direct) way to specify a data source using the <ApiLink type="igPivotGrid" member="dataSourceOptions" section="options" label="dataSourceOptions" /> property of the igPivotDataSelector. (See [**Adding igPivotGrid summary**](#summary).)
...
- [Adding \{environment:ProductNameMVC\} igPivotGrid](/igpivotgrid-adding-using-the-mvc-helper.mdx): This topic demonstrates how to add the `igPivotGrid`™ control to an ASP.NET MVC application.
