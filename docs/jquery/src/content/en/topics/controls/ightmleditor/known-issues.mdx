...
-	[Known Issues](//known-issues/known-issues-revision-history/revision-history.mdx): Provides reference information for known issues and limitations of all &#123;environment:ProductName&#125; controls.
...
#### Cut, Copy, and Paste toolbar functionality works only in Internet Explorer workaround

For other browsers like Firefox, Chrome, Safari, and Opera, you can use keyboard shortcuts like Ctrl+C, Ctrl+X, Ctrl+V.

#### In Firefox, Symbol, Webdings, and Wingdings fonts are rendered differently than on other browsers.
...
#### In Firefox, if the igHtmlEditor is used in an HTML form with input fields after it, when pressing F5 the input fields are filled with the text from the combo boxes.
...
#### When an igHtmlEditor is used inside an igDialog, the editor is not sized correctly
...
-	[Accessibility Compliance(igHtmlEditor)](/ightmleditor-accessibility-compliance.mdx): This topic explains the accessibility features of the `igHtmlEditor` and gives information on how to achieve accessibility compliance for pages containing `igHtmlEditor`.
