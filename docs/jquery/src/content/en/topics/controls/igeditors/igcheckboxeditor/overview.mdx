---
title: "igCheckboxEditor Overview"
slug: igcheckboxeditor-overview
---

# igCheckboxEditor Overview

The \{environment:ProductName\} check box editor, or "`igCheckboxEditor`", is a control that renders a check box field, which permits users to make a binary choice between one or multiple mutually exclusive options and allows the user to submit the check box value to the server.
...
3.  For jQuery implementations create an INPUT,a SPAN or a DIV as the target element in HTML. This step is optional for ASP.NET MVC implementations as \{environment:ProductNameMVC\} creates the containing element for you.
