---
title: "Configuring igEditors at Runtime"
...
The \{environment:ProductNameMVC\} internally renders an `igEditor` control. So your code should look like this:
...
The following example instantiates an \{environment:ProductNameMVC\} `MaskEditor` control with the HTML helper and then changes the input mask after initialization.
...
The following example instantiates an \{environment:ProductNameMVC\} `PercentEditor` control using the HTML helper and changes the maximum decimals after initialization.
