...
Option| Description
---|---
[allowNullValue](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:allowNullValue)|<a name='allowNullValue'></a>If that option is false, and editor has no value, then value is set to an empty string.
[disabled](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:disabled)|<a name='disabled'></a>Use to set the disabled attribute. Does not allow editing. Disables all the buttons and iteracitons applied. On submit the current value is not sent into the request: `$('#textEditor').igTextEditor({ disabled: true })`.
[buttonType](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:buttonType)|<a name='buttonType'></a>Sets gets visibility of spin, clear and drop-down button.
[listItemHoverDuration](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:listItemHoverDuration)|<a name='listItemHoverDuration'></a>Use to set the hover/unhover animation duration of items inside the dropdown list: `$('#textEditor').igTextEditor({ listItems: ["A", "B", "C", "D"], listItemHoverDuration: 400 })`.
[dropDownAttachedToBody](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:dropDownAttachedToBody)|<a name='dropDownAttachedToBody'></a>Gets sets location of drop-down list.
[dropDownAnimationDuration](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:dropDownAnimationDuration)|<a name='dropDownAnimationDuration'></a>Gets sets show/hide drop-down list animation duration in milliseconds.
[visibleItemsCount](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:visibleItemsCount)|<a name='visibleItemsCount'></a>Use to set how many items should be shown at once: `$('#textEditor').igTextEditor({ listItems: ["A", "B", "C", "D"], visibleItemsCount: 2 })`.
[isLimitedToListValues](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:isLimitedToListValues)|<a name='isLimitedToListValues'></a>Use to set the ability to allow values only set into the list items. Matching is case-insensitive: `$('#textEditor').igTextEditor({ listItems: ["A", "B", "C", "D"], isLimitedToListValues: true })`.
[placeHolder](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:placeHolder)|<a name='placeHolder'></a>Gets sets text which appears in editor when editor has no focus and "value" in editor is null or empty string.
[revertIfNotValid](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:revertIfNotValid)|<a name='revertIfNotValid'></a>Use to set the editor to revert value to previous value in case of not valid value on blur, or enter key. If set to false clear is called: `$('#textEditor').igTextEditor({ listItems: ["A", "B", "C", "D"], isLimitedToListValues: true, revertIfNotValid: false })`.
[preventSubmitOnEnter](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:preventSubmitOnEnter)|<a name='preventSubmitOnEnter'></a>Use to set the ability of the editor to prevent form submition on enter key pressed: `$('#textEditor').igTextEditor({ preventSubmitOnEnter: true })`.
[dropDownOrientation](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:dropDownOrientation)|<a name='dropDownOrientation'></a>Use to set drop down opening orientation for the dorp down list when open button is clicked: `$('#textEditor').igTextEditor({ listItems: ["A", "B", "C", "D"], dropDownOrientation: 'top' })`.
[suppressNotifications](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options:suppressNotifications)|<a name='suppressNotifications'></a>Disables default notifications for basic validation scenarios built in the editors such as required list selection, value wrapping around or spin limits.

[See all options](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#options)
...
Method|Description
---|---
[inputName](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:inputName)|<a name='inputName'></a>Gets sets name attribute applied to the editor element.
[displayValue](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:displayValue)|<a name='displayValue'></a>Returns a string with all the characters shown in the input.
[editorContainer](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:editorContainer)|<a name='editorContainer'></a>Gets reference to jquery object which is used as top/outer element of igEditor.
[dropDownContainer](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:dropDownContainer)|<a name='dropDownContainer'></a>Gets reference to jquery object which is used as container of drop-down.
[showDropDown](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:showDropDown)|<a name='showDropDown'></a>Show the dropdown list.
[hideDropDown](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:hideDropDown)|<a name='hideDropDown'></a>Hide the dropdown list.
[dropDownVisible](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:dropDownVisible)|<a name='dropDownVisible'></a>Returns the visibility state of the drop down listing the items.
[dropDownButton](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:dropDownButton)|<a name='dropDownButton'></a>Returns a reference to the dropdown button of the editor.
[clearButton](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:clearButton)|<a name='clearButton'></a>Returns a reference to the clear button of the editor.
[getSelectionStart](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:getSelectionStart)|<a name='getSelectionStart'></a>Gets start index of the selected text in editor.
[getSelectionEnd](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:getSelectionEnd)|<a name='getSelectionEnd'></a>Gets end index of the selected text in editor.
[insert](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:insert)|<a name='insert'></a>Paste text at location of caret.
[spinUp](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:spinUp)|<a name='spinUp'></a>Selects the previous item from the drop-down list.
[spinDown](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:spinDown)|<a name='spinDown'></a>Selects the next item from the drop-down list.
[spinUpButton](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:spinUpButton)|<a name='spinUpButton'></a>Returns a reference to the spin up button of the editor.
[spinDownButton](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods:spinDownButton)|<a name='spinDownButton'></a>Returns a reference to the spin down button of the editor.

[See all API methods](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#methods)
...
Event|Description
---|---
[rendering](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#events:rendering)|<a name='rendering'></a>Event which is raised before rendering of the editor completes.
[rendered](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#events:rendered)|<a name='rendered'></a>Event which is raised after rendering of the editor completes.
[dropDownListOpening](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#events:dropDownListOpening)|<a name='dropDownListOpening'></a>Event which is raised when the drop down is opening.
[dropDownListOpened](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#events:dropDownListOpened)|<a name='dropDownListOpened'></a>Event which is raised when the drop down is already opened.
[dropDownListClosing](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#events:dropDownListClosing)|<a name='dropDownListClosing'></a>Event which is raised when the drop down is closing.
[dropDownListClosed](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#events:dropDownListClosed)|<a name='dropDownListClosed'></a>Event which is raised when the drop down is already closed.
[dropDownItemSelecting](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#events:dropDownItemSelecting)|<a name='dropDownItemSelecting'></a>Event which is raised when the drop down list item is selecting.
[dropDownItemSelected](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#events:dropDownItemSelected)|<a name='dropDownItemSelected'></a>Event which is raised when the drop down list item is selected.
[mouseout](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#events:mouseout)|<a name='mouseout'></a>Event which is raised on mouseleave at any part of editor including drop-down list.

[See all events](&#123;environment:jQueryApiUrl&#125;/ui.igtexteditor#events)
