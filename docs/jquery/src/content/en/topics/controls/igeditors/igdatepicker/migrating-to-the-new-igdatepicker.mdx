...
Option| Description
---|---
[limitSpinToCurrentField](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#options:limitSpinToCurrentField)|<a name='limitSpinToCurrentField'></a>Sets gets ability to modify only 1 date field on spin events.
[allowNullValue](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#options:allowNullValue)|<a name='allowNullValue'></a>If that option is false, and editor has no value, then value is set to an empty string.
[disabled](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#options:disabled)|<a name='disabled'></a>Use to set the disabled attribute. Does not allow editing. Disables all the buttons and iteracitons applied. On submit the current value is not sent into the request: `$('#datePicker').igDatePicker({ disabled: true })`.
[buttonType](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#options:buttonType)|<a name='buttonType'></a>Sets gets visibility of spin, clear and drop-down button.
[dropDownAttachedToBody](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#options:dropDownAttachedToBody)|<a name='dropDownAttachedToBody'></a>Gets sets location of drop-down list.
[dropDownAnimationDuration](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#options:dropDownAnimationDuration)|<a name='dropDownAnimationDuration'></a>Gets sets show/hide drop-down list animation duration in milliseconds.
[placeHolder](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#options:placeHolder)|<a name='placeHolder'></a>Gets sets text which appears in editor when editor has no focus and "value" in editor is null or empty string.
[revertIfNotValid](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#options:revertIfNotValid)|<a name='revertIfNotValid'></a>Use to set the editor to revert value to previous value in case of not valid value on blur, or enter key. If set to false clear is called.
[preventSubmitOnEnter](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#options:preventSubmitOnEnter)|<a name='preventSubmitOnEnter'></a>Use to set the ability of the editor to prevent form submition on enter key pressed: `$('#datePicker').igDatePicker({ preventSubmitOnEnter: true })`.
[dropDownOrientation](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#options:dropDownOrientation)|<a name='dropDownOrientation'></a>Use to set drop down opening orientation for the dorp down list when open button is clicked.
[suppressNotifications](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#options:suppressNotifications)|<a name='suppressNotifications'></a>Disables default notifications for basic validation scenarios built in the editors such as required list selection, value wrapping around or spin limits.

[See all options](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#options)
...
Method|Description
---|---
[getSelectedDate](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:getSelectedDate)|Gets selected date.
[selectDate](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:selectDate)|Sets selected date.
[getCalendar](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:getCalendar)|<a name='getCalendar'></a>Returns a reference to the jQuery calendar used as a picker selector.
[inputName](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:inputName)|<a name='inputName'></a>Gets sets name attribute applied to the editor element.
[displayValue](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:displayValue)|<a name='displayValue'></a>Returns a string with all the characters shown in the input.
[editorContainer](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:editorContainer)|<a name='editorContainer'></a>Gets reference to jquery object which is used as top/outer element of igEditor.
[showDropDown](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:showDropDown)|<a name='showDropDown'></a>Show the dropdown list.
[hideDropDown](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:hideDropDown)|<a name='hideDropDown'></a>Hide the dropdown list.
[dropDownVisible](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:dropDownVisible)|<a name='dropDownVisible'></a>Returns the visibility state of the drop down listing the items.
[dropDownButton](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:dropDownButton)|<a name='dropDownButton'></a>Returns a reference to the dropdown button of the editor.
[clearButton](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:clearButton)|<a name='clearButton'></a>Returns a reference to the clear button of the editor.
[getSelectionStart](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:getSelectionStart)|<a name='getSelectionStart'></a>Gets start index of the selected text in editor.
[getSelectionEnd](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:getSelectionEnd)|<a name='getSelectionEnd'></a>Gets end index of the selected text in editor.
[insert](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:insert)|<a name='insert'></a>Paste text at location of caret.
[spinUp](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:spinUp)|<a name='spinUp'></a>Increase date-time period, depending on the cursor position.
[spinDown](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:spinDown)|<a name='spinDown'></a>Decrease date-time period, depending on the cursor position.
[spinUpButton](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:spinUpButton)|<a name='spinUpButton'></a>Returns a reference to the spin up button of the editor.
[spinDownButton](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods:spinDownButton)|<a name='spinDownButton'></a>Returns a reference to the spin down button of the editor.

[See all API methods](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#methods)
...
Event|Description
---|---
[rendering](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#events:rendering)|<a name='rendering'></a>Event which is raised before rendering of the editor completes.
[rendered](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#events:rendered)|<a name='rendered'></a>Event which is raised after rendering of the editor completes.
[dropDownListOpening](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#events:dropDownListOpening)|<a name='dropDownListOpening'></a>Event which is raised when the drop down is opening.
[dropDownListOpened](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#events:dropDownListOpened)|<a name='dropDownListOpened'></a>Event which is raised when the drop down is already opened.
[dropDownListClosed](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#events:dropDownListClosed)|<a name='dropDownListClosed'></a>Event which is raised when the drop down is already closed.
[mouseout](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#events:mouseout)|<a name='mouseout'></a>Event which is raised on mouseleave at any part of editor including drop-down list.

[See all events](&#123;environment:jQueryApiUrl&#125;/ui.igdatepicker#events)
