...
-	[`enableUTCDates`](&#123;environment:jQueryApiUrl&#125;/ui.igdateeditor#options:enableUTCDates) - is no longer related to the display value of the editor, it only controls how the date will be serialized. Enables/Disables serializing client date as UTC ISO 8061 string instead of using the local time and zone values. The option is only applied in "date" [`dataMode`](&#123;environment:jQueryApiUrl&#125;/ui.igdateeditor#options:dataMode).
