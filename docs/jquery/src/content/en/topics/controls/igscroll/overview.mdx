...
The igScroll allows specifying two different display types - native or custom, depending on whether you want the native scrollbars to be displayed or the custom ones. The option for setting this is the [`scrollbarType`](&#123;environment:jQueryApiUrl&#125;/ui.igscroll#options:scrollbarType) option.
...
There is inertia enabled by default, so the content will be scrolled initially fast and then the speed will gradually decrease and the scrolling will stop. This behavior can be further modified via the [`inertiaDuration`](&#123;environment:jQueryApiUrl&#125;/ui.igscroll#options:inertiaDuration) option.
...
```
 ...
 The DOM manipulation done by the igScroll can be disabled by setting the [`modifyDOM`](&#123;environment:jQueryApiUrl&#125;/ui.igscroll#options:modifyDOM) property to false. 
...
- [`syncedElemsV`](&#123;environment:jQueryApiUrl&#125;/ui.igscroll#options:syncedElemsV) - Allows setting elements (could be html elements or jQuery elements) that will be linked to the main content container vertically. When the content is scrolled on the Y axis, the linked elements will scroll up/down.
 
- [`syncedElemsH`](&#123;environment:jQueryApiUrl&#125;/ui.igscroll#options:syncedElemsH) - Allows setting elements (could be html elements or jQuery elements) that will be linked to the main content container horizontally. When the content is scrolled on the X axis, the linked elements will scroll left/right.
...
3. Next initialize the two igScrolls (for each scrollable container) and set the [`syncedElemsV`](&#123;environment:jQueryApiUrl&#125;/ui.igscroll#options:syncedElemsV) and [`syncedElemsH`](&#123;environment:jQueryApiUrl&#125;/ui.igscroll#options:syncedElemsH) properties for each of the scrolls as follows:
...
> **Note**: When implementing this with [`modifyDOM`](&#123;environment:jQueryApiUrl&#125;/ui.igscroll#options:modifyDOM) set to false the target elements for the `syncedElemsV`/`syncedElemsH` options should be the container elements, not the container wrapper element. Refer to the example DOM structure with `modifyDOM`:false in the [DOM structure](#structure-igScroll) section of this topic.
