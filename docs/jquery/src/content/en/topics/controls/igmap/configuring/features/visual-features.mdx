---
title: "Configuring the Visual Features (igMap)"
...
## Visual Features Configuration Summary
...
---|---|---
Cross hairs visibility|By default, the control does not render cross hairs. Configuring this setting displays the crosshairs in the control.|In JavaScript:<ul><li><ApiLink type="igMap" member="crosshairVisibility" section="options" label="crosshairVisibility" /></li></ul>In ASP.NET MVC:<ul><li>[CrosshairVisibility()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.SeriesViewer`3~CrosshairVisibility.html)</li></ul>
Tooltips visibility|By default, the control does not render tooltips. Configuring this setting displays the tooltips.|In JavaScript:<ul><li> <ApiLink type="igMap" member="series.showTooltip" section="options" label={'series["name"].showTooltip'} /></li></ul>In ASP.NET MVC:<ul><li>[Series.ShowTooltip()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.Series`3~ShowTooltip.html)</li></ul>
Tooltip look-and-feel|The look-and-feel of the tooltips is configurable through a template .|In JavaScript:<ul><li><ApiLink type="igMap" member="series.tooltipTemplate" section="options" label={'series["name"].tooltipTemplate'} /></li></ul>In ASP.NET MVC:<ul><li>[Series.TooltipTemplate()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.Series`3~TooltipTemplate.html)</li></ul>
Marker type|The type of the marker is configurable.|In JavaScript:<ul><li><ApiLink type="igMap" member="series.markerType" section="options" label={'series["name"].markerType'} /></li></ul>In ASP.NET MVC:<ul><li>[Series.MarkerType()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.GeographicSymbolSeries`1~MarkerType.html)</li></ul>
Custom marker|A custom marker type can be configured through a template. Configure an object with callback functions that render content directly on the canvas element used for the map.|In JavaScript:<ul><li> <ApiLink type="igMap" member="series.markerTemplate" section="options" label={'series["name"].markerTemplate'} /></li></ul>In ASP.NET MVC:<ul><li>[Series.MarkerTemplate()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.GeographicSymbolSeries`1~MarkerTemplate.html)</li></ul>
Marker collision avoidance|There is built-in collision avoidance logic for the control’s behavior if two or more markers overlap. By default, overlapping markers are drawn one over the other, but you can configure your own logic.|In JavaScript: <ul><li> <ApiLink type="igMap" member="series.markerCollisionAvoidance" section="options" label={'series ["name"].markerCollisionAvoidance'} /></li></ul>In ASP.NET MVC:<ul><li>[Series.MarkerCollisionAvoidance()](Infragistics.Web.Mvc~Infragistics.Web.Mvc.GeographicSymbolSeries`1~MarkerCollisionAvoidance.html)</li></ul>


## Marker Template Object Reference

### Marker template object reference summary
...
### Marker template object reference chart
...
### Measure callback function reference chart
...
### Render callback function reference chart
...
[Configuring the Marker Template With the SimpleTextMarkerTemplate](#simple-text-marker-template)|This code example shows how to configure a custom marker template using the `SimpleTextMarkerTemplate` helper class from the &#123;environment:ProductName&#125; library.
...
## Code Example: Configuring the Tooltip Template
...
## Code Example: Configuring the Marker Template in JavaScript
...
## Code Example: Configuring the Marker Template in ASP.NET MVC
...
## Code Example: Configuring the Marker Template With the SimpleTextMarkerTemplate
...
The example employs a helper widget called `SimpleTextMarkerTemplate` from the &#123;environment:ProductName&#125; created in order to simplify using custom markers. The widget has several options for configuring the tooltip’s appearance.
...
## Code Example: Configuring a Custom Marker Template
...
-	[Configuring the Navigation Features (igMap)](/igmap-configuring-navigation-features.mdx):This topic explains, with code examples, how to configure navigation features of the `igMap` control and how to use its API to define the position and size of the visible portion of the map.
...
-	[Marker Template](&#123;environment:SamplesUrl&#125;/map/marker-template):This sample demonstrates how to create a custom marker template in a map control.
