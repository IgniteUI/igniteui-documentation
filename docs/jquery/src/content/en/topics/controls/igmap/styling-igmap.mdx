...
Detailed information about using themes with the &#123;environment:ProductName&#125; library is available in the [Styling and Theming in &#123;environment:ProductName&#125;](//general-and-getting-started/styling-and-theming/deployment-guide-styling-and-theming.mdx) topic.

>**Note:** The base theme of &#123;environment:ProductName&#125; is not required for maps and you may safely omit it on pages containing only maps.
...
&#123;environment:ProductName&#125; offers the following themes you can use with the `igMap` control:
...
<table cellspacing="0" cellpadding="0" class="table">
	<tbody>
		...
		<tr>
			...
			<td>Path: &#123;IG CSS root&#125;/themes/infragistics/ File: infragistics.theme.css This theme defines general visual features for all &#123;environment:ProductName&#125; controls. Detailed information for using the IG theme is available in the <a class="ig-topic-link" href="Deployment-Guide-Styling-and-Theming.html" data-auto-update-caption="true">Styling and Theming in &#123;environment:ProductName&#125;</a> topic.</td>
</tr>

		<tr>
			<td>Metro</td>
			<td><img alt="" src="images/Styling_with_Themes_igMap_2.png" width="319" height="230"/></td>
			<td>Path: &#123;IG CSS root&#125;/themes/metro/ File: infragistics.theme.css This theme defines visual features with regard to the new Windows® 8 Metro user interface and touch enabled devices. It features sharp corners and some different colors on the OPD panel.</td>
</tr>
	</tbody>
</table>
...
---|---|---
<ApiLink type="igMap" member="backgroundContent" section="options" label="backgroundContent" />|Configures the background content provider to be used for plotting the map. Currently supported providers are Bing® Maps and OpenStreetMap. For details, see the [Configuring the Map Provider (igMap)](./03_Configuring/00_igMap_Configuring_Map Provider.mdx) topic.|All series
<ApiLink type="igMap" member="plotAreaBackground" section="options" label="plotAreaBackground" />|Configures the brush used for the background for the current plot area, i.e. the background of the map which is shown while some map tiles are still being downloaded.|All series
<ApiLink type="igMap" member="series.brush" section="options" label="series[“key”].brush" />|Configures the fill color for the main visual element of map series with name key.|Shape
<ApiLink type="igMap" member="series.outline" section="options" label="series[“key”].outline" />|Configures the outline color for the main visual element of map series with name key.|<ul><li>Shape</li><li>Polyline</li></ul>
<ApiLink type="igMap" member="series.markerBrush" section="options" label="series[“key”].markerBrush" />|Configures the fill color for the markers for map series with name key.|<ul><li>Symbol</li><li>Shape</li></ul>
<ApiLink type="igMap" member="series.markerOutline" section="options" label="series[“key”].markerOutline" />|Configures the outline color for the map series with name key.|<ul><li>Symbol</li><li>Shape</li></ul>
<ApiLink type="igMap" member="series.colorScale" section="options" label="series[“key”].colorScale" />|Configures a palette of colors to be used to represent the data range for geographic scatter area series.|Scatter area
<ApiLink type="igMap" member="series.fillScale" section="options" label="series[“key”].fillScale" />|Configures a palette of colors to be used to plot lines for geographic contour line series.|Contour line
<ApiLink type="igMap" member="series.shapeStyleSelector" section="options" label="series[“key”].shapeStyleSelector" />|Configures custom function for the  geographic shape series with name key which returns brush and outline settings for individual shapes.|Shape
...
- [Configuring the Visual Features (igMap)](/configuring/features/igmap-configuring-visual-features.mdx): This topic shows how to configure visual features of the `igMap` control like cross hairs, tooltip templates, marker templates and other.

- [Configuring the Map Series (igMap)](/configuring/series/igmap-creating-different-kinds-maps.mdx): This topic is a landing page linking to the topics explaining how to configure all supported map visualizations by the `igMap` control and how to use different background content (map providers).
...
- [Geographic Polyline Series](&#123;environment:SamplesUrl&#125;/map/geo-polyline-series): This sample shows how to bind shape and database files and configures geographic polyline map series.

- [Geographic Scatter Area Series](&#123;environment:SamplesUrl&#125;/map/geo-scatter-area):This sample demonstrates how to bind pre-triangulated (ITF) files to a map control with the help of the geographic scatter area series.

- [Geographic Contour Line Series](&#123;environment:SamplesUrl&#125;/map/geo-contour-line): This sample demonstrates how to bind pre-triangulated (ITF) files to a map control and configure geographic contour line series.
