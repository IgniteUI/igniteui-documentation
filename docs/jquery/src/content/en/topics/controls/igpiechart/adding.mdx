...
- [igPieChart Overview](/igpiechart-overview.mdx): This topic provides conceptual information about the `igPieChart` control including its main features, minimum requirements for using charts and user functionality.
...
```
	...
	**Referencing resources in MVC using the MVC Loader**
	...
	For MVC views &#123;environment:ProductNameMVC&#125; Loader must be used:
...
```
	...
	The &#123;environment:ProductNameMVC&#125; Loader automatically detects required resources and specifying resources is not necessary.

	**Referencing resources manually**

	For manual loading, refer to the [Minimum Requirements](./00_igPieChart_Overview.mdx#min-requirements) block in the [igPieChart Overview](/igpiechart-overview.mdx) topic to see which resource files must be linked to use pie charts.
...
```
	...
	**ASP.NET Example**
	...
	For ASP.NET MVC, no container elements are needed because the &#123;environment:ProductNameMVC&#125; adds the required markup automatically.

3. Add the data source.

	**HTML example**

	**For the HTML example some JavaScript code must be added that defines an array with several data records for company budget data.** In order to get information on binding to other data sources you may refer to the [Data Binding (igPieChart)](/igpiechart-databinding.mdx) topic.
...
```
	...
	**ASP.NET example**
	...
	The code below instantiates and sets the main features of the `igPieChart` using the &#123;environment:ProductNameMVC&#125; PieChart provided in the `Infragistics.Web.Mvc` assembly. The data model is associated with the control with the PieChart(Model) call and the rest of the calls act similarly to the HTML example.
...
- [jQuery and MVC API Reference Links (igPieChart)](/igpiechart-api-links.mdx): This topic provides links to the API documentation for jQuery and &#123;environment:ProductNameMVC&#125; class for `igDataChart`™ control.
