...
- [igDataSource](//data-sources/igdatasource/igdatasource.mdx): This is a reference page that links to all topics related to the `igDataSource`™.

- [igDataSource Overview](//data-sources/igdatasource/igdatasource-overview.mdx): This topic introduces the `igDataSource` control and demonstrates its basic usage and features.

- [igPieChart Overview](/igpiechart-overview.mdx): Contains basic information about the `igPieChart` control for displaying pie charts in web pages.

- [Adding an igPieChart](/igpiechart-adding.mdx): Provides step by step instructions for adding an `igPieChart` to a web page.
...
Data binding for `igPieChart` is done identically to binding for other controls from the &#123;environment:ProductName&#125;™ library. The way to bind data is by either by assigning a data source to the `dataSource` option or to provide a URL in the `dataSourceUrl` if data is provided by a web or WCF service.


## Bind to a JavaScript Array
...
-   An HTML5 web page
-   All required JavaScript and CSS files added to your web site or web application project. For detailed information on instantiation and configuration of an `igPieChart` see [Adding an igPieChart](/igpiechart-adding.mdx).
...
```
	...
	2. [Binding to XML](//data-sources/igdatasource/binding-to-xml.mdx) requires a <ApiLink pkg="ig" type="dataschema" label="igDataSchema" /> to define data fields. This component handles transformations for Array, JSON and XML data objects for the <ApiLink pkg="ig" type="datasource" label="igDataSource" />. Specify where the records in the date source are located using <ApiLink pkg="ig" type="dataschema" member="schema.searchField" section="options" label="searchField" /> option. Notice how the <ApiLink pkg="ig" type="dataschema" member="schema.fields" section="options" label="fields" /> are defined by specifying an `“xpath”` property for every field definition. This is unique to XML data sources. In this way, you are telling the data source the path to the property you are binding, in every hierarchical object that is in the list. 
...
```
	...
	3. Create an [Infragistics datasource](//data-sources/igdatasource/igdatasource-overview.mdx) which can be consumed by the chart from the already configured XML Data and `igDataSchema`. 
...
## Bind to an IQueryable&lt;T&gt; in ASP.NET MVC

### Introduction

This procedure shows how to bind a list of data objects from a backend controller method to a pie chart using the ASP.NET helper provided in the &#123;environment:ProductName&#125; library.
...
The following steps demonstrate how to instantiate and bind an `igPieChart` control in ASP.NET MVC by providing a list of data objects to a strongly typed view and use the &#123;environment:ProductNameMVC&#125; DataChart.
...
```
	...
	Note how the list of `DepartmentSpending` objects is converted to an `IQueryable< DepartmentSpending>` before submitting to the view. This can alternatively be done in the &#123;environment:ProductNameMVC&#125; PieChart call in the view but the implementation provided is cleaner.
...
- [Binding igDataSource to Client-Side Data](//data-sources/igdatasource/binding-igdatasource-to-client-side-data.mdx): This topic explains how to bind `igDataSource` to client-side JavaScript arrays and JSON data.

- [Binding igDataSource to REST Services](//data-sources/igdatasource/binding-to-rest-services.mdx): This topic explains how to bind an `igDataSource` control to REST services.

- [Binding igDataSource to WCF Data Services](//data-sources/igdatasource/binding-to-wcf-data-services.mdx): This topic explains how to bind an `igDataSource` component to a WCF service.
...
- [Pie Chart Overview](&#123;environment:SamplesUrl&#125;/pie-chart/overview):This sample demonstrates creating a simple pie chart and configuring some of its  features.