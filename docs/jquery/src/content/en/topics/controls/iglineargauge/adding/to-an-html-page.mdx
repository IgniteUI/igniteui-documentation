...
4. Configure the scale.
    ...
    In order to customize the values of the scale, you need to set its [minimumValue](&#123;environment:jQueryApiUrl&#125;/ui.igLinearGauge#options:minimumValue) and [maximumValue](&#123;environment:jQueryApiUrl&#125;/ui.igLinearGauge#options:maximumValue) properties. In this example, the scale will start at 5 and end at 55.
...
5. Add a needle.
    ...
    The primary measure of the `igLinearGauge` is visualized by its needle. Its value is managed by the [value](&#123;environment:jQueryApiUrl&#125;/ui.igLinearGauge#options:value) property setting. For this example, set the value property to 35.
...
6. Add comparative ranges.
    ...
    In order to compare the value displayed by the performance bar against some meaningful range(s) of values, these comparative ranges need to be displayed on the scale. Comparative ranges are managed by [ranges](&#123;environment:jQueryApiUrl&#125;/ui.igLinearGauge#options:ranges) property within which several individual ranges can be defined, each of which having its own starting and ending values ([startValue](&#123;environment:jQueryApiUrl&#125;/ui.igLinearGauge#options:startValue) and [endValue](&#123;environment:jQueryApiUrl&#125;/ui.igLinearGauge#options)) and color ([brush](&#123;environment:jQueryApiUrl&#125;/ui.igLinearGauge#options:brush)). For this example, configure 3 comparative ranges, each of a different shade of gray, starting at the 0, 15, and 30 tick marks of the scale, respectively.
