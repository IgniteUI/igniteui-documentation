---
title: "igTree Known Limitations"
...
The table below briefly describes the known issues/limitations of the \{environment:ProductName\}® \{environment:ProductVersion\} release for the `igTree`™ control. Detailed explanations and the possible workarounds are provided for some of the issues in the blocks following the table.
...
Upgrade to a later version of jQuery after 1.4.4 or upgrade to the latest service release of \{environment:ProductName\} where the `igTree` control resolves this issue internally.
...
-   [Known Issues](//known-issues/known-issues-revision-history/revision-history.mdx)
