...
The `infragistics.util` files contain an utility function `$.ig.formatter` that is used to format dates, numbers and strings across the &#123;environment:ProductName&#125; controls. For example the igGrid uses it in the [format](&#123;environment:jQueryApiUrl&#125;/ui.iggrid#options:columns.format) option.
