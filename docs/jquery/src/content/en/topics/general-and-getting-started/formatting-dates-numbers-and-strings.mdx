---
title: "Formatting Dates, Numbers and Strings"
...
The `infragistics.util` files contain an utility function `$.ig.formatter` that is used to format dates, numbers and strings across the \{environment:ProductName\} controls. For example the igGrid uses it in the <ApiLink type="iggrid" member="columns.format" section="options" label="format" /> option.
