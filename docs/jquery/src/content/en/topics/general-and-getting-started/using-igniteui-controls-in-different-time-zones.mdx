...
-	[`enableUTCDates`](&#123;environment:jQueryApiUrl&#125;/ui.igdateeditor#options:enableUTCDates) -  Enables/Disables serializing client date as UTC ISO 8061 string instead of using the local time and zone values. The option is only applied in "date" [`dataMode`](&#123;environment:jQueryApiUrl&#125;/ui.igdateeditor#options:dataMode).
...
The following examples will demonstrate how MVC wrapper and client widget works, when [`displayTimeOffset`](&#123;environment:jQueryApiUrl&#125;/ui.igdateeditor#options:displayTimeOffset) and [`enableUTCDates`](&#123;environment:jQueryApiUrl&#125;/ui.igdateeditor#options:enableUTCDates) options are used.
...
From the example above, it can be seen that when we define a server hour in the editor MVC wrapper, in the client we will always see that server hour, ignoring the client time zone offset. This is the default behavior, if we want each of the clients to display the time in their time zones, then we need to set the [`displayTimeOffset`](&#123;environment:jQueryApiUrl&#125;/ui.igdateeditor#options:displayTimeOffset) wrapper option to `null`. Using the previous example and setting the option, on the client the `displayTimeOffset` option will be ignored and the time will show according the specific time zone:
