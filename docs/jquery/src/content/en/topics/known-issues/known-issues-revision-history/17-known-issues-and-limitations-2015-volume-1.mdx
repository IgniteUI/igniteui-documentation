---
title: "Known Issues and Limitations in 2015 Volume 1"
...
This summarizes the known issues and limitations of the \{environment:ProductName\}™ 2015 Volume 1 release. Information about previous releases can be found [here](/known-issues-revision-history.mdx).
...
| Issue | Description | Status |
| --- | --- | --- |
| Placeholder behavior when element is `` or `` | If the editor’s base element is an `` or `` element and `buttons`, `theme`, or <ApiLink type="igeditor" member="renderInContainer" section="options" label="renderInContainer" /> options are enabled, then the editor’s base element is removed from its parent and wrapped in a `` element. | ![](../../images/images/negative.png) |
| Editor not rendering correctly in some browsers when in a `` base element | If the base element for the editor is a `` element (or some other special container different from the `` or `` elements), then some browsers may not render the control. For example, rendering fails in the Safari® browser when using a `` as the base element for the `igCurrencyEditor`™ control. **Workaround**, Change the default value of the `display` option from inline-block to block. | ![](../../images/images/positive.png) |
| Changing theme after initialization not supported under certain conditions | Changing the `theme` option is not supported after initialization when the base element is either `` or ``and if the `button`, <ApiLink type="igeditor" member="renderInContainer" section="options" label="renderInContainer" />, and `theme` options are disabled. | ![](../../images/images/negative.png) |
...
| Base `` element auto-replaced with `` element | If the base element is a `` element, it is replaced with an `` element. The drop-down button and list Items are created programmatically and the content is filled with the options of the original `` element. | ![](../../images/images/negative.png) |
| Width and height options can be set only absolutely (in pixels) under certain conditions | If the editor’s base element is not an `` or `` element and `buttons`, `theme`, or <ApiLink type="igeditor" member="renderInContainer" section="options" label="renderInContainer" /> options are enabled, then the width and height options can be set only absolutely (in pixels). **Workaround**, If the editor is rendered as an `` or `` element without a ``element wrapper, then the width and height can be set to a string with percent and other HTML units. | ![](../../images/images/positive.png) |
| Some options cannot change values after initialization | Changing value after initialization for `button`, <ApiLink type="igeditor" member="textMode" section="options" label="textMode" />, and <ApiLink type="igeditor" member="renderInContainer" section="options" label="renderInContainer" /> options is not supported. | ![](../../images/images/positive.png) |
| Limitations to filtering an entry by list | Options related to filtering an entry by list, such as <ApiLink type="igeditor" member="listMatchIgnoreCase" section="options" label="listMatchIgnoreCase" />, <ApiLink type="igeditor" member="listMatchOnly" section="options" label="listMatchOnly" />, <ApiLink type="igeditor" member="listMatchContains" section="options" label="listMatchContains" />and <ApiLink type="igeditor" member="listAutoComplete" section="options" label="listAutoComplete" />, have effect only for an editor with `type` set to either text or `igTextEditor`. | ![](../../images/images/negative.png) |
| Specifics about disabling all triggers for a drop-down | To disable all triggers for a drop-down, the <ApiLink type="igeditor" member="dropDownTriggers" section="options" label="dropDownTriggers" /> option should be set to either an empty string or null. | ![](../../images/images/negative.png) |
| Erroneous behavior when user input exceeds the length of the input field | If the user enters text longer than the width of the edit field: Caret in some browsers may go out of view. (The editor control’s has logic designed to minimize this occurrence, but it does not cover 100% of the cases). Some browsers may fail to instantly adjust the caret position while characters are entered. The caret may also briefly (for a split-second) jump to the end of the text and back. Although the actual entry point for the next key and the actual location of caret should be correct. In some browsers, particularly Firefox®, text may jump/shake horizontally on every keypress. | ![](../../images/images/negative.png) |
| Erroneous behavior when under certain conditions | If the base element is `` (`textMode=”multiline”`) and multiple lines are entered, that make the field scroll: Vertical scroll may fail and the caret may go out of view after multiple keyboard entries, when Shift+Enter (new line) is entered. This may happen if the caret has been located in the middle of text. In some browsers, particularly Firefox, text may jump vertically and horizontally on every keypress. | ![](../../images/images/negative.png) |
| Specific referencing requirements for the date picker | If an editor has a type set to either *datepicker* or `igDatePicker`™, then it is dependent on jquery-datepicker and requires reference to either `jquery.ui.datepicker.js` or to combined library such as `jqueryui/1.8.7/jquery-ui.js`. | ![](../../images/images/negative.png) |
| Date picker not supporting multi-line mode | If an editor has a type set to either *datepicker* or `igDatePicker`, it does not support *multiline* <ApiLink type="igeditor" member="textMode" section="options" label="textMode" /> option. | ![](../../images/images/negative.png) |
| Date picker not supporting `` base element | If an editor has a type set to either *datepicker* or `igDatePicker`, it does not support `` as a base element. | ![](../../images/images/negative.png) |
...
| Specific referencing requirements for the numeric and date editors | Numeric and Date editors depend on `$.ig._regional`, therefore, that object should be available in a reference to `ig.util.js` or to a combined `ig-js` file. | ![](../../images/images/negative.png) |
...
| Change in styling | The layout of HTML elements has modified and rounded corners are rendered around the whole editor, not only around buttons. **Workaround**, Applications can disable rounded corners by providing custom CSS or overriding default `igEditor` CSS classes. | ![](../../images/images/positive.png) |
| Spin buttons issue | Spin buttons are rendered horizontally. | ![](../../images/images/negative.png) |
| Editors rendering failure | Rendering may fail if the base element is ``. **Workaround**, Applications should not restrict the width of a parent element if the base element is `` or it should ensure that the actual width of the `` element is no less than the width of the editor control. The parent element is not only the parent table but the whole upchain of parents in the DOM hierarchy. | ![](../../images/images/positive.png) |
| `igDateEditor` and `igDatePicker` Knockout extensions have limitation in Immediate update mode | `igDateEditor`™ and `igDatePicker` Knockout extensions have a limitation when the `updateMode` property is set to immediate. The Immediate mode is not supported when date format is different than en-US., When non en-US format is used, Immediate mode, works correct only if the date is selected from the date picker, or a valid date is pasted inside the editor. If the value is typed, character by character, the wrong date is populated. The limitation is due to limitation of the JavaScript Date object constructor. **Workaround**, Either use the default update mode – On Change – or force the user to select the date from the `igDatePicker` component and not to enter it manually. | ![](../../images/images/positive.png) |
...
| Misalignment of fixed and unfixed parts of the rows in IE9+ | In Internet Explorer 9 or higher, if you fix a column in an igGrid and scroll down to the middle of the grid, you will see misalignment between the fixed and unfixed parts of the rows. This issue is caused by the IE9 engine. | ![](../../images/images/positive.png) |
| Grid and its columns widths are mandatory and should be defined in pixels units | Grid and its columns (either explicitly or using the <ApiLink type="iggrid" member="defaultColumnWidth" section="options" label="defaultColumnWidth" /> option) widths are mandatory and should be defined in pixels units. | ![](../../images/images/negative.png) |
...
Limitation when using GroupBy feature and continuous virtualization | When `igGrid`’s continuous virtualization is in use and the Group By feature is enabled, scrolling vertically causes the groups to revert to their initial state (expanded or collapsed depending on the <ApiLink type="iggridgroupby" member="initialExpand" section="options" label="initialExpand" /> option). | ![](../../images/images/negative.png)
...
Excel Navigation mode supported only for Cell Edit and Row Edit modes. | When <ApiLink type="iggridupdating" member="excelNavigationMode" section="options" label="excelNavigationMode" /> is enabled, navigating with the arrows is only possible with *"cell"* or *"row"* <ApiLink type="iggridupdating" member="editMode" section="options" label="editMode" />. Other Edit modes (*“rowedittemplate”*, *none*, *null*) are not supported with <ApiLink type="iggridupdating" member="excelNavigationMode" section="options" label="excelNavigationMode" />. | ![](../../images/images/negative.png)
...
`rowEditDialogMaxHeight` property renamed to <ApiLink type="iggridupdating" member="rowEditDialogContentHeight" section="options" label="rowEditDialogContentHeight" /> | This property is related to the Row Edit Template (<ApiLink type="iggridupdating" member="editMode" section="options" label="editMode" />=*“rowedittemplate”*). It is used to set the height of the row edit dialog content. This is a breaking change from the previous versions’ functionality. | ![](../../images/images/negative.png)
