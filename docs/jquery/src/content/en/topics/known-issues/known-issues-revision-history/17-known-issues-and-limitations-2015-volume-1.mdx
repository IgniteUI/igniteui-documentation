...
| Issue | Description | Status |
| --- | --- | --- |
| Placeholder behavior when element is `` or `` | If the editor’s base element is an `` or `` element and `buttons`, `theme`, or [`renderInContainer`](&#123;environment:jQueryApiUrl}/ui.igeditor#options:renderInContainer) options are enabled, then the editor’s base element is removed from its parent and wrapped in a `` element. | ![](../../images/images/negative.png) |
...
| Changing theme after initialization not supported under certain conditions | Changing the `theme` option is not supported after initialization when the base element is either `` or ``and if the `button`, [`renderInContainer`](&#123;environment:jQueryApiUrl}/ui.igeditor#options:renderInContainer), and `theme` options are disabled. | ![](../../images/images/negative.png) |
...
| Width and height options can be set only absolutely (in pixels) under certain conditions | If the editor’s base element is not an `` or `` element and `buttons`, `theme`, or [`renderInContainer`](&#123;environment:jQueryApiUrl}/ui.igeditor#options:renderInContainer) options are enabled, then the width and height options can be set only absolutely (in pixels). **Workaround**, If the editor is rendered as an `` or `` element without a ``element wrapper, then the width and height can be set to a string with percent and other HTML units. | ![](../../images/images/positive.png) |
