---
title: "Known Issues and Limitations in 2016 Volume 2"
...
This summarizes the known issues and limitations of the &#123;environment:ProductName&#125;™ 2016 Volume 2 release. Information about previous releases can be found [here](/known-issues-revision-history.mdx).
...
| --- | --- | --- |
| On Internet Explorer 7 and 8 Array.prototype is extended with some helper functions | On Internet Explorer 7 and 8 Array.prototype is extended with the following functions (by the infragistics.util files): add indexOf copy contains insert removeAt removeItem getEnumerator count item getLength clear resize filter insertRange insertRange1 clone This affects iteration over an array with a for…in loop, as those functions also will be iterated. | ![](../../images/images/negative.png) |
...
| Specific referencing requirements for the date picker | `igDatePicker`™ is dependent on jquery-datepicker and requires reference to either `jquery.ui.datepicker.js` or to combined library such as `jqueryui/1.9.1/jquery-ui.js`. | ![](../../images/images/negative.png) |
...
| Caption rendering differs whether there is height set for the grid or not. | 1. When there is grid <ApiLink type="iggrid" member="caption" section="options" label="caption" /> and <ApiLink type="iggrid" member="height" section="options" label="height" /> set two captions are rendered, one for the fixed and one for the unfixed area., 2. When there is grid caption without height set only one caption is rendered for the grid., The difference is caused by the more complex DOM structure used when there is grid height set. | ![](../../images/images/negative.png) |
