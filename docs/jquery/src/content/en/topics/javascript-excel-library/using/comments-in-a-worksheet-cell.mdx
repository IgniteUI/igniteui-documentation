...
Cell Comments are used to explain or remind the content of a cell or range of cells. For example, they may be used to explain the formulas that reside in the cells they are attached to. As in Microsoft® Excel®, it is possible to add cell comments to your Excel Worksheet using the [`WorksheetCellComment`](&#123;environment:jQueryApiUrl&#125;/ig.excel.WorksheetCellComment "Link to the Web API Reference Guide to the WorksheetCellComment member.") WorksheetCellComment member of the JavaScript Excel Library.
